...
START_TEST(test_add) {
    ...
    for (i = 0; i < len; ++i) {
        ...
        fail_unless(xhash_get_str(ht, buffer) == (a + i), "bad hashable item returned");
    }
    xhash_free(ht);
}
...
START_TEST(test_find) {
    ...
    for (i = 0; i < g_hashableslen; ++i) {
        snprintf(buffer, sizeof(buffer), "%d", i);
        fail_unless(xhash_get_str(ht, buffer) == (g_hashables + i), "bad hashable item returned");
    }
}
...
START_TEST(test_delete)
        ...
        fail_unless(xhash_count(ht) == g_hashableslen, "invalid delete has been done");
        result = test_delete_helper();
        fail_unless(result == 0, "no item should have been deleted, but %d were deleted", result);
        ...
        result = test_delete_helper();
        fail_unless(result == 3, "bad number of items were deleted: %d", result);}
...
START_TEST(test_count) {
    ...
    hashable_t a[4] = {{"0", 0},
                       ...
                       {"2", 2},
                       {"3", 3}};
    fail_unless(xhash_count(ht) == g_hashableslen, "invalid count (fixture table)");
    ht = xhash_init(hashable_identify, NULL);
    xhash_add(ht, a);
    xhash_add(ht, a + 1);
    xhash_add(ht, a + 2);
...
START_TEST(test_walk)
        ...
        for (i = 0; i < g_hashableslen; ++i) {
            fail_unless(g_hashables[i].idn == UINT32_MAX, "hashable item was not walked over");
        }}
END_TEST
