...
START_TEST(test_xtree_creation_unmanaged)
        ...
        fail_unless(tree->root == NULL, "tree has a root on creation");
        fail_unless(tree->count == 0, "tree has nodes on creation");
        fail_unless(tree->depth == 0, "tree has a depth on creation");
        fail_unless(xtree_depth_const(tree) == 0, "tree depth is not 0 on creation");
        fail_unless(tree->state == XTREE_STATE_DEPTHCACHED, "tree is not cached on creation");}
...
START_TEST(test_xtree_add_root_node_unmanaged) {
    ...
    fail_unless(xtree_add_child(tree, NULL, fake_addr, XTREE_APPEND) != NULL, "unable to add root node");
    fail_unless(tree->root != NULL, "root node has not been allocated");
    fail_unless(tree->free == NULL, "bad free function in the tree");
    fail_unless(tree->count == 1, "there should be at least one node and only one in node count");
    fail_unless(xtree_depth_const(tree) == 1, "tree should have a depth of one (depth %d)", xtree_depth_const(tree));
    fail_unless(tree->root->data == (void *) 1, "node data is incorrect");
    fail_unless(tree->root->parent == NULL, "root node has a parent");
    fail_unless(tree->root->start == NULL && tree->root->end == NULL, "root node should not already have child in it");
    fail_unless(tree->root->next == NULL && tree->root->previous == NULL, "root node have invalid siblings");

    xtree_refresh_depth(tree);
    fail_unless(tree->depth == 1, "root node refreshed should have one depth (root level)");
    fail_unless(tree->state == XTREE_STATE_DEPTHCACHED, "root node should now have its depth been cached");
    ...
    fail_unless(tree->root->data == (void *) 1, "xtree_add_child generated an operation and should not in context");
    fail_unless(tree->root->start == NULL, "xtree_add_child had added an invalid child");
    fail_unless(tree->root->start == tree->root->end, "xtree_add_child invalidated root node child list");

    fail_unless(xtree_add_child(tree, tree->root, fake_addr, XTREE_APPEND) != NULL,
                "unable to add child node to root node");
    fail_unless(tree->count == 2, "bad tree node count");
    fail_unless(xtree_depth_const(tree) == 2, "bad depth after root's first child");
    fail_unless(tree->state != XTREE_STATE_DEPTHCACHED, "tree should not have already cached level count");

    fail_unless(
            tree->root && tree->root->data == (void *) 1 && tree->root->parent == NULL && tree->root->next == NULL &&
            tree->root->previous == NULL, "root node has badly been modified");
    fail_unless(!!tree->root->start, "root has no child, but should have one");
    fail_unless(tree->root->start == tree->root->end, "root child list is inconsistent");

    fail_unless(tree->root->start->data == (void *) 2, "bad child data");
    fail_unless(tree->root->start->parent == tree->root, "child parent does not point to root node");
    fail_unless(!tree->root->start->start, "child should be unique for now");
    fail_unless(tree->root->start->start == tree->root->start->end, "child children list is inconsistent");
    fail_unless(!tree->root->start->next && !tree->root->start->previous, "child should not have siblings");

    ++fake_addr;
    fail_unless(xtree_add_child(tree, tree->root, fake_addr, XTREE_APPEND) != NULL, "unable to add second child");

    fail_unless(tree->root->start != tree->root->end, "root should have more children");
    fail_unless(tree->root->start->next == tree->root->end && tree->root->end->previous == tree->root->start &&
                tree->root->end->next == NULL && tree->root->start->previous == NULL,
                "root children list is inconsistent");
    fail_unless(tree->root->end->data == (void *) 3, "root second child has bad data");
}
...
static void myfree(xtree_node_t *x) {
    int *item = (int *) x->data;
    fail_unless(*item < 10 && *item >= 0, "bad data passed to freeing function");
    fail_unless(test_table[*item] == 1, "item was duplicated/corrupted");
    test_table[*item] = 0;
    xfree(item);
}
...
START_TEST(test_xtree_freeing_elements)
        ...
        fail_unless(x != NULL, "unable to allocate memory for test");
        *x = i;
        test_table[i] = 1;
        ...
        for (i = 0; i < 10; ++i) {
            fail_unless(test_table[i] == 0, "one element has not been freed in the table (num %d)", i);
        }}
...
START_TEST(test_xtree_with_add_child) {
    ...
    fail_unless(xtree_add_child(tree, NULL, fake_addr, XTREE_APPEND) != NULL, NULL);
    ++fake_addr;
    fail_unless(xtree_add_child(tree, tree->root, fake_addr, XTREE_APPEND) != NULL, NULL);
    ++fake_addr;
    fail_unless(xtree_add_child(tree, tree->root, fake_addr, XTREE_APPEND) != NULL, NULL);
    ++fake_addr;
    fail_unless(xtree_add_child(tree, tree->root->start, fake_addr, XTREE_APPEND) != NULL, NULL);
    ++fake_addr;
    fail_unless(xtree_add_child(tree, tree->root, fake_addr, XTREE_APPEND) != NULL, NULL);
    ++fake_addr;
    fail_unless(xtree_add_child(tree, tree->root, fake_addr, XTREE_PREPEND) != NULL, NULL);
    ++fake_addr;
    fail_unless(xtree_add_child(tree, tree->root->start->next, fake_addr, XTREE_PREPEND) != NULL, NULL);

    fail_unless(tree->root->start->data == (void *) 6 && tree->root->start->next->data == (void *) 2 &&
    ...
    fail_unless(tree->root->end->data == (void *) 5 && tree->root->end->previous->data == (void *) 3 &&
    ...
    fail_unless(tree->root->start->previous == NULL && tree->root->end->next == NULL, "bad tree edges");
    ...
    fail_unless(level1_2->start->data == (void *) 7 && level1_2->start->start == NULL &&
                level1_2->start->previous == NULL && level1_2->start->next == level1_2->end &&
                level1_2->end->data == (void *) 4 && level1_2->end->next == NULL && level1_2->end->start == NULL,
...
START_TEST(test_xtree_with_add_sibling) {
    ...
    fail_unless(xtree_add_sibling(tree, NULL, fake_addr, XTREE_APPEND) != NULL, NULL); /* 1 */
    ++fake_addr;
    fail_unless(xtree_add_child(tree, tree->root, fake_addr, XTREE_APPEND) != NULL, NULL); /* 2 */
    fail_unless(xtree_add_sibling(tree, tree->root, fake_addr, XTREE_APPEND) == NULL,
                "add_sibling should return null when used with root node");
    ++fake_addr;
    fail_unless(xtree_add_sibling(tree, tree->root->start, fake_addr, XTREE_APPEND) != NULL, NULL); /* 3 */
    ++fake_addr;
    fail_unless(xtree_add_sibling(tree, tree->root->end, fake_addr, XTREE_PREPEND) != NULL, NULL); /* 4 */
    ++fake_addr;
    fail_unless(xtree_add_sibling(tree, tree->root->end, fake_addr, XTREE_APPEND) != NULL, NULL); /* 5 */
    ++fake_addr;
    fail_unless(xtree_add_sibling(tree, tree->root->start, fake_addr, XTREE_APPEND) != NULL, NULL); /* 6 */
    ++fake_addr;
    fail_unless(xtree_add_sibling(tree, tree->root->start, fake_addr, XTREE_PREPEND) != NULL, NULL); /* 7 */

    fail_unless(tree->root->data == (void *) 1, "bad root node");

    fail_unless(tree->root->start->data == (void *) 7 && tree->root->start->next->data == (void *) 2 &&
                tree->root->start->next->next->data == (void *) 6 &&
                tree->root->start->next->next->next->data == (void *) 4, "bad tree structure browsing forward");
    fail_unless(tree->root->end->data == (void *) 5 && tree->root->end->previous->data == (void *) 3 &&
...
START_TEST(test_xtree_depth) {
    ...
    fail_unless(~tree->state & XTREE_STATE_DEPTHCACHED, "state is cached, should not be");
    ...
    fail_unless(xtree_depth_const_node(tree, tree->root->start), "bad subtree level depth");
    fail_unless(xtree_depth_const_node(tree, tree->root->start->next), "bad subtree level depth");
    fail_unless(xtree_depth_const_node(tree, tree->root->start->next->start), "bad subtree level depth");
}
...
static uint8_t action_test(xtree_node_t *node, uint8_t which, uint32_t level, void *arg) {
    walk_test_t *data = (walk_test_t *) arg;
    if (data) {
        data->executed = 1;
        if (data->table_pos->node_data == node->data && data->table_pos->which == which &&
...
START_TEST(test_xtree_walk)
        ...
        node = xtree_add_child(tree, tree->root->start->next->end, (void*)8, XTREE_APPEND);
        fail_unless(node == tree->root->start->next->end->start, "fail to add required node for tests");
        ...
        node = xtree_walk(tree, tree->root, UINT32_MAX, 0, action_test, &walk_data);
        fail_unless(node == NULL, "invalid case, however returned not null");
        fail_unless(walk_data.executed == 0, "invalid case (min > max) but got executed");
        fail_unless(walk_data.error == 0, "invalid case, error detected but should not have been executed");
        fail_unless(walk_data.table_pos == table, "invalid case table_pos advanced but should not");

        /* test tree walk through */
        node = xtree_walk(tree, NULL, 0, UINT32_MAX, action_test, &walk_data);
        fail_unless(walk_data.executed == 1, "should have executed at least one time");
        fail_unless(walk_data.table_pos != NULL, "invalid pointer value for table_pos");
#if 0
...
compare_test(const void *node_data, const void *arg) {
...
START_TEST(test_xtree_find)
        ...
        fail_unless(node == NULL, "bad result (should be NULL): %x", (node)?node->data:NULL);
        ...
        fail_unless(node != NULL, "result is null however it should have been found");
        fail_unless(node == tree->root, "root node should have been found, but found : %x", (node)?node->data:NULL);

        node = xtree_find(tree, compare_test, (void*)4);
        fail_unless(node != NULL, "result is null however it should have been found");
        fail_unless(tree->root->start->next->end == node, "bad result (search 4): %x", (node)?node->data:NULL);

        node = xtree_find(tree, compare_test, (void*)5);
        fail_unless(node != NULL, "result is null however it should have been found");
        fail_unless(tree->root->end == node, "bad result (search 5): %x", (node)?node->data:NULL);

        /* test node with parent and with childs */
        node = xtree_find(tree, compare_test, (void*)2);
        fail_unless(node != NULL, "result is null however it should have been found");
        fail_unless(tree->root->start->next == node, "bad result (search 2): %x", (node)?node->data:NULL);
...
START_TEST(test_xtree_delete) {
    ...
    fail_unless(tree->state & XTREE_STATE_DEPTHCACHED, "level should still be cached");
    ...
    /* tree structure */
    fail_unless(xtree_delete(tree, tree->root->start) == tree->root, "parent of 6 should have been root node");
    fail_unless(xtree_depth(tree) == 3, NULL);
    fail_unless(tree->root->start->data == (void *) 2 && tree->root->start->next->data == (void *) 3 &&
                tree->root->start->next->next->data == (void *) 5, "children should be now 2 -> 3 -> 5");
    fail_unless(tree->root->start->previous == NULL, "bad children list edges");
    fail_unless(xtree_get_count(tree) == 6, "bad count update");
    fail_unless(tree->state & XTREE_STATE_DEPTHCACHED, "level should still be cached");
    fail_unless(tree->depth == 3 && xtree_depth(tree) == 3, "depth should not have changed");

    /* structure and depth changing */
    fail_unless(xtree_delete(tree, tree->root->start->start) == tree->root->start,
    ...
    fail_unless(tree->root->start->start == NULL && tree->root->start->end == NULL, "bad edges for node 2");
    fail_unless(tree->root->start->data == (void *) 2 && tree->root->start->next->data == (void *) 3 &&
                tree->root->start->next->next->data == (void *) 5, "tree deconstruction");
    fail_unless(tree->root->start->previous == NULL && tree->root->end->next == NULL, "tree edges deconstruction");
    fail_unless(~tree->state & XTREE_STATE_DEPTHCACHED, "level should not be cached");
    fail_unless(xtree_depth(tree) == 2, "the last removal should have reduced depth");
...
START_TEST(test_xtree_get_parents) {
    ...
    fail_unless(xtree_get_parents(NULL, tree->root->start, NULL) == NULL, "bad behavior");
    ...
    fail_unless(xtree_get_parents(tree, tree->root->start, NULL) == NULL, "bad behavior");
    fail_unless(xtree_get_parents(tree, tree->root, &size) == NULL, "bad behavior");
    ...
    fail_unless(parents[0] == tree->root, "parents list of 6 should be root node");
    xfree(parents);
    ...
    fail_unless(parents != NULL, "should have a parent here");
    fail_unless(size == 1, "should have parents' list size == 1");
    fail_unless(parents[0] == tree->root, "parents list of 2 should be root node");
    xfree(parents);
    ...
    fail_unless(parents != NULL, "should have a parent here");
    fail_unless(size == 1, "should have parents' list size == 1");
    fail_unless(parents[0] == tree->root, "parents list of 3 should be root node");
    xfree(parents);
    ...
    fail_unless(parents != NULL, "should have a parent here");
    fail_unless(size == 1, "should have parents' list size == 1");
    fail_unless(parents[0] == tree->root, "parents list of 5 should be root node");
    xfree(parents);
    ...
    fail_unless(parents[0] == tree->root->start->next, "parents[0] of 7 should be node 2 (actually %x)",
                (parents[0]) ? parents[0]->data : NULL);
    fail_unless(parents[1] == tree->root, "parents[1] of 7 should be root node");
    xfree(parents);
    ...
    fail_unless(parents != NULL, "should have parents here");
    fail_unless(size == 2, "should have parents' list size == 2");
    fail_unless(parents[0] == tree->root->start->next, "parents[0] of 4 should be node 2 (actually %x)",
                (parents[0]) ? parents[0]->data : NULL);
    fail_unless(parents[1] == tree->root, "parents[1] of 7 should be root node");
    xfree(parents);
}
...
START_TEST(test_xtree_common) {
    ...
    fail_unless(node == tree->root, "bad returned node : %x", (node) ? node->data : NULL);
...
START_TEST(test_xtree_get_leaves) {
    ...
    fail_unless(nodes[0] == tree->root->start->next->start, "nodes[0] != nodes 7");
    fail_unless(nodes[1] == tree->root->start->next->end, "nodes[1] != nodes 4");
    xfree(nodes);
    ...
    fail_unless(nodes[2] == tree->root->start->next->end, "bad leaves result");
    fail_unless(nodes[3] == tree->root->start->next->next, "bad leaves result");
    fail_unless(nodes[4] == tree->root->end, "bad leaves result");
    xfree(nodes);
}
