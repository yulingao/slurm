...
int main(int argc, char **argv) {
    ...
    if ((rc = PMI_Get_clique_ranks(clique_ranks, clique_size)) != PMI_SUCCESS) {
    ...
    for (i = 0; i < pmi_size; i++) {
        ...
        if ((rc = PMI_KVS_Get(kvs_name, key, val, val_len)) != PMI_SUCCESS) {
...
#endif
        if ((rc = PMI_KVS_Iter_next(kvs_name, key, key_len, val, val_len)) != PMI_SUCCESS) {
    ...
    for (i = 0; i < pmi_size; i++) {
        ...
        if ((val[0] != 'D') || ((atoi(&val[1]) - OFFSET_2) != i)) {
            printf("FAILURE: Bad keypair %s=%s, task %d\n", key, val, pmi_rank);
            exit(1);
        }
#if _DEBUG
...
#endif
    if ((rc = PMI_KVS_Get(kvs_name, "KVS_KEY", val, val_len)) != PMI_SUCCESS) {
    ...
    if ((rc = PMI_KVS_Get(kvs_name, "KVS_KEY", val, val_len)) != PMI_ERR_INVALID_KVS) {
        printf("FAILURE: PMI_KVS_Get(%s, KVS_KEY): %d, task %d\n", kvs_name, rc, pmi_rank);
        exit(1);
    }
