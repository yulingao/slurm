...
static int _set_options(const int argc, char **argv) {
    ...
    static struct option long_options[] = {{"configdir", required_argument, 0, 'c'},
    ...
    while ((cc = getopt_long(argc, argv, "cmt:Uv", long_options, &option_index)) != EOF) {
        switch (cc) {
            case 'c':
...
int _run_test(char **testcase, int lines) {
    ...
    if (hl_count != (lines - 1)) {
        info("Expected #lines is %d, not #returned %d", lines, hl_count);
        _print_test(testcase, lines);
    ...
    for (i = 0; i < hl_count; i++) {
        ...
        if (strcmp(list, testcase[i + 1]) != 0) {
            info("List[%d]=%s not expected %s", i, list, testcase[i + 1]);
            _print_test(testcase, lines);
            xfree(list);
            rc = SLURM_ERROR;
            goto clean;
