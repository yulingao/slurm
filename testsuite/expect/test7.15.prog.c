...
int main(int ac, char **av) {
    ...
    for (i = 1; i < SIGRTMAX; i++) {
        ...
        if (act.sa_handler == SIG_IGN) {
            ...
            if (i == SIGFPE)
                fprintf(stderr,
                        "%s: Terminals like rxvt-unicode/aterm will ignore SIGFPE.  Rerun this test where the slurmd isn't started from that terminal if you get this message.\n",
                        hostname);
            rc = 1;
        } else if (act.sa_handler != SIG_DFL) {
            fprintf(stderr, "%s: Signal %d has handler function!\n", hostname, i);
            rc = 1;
        }
    }
    return (rc);
