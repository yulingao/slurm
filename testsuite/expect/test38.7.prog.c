...
static void pass_its_neighbor(const int rank, const int size) {
    ...
    MPI_Irecv((void *) &in_buf, sizeof(rank_info_t), MPI_CHAR, ((rank + size - 1) % size), COMM_TAG, MPI_COMM_WORLD,
              &request[0]);
    MPI_Isend((void *) &out_buf, sizeof(rank_info_t), MPI_CHAR, ((rank + 1) % size), COMM_TAG, MPI_COMM_WORLD,
              &request[1]);
