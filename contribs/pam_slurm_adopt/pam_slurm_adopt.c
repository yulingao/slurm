...
typedef enum {
    CALLERID_ACTION_NEWEST, CALLERID_ACTION_ALLOW, CALLERID_ACTION_IGNORE, CALLERID_ACTION_DENY,
} callerid_action_t;
...
static int _adopt_process(pam_handle_t *pamh, pid_t pid, step_loc_t *stepd) {
    ...
    if (!stepd)
        return -1;
    debug("_adopt_process: trying to get %u.%u to adopt %d", stepd->jobid, stepd->stepid, pid);
    fd = stepd_connect(stepd->directory, stepd->nodename, stepd->jobid, stepd->stepid, &protocol_version);
    if (fd < 0) {
        /* It's normal for a step to exit */
        debug3("unable to connect to step %u.%u on %s: %m", stepd->jobid, stepd->stepid, stepd->nodename);
        return -1;
    }
    ...
    if ((rc == PAM_SUCCESS) && !opts.disable_x11) {
        int display;
        char *xauthority;
        display = stepd_get_x11_display(fd, stepd->protocol_version, &xauthority);
    ...
    if (rc == PAM_SUCCESS)
        ...
        info("Process %d adoption FAILED for job %u", pid, stepd->jobid);
...
static uid_t _get_job_uid(step_loc_t *stepd) {
    ...
    if ((int32_t) uid == -1)
        debug3("unable to determine uid of step %u.%u on %s", stepd->jobid, stepd->stepid, stepd->nodename);
...
static time_t _cgroup_creation_time(char *uidcg, uint32_t job_id) {
    ...
    if (snprintf(path, PATH_MAX, "%s/job_%u", uidcg, job_id) >= PATH_MAX) {
        info("snprintf: '%s/job_%u' longer than PATH_MAX of %d", uidcg, job_id, PATH_MAX);
        return 0;
    }
...
static int _indeterminate_multiple(pam_handle_t *pamh, List steps, uid_t uid, step_loc_t **out_stepd) {
    ...
    if (snprintf(uidcg, PATH_MAX, "%s/memory/slurm%s/uid_%u", cg_conf->cgroup_mountpoint, cgroup_suffix, uid) >=
        PATH_MAX) {
        info("snprintf: '%s/memory/slurm%s/uid_%u' longer than PATH_MAX of %d", cg_conf->cgroup_mountpoint,
    ...
    while ((stepd = list_next(itr))) {
        ...
        if (stepd->stepid == SLURM_EXTERN_CONT && (uid == _get_job_uid(stepd))) {
            cgroup_time = _cgroup_creation_time(uidcg, stepd->jobid);
    ...
    if (rc != PAM_SUCCESS) {
        if (opts.action_no_jobs == CALLERID_ACTION_DENY) {
            ...
            debug("uid %u owns no jobs but action_no_jobs=allow", uid);
            rc = PAM_SUCCESS;
        }
    }
...
static int _rpc_network_callerid(struct callerid_conn *conn, char *user_name, uint32_t *job_id) {
    ...
    if (slurm_network_callerid(req, job_id, node_name, MAXHOSTNAMELEN) != SLURM_SUCCESS) {
        debug("From %s port %d as %s: unable to retrieve callerid data from remote slurmd", ip_src_str, req.port_src,
              user_name);
        return SLURM_ERROR;
    } else if (*job_id == NO_VAL) {
        debug("From %s port %d as %s: job indeterminate", ip_src_str, req.port_src, user_name);
        return SLURM_ERROR;
    } else {
        info("From %s port %d as %s: member of job %u", ip_src_str, req.port_src, user_name, *job_id);
...
static int _try_rpc(pam_handle_t *pamh, struct passwd *pwd) {
    ...
    if (inet_ntop(conn.af, &conn.ip_src, ip_src_str, INET6_ADDRSTRLEN) == NULL) {
    ...
    info("From %s port %d as %s: unable to determine source job", ip_src_str, conn.port_src, pwd->pw_name);
...
log_level_t _parse_log_level(pam_handle_t *pamh, const char *log_level_str) {
    ...
    if (endptr && endptr[0]) {
        ...
        if (!strcasecmp(log_level_str, "quiet"))
            ...
            pam_syslog(pamh, LOG_ERR, "unrecognized log level %s, setting to max", log_level_str);
        ...
        if (u >= LOG_LEVEL_END) {
            pam_syslog(pamh, LOG_ERR, "log level %u too high, lowering to max", u);
            u = (unsigned int) LOG_LEVEL_END - 1;
        }
    }
    return u;
    ...
    for (; argc-- > 0; ++argv) {
        if (!xstrncasecmp(*argv, "single_job_skip_rpc=0", 21))
            ...
            if (!xstrncasecmp(v, "deny", 4))
                ...
                pam_syslog(pamh, LOG_ERR, "unrecognized action_no_jobs=%s, setting to 'deny'", v);
            }
            ...
            if (!xstrncasecmp(v, "allow", 5))
                ...
                pam_syslog(pamh, LOG_ERR, "unrecognized action_unknown=%s, setting to 'newest'", v);
            }
            ...
            if (!xstrncasecmp(v, "allow", 5))
                ...
                pam_syslog(pamh, LOG_ERR, "unrecognized action_generic_failure=%s, setting to 'allow'", v);
            }
            ...
            if (!xstrncasecmp(v, "allow", 5))
                ...
                opts.action_adopt_failure = CALLERID_ACTION_DENY;
            else {
                pam_syslog(pamh, LOG_ERR, "unrecognized action_adopt_failure=%s, setting to 'allow'", v);
            }
...
__attribute__((unused)), int argc, const char **argv) {
    ...
    if (!opts.ignore_root &&
        (opts.action_unknown == CALLERID_ACTION_DENY || opts.action_no_jobs != CALLERID_ACTION_ALLOW ||
         opts.action_adopt_failure != CALLERID_ACTION_ALLOW || opts.action_generic_failure != CALLERID_ACTION_ALLOW)) {
        /* Let's get verbose */
        info("===============================");
        info("Danger!!!");
    ...
    if (user_jobs == 0) {
        if (opts.action_no_jobs == CALLERID_ACTION_DENY) {
            ...
            debug("uid %u owns no jobs but action_no_jobs=ignore", pwd.pw_uid);
            rc = PAM_IGNORE;
        }
        goto cleanup;
    } else if (user_jobs == 1) {
        if (opts.single_job_skip_rpc) {
            info("Connection by user %s: user has only one job %u", user_name, stepd->jobid);
            ...
            if (slurmrc == SLURM_SUCCESS || (opts.action_adopt_failure == CALLERID_ACTION_ALLOW))
                rc = PAM_SUCCESS;
            else {
                send_user_msg(pamh, "Access denied by "
                                    PAM_MODULE_NAME
