...
static int _load_config(void) {
    ...
    struct stat statbuf;
    s_p_options_t options[] = {{"SlurmdSpoolDir", S_P_STRING},
                               {"NodeName",       S_P_STRING},
                               {NULL}};
...
static struct passwd *_pw_internal(int mode, uid_t uid, const char *name) {
    ...
    while ((stepd = list_next(itr))) {
        ...
        if ((pwd = stepd_getpw(fd, stepd->protocol_version, mode, uid, name)))
            break;
    }
...
static int _internal_getpw(int mode, uid_t uid, const char *name, struct passwd *pwd, char *buf, size_t buflen,
    ...
    if ((len_name + len_passwd + len_gecos + len_dir + len_shell + 5) > buflen) {
...
int _nss_slurm_getpwnam_r(const char *name, struct passwd *pwd, char *buf, size_t buflen, struct passwd **result) {
    return _internal_getpw(GETPW_MATCH_USER_AND_PID, NO_VAL, name, pwd, buf, buflen, result);

}

int _nss_slurm_getpwuid_r(uid_t uid, struct passwd *pwd, char *buf, size_t buflen, struct passwd **result) {
    return _internal_getpw(GETPW_MATCH_USER_AND_PID, uid, NULL, pwd, buf, buflen, result);
}
...
int _nss_slurm_getpwent_r(struct passwd *pwd, char *buf, size_t buflen, struct passwd **result) {
    /*
    ...
    return _internal_getpw(GETPW_MATCH_PID, NO_VAL, NULL, pwd, buf, buflen, result);
}
...
static struct group **_gr_internal(int mode, gid_t uid, const char *name) {
    ...
    while ((stepd = list_next(itr))) {
        ...
        if ((grps = stepd_getgr(fd, stepd->protocol_version, mode, uid, name)))
            break;
    }
    list_iterator_destroy(itr);
    FREE_NULL_LIST(steps);
...
static int _internal_getgr(int mode, gid_t gid, const char *name, struct group *grp, char *buf, size_t buflen,
                           struct group **result) {
     ...
     * alignment issues.
     */
    if ((len_name + len_passwd + len_mem + 3) > buflen) {
...
int _nss_slurm_getgrnam_r(const char *name, struct group *pwd, char *buf, size_t buflen, struct group **result) {
    return _internal_getgr(GETGR_MATCH_GROUP_AND_PID, NO_VAL, name, pwd, buf, buflen, result);

}

int _nss_slurm_getgrgid_r(gid_t gid, struct group *pwd, char *buf, size_t buflen, struct group **result) {
    return _internal_getgr(GETGR_MATCH_GROUP_AND_PID, gid, NULL, pwd, buf, buflen, result);
}
...
int _nss_slurm_getgrent_r(struct group *grp, char *buf, size_t buflen, struct group **result) {
    return _internal_getgr(GETGR_MATCH_PID, NO_VAL, NULL, grp, buf, buflen, result);
}
