...
static int loop_through_switches(netloc_topology_t *topology, netloc_map_t *map, netloc_dt_lookup_table_t *switches);

// Loop through and parse all of the edges for a switch
static int loop_through_edges(netloc_topology_t *topology, netloc_map_t *map, netloc_node_t *node, const char *src_name,
                              FILE *f_temp);


// Add a switch connection and its link speed to the switch list
static int
add_switch_connection(netloc_edge_t **edges, int idx, int num_edges, const char *src_name, const char *dst_name,
                      char *switch_str);
...
static int add_node_connection(netloc_topology_t *topology, netloc_map_t *map, netloc_edge_t *edge, char *node_str);

// Find a node group that matches the specifications given
static int find_node_group(int cpus, int cores_per_socket, int threads_per_core, int memory, const char *dst_name);

// Make a new node group in the table and fill in information
static void make_new_node_group(int cpus, int cores_per_socket, int threads_per_core, int memory, const char *dst_name);
...
static int
get_node_name_and_topology(netloc_topology_t *topology, netloc_map_t *map, netloc_node_t *node, const char **name,
                           hwloc_topology_t *hw_topo);

// Gets the name of a switch in the network
static int get_switch_name(netloc_topology_t *topology, netloc_map_t *map, netloc_node_t *node, const char **name);
...
static int make_new_switch_name(netloc_topology_t *topology, netloc_map_t *map, netloc_node_t *node, const char **name);
...
int main(int argc, char **argv) {
    ...
    if (0 != parse_args(argc, argv)) {
        printf("Usage: %s\n"
               ...
               "\t[%s|%s] [--help|-h]\n", argv[0], ARG_DATADIR, ARG_SHORT_DATADIR, ARG_OUTDIR, ARG_SHORT_OUTDIR,
               ARG_FABRIC, ARG_SHORT_FABRIC, ARG_VERBOSE, ARG_SHORT_VERBOSE);
...
static int parse_args(int argc, char **argv) {
    ...
    for (i = 1; i < argc; ++i) {
        ...
        if ((0 == strncmp(ARG_OUTDIR, argv[i], strlen(ARG_OUTDIR))) ||
            ...
            return NETLOC_ERROR;
        } else if (0 == strcmp(ARG_FABRIC, argv[i]) || 0 == strcmp(ARG_SHORT_FABRIC, argv[i])) {
            i++;
            if (i >= argc) {
                fprintf(stderr, "Error: Must supply an argument to %s (fabric ID)\n", ARG_FABRIC);
            }
...
static netloc_topology_t setup_topology(char *data_uri) {
    ...
    free(search_uri);
    if (NETLOC_SUCCESS != ret) {
        fprintf(stderr, "Error: netloc_find_network return error (%d)\n"
...
static int loop_through_switches(netloc_topology_t *topology, netloc_map_t *map, netloc_dt_lookup_table_t *switches) {
    ...
    while (!netloc_lookup_table_iterator_at_end(hti)) {
        ...
        netloc_node_t *node = (netloc_node_t *) netloc_lookup_table_access(*switches, key);
        ...
        if (NETLOC_SUCCESS != ret) {
            if (verbose) {
                fprintf(stderr, "Did not find data for any nodes attached to switch %s\n",
                        netloc_pretty_print_node_t(node));
            }
            continue;
        }
...
static int loop_through_edges(netloc_topology_t *topology, netloc_map_t *map, netloc_node_t *node, const char *src_name,
    ...
    if (NETLOC_SUCCESS != ret) {
        fprintf(stderr, "Error: get_all_edges_by_id returned %d for"
    ...
    for (i = 0; i < num_edges; i++) {
        (verbose) ? printf("\tEdge %2d - Speed: %s, Width: %s - ", i, edges[i]->speed, edges[i]->width) : 0;

        if (NETLOC_NODE_TYPE_SWITCH == edges[i]->dest_node->node_type) {
            // get the dest_node name
            const char *dst_name;
            ret = get_switch_name(topology, map, edges[i]->dest_node, &dst_name);
            if (NETLOC_SUCCESS != ret) {
                if (verbose) {
                    fprintf(stderr, "Did not find data for any nodes attached to switch %s\n",
                            netloc_pretty_print_node_t(node));
            ...
            ret = add_switch_connection(edges, i, num_edges, src_name, dst_name, switch_str);
...
add_switch_connection(netloc_edge_t **edges, int idx, int num_edges, const char *src_name, const char *dst_name,
    ...
    (verbose) ? printf("Dst:%9s - (%s - %s) [%20s][%18lu]/[%7s] - (%d edges)\n", dst_name,
                       netloc_decode_network_type(dn->network_type), netloc_decode_node_type(dn->node_type),
                       dn->physical_id, dn->physical_id_int, dn->logical_id, dn->num_edges) : 0;
        ...
        if (edges[i]->dest_node->physical_id_int == current_ID) {
            ...
            if (0 >= link_speed) {
                fprintf(stderr, "\nError: invalid connection width %s or "
                                "speed %s between %s and %s\n", edges[idx]->width, edges[idx]->speed, src_name,
                        dst_name);
                return NETLOC_ERROR;
            }
...
static int add_node_connection(netloc_topology_t *topology, netloc_map_t *map, netloc_edge_t *edge, char *node_str) {
    ...
    ret = get_node_name_and_topology(topology, map, edge->dest_node, &dst_name, &dst_hw_topo);
    ...
    // Find a node group that matches the specifications given
    ret = find_node_group(cpus, cores_per_socket, threads_per_core, memory, dst_name);

    // if couldn't find a matching node group, create a new one
    if (ret == node_group_cnt) {
        // Make a new node group in the table and fill in information
        make_new_node_group(cpus, cores_per_socket, threads_per_core, memory, dst_name);
    }

    netloc_node_t *dn = edge->dest_node;
    (verbose) ? printf("(%s - %s) [%20s][%18lu]/[%7s] - (%d edges)\n", netloc_decode_network_type(dn->network_type),
                       netloc_decode_node_type(dn->node_type), dn->physical_id, dn->physical_id_int, dn->logical_id,
                       dn->num_edges) : 0;
...
static int find_node_group(int cpus, int cores_per_socket, int threads_per_core, int memory, const char *dst_name) {
    ...
    for (j = 0; j < node_group_cnt; j++) {
        ...
        if ((node_group_table[j].cpus == cpus) && (node_group_table[j].memory == memory) &&
            ...
            if ((strlen(node_group_table[j].node_name) + strlen(dst_name) + 3) >= node_group_table[j].node_name_len) {
                node_group_table[j].node_name_len *= 2;
                char *temp_node_name = (char *) realloc(node_group_table[j].node_name,
            ...
            sprintf(node_group_table[j].node_name, "%s,%s", node_group_table[j].node_name, dst_name);
            return j;
        }
    }
    return j;
...
// Make a new node group in the table and fill in information
static void
make_new_node_group(int cpus, int cores_per_socket, int threads_per_core, int memory, const char *dst_name) {
    ...
    if (node_group_cnt >= node_groups_max) {
        node_groups_max *= 2;
        node_group *temp_node_group = realloc(node_group_table, sizeof(node_group) * node_groups_max);
...
int save_topology_data_to_file() {
    ...
    for (j = 0; j < node_group_cnt; j++) {
        fprintf(f, "# NodeName=%s CPUs=%d RealMemory=%d CoresPerSocket=%d "
                   "ThreadsPerCore=%d State=UNKNOWN\n", node_group_table[j].node_name, node_group_table[j].cpus,
                node_group_table[j].memory, node_group_table[j].cores_per_socket, node_group_table[j].threads_per_core);
    }
    fprintf(f, "\n###########################################################"
    ...
    if (ret != switch_name_cnt) {
        *name = switch_name_table[ret]->sw_name;
        ...
        if (switch_name_cnt == switch_name_max) {
            switch_name_max *= 2;
            switch_name **temp_switch_name_table = realloc(switch_name_table, sizeof(switch_name) * switch_name_max);
...
// Make a new switch_name entry in the table and fill in information
static int
make_new_switch_name(netloc_topology_t *topology, netloc_map_t *map, netloc_node_t *node, const char **name) {
    int ret, i, num_edges;
    netloc_edge_t **edges = NULL;
    ...
    ret = netloc_get_all_edges(*topology, node, &num_edges, &edges);
    if (NETLOC_SUCCESS != ret) {
        fprintf(stderr, "Error: netloc_get_all_edges returned %d for"
    ...
    for (i = 0; i < num_edges; i++) {
        if (NETLOC_NODE_TYPE_HOST == edges[i]->dest_node->node_type) {
            ...
            ret = get_node_name_and_topology(topology, map, edges[i]->dest_node, &node_name, &dst_hw_topo);
    ...
    if (num_edges == i) {
        if (verbose) {
            fprintf(stderr, "Skipping switch because no data was available for attached nodes:\n"
                            "\t%s\n", netloc_pretty_print_node_t(node));
        }
