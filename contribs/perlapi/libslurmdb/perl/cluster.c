...
int av_to_cluster_grouping_list(AV *av, List grouping_list) {
    SV **svp;
    char *str = NULL;
    int i, elements = 0;
    ...
    for (i = 0; i < elements; i++) {
        if ((svp = av_fetch(av, i, FALSE))) {
            ...
            Perl_warn(aTHX_
            "error fetching group from grouping list");
            return -1;
        }
    }
    return 0;
...
int hv_to_assoc_cond(HV *hv, slurmdb_assoc_cond_t *assoc_cond) {
    AV *element_av;
    SV **svp;
    char *str = NULL;
    int i, elements = 0;
...
int hv_to_cluster_cond(HV *hv, slurmdb_cluster_cond_t *cluster_cond) {
    AV *element_av;
    char *str = NULL;
    int i, elements = 0;
...
int hv_to_job_cond(HV *hv, slurmdb_job_cond_t *job_cond) {
    AV *element_av;
    SV **svp;
    char *str = NULL;
    int i, elements = 0;
...
int hv_to_user_cond(HV *hv, slurmdb_user_cond_t *user_cond) {
    AV *element_av;
    SV **svp;
    char *str = NULL;
    int i, elements = 0;
...
int tres_rec_to_hv(slurmdb_tres_rec_t *rec, HV *hv) {
...
int report_job_grouping_to_hv(slurmdb_report_job_grouping_t *rec, HV *hv) {
    AV *my_av;
    HV *rh;
    ...
    if (rec->tres_list) {
        ...
        while ((tres_rec = slurm_list_next(itr))) {
            ...
            if (tres_rec_to_hv(tres_rec, rh) < 0) {
                Perl_warn(aTHX_
                "Failed to convert a tres_rec to a hv");
...
int report_acct_grouping_to_hv(slurmdb_report_acct_grouping_t *rec, HV *hv) {
    AV *my_av;
    HV *rh;
    ...
    if (rec->groups) {
        ...
        while ((jgr = slurm_list_next(itr))) {
            ...
            if (report_job_grouping_to_hv(jgr, rh) < 0) {
                Perl_warn(aTHX_
                "Failed to convert a report_job_grouping to a hv");
                slurm_list_iterator_destroy(itr);
                return -1;
            } else {
                av_push(my_av, newRV((SV *) rh));
...
int report_cluster_grouping_to_hv(slurmdb_report_cluster_grouping_t *rec, HV *hv) {
    AV *my_av;
    HV *rh;
    ...
    if (rec->acct_list) {
        ...
        while ((agr = slurm_list_next(itr))) {
            ...
            if (report_acct_grouping_to_hv(agr, rh) < 0) {
                Perl_warn(aTHX_
                "Failed to convert a report_acct_grouping to a hv");
                slurm_list_iterator_destroy(itr);
                return -1;
            } else {
                av_push(my_av, newRV((SV *) rh));
...
int cluster_grouping_list_to_av(List list, AV *av) {
    HV *rh;
    ListIterator itr = NULL;
    ...
    if (list) {
        ...
        while ((rec = slurm_list_next(itr))) {
            ...
            if (report_cluster_grouping_to_hv(rec, rh) < 0) {
                Perl_warn(aTHX_
                "Failed to convert a report_cluster_grouping to a hv");
                slurm_list_iterator_destroy(itr);
                return -1;
            } else {
...
int cluster_accounting_rec_to_hv(slurmdb_cluster_accounting_rec_t *ar, HV *hv) {
    HV *rh;
...
int cluster_rec_to_hv(slurmdb_cluster_rec_t *rec, HV *hv) {
    AV *my_av;
    HV *rh;
    ListIterator itr = NULL;
    ...
    if (rec->accounting_list) {
        ...
        while ((ar = slurm_list_next(itr))) {
            ...
            if (cluster_accounting_rec_to_hv(ar, rh) < 0) {
                Perl_warn(aTHX_
                "Failed to convert a cluster_accounting_rec to a hv");
                slurm_list_iterator_destroy(itr);
                return -1;
            } else {
                av_push(my_av, newRV((SV *) rh));
...
int report_assoc_rec_to_hv(slurmdb_report_assoc_rec_t *rec, HV *hv) {
    AV *my_av;
    HV *rh;
    slurmdb_tres_rec_t *tres_rec = NULL;
    ListIterator itr = NULL;
...
int report_cluster_rec_to_hv(slurmdb_report_cluster_rec_t *rec, HV *hv) {
    AV *my_av;
    HV *rh;
    ...
    if (rec->assoc_list) {
        ...
        while ((ar = slurm_list_next(itr))) {
            ...
            if (report_assoc_rec_to_hv(ar, rh) < 0) {
                Perl_warn(aTHX_
                "Failed to convert a report_assoc_rec to a hv");
                slurm_list_iterator_destroy(itr);
                return -1;
            } else {
                av_push(my_av, newRV((SV *) rh));
    ...
    if (rec->user_list) {
        ...
        while ((ur = slurm_list_next(itr))) {
            ...
            if (report_user_rec_to_hv(ur, rh) < 0) {
                Perl_warn(aTHX_
                "Failed to convert a report_user_rec to a hv");
                slurm_list_iterator_destroy(itr);
                return -1;
            } else {
                av_push(my_av, newRV((SV *) rh));
...
int report_cluster_rec_list_to_av(List list, AV *av) {
    HV *rh;
    ListIterator itr = NULL;
    ...
    if (list) {
        ...
        while ((rec = slurm_list_next(itr))) {
            ...
            if (report_cluster_rec_to_hv(rec, rh) < 0) {
                Perl_warn(aTHX_
                "Failed to convert a report_cluster_rec to a hv");
                slurm_list_iterator_destroy(itr);
                return -1;
            } else {
                av_push(av, newRV((SV *) rh));
...
int report_user_rec_to_hv(slurmdb_report_user_rec_t *rec, HV *hv) {
    AV *my_av;
    HV *rh;
    char *acct;
    slurmdb_report_assoc_rec_t *ar = NULL;
...
int stats_to_hv(slurmdb_stats_t *stats, HV *hv) {
...
int step_rec_to_hv(slurmdb_step_rec_t *rec, HV *hv) {
...
int job_rec_to_hv(slurmdb_job_rec_t *rec, HV *hv) {
...
int hv_to_qos_cond(HV *hv, slurmdb_qos_cond_t *qos_cond) {
    AV *element_av;
    char *str = NULL;
    int i, elements = 0;
...
int qos_rec_to_hv(slurmdb_qos_rec_t *rec, HV *hv, List all_qos) {
    char *preempt = NULL;
