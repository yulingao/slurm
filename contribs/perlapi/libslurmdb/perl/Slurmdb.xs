...
int slurmdb_connection_close(db_conn)

void *db_conn

SV *slurmdb_clusters_get(db_conn, conditions)
...
if (
hv_to_cluster_cond(conditions, cluster_cond
) < 0) {
XSRETURN_UNDEF;
}
...
while ((
rec = slurm_list_next(itr)
)) {
rh = (HV *) sv_2mortal((SV *) newHV());
if (
cluster_rec_to_hv(rec, rh
) < 0) {
XSRETURN_UNDEF;
}
av_push(results, newRV((SV *)
rh));
}
...
if (
hv_to_assoc_cond(assoc_condition, assoc_cond
) < 0) {
XSRETURN_UNDEF;
}
results = (AV *) sv_2mortal((SV * )
...
if (list) {
if (
report_cluster_rec_list_to_av(list, results
) < 0) {
XSRETURN_UNDEF;
}
slurm_list_destroy(list);
...
if (
hv_to_job_cond(job_condition, job_cond
) < 0) {
XSRETURN_UNDEF;
}
if (
av_to_cluster_grouping_list(grouping_array, grouping_list
) < 0) {
XSRETURN_UNDEF;
}
results = (AV *) sv_2mortal((SV * )
...
CODE:
list = slurmdb_report_job_sizes_grouped_by_account(db_conn, job_cond, grouping_list, flat_view, acct_as_parent);
if (list) {
if (
cluster_grouping_list_to_av(list, results
) < 0) {
XSRETURN_UNDEF;
}
slurm_list_destroy(list);
...
if (
hv_to_user_cond(user_condition, user_cond
) < 0) {
XSRETURN_UNDEF;
}
results = (AV *) sv_2mortal((SV * )
...
CODE:
list = slurmdb_report_user_top_usage(db_conn, user_cond, group_accounts);
if (list) {
if (
report_cluster_rec_list_to_av(list, results
) < 0) {
...
if (
hv_to_job_cond(conditions, job_cond
) < 0) {
XSRETURN_UNDEF;
}
results = (AV *) sv_2mortal((SV * )
...
if (
hv_to_qos_cond(conditions, qos_cond
) < 0) {
XSRETURN_UNDEF;
}
results = (AV *) sv_2mortal((SV * )
