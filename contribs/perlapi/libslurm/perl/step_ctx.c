 ...
 */
int hv_to_slurm_step_ctx_params(HV *hv, slurm_step_ctx_params_t *params) {
 ...
 */
int hv_to_slurm_step_launch_params(HV *hv, slurm_step_launch_params_t *params) {
    int i, num_keys;
    STRLEN vlen;
    I32 klen;
    SV **svp;
    ...
    if ((svp = hv_fetch(hv, "argv", 4, FALSE))) {
        if (SvROK(*svp) && SvTYPE(SvRV(*svp)) == SVt_PVAV) {
            ...
            if (params->argc > 0) {
                ...
                Newz(0, params->argv, (int32_t) (params->argc + 1),
                char*);
    ...
    if ((svp = hv_fetch(hv, "env", 3, FALSE))) {
        if (SvROK(*svp) && SvTYPE(SvRV(*svp)) == SVt_PVHV) {
            ...
            while ((val = hv_iternextsv(environ_hv, &env_key, &klen))) {
                ...
                Newz(0, (*(params->env + i)), klen + vlen + 2,
                char);
    ...
    if ((svp = hv_fetch(hv, "spank_job_env", 13, FALSE))) {
        if (SvROK(*svp) && SvTYPE(SvRV(*svp)) == SVt_PVHV) {
            ...
            Newz(0, params->spank_job_env, num_keys + 1,
            char*);
            ...
            while ((val = hv_iternextsv(environ_hv, &env_key, &klen))) {
                env_val = SvPV(val, vlen);
                Newz(0, (*(params->spank_job_env + i)), klen + vlen + 2,
                char);
 ...
 */
static void _free_env(char **environ) {
    int i;
    if (!environ)
        return;
    for (i = 0; *(environ + i); i++)
 ...
 */
void free_slurm_step_launch_params_memory(slurm_step_launch_params_t *params) {
    if (params->argv)
...
static int launch_tasks_response_msg_to_hv(launch_tasks_response_msg_t *resp_msg, HV *hv) {
    AV *av, *av2;
    int i;
...
static int task_exit_msg_to_hv(task_exit_msg_t *exit_msg, HV *hv) {
    AV *av;
    int i;
...
static void set_thread_perl(void) {
...
static void clear_thread_callbacks(void *arg) {
...
static void set_thread_callbacks(void) {
    CLONE_PARAMS params;
...
static void step_complete_cb(srun_job_complete_msg_t *comp_msg) {
    HV *hv;
...
static void step_signal_cb(int signo) {
    thread_callbacks_t *cbs = NULL;
...
static void step_timeout_cb(srun_timeout_msg_t *timeout_msg) {
    HV *hv;
    thread_callbacks_t *cbs = NULL;
...
static void task_start_cb(launch_tasks_response_msg_t *resp_msg) {
    HV *hv;
    thread_callbacks_t *cbs = NULL;
...
static void task_finish_cb(task_exit_msg_t *exit_msg) {
    HV *hv;
    thread_callbacks_t *cbs = NULL;
...
slurm_step_launch_callbacks_t slcb = {step_complete_cb, step_signal_cb, step_timeout_cb, task_start_cb, task_finish_cb};
