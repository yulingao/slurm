 ...
 */
int hv_to_job_desc_msg(HV *hv, job_desc_msg_t *job_desc) {
    SV **svp;
    HV *environ_hv;
    AV *argv_av;
    SV *val;
    ...
    if ((svp = hv_fetch(hv, "argv", 4, FALSE))) {
        if (SvROK(*svp) && SvTYPE(SvRV(*svp)) == SVt_PVAV) {
            ...
            if (job_desc->argc > 0) {
                Newz(0, job_desc->argv, (int32_t) (job_desc->argc + 1),
                char*);
    ...
    if ((svp = hv_fetch(hv, "environment", 11, FALSE))) {
        if (SvROK(*svp) && SvTYPE(SvRV(*svp)) == SVt_PVHV) {
            ...
            Newz(0, job_desc->environment, num_keys + 1,
            char*);
            ...
            while ((val = hv_iternextsv(environ_hv, &env_key, &klen))) {
                ...
                Newz(0, (*(job_desc->environment + i)), klen + vlen + 2,
                char);
    ...
    if ((svp = hv_fetch(hv, "spank_job_env", 13, FALSE))) {
        if (SvROK(*svp) && SvTYPE(SvRV(*svp)) == SVt_PVHV) {
            ...
            Newz(0, job_desc->spank_job_env, num_keys + 1,
            char*);
            ...
            while ((val = hv_iternextsv(environ_hv, &env_key, &klen))) {
                env_val = SvPV(val, vlen);
                Newz(0, (*(job_desc->spank_job_env + i)), klen + vlen + 2,
                char);
 ...
 */
void free_job_desc_msg_memory(job_desc_msg_t *msg) {
    if (msg->argv)
        Safefree(msg->argv);
 ...
 */
int resource_allocation_response_msg_to_hv(resource_allocation_response_msg_t *resp_msg, HV *hv) {
    AV *av;
    int i;
 ...
 */
int submit_response_msg_to_hv(submit_response_msg_t *resp_msg, HV *hv) {
    STORE_FIELD(hv, resp_msg, job_id, uint32_t);
 ...
 */
int job_sbcast_cred_msg_to_hv(job_sbcast_cred_msg_t *msg, HV *hv) {
    AV *av;
    int i;
...
int srun_job_complete_msg_to_hv(srun_job_complete_msg_t *msg, HV *hv) {
    STORE_FIELD(hv, msg, job_id, uint32_t);
    STORE_FIELD(hv, msg, step_id, uint32_t);
    return 0;
}

int srun_timeout_msg_to_hv(srun_timeout_msg_t *msg, HV *hv) {
    STORE_FIELD(hv, msg, job_id, uint32_t);
    STORE_FIELD(hv, msg, step_id, uint32_t);
...
void sarb_cb(uint32_t job_id) {
    dSP;

    if (sarb_cb_sv == NULL || sarb_cb_sv == &PL_sv_undef)
        return;
...
static int srun_ping_msg_to_hv(srun_ping_msg_t *msg, HV *hv) {
    STORE_FIELD(hv, msg, job_id, uint32_t);
    STORE_FIELD(hv, msg, step_id, uint32_t);
    return 0;
}

static int srun_user_msg_to_hv(srun_user_msg_t *msg, HV *hv) {
    STORE_FIELD(hv, msg, job_id, uint32_t);
    STORE_FIELD(hv, msg, msg, charp);
    return 0;
}

static int srun_node_fail_msg_to_hv(srun_node_fail_msg_t *msg, HV *hv) {
    STORE_FIELD(hv, msg, job_id, uint32_t);
...
static void ping_cb(srun_ping_msg_t *msg) {
    HV *hv;
    dSP;

    if (ping_cb_sv == NULL || ping_cb_sv == &PL_sv_undef) {
        return;
    }
    hv = newHV();
...
static void job_complete_cb(srun_job_complete_msg_t *msg) {
    HV *hv;
    dSP;

    if (job_complete_cb_sv == NULL || job_complete_cb_sv == &PL_sv_undef) {
        return;
    }
    hv = newHV();
...
static void timeout_cb(srun_timeout_msg_t *msg) {
    HV *hv;
    dSP;

    if (timeout_cb_sv == NULL || timeout_cb_sv == &PL_sv_undef) {
        return;
    }
    hv = newHV();
...
static void user_msg_cb(srun_user_msg_t *msg) {
    HV *hv;
    dSP;

    if (user_msg_cb_sv == NULL || user_msg_cb_sv == &PL_sv_undef) {
        return;
    }
    hv = newHV();
...
static void node_fail_cb(srun_node_fail_msg_t *msg) {
    HV *hv;
    dSP;

    if (node_fail_cb_sv == NULL || node_fail_cb_sv == &PL_sv_undef) {
        return;
    }
    hv = newHV();
...
slurm_allocation_callbacks_t sacb = {ping_cb, job_complete_cb, timeout_cb, user_msg_cb, node_fail_cb};
