 ...
 */
int slurm_ctl_conf_to_hv(slurm_ctl_conf_t *conf, HV *hv) {
    AV *av;
    int i;
 ...
 */
int hv_to_slurm_ctl_conf(HV *hv, slurm_ctl_conf_t *conf) {
    SV **svp;
    AV *av;
    STRLEN len;
    int i, n;
    ...
    if (svp && SvROK(*svp) && SvTYPE(SvRV(*svp)) == SVt_PVAV) {
        ...
        for (i = 0; i < n; i++) {
            conf->control_addr[i] = (char *) SvPV(*(av_fetch(av, i, FALSE)), len);
        }
    } else {
        /* nothing to do */
    }
    ...
    if (svp && SvROK(*svp) && SvTYPE(SvRV(*svp)) == SVt_PVAV) {
        ...
        for (i = 0; i < n; i++) {
            conf->control_machine[i] = (char *) SvPV(*(av_fetch(av, i, FALSE)), len);
        }
    } else {
        /* nothing to do */
    }
 ...
 */
int slurmd_status_to_hv(slurmd_status_t *status, HV *hv) {
 ...
 */
int hv_to_slurmd_status(HV *hv, slurmd_status_t *status) {
 ...
 */
int hv_to_step_update_request_msg(HV *hv, step_update_request_msg_t *update_msg) {
