...
static uint32_t _threads_per_core(char *host) {
    ...
    for (i = 0; i < job_node_ptr->record_count; i++) {
        if (job_node_ptr->node_array[i].name && !strcmp(host, job_node_ptr->node_array[i].name)) {
...
static int _job_resrcs_to_hv(job_info_t *job_info, HV *hv) {
    ...
    if (!job_resrcs || !job_resrcs->core_bitmap || ((last = slurm_bit_fls(job_resrcs->core_bitmap)) == -1))
        return 0;
 ...
 */
        if (strcmp(tmp1, tmp2) || (last_mem_alloc_ptr != job_resrcs->memory_allocated) ||
            ...
            if (slurm_hostlist_count(hl_last)) {
                ...
                hv_store_uint64_t(nr_hv, "mem", last_mem_alloc_ptr ? last_mem_alloc : 0);
 ...
 */
int job_info_to_hv(job_info_t *job_info, HV *hv) {
    int j;
    AV *av;
 ...
 */
int hv_to_job_info(HV *hv, job_info_t *job_info) {
    SV **svp;
    AV *av;
    int i, n;
 ...
 */
int job_info_msg_to_hv(job_info_msg_t *job_info_msg, HV *hv) {
    int i;
    HV *hv_info;
    AV *av;
 ...
 */
int hv_to_job_info_msg(HV *hv, job_info_msg_t *job_info_msg) {
    SV **svp;
    AV *av;
    int i, n;
