 ...
 */
int node_info_to_hv(node_info_t *node_info, HV *hv) {
    ...
    slurm_get_select_nodeinfo(node_info->select_nodeinfo, SELECT_NODEDATA_SUBCNT, NODE_STATE_ALLOCATED, &alloc_cpus);
 ...
 */
int hv_to_node_info(HV *hv, node_info_t *node_info) {
 ...
 */
int node_info_msg_to_hv(node_info_msg_t *node_info_msg, HV *hv) {
    int i;
    HV *hv_info;
    AV *av;
    ...
    for (i = 0; i < node_info_msg->record_count; i++) {
        hv_info = newHV();
        if (node_info_msg->node_array[i].name && node_info_to_hv(node_info_msg->node_array + i, hv_info) < 0) {
 ...
 */
int hv_to_node_info_msg(HV *hv, node_info_msg_t *node_info_msg) {
    SV **svp;
    AV *av;
    int i, n;
 ...
 */
int hv_to_update_node_msg(HV *hv, update_node_msg_t *update_msg) {
