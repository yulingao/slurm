...
static void _send_denial_msg(pam_handle_t *pamh, struct _options *opts, const char *user, uid_t uid);
...
PAM_EXTERN int pam_sm_acct_mgmt(pam_handle_t *pamh, int flags, int argc, const char **argv) {
    ...
    if ((retval != PAM_SUCCESS) || (user == NULL) || (*user == '\0')) {
        _log_msg(LOG_ERR, "unable to identify user: %s", pam_strerror(pamh, retval));
    ...
    if ((auth != PAM_SUCCESS) || (!opts.disable_sys_info)) {
        _log_msg(LOG_INFO, "access %s for user %s (uid=%d)", (auth == PAM_SUCCESS) ? "granted" : "denied", user, uid);
    }
 ...
 * OUT name
 */
static int _gethostname_short(char *name, size_t len) {
...
static int _slurm_match_allocation(uid_t uid) {
    ...
    if (!(nodename = slurm_conf_get_nodename(hostname))) {
        if (!(nodename = slurm_conf_get_aliased_nodename())) {
            ...
            if (!(nodename = slurm_conf_get_nodename("localhost"))) {
                _log_msg(LOG_ERR, "slurm_conf_get_aliased_nodename: "
                                  "no hostname found");
                return 0;
            }
        }
    ...
    if (slurm_load_job_user(&msg, uid, SHOW_ALL) < 0) {
        _log_msg(LOG_ERR, "slurm_load_job_user: %s", slurm_strerror(errno));
        return 0;
    }
    ...
    for (i = 0; i < msg->record_count; i++) {
        ...
        if (j->job_state == JOB_RUNNING) {
            ...
            if (_hostrange_member(nodename, j->nodes)) {
                DBG ("user %ld allocated node %s in job %ld", uid, nodename, j->job_id);
                authorized = 1;
                break;
            } else {
                char *nodename;
...
static void _send_denial_msg(pam_handle_t *pamh, struct _options *opts, const char *user, uid_t uid) {
     ...
     */
    n = snprintf(str, sizeof(str), "%sAccess denied: user %s (uid=%d) has no active jobs on this node.%s",
...
extern void libpam_slurm_init(void) {
    ...
    if (snprintf(libslurmname, sizeof(libslurmname), "libslurm.so.%d", SLURM_API_CURRENT) >= sizeof(libslurmname)) {
        _log_msg(LOG_ERR, "Unable to write libslurmname\n");
    } else if ((slurm_h = dlopen(libslurmname, RTLD_NOW | RTLD_GLOBAL))) {
        return;
    } else {
