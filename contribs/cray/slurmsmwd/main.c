...
static const char *event_description[] = {"Invalid Event", "ec_node_failed", "ec_node_unavailable"};

typedef enum event_type {
    EVENT_INVALID = 0, EVENT_NODE_FAILED, EVENT_NODE_UNAVAILABLE, EVENT_LIMIT
} event_type_t;
...
static int _getnid(const char *cname, int dim) {
    ...
    sscanf(cname, "c%d-%dc%ds%dn%d", &cabinet, &row, &chassis, &slot, &node);

    return cabinet * nodes_per_cabinet + row * nodes_per_row + chassis * nodes_per_chassis + slot * nodes_per_slot +
           node;
}
...
static void *_xtconsumer_listen(void *arg) {
    int xtc_fd = 0;
    char *xtc_argv[] = {"xtconsumer", "-b", "ec_node_unavailable", "ec_node_failed"};
    char *line_ptr = NULL;
    char *buffer = NULL;
    size_t buffer_sz = 0;
    size_t buffer_off = 0;
    ...
    while (!stop_running) {
        ...
        if ((fds.revents & POLLIN) == 0)
            break;
        i = read(xtc_fd, buffer + buffer_off, buffer_sz - buffer_off);
        ...
        while ((line_ptr = strchr(buffer, '\n')) != NULL) {
            ...
            *line_ptr = '\0';
            ...
            if (event == EVENT_NODE_FAILED || event == EVENT_NODE_UNAVAILABLE) {
                info("received event: %s, nodelist: %s", event_description[event], node_list);
...
static void _update_logging(void) {
    ...
    if (debug_level) {
        slurmsmwd_debug_level = MIN((LOG_LEVEL_INFO + debug_level), (LOG_LEVEL_END - 1));
    }
...
int main(int argc, char **argv) {
    ...
    if (pthread_create(&signal_handler_thread, &thread_attr, _signal_handler, NULL))
        fatal("pthread_create %m");
    slurm_attr_destroy(&thread_attr);

    slurm_attr_init(&thread_attr);
    if (pthread_create(&processing_thread, &thread_attr, &_process_data, NULL))
        fatal("pthread_create %m");
    slurm_attr_destroy(&thread_attr);

    while (!stop_running) {
        slurm_attr_init(&thread_attr);
        if (pthread_create(&xtc_thread, &thread_attr, &_xtconsumer_listen, NULL))
            fatal("pthread_create %m");
        slurm_attr_destroy(&thread_attr);
