...
static s_p_hashtbl_t *_config_make_tbl(char *filename) {
    ...
    if (!(tbl = s_p_hashtbl_create(knl_conf_file_options))) {
        error("%s: s_p_hashtbl_create error: %s", prog_name, slurm_strerror(slurm_get_errno()));
        return tbl;
    }

    if (s_p_parse_file(tbl, NULL, filename, false) == SLURM_ERROR) {
        error("%s: s_p_parse_file error: %s", prog_name, slurm_strerror(slurm_get_errno()));
...
static char *_run_script(char **script_argv, int *status) {
    ...
    if (pipe(pfd) != 0) {
        error("%s: pipe(): %s", prog_name, slurm_strerror(slurm_get_errno()));
        *status = 127;
    ...
    if ((cpid = fork()) == 0) {
        ...
        error("%s: execv(): %s", prog_name, slurm_strerror(slurm_get_errno()));
        exit(127);
    } else if (cpid < 0) {
        close(pfd[0]);
        close(pfd[1]);
        error("%s: fork(): %s", prog_name, slurm_strerror(slurm_get_errno()));
        *status = 127;
        resp = xstrdup("System error");
        return resp;
    } else {
        ...
        while (1) {
            ...
            if (new_wait <= 0) {
                error("%s: poll() timeout @ %d msec", prog_name, capmc_timeout);
                break;
            }
            ...
            if (i == 0) {
                ...
                error("%s: poll(): %s", prog_name, slurm_strerror(slurm_get_errno()));
                break;
            }
            ...
            if (i == 0) {
                ...
                if (errno == EAGAIN)
                    continue;
                error("%s: read(): %s", prog_name, slurm_strerror(slurm_get_errno()));
                break;
            } else {
                resp_offset += i;
...
static int _update_all_nodes(char *host_list) {
    ...
    if (mcdram_mode) {
        ...
        * Example: "capmc set_mcdram_cfg –n 43 –m cache" */
        ...
        for (retry = 0;; retry++) {
            ...
            if ((status == 0) || (resp_msg && strcasestr(resp_msg, "Success"))) {
                debug("%s: set_mcdram_cfg sent to %s", prog_name, argv[5]);
                xfree(resp_msg);
                break;
            }
            error("%s: capmc(%s,%s,%s,%s,%s): %d %s", prog_name, argv[1], argv[2], argv[3], argv[4], argv[5], status,
                  resp_msg);
            if (resp_msg && strstr(resp_msg, "Could not lookup") && (retry <= capmc_retries)) {
    ...
    if (numa_mode && (rc == 0)) {
        ...
        for (retry = 0;; retry++) {
            resp_msg = _run_script(argv, &status);
            if ((status == 0) || (resp_msg && strcasestr(resp_msg, "Success"))) {
                debug("%s: set_numa_cfg sent to %s", prog_name, argv[5]);
                xfree(resp_msg);
                break;
            }
            error("%s: capmc(%s,%s,%s,%s,%s): %d %s", prog_name, argv[1], argv[2], argv[3], argv[4], argv[5], status,
...
//		argv[5] = "Change KNL mode";
        for (retry = 0;; retry++) {
            resp_msg = _run_script(argv, &status);
            if ((status == 0) || (resp_msg && strcasestr(resp_msg, "Success"))) {
                debug("%s: node_reinit sent to %s", prog_name, argv[3]);
                xfree(resp_msg);
                break;
            }
            error("%s: capmc(%s,%s,%s): %d %s", prog_name, argv[1], argv[2], argv[3], status, resp_msg);
            if (resp_msg && (strstr(resp_msg, "Could not lookup") || strstr(resp_msg, "Internal server error")) &&
                (retry <= capmc_retries)) {
                /* State Manager is down. Sleep and retry */
                sleep(1);
                xfree(resp_msg);
...
static uint32_t *_json_parse_nids(json_object *jobj, char *key, int *num) {
    ...
    *num = 0;
    ...
    if (!j_array) {
        debug("%s: key=%s not found in nid specification", prog_name, key);
        return NULL;
    }
    ...
    for (i = 0; i < cnt; i++) {
        ...
        if (j_type != json_type_int) {
            error("%s: Unable to parse nid specification", prog_name);
            break;
        } else {
...
static void _wait_all_nodes_on(void) {
    ...
    while ((difftime(time(NULL), start_time) < (30 * 60)) && (bit_set_count(node_bitmap) > 0)) {
...
int main(int argc, char *argv[]) {
    ...
    if (argc == 3) {
        ...
        while (tok) {
            printf("%s\n", tok);
            if (!strcasecmp(tok, "a2a") || !strcasecmp(tok, "hemi") || !strcasecmp(tok, "quad") ||
                !strcasecmp(tok, "snc2") || !strcasecmp(tok, "snc4")) {
                xfree(numa_mode);
                numa_mode = xstrdup(tok);
            } else if (!strcasecmp(tok, "cache") || !strcasecmp(tok, "split") || !strcasecmp(tok, "equal") ||
                       !strcasecmp(tok, "flat")) {
                xfree(mcdram_mode);
    ...
    if (_update_all_nodes(argv[1]) != 0) {
        ...
        node_msg.node_state = NODE_STATE_POWER_SAVE | NODE_STATE_POWER_UP;
