...
const char *etypes[] = {"Center", "Node", NULL};
...
int layouts_p_update_done(layout_t *layout, entity_t **e_array, int e_cnt) {
    int i;
    debug3("layouts/power: receiving update callback for %d entities", e_cnt);
    for (i = 0; i < e_cnt; i++) {
        if (e_array[i] == NULL) {
            debug3("layouts/power: skipping update of nullified"
                   "entity[%d]", i);
        } else {
            debug3("layouts/power: updating entity[%d]=%s", i, e_array[i]->name);
        }
    }
    return 1;
}
