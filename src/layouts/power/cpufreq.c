...
const layouts_keyspec_t keyspec[] = {
        ...
        {"CurrentSumPower",  L_T_UINT32, KEYSPEC_UPDATE_CHILDREN_SUM, "CurrentPower"},
        {"IdleSumWatts",     L_T_UINT32, KEYSPEC_UPDATE_CHILDREN_SUM, "IdleWatts"},
        {"MaxSumWatts",      L_T_UINT32, KEYSPEC_UPDATE_CHILDREN_SUM, "MaxWatts"},
        {"CurrentPower",     L_T_UINT32, KEYSPEC_UPDATE_CHILDREN_SUM, "CurrentCorePower"},
        {"IdleWatts",        L_T_UINT32, KEYSPEC_UPDATE_CHILDREN_SUM, "IdleCoreWatts"},
        {"MaxWatts",         L_T_UINT32, KEYSPEC_UPDATE_CHILDREN_SUM, "MaxCoreWatts"},
...
const char *etypes[] = {"Center", "Node", "Core", NULL};
...
int layouts_p_update_done(layout_t *layout, entity_t **e_array, int e_cnt) {
    int i;
    debug3("layouts/power_cpufreq: receiving update callback for %d entities", e_cnt);
    for (i = 0; i < e_cnt; i++) {
        if (e_array[i] == NULL) {
            debug3("layouts/power_cpufreq: skipping update of nullified"
                   "entity[%d]", i);
        } else {
            debug3("layouts/power_cpufreq: updating entity[%d]=%s", i, e_array[i]->name);
        }
    }
    return 1;
}
