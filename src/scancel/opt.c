...
{
	if (opt.account == NULL && opt.batch == false && opt.interactive == false && opt.job_name == NULL && opt.partition == NULL && opt.qos == NULL && opt.reservation == NULL && opt.signal == NO_VAL16 && opt.state == JOB_END && opt.user_id == 0 && opt.user_name == NULL && opt.wckey == NULL && opt.nodelist == NULL)
	{
		return true;
	}
	return false;
...
/*
opt_default(): 被initialize_and_process_args调用，来设置默认值
...
/*
opt_env():被initialize_and_process_args()调用，通过环境变量设置选项。请参阅上面的注释了解如何扩展srun以处理不同的vars（变量）
...
/*
opt_args(): 通过命令行args和getopt_long设置选项
