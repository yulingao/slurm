...
static int _set_resv_cond(int *start, int argc, char **argv, slurmdb_reservation_cond_t *resv_cond, List format_list) {
    int i;
    int set = 0;
    int end = 0;
    ...
    for (i = (*start); i < argc; i++) {
        ...
        if (!end && !xstrncasecmp(argv[i], "all_clusters", MAX(command_len, 1))) {
            ...
            if (!resv_cond->name_list)
                resv_cond->name_list = list_create(slurm_destroy_char);
            slurm_addto_char_list(resv_cond->name_list, argv[i] + end);
            set = 1;
        } else if (!xstrncasecmp(argv[i], "Clusters", MAX(command_len, 1))) {
            slurm_addto_char_list(resv_cond->cluster_list, argv[i] + end);
            set = 1;
            ...
            resv_cond->flags = parse_resv_flags(argv[i] + end, __func__);
            set = 1;
            ...
            if (!resv_cond->id_list)
                resv_cond->id_list = list_create(slurm_destroy_char);
            ...
            if (resv_cond->nodes) {
                error("You already specified nodes '%s' "
                      " combine your request into 1 nodes=.", resv_cond->nodes);
                exit_code = 1;
                break;
            }
...
static int _setup_print_fields_list(List format_list) {
    ...
    while ((object = list_next(itr))) {
        ...
        if (!xstrncasecmp("allocated", object, MAX(command_len, 2))) {
            ...
            field->len = 9;
            field->print_routine = print_fields_str;
        } else if (!xstrncasecmp("End", object, MAX(command_len, 2))) {
            ...
            field->len = 9;
            field->print_routine = print_fields_str;
        } else if (!xstrncasecmp("Nodes", object, MAX(command_len, 2))) {
            field->type = PRINT_RESV_NODES;
            field->name = xstrdup("Nodes");
            field->len = 15;
            field->print_routine = print_fields_str;
        } else if (!xstrncasecmp("ReservationId", object, MAX(command_len, 2))) {
            field->type = PRINT_RESV_ID;
            field->name = xstrdup("Id");
            field->len = 8;
            field->print_routine = print_fields_uint;
        } else if (!xstrncasecmp("Start", object, MAX(command_len, 2))) {
            field->type = PRINT_RESV_START;
            field->name = xstrdup("Start");
            field->len = 19;
            field->print_routine = print_fields_date;
        } else if (!xstrncasecmp("TotalTime", object, MAX(command_len, 2))) {
            ...
            field->len = 14;
            field->print_routine = print_fields_str;
        } else if (!xstrncasecmp("TresTime", object, MAX(command_len, 2)) ||
                   !xstrncasecmp("CpuTime", object, MAX(command_len, 5))) {
...
static List _get_resv_list(int argc, char **argv, char *report_name, List format_list) {
    slurmdb_reservation_cond_t *resv_cond = xmalloc(sizeof(slurmdb_reservation_cond_t));
    int i = 0;
    List resv_list = NULL;
...
static void _resv_tres_report(slurmdb_reservation_rec_t *resv_ptr, slurmdb_tres_rec_t *resv_tres) {
    ...
    if (resv_ptr->tres_list &&
        (tres_rec = list_find_first(resv_ptr->tres_list, slurmdb_find_tres_in_list, &resv_tres->id))) {
    ...
    while ((field = list_next(iter))) {
        switch (field->type) {
            case PRINT_RESV_NAME:
                field->print_routine(field, resv_ptr->name, (curr_inx == field_count));
                break;
            case PRINT_RESV_CLUSTER:
                field->print_routine(field, resv_ptr->cluster, (curr_inx == field_count));
                break;
            case PRINT_RESV_TRES_CNT:
                field->print_routine(field, tres_alloc_cnt, (curr_inx == field_count));
                break;
            case PRINT_RESV_ID:
                field->print_routine(field, resv_ptr->id, (curr_inx == field_count));
                break;
            case PRINT_RESV_TRES_ALLOC:
                field->print_routine(field, tres_alloc_secs, total_reported, (curr_inx == field_count));
                break;
            case PRINT_RESV_TRES_IDLE:
                field->print_routine(field, idle_secs, total_reported, (curr_inx == field_count));
                break;
            case PRINT_RESV_NODES:
                field->print_routine(field, resv_ptr->nodes, (curr_inx == field_count));
                break;
            case PRINT_RESV_ASSOCS:
                field->print_routine(field, resv_ptr->assocs, (curr_inx == field_count));
                break;
            case PRINT_RESV_START:
                field->print_routine(field, resv_ptr->time_start, (curr_inx == field_count));
                break;
            case PRINT_RESV_END:
                field->print_routine(field, resv_ptr->time_end, (curr_inx == field_count));
                break;
            case PRINT_RESV_FLAGS:
                temp_char = reservation_flags_string(resv_ptr->flags);
                field->print_routine(field, temp_char, (curr_inx == field_count));
                break;
            case PRINT_RESV_TIME:
                field->print_routine(field, (uint32_t) total_time, (curr_inx == field_count));
                break;
            case PRINT_RESV_TRES_NAME:
                xstrfmtcat(tres_tmp, "%s%s%s", resv_tres->type, resv_tres->name ? "/" : "",
            ...
            case PRINT_RESV_TRES_USAGE:
                field->print_routine(field, total_reported, (curr_inx == field_count));
                break;
            default:
...
extern int resv_utilization(int argc, char **argv) {
    ...
    if (!(resv_list = _get_resv_list(argc, argv, "Reservation Utilization", format_list)))
        goto end_it;

    if (!list_count(format_list)) {
        slurm_addto_char_list(format_list, "Cl,name,start,end,TresName,al,i");
    }
    ...
    while ((resv_ptr = list_next(resv_itr))) {
        ...
        slurmdb_tres_list_from_string(&resv_tres_list, resv_ptr->tres_str, TRES_STR_FLAG_NONE);
        if (!resv_tres_list)
            continue;
        ...
        while ((resv_tres = list_next(tres_itr))) {
            ...
            if (!(req_tres = list_find_first(req_tres_list, slurmdb_find_tres_in_list, &resv_tres->id))) {
                debug2("TRES id %d is not in the requested TRES list", resv_tres->id);
                continue;
            }
