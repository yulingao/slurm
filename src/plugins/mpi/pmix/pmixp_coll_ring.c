...
int pmixp_coll_ring_unpack(Buf buf, pmixp_coll_type_t *type, pmixp_coll_ring_msg_hdr_t *ring_hdr, pmixp_proc_t **r,
                           size_t *nr) {
...
static int
_ring_forward_data(pmixp_coll_ring_ctx_t *coll_ctx, uint32_t contrib_id, uint32_t hop_seq, void *data, size_t size) {
...
#ifdef PMIXP_COLL_DEBUG
    PMIXP_DEBUG("%p: transit data to nodeid=%d, seq=%d, hop=%d, size=%lu, contrib=%d", coll_ctx, _ring_next_id(coll),
                hdr.seq, hdr.hop_seq, hdr.msgsize, hdr.contrib_id);
#endif
    if (!buf) {
        rc = SLURM_ERROR;
        goto exit;
    ...
    do {
        ...
        switch (coll_ctx->state) {
            ...
            case PMIXP_COLL_RING_FINALIZE:
                ...
                break;
            default: PMIXP_ERROR("%p: unknown state = %d", coll_ctx, (int) coll_ctx->state);
        }
    } while (ret);
}
...
inline static int
_pmixp_coll_contrib(pmixp_coll_ring_ctx_t *coll_ctx, int contrib_id, uint32_t hop, char *data, size_t size) {
    pmixp_coll_t *coll = _ctx_get_coll(coll_ctx);
    char *data_ptr = NULL;
    int ret;
...
int pmixp_coll_ring_check(pmixp_coll_t *coll, pmixp_coll_ring_msg_hdr_t *hdr) {
    ...
    if (hdr->nodeid != _ring_prev_id(coll)) {
        ...
        PMIXP_ERROR("%p: unexpected contrib from %s:%u, expected is %d", coll, nodename, hdr->nodeid,
                    _ring_prev_id(coll));
        return SLURM_ERROR;
    }
...
#ifdef PMIXP_COLL_DEBUG
        nodename = pmixp_info_job_host(hdr->nodeid);
        PMIXP_ERROR("Wrong collective seq. #%d from nodeid %u, current is %d, skip this message", hdr->seq, hdr->nodeid,
                    coll->seq);
#endif
        return SLURM_ERROR;
    }
...
int pmixp_coll_ring_neighbor(pmixp_coll_t *coll, pmixp_coll_ring_msg_hdr_t *hdr, Buf buf) {
...
#ifdef PMIXP_COLL_DEBUG
    PMIXP_DEBUG("%p: contrib/nbr: seqnum=%u, state=%d, nodeid=%d, contrib=%d, seq=%d, size=%lu", coll_ctx,
                coll_ctx->seq, coll_ctx->state, hdr->nodeid, hdr->contrib_id, hdr->hop_seq, hdr->msgsize);
#endif
...
#ifdef PMIXP_COLL_DEBUG
        PMIXP_DEBUG("%p: unexpected ring seq number=%d, expect=%d, coll seq=%d", coll, hdr->hop_seq, hop_seq,
                    coll->seq);
#endif
        goto exit;
    }
    ...
    if (coll_ctx->contrib_map[hdr->contrib_id]) {
        ...
        PMIXP_DEBUG("%p: double receiving was detected from %d, "
                    "local seq=%d, seq=%d, rejected", coll, hdr->contrib_id, coll->seq, hdr->seq);
#endif
        goto exit;
    }
...
void pmixp_coll_ring_log(pmixp_coll_t *coll) {
    ...
    for (i = 0; i < PMIXP_COLL_RING_CTX_NUM; i++) {
        ...
        if (coll_ctx->in_use) {
            ...
            PMIXP_ERROR("\t seq=%d contribs: loc=%d/prev=%d/fwd=%d", coll_ctx->seq, coll_ctx->contrib_local,
            ...
            if (coll_ctx->ring_buf) {
                PMIXP_ERROR("\t buf (offset/size): %u/%u", get_buf_offset(coll_ctx->ring_buf),
