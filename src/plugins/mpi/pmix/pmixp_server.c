...
static int _auth_cred_create(Buf buf) {
    ...
    if (rc) PMIXP_ERROR("Packing authentication credential: %m");
...
static int _auth_cred_verify(Buf buf) {
    ...
    if (rc) PMIXP_ERROR("Verifying authentication credential: %m");
    g_slurm_auth_destroy(auth_cred);
    return rc;
}
...
static void _process_server_request(pmixp_base_hdr_t *hdr, Buf buf) {
    ...
    switch (hdr->type) {
        ...
        case PMIXP_MSG_INIT_DIRECT: PMIXP_DEBUG("Direct connection init from %d", hdr->nodeid);
            break;
#ifndef NDEBUG
        ...
        default: PMIXP_ERROR("Unknown message type %d", hdr->type);
            break;
    }
...
static int _slurm_send(pmixp_ep_t *ep, pmixp_base_hdr_t bhdr, Buf buf) {
    ...
    switch (ep->type) {
        ...
        default: PMIXP_ERROR("Bad value of the EP type: %d", (int) ep->type);
            abort();
    }
...
static int _pmixp_server_cperf_iter(pmixp_coll_type_t type, char *data, int ndata) {
    ...
    switch (type) {
        ...
        case PMIXP_COLL_TYPE_FENCE_TREE:
            ...
            break;
        default: PMIXP_ERROR("Uncnown coll type");
            return SLURM_ERROR;
    }
