...
char *assoc_req_inx[] = {"id_assoc", "lft", "rgt", "user", "acct", "`partition`", "shares", "grp_tres_mins",
                         "grp_tres_run_mins", "grp_tres", "grp_jobs", "grp_jobs_accrue", "grp_submit_jobs", "grp_wall",
                         "max_tres_mins_pj", "max_tres_run_mins", "max_tres_pj", "max_tres_pn", "max_jobs",
                         "max_jobs_accrue", "min_prio_thresh", "max_submit_jobs", "max_wall_pj", "parent_acct",
                         "priority", "def_qos_id", "qos", "delta_qos", "is_def",};
enum {
    ASSOC_REQ_ID,
    ASSOC_REQ_LFT,
    ASSOC_REQ_RGT,
...
static char *get_parent_limits_select = "select @par_id, @mj, @mja, @mpt, @msj, "
...
static char *aassoc_req_inx[] = {"id_assoc", "parent_acct", "lft", "rgt", "deleted"};

enum {
    AASSOC_ID, AASSOC_PACCT, AASSOC_LFT, AASSOC_RGT, AASSOC_DELETED, AASSOC_COUNT
};

static char *massoc_req_inx[] = {"id_assoc", "acct", "parent_acct", "user", "`partition`", "lft", "rgt", "qos",
                                 "grp_tres_mins", "grp_tres_run_mins", "grp_tres", "max_tres_mins_pj",
                                 "max_tres_run_mins", "max_tres_pj", "max_tres_pn",};
...
static char *rassoc_req_inx[] = {"id_assoc", "lft", "acct", "parent_acct", "user", "`partition`"};

enum {
    RASSOC_ID, RASSOC_LFT, RASSOC_ACCT, RASSOC_PACCT, RASSOC_USER, RASSOC_PART, RASSOC_COUNT
};
 ...
 */
static int
_reset_default_assoc(mysql_conn_t *mysql_conn, slurmdb_assoc_rec_t *assoc, char **query, bool add_to_update) {
    time_t now = time(NULL);
    int rc = SLURM_SUCCESS;

    xassert(query);

    if ((assoc->is_def != 1) || !assoc->cluster || !assoc->acct || !assoc->user)
        return SLURM_ERROR;

    xstrfmtcat(*query, "update \"%s_%s\" set is_def=0, mod_time=%ld "
                       "where (user='%s' && acct!='%s' && is_def=1);", assoc->cluster, assoc_table, (long) now,
    ...
    if (add_to_update) {
		...
		*/
        xstrfmtcat(sel_query, "select id_assoc from \"%s_%s\" "
                              "where (user='%s' && acct!='%s' && is_def=1);", assoc->cluster, assoc_table, assoc->user,
                   assoc->acct);
        if (debug_flags & DEBUG_FLAG_DB_ASSOC)
            DB_DEBUG(mysql_conn->conn, "query\n%s", sel_query);
        if (!(result = mysql_db_query_ret(mysql_conn, sel_query, 1))) {
            xfree(sel_query);
            rc = SLURM_ERROR;
            goto end_it;
        }
        ...
        while ((row = mysql_fetch_row(result))) {
            slurmdb_assoc_rec_t *mod_assoc = xmalloc(sizeof(slurmdb_assoc_rec_t));
            slurmdb_init_assoc_rec(mod_assoc, 0);

            mod_assoc->cluster = xstrdup(assoc->cluster);
            mod_assoc->id = slurm_atoul(row[0]);
            mod_assoc->is_def = 0;
            if (addto_update_list(mysql_conn->update_list, SLURMDB_MODIFY_ASSOC, mod_assoc) != SLURM_SUCCESS) {
...
static int
_check_coord_qos(mysql_conn_t *mysql_conn, char *cluster_name, char *account, char *coord_name, List qos_list) {
    char *query;
    ...
    query = xstrdup_printf("call get_coord_qos('%s', '%s', '%s', '%s');", assoc_table, account, cluster_name,
                           coord_name);
 ...
 */
static int _make_sure_users_have_default(mysql_conn_t *mysql_conn, List user_list) {
    ...
    while ((user = list_next(itr))) {
        while ((cluster = list_next(clus_itr))) {
            ...
            MYSQL_ROW row;
            char *acct = NULL;
            query = xstrdup_printf("select distinct is_def, acct from "
                                   "\"%s_%s\" where user='%s' FOR UPDATE;", cluster, assoc_table, user);
            debug4("%d(%s:%d) query\n%s", mysql_conn->conn, THIS_FILE, __LINE__, query);
			...
			*/
            ...
            query = xstrdup_printf("update \"%s_%s\" set is_def=1 where "
                                   "user='%s' and acct='%s';", cluster, assoc_table, user, acct);
            xfree(acct);
            if (debug_flags & DEBUG_FLAG_DB_ASSOC)
                DB_DEBUG(mysql_conn->conn, "query\n%s", query);
 ...
 */
static int _move_account(mysql_conn_t *mysql_conn, uint32_t *lft, uint32_t *rgt, char *cluster, char *id, char *parent,
                         time_t now) {
    int rc = SLURM_SUCCESS;
    MYSQL_RES *result = NULL;
    MYSQL_ROW row;
    ...
    uint32_t diff = 0;
    uint32_t width = 0;
    char *query = xstrdup_printf("SELECT lft from \"%s_%s\" where acct='%s' && user='';", cluster, assoc_table, parent);
    if (debug_flags & DEBUG_FLAG_DB_ASSOC)
        DB_DEBUG(mysql_conn->conn, "query\n%s", query);
    if (!(result = mysql_db_query_ret(mysql_conn, query, 0))) {
        xfree(query);
    ...
    if (diff == 0) {
        if (debug_flags & DEBUG_FLAG_DB_ASSOC)
            DB_DEBUG(mysql_conn->conn, "Trying to move association to the same "
    ...
    /* every thing below needs to be a %d not a %u because we are
	   looking for -1 */
    xstrfmtcat(query, "update \"%s_%s\" set mod_time=%ld, deleted = deleted + 2, "
                      "lft = lft + %d, rgt = rgt + %d "
                      "WHERE lft BETWEEN %d AND %d;", cluster, assoc_table, now, diff, diff, *lft, *rgt);

    xstrfmtcat(query, "UPDATE \"%s_%s\" SET mod_time=%ld, rgt = rgt + %d WHERE "
                      "rgt > %d && deleted < 2;"
                      "UPDATE \"%s_%s\" SET mod_time=%ld, lft = lft + %d WHERE "
                      "lft > %d && deleted < 2;", cluster, assoc_table, now, width, par_left, cluster, assoc_table, now,
               width, par_left);
    ...
    xstrfmtcat(query, "UPDATE \"%s_%s\" SET mod_time=%ld, rgt = rgt - %d WHERE "
                      ...
                      "UPDATE \"%s_%s\" SET mod_time=%ld, lft = lft - %d WHERE "
                      "(%d < 0 && lft > %d && deleted < 2) "
                      "|| (%d > 0 && lft > %d);", cluster, assoc_table, now, width, diff, *rgt, diff, *lft, cluster,
               assoc_table, now, width, diff, *rgt, diff, *lft);

    xstrfmtcat(query, "update \"%s_%s\" set mod_time=%ld, "
                      "deleted = deleted - 2 WHERE deleted > 1;", cluster, assoc_table, now);
    xstrfmtcat(query, "update \"%s_%s\" set mod_time=%ld, "
                      "parent_acct='%s' where id_assoc = %s;", cluster, assoc_table, now, parent, id);
    /* get the new lft and rgt if changed */
    xstrfmtcat(query, "select lft, rgt from \"%s_%s\" where id_assoc = %s", cluster, assoc_table, id);
    if (debug_flags & DEBUG_FLAG_DB_ASSOC)
        DB_DEBUG(mysql_conn->conn, "query\n%s", query);
    if (!(result = mysql_db_query_ret(mysql_conn, query, 1))) {
        xfree(query);
    ...
    xfree(query);
    if ((row = mysql_fetch_row(result))) {
        debug4("lft and rgt were %u %u and now is %s %s", *lft, *rgt, row[0], row[1]);
 ...
 */
static int _move_parent(mysql_conn_t *mysql_conn, uid_t uid, uint32_t *lft, uint32_t *rgt, char *cluster, char *id,
                        char *old_parent, char *new_parent, time_t now) {
    MYSQL_RES *result = NULL;
    MYSQL_ROW row;
    char *query = NULL;
    int rc = SLURM_SUCCESS;
	 ...
	 */
    query = xstrdup_printf("select id_assoc, lft, rgt from \"%s_%s\" "
                           "where lft between %d and %d "
                           "&& acct='%s' && user='' order by lft;", cluster, assoc_table, *lft, *rgt, new_parent);
    if (debug_flags & DEBUG_FLAG_DB_ASSOC)
        DB_DEBUG(mysql_conn->conn, "query\n%s", query);
    if (!(result = mysql_db_query_ret(mysql_conn, query, 0))) {
        xfree(query);
    ...
    if ((row = mysql_fetch_row(result))) {
        ...
        debug4("%s(%s) %s,%s is a child of %s", new_parent, row[0], row[1], row[2], id);
        rc = _move_account(mysql_conn, &child_lft, &child_rgt, cluster, row[0], old_parent, now);
    }
	 ...
	 * have changed.
	 */
    query = xstrdup_printf("select lft, rgt from \"%s_%s\" where id_assoc=%s;", cluster, assoc_table, id);
    if (debug_flags & DEBUG_FLAG_DB_ASSOC)
        DB_DEBUG(mysql_conn->conn, "query\n%s", query);
    if (!(result = mysql_db_query_ret(mysql_conn, query, 0))) {
        xfree(query);
    ...
    if ((row = mysql_fetch_row(result))) {
        *lft = slurm_atoul(row[0]);
        *rgt = slurm_atoul(row[1]);
        rc = _move_account(mysql_conn, lft, rgt, cluster, id, new_parent, now);
    } else {
...
static uint32_t _get_parent_id(mysql_conn_t *mysql_conn, char *parent, char *cluster) {
    ...
    query = xstrdup_printf("select id_assoc from \"%s_%s\" where user='' "
                           "and deleted = 0 and acct='%s';", cluster, assoc_table, parent);
    debug4("%d(%s:%d) query\n%s", mysql_conn->conn, THIS_FILE, __LINE__, query);
    ...
    if ((row = mysql_fetch_row(result))) {
        ...
        error("no association for parent %s on cluster %s", parent, cluster);
    mysql_free_result(result);

    return parent_id;
}

static int _set_assoc_lft_rgt(mysql_conn_t *mysql_conn, slurmdb_assoc_rec_t *assoc) {
    MYSQL_RES *result = NULL;
    MYSQL_ROW row;
    char *query = NULL;
    int rc = SLURM_ERROR;
    ...
    query = xstrdup_printf("select lft, rgt from \"%s_%s\" "
                           "where id_assoc=%u;", assoc->cluster, assoc_table, assoc->id);
    debug4("%d(%s:%d) query\n%s", mysql_conn->conn, THIS_FILE, __LINE__, query);
...
static int _set_assoc_limits_for_add(mysql_conn_t *mysql_conn, slurmdb_assoc_rec_t *assoc) {
    MYSQL_RES *result = NULL;
    MYSQL_ROW row;
    char *query = NULL;
    char *parent = NULL;
    ...
    query = xstrdup_printf("call get_parent_limits('%s', "
                           "'%s', '%s', %u); %s", assoc_table, parent, assoc->cluster, 0, get_parent_limits_select);
    debug4("%d(%s:%d) query\n%s", mysql_conn->conn, THIS_FILE, __LINE__, query);
    if (!(result = mysql_db_query_ret(mysql_conn, query, 1))) {
        xfree(query);
        return SLURM_ERROR;
	 ...
	 * TRES.
	 */
    slurmdb_combine_tres_strings(&assoc->max_tres_pj, row[ASSOC2_REQ_MTPJ], tres_str_flags);
    slurmdb_combine_tres_strings(&assoc->max_tres_pn, row[ASSOC2_REQ_MTPN], tres_str_flags);
    slurmdb_combine_tres_strings(&assoc->max_tres_mins_pj, row[ASSOC2_REQ_MTMPJ], tres_str_flags);
    slurmdb_combine_tres_strings(&assoc->max_tres_run_mins, row[ASSOC2_REQ_MTRM], tres_str_flags);
    ...
    if (row[ASSOC2_REQ_DELTA_QOS][0])
        slurm_addto_char_list(assoc->qos_list, row[ASSOC2_REQ_DELTA_QOS] + 1);
    if (qos_delta) {
 ...
 */
static int
_modify_unset_users(mysql_conn_t *mysql_conn, slurmdb_assoc_rec_t *assoc, char *acct, uint32_t lft, uint32_t rgt,
    ...
    char *assoc_inx[] = {"id_assoc", "user", "acct", "`partition`", "max_jobs", "max_jobs_accrue", "min_prio_thresh",
                         "max_submit_jobs", "max_tres_pj", "max_tres_pn", "max_wall_pj", "max_tres_mins_pj",
                         "max_tres_run_mins", "priority", "def_qos_id", "qos", "delta_qos", "lft", "rgt"};
    ...
    query = xstrdup_printf("select distinct %s from \"%s_%s\" "
                           ...
                           "((user = '' && parent_acct = '%s') || "
                           "(user != '' && acct = '%s')) "
                           "order by lft;", object, assoc->cluster, assoc_table, lft, rgt, acct, acct);
    xfree(object);
    if (debug_flags & DEBUG_FLAG_DB_ASSOC)
        DB_DEBUG(mysql_conn->conn, "query\n%s", query);
    if (!(result = mysql_db_query_ret(mysql_conn, query, 0))) {
    ...
    while ((row = mysql_fetch_row(result))) {
        ...
        if (assoc->max_tres_pj) {
            ...
            slurmdb_combine_tres_strings(&tmp_char, assoc->max_tres_pj, tres_str_flags);
        ...
        if (assoc->max_tres_pn) {
            ...
            slurmdb_combine_tres_strings(&tmp_char, assoc->max_tres_pn, tres_str_flags);
        ...
        if (assoc->max_tres_mins_pj) {
            ...
            slurmdb_combine_tres_strings(&tmp_char, assoc->max_tres_mins_pj, tres_str_flags);
        ...
        if (assoc->max_tres_run_mins) {
            ...
            slurmdb_combine_tres_strings(&tmp_char, assoc->max_tres_run_mins, tres_str_flags);
        ...
        if (!row[ASSOC_QOS][0] && assoc->qos_list) {
            ...
            if (row[ASSOC_DELTA_QOS][0]) {
                ...
                slurm_addto_char_list(delta_qos_list, row[ASSOC_DELTA_QOS] + 1);
                delta_itr = list_iterator_create(delta_qos_list);
            }
			...
			*/
            while ((qos_char = list_next(qos_itr))) {
                if (delta_itr && qos_char[0] != '=') {
                    while ((delta_char = list_next(delta_itr))) {

                        if ((qos_char[0] != delta_char[0]) && (!xstrcmp(qos_char + 1, delta_char + 1)))
                            break;
                    }
                ...
                list_append(mod_assoc->qos_list, xstrdup(qos_char));
            }
            list_iterator_destroy(qos_itr);
            if (delta_itr)
                list_iterator_destroy(delta_itr);
            FREE_NULL_LIST(delta_qos_list);
            if (list_count(mod_assoc->qos_list) || !list_count(assoc->qos_list))
                modified = 1;
            else {
        ...
        if (modified) {
            ...
            if (!row[ASSOC_USER][0]) {
				 ...
				 */
                _modify_unset_users(mysql_conn, mod_assoc, row[ASSOC_ACCT], slurm_atoul(row[ASSOC_LFT]),
                                    slurm_atoul(row[ASSOC_RGT]), ret_list, moved_parent);
                slurmdb_destroy_assoc_rec(mod_assoc);
                continue;
            }
            ...
            if (row[ASSOC_PART][0]) {
                ...
                object = xstrdup_printf("C = %-10s A = %-20s U = %-9s P = %s", assoc->cluster, row[ASSOC_ACCT],
                                        row[ASSOC_USER], row[ASSOC_PART]);
            } else {
                object = xstrdup_printf("C = %-10s A = %-20s U = %-9s", assoc->cluster, row[ASSOC_ACCT],
                                        row[ASSOC_USER]);
            }
            ...
            if (moved_parent)
                slurmdb_destroy_assoc_rec(mod_assoc);
            else if (addto_update_list(mysql_conn->update_list, SLURMDB_MODIFY_ASSOC, mod_assoc) != SLURM_SUCCESS) {
                slurmdb_destroy_assoc_rec(mod_assoc);
...
static char *_setup_assoc_cond_qos(slurmdb_assoc_cond_t *assoc_cond, char *cluster_name) {
    int set = 0;
	...
	*/
    if (assoc_cond->qos_list && list_count(assoc_cond->qos_list)) {
        ...
        xstrfmtcat(extra, ", \"%s_%s\" as t2 where "
                          "(t1.lft between t2.lft and t2.rgt) && (", cluster_name, assoc_table);
        set = 0;
        itr = list_iterator_create(assoc_cond->qos_list);
        while ((object = list_next(itr))) {
            if (set)
                xstrcat(extra, " || ");
            xstrfmtcat(extra, "(%s.qos like '%%,%s' "
                              "|| %s.qos like '%%,%s,%%' "
                              "|| %s.delta_qos like '%%,+%s' "
                              "|| %s.delta_qos like '%%,+%s,%%')", prefix, object, prefix, object, prefix, object,
                       prefix, object);
            set = 1;
        }
        list_iterator_destroy(itr);
...
static int _setup_assoc_cond_limits(slurmdb_assoc_cond_t *assoc_cond, const char *prefix, char **extra) {
    int set = 0;
    ListIterator itr = NULL;
    char *object = NULL;
    ...
    if (assoc_cond->def_qos_id_list && list_count(assoc_cond->def_qos_id_list)) {
        set = 0;
        xstrcat(*extra, " && (");
        ...
        while ((object = list_next(itr))) {
            if (set)
                xstrcat(*extra, " || ");
            xstrfmtcat(*extra, "%s.def_qos_id='%s'", prefix, object);
            set = 1;
        }
        list_iterator_destroy(itr);
        xstrcat(*extra, ")");
    ...
    if (assoc_cond->partition_list && list_count(assoc_cond->partition_list)) {
        set = 0;
        xstrcat(*extra, " && (");
        ...
        while ((object = list_next(itr))) {
            if (set)
                xstrcat(*extra, " || ");
            xstrfmtcat(*extra, "%s.partition='%s'", prefix, object);
            set = 1;
        }
        list_iterator_destroy(itr);
        xstrcat(*extra, ")");
    ...
    if (assoc_cond->parent_acct_list && list_count(assoc_cond->parent_acct_list)) {
        set = 0;
        xstrcat(*extra, " && (");
...
static int _process_modify_assoc_results(mysql_conn_t *mysql_conn, MYSQL_RES *result, slurmdb_assoc_rec_t *assoc,
                                         slurmdb_user_rec_t *user, char *cluster_name, char *sent_vals, bool is_admin,
                                         bool same_user, List ret_list) {
    ListIterator itr = NULL;
    MYSQL_ROW row;
    int added = 0;
    int rc = SLURM_SUCCESS;
    ...
    while ((row = mysql_fetch_row(result))) {
		...
		*/
            ...
            if (!coord) {
                if (row[MASSOC_PACCT][0])
                    error("User %s(%d) can not modify "
                          ...
                          "parent account '%s'.", user->name, user->uid, row[MASSOC_ACCT], row[MASSOC_PACCT]);
                else
                    error("User %s(%d) does not have the "
                          "ability to modify the account "
                          "(%s).", user->name, user->uid, row[MASSOC_ACCT]);

                rc = ESLURM_ACCESS_DENIED;
                goto end_it;
            } else if (_check_coord_qos(mysql_conn, cluster_name, account, user->name, assoc->qos_list) ==
                       SLURM_ERROR) {
                assoc_mgr_lock_t locks = {NO_LOCK, NO_LOCK, READ_LOCK, NO_LOCK, NO_LOCK, NO_LOCK, NO_LOCK};
                char *requested_qos;

                assoc_mgr_lock(&locks);
                requested_qos = get_qos_complete_str(assoc_mgr_qos_list, assoc->qos_list);
                assoc_mgr_unlock(&locks);
                error("Coordinator %s(%d) does not have the "
                      "access to all the qos requested (%s), "
                      "so they can't modify account "
                      "%s with it.", user->name, user->uid, requested_qos, account);
                xfree(requested_qos);
                rc = ESLURM_ACCESS_DENIED;
                goto end_it;
            }
        ...
        if (row[MASSOC_PART][0]) {
            // see if there is a partition name
            object = xstrdup_printf("C = %-10s A = %-20s U = %-9s P = %s", cluster_name, row[MASSOC_ACCT],
                                    row[MASSOC_USER], row[MASSOC_PART]);
        } else if (row[MASSOC_USER][0]) {
            object = xstrdup_printf("C = %-10s A = %-20s U = %-9s", cluster_name, row[MASSOC_ACCT], row[MASSOC_USER]);
        } else {
            if (assoc->parent_acct) {
                if (!xstrcasecmp(row[MASSOC_ACCT], assoc->parent_acct)) {
                    error("You can't make an account be a "
                          "child of it's self");
                    continue;
                }
                rc = _move_parent(mysql_conn, user->uid, &lft, &rgt, cluster_name, row[MASSOC_ID], row[MASSOC_PACCT],
                                  assoc->parent_acct, now);

                if ((rc == ESLURM_INVALID_PARENT_ACCOUNT) || (rc == ESLURM_SAME_PARENT_ACCOUNT)) {
                    continue;
            ...
            if (row[MASSOC_PACCT][0]) {
                object = xstrdup_printf("C = %-10s A = %s of %s", cluster_name, row[MASSOC_ACCT], row[MASSOC_PACCT]);
            } else {
                object = xstrdup_printf("C = %-10s A = %s", cluster_name, row[MASSOC_ACCT]);
            }
            account_type = 1;
        }
        list_append(ret_list, object);
        ...
        if (xstrcmp(orig_acct, "root") || row[MASSOC_USER][0]) {
            ...
            /* If there is a variable cleared here we need to make
			   sure we get the parent's information, if any. */
            query = xstrdup_printf("call get_parent_limits('%s', "
                                   "'%s', '%s', %u); %s", assoc_table, account, cluster_name, 0,
                                   get_parent_limits_select);
            debug4("%d(%s:%d) query\n%s", mysql_conn->conn, THIS_FILE, __LINE__, query);
            if (!(result2 = mysql_db_query_ret(mysql_conn, query, 1))) {
                xfree(query);
                break;
            }
            xfree(query);
            ...
            if ((row2 = mysql_fetch_row(result2))) {
                if (assoc->def_qos_id == INFINITE && row2[ASSOC2_REQ_DEF_QOS])
                    alt_assoc.def_qos_id = slurm_atoul(row2[ASSOC2_REQ_DEF_QOS]);

                if ((assoc->max_jobs == INFINITE) && row2[ASSOC2_REQ_MJ])
                    alt_assoc.max_jobs = slurm_atoul(row2[ASSOC2_REQ_MJ]);
                if ((assoc->max_jobs_accrue == INFINITE) && row2[ASSOC2_REQ_MJA])
                    alt_assoc.max_jobs_accrue = slurm_atoul(row2[ASSOC2_REQ_MJA]);
                if ((assoc->min_prio_thresh == INFINITE) && row2[ASSOC2_REQ_MPT])
                    alt_assoc.min_prio_thresh = slurm_atoul(row2[ASSOC2_REQ_MPT]);
                if ((assoc->max_submit_jobs == INFINITE) && row2[ASSOC2_REQ_MSJ])
                    alt_assoc.max_submit_jobs = slurm_atoul(row2[ASSOC2_REQ_MSJ]);
                if ((assoc->max_wall_pj == INFINITE) && row2[ASSOC2_REQ_MWPJ])
                    alt_assoc.max_wall_pj = slurm_atoul(row2[ASSOC2_REQ_MWPJ]);
                if ((assoc->priority == INFINITE) && row2[ASSOC2_REQ_PRIO])
                    alt_assoc.priority = slurm_atoul(row2[ASSOC2_REQ_PRIO]);
                ...
                if (row2[ASSOC2_REQ_MTPJ][0])
                    alt_assoc.max_tres_pj = row2[ASSOC2_REQ_MTPJ];
                if (row2[ASSOC2_REQ_MTPN][0])
                    alt_assoc.max_tres_pn = row2[ASSOC2_REQ_MTPN];
                if (row2[ASSOC2_REQ_MTMPJ][0])
                    alt_assoc.max_tres_mins_pj = row2[ASSOC2_REQ_MTMPJ];
                if (row2[ASSOC2_REQ_MTRM][0])
                    alt_assoc.max_tres_run_mins = row2[ASSOC2_REQ_MTRM];
            }
        }
        mod_assoc = xmalloc(sizeof(slurmdb_assoc_rec_t));
        slurmdb_init_assoc_rec(mod_assoc, 0);
        ...
        mod_tres_str(&mod_assoc->grp_tres, assoc->grp_tres, row[MASSOC_GT], NULL, "grp_tres", &vals, mod_assoc->id, 1);
        mod_tres_str(&mod_assoc->grp_tres_mins, assoc->grp_tres_mins, row[MASSOC_GTM], NULL, "grp_tres_mins", &vals,
                     mod_assoc->id, 1);
        mod_tres_str(&mod_assoc->grp_tres_run_mins, assoc->grp_tres_run_mins, row[MASSOC_GTRM], NULL,
                     "grp_tres_run_mins", &vals, mod_assoc->id, 1);
        ...
        mod_tres_str(&mod_assoc->max_tres_pj, assoc->max_tres_pj, row[MASSOC_MTPJ], alt_assoc.max_tres_pj,
                     "max_tres_pj", &vals, mod_assoc->id, 1);
        mod_tres_str(&mod_assoc->max_tres_pn, assoc->max_tres_pn, row[MASSOC_MTPN], alt_assoc.max_tres_pn,
                     "max_tres_pn", &vals, mod_assoc->id, 1);
        mod_tres_str(&mod_assoc->max_tres_mins_pj, assoc->max_tres_mins_pj, row[MASSOC_MTMPJ],
                     alt_assoc.max_tres_mins_pj, "max_tres_mins_pj", &vals, mod_assoc->id, 1);
        mod_tres_str(&mod_assoc->max_tres_run_mins, assoc->max_tres_run_mins, row[MASSOC_MTRM],
                     alt_assoc.max_tres_run_mins, "max_tres_run_mins", &vals, mod_assoc->id, 1);
        ...
        if (assoc->qos_list && list_count(assoc->qos_list)) {
            ListIterator new_qos_itr = list_iterator_create(assoc->qos_list);
            ...
            while ((new_qos = list_next(new_qos_itr))) {
                if (new_qos[0] == '-' || new_qos[0] == '+') {
                    list_append(mod_assoc->qos_list, xstrdup(new_qos));
                } else if (new_qos[0]) {
                    list_append(mod_assoc->qos_list, xstrdup_printf("=%s", new_qos));
                }
				...
				*/
                if (new_qos[0] == '-') {
                    xstrfmtcat(vals, ", qos=if (qos='', '', "
                                     ...
                                     "delta_qos, ',+%s,', ','), "
                                     "',-%s,', ','), "
                                     "',%s,'), ',,', ','), '')", new_qos + 1, new_qos + 1, new_qos + 1, new_qos);
                } else if (new_qos[0] == '+') {
                    xstrfmtcat(vals, ", qos=if (qos='', '', "
                                     "replace(concat("
                                     "replace(qos, ',%s,', ','), "
                                     ...
                                     "delta_qos, ',+%s,', ','), "
                                     "',-%s,', ','), "
                                     "',%s,'), ',,', ','), '')", new_qos + 1, new_qos + 1, new_qos + 1, new_qos + 1,
                               new_qos);
                } else if (new_qos[0]) {
            ...
            if (!set_qos_vals && tmp_qos) {
                xstrfmtcat(vals, ", qos='%s%s', delta_qos=''", tmp_qos, adding_straight ? "," : "");
            }
            xfree(tmp_qos);
        ...
        if (account_type) {
            _modify_unset_users(mysql_conn, mod_assoc, row[MASSOC_ACCT], lft, rgt, ret_list, moved_parent);
			...
			*/
            ...
            if ((rc = _reset_default_assoc(mysql_conn, &tmp_assoc, &reset_query, moved_parent ? 0 : 1)) !=
                SLURM_SUCCESS) {
                slurmdb_destroy_assoc_rec(mod_assoc);
                xfree(reset_query);
                goto end_it;
    ...
    if (assoc->parent_acct) {
        if (((rc == ESLURM_INVALID_PARENT_ACCOUNT) || (rc == ESLURM_SAME_PARENT_ACCOUNT)) && added)
            rc = SLURM_SUCCESS;
    }
    ...
    if (vals && vals[0]) {
        ...
        rc = modify_common(mysql_conn, DBD_MODIFY_ASSOCS, now, user_name, assoc_table, name_char, vals, cluster_name);
        xfree(user_name);
        if (rc == SLURM_ERROR) {
    ...
    if (moved_parent) {
        ...
        memset(&local_assoc_cond, 0, sizeof(slurmdb_assoc_cond_t));
        local_assoc_cond.cluster_list = list_create(NULL);
        list_append(local_assoc_cond.cluster_list, cluster_name);
        local_assoc_list = as_mysql_get_assocs(mysql_conn, user->uid, &local_assoc_cond);
		...
		*/
        ...
        while ((local_assoc = list_next(local_itr))) {
            if (addto_update_list(mysql_conn->update_list, SLURMDB_MODIFY_ASSOC, local_assoc) == SLURM_SUCCESS)
    ...
    if (reset_query) {
        if (debug_flags & DEBUG_FLAG_DB_ASSOC)
            DB_DEBUG(mysql_conn->conn, "query\n%s", reset_query);
        if ((rc = mysql_db_query(mysql_conn, reset_query)) != SLURM_SUCCESS)
...
static int
_process_remove_assoc_results(mysql_conn_t *mysql_conn, MYSQL_RES *result, slurmdb_user_rec_t *user, char *cluster_name,
                              char *name_char, bool is_admin, List ret_list, bool *jobs_running) {
    ListIterator itr = NULL;
    MYSQL_ROW row;
    int rc = SLURM_SUCCESS;
    ...
    while ((row = mysql_fetch_row(result))) {
        ...
        if (!is_admin) {
            ...
            itr = list_iterator_create(user->coord_accts);
            while ((coord = list_next(itr))) {
                if (!xstrcasecmp(coord->name, row[RASSOC_ACCT]))
                    break;
            }
            list_iterator_destroy(itr);
            ...
            if (!coord) {
                error("User %s(%d) does not have the "
                      "ability to change this account (%s)", user->name, user->uid, row[RASSOC_ACCT]);
                rc = ESLURM_ACCESS_DENIED;
                goto end_it;
            }
        }
        if (row[RASSOC_PART][0]) {
            // see if there is a partition name
            object = xstrdup_printf("C = %-10s A = %-10s U = %-9s P = %s", cluster_name, row[RASSOC_ACCT],
                                    row[RASSOC_USER], row[RASSOC_PART]);
        } else if (row[RASSOC_USER][0]) {
            object = xstrdup_printf("C = %-10s A = %-10s U = %-9s", cluster_name, row[RASSOC_ACCT], row[RASSOC_USER]);
        } else {
            if (row[RASSOC_PACCT][0]) {
                object = xstrdup_printf("C = %-10s A = %s of %s", cluster_name, row[RASSOC_ACCT], row[RASSOC_PACCT]);
            } else {
                object = xstrdup_printf("C = %-10s A = %s", cluster_name, row[RASSOC_ACCT]);
            }
        }
        list_append(ret_list, object);
        if (assoc_char)
    ...
    if ((rc = as_mysql_get_modified_lfts(mysql_conn, cluster_name, smallest_lft)) != SLURM_SUCCESS)
        goto end_it;
    ...
    rc = remove_common(mysql_conn, DBD_REMOVE_ASSOCS, now, user_name, assoc_table, name_char, assoc_char, cluster_name,
...
static int _cluster_get_assocs(mysql_conn_t *mysql_conn, slurmdb_user_rec_t *user, slurmdb_assoc_cond_t *assoc_cond,
                               char *cluster_name, char *fields, char *sent_extra, bool is_admin, List sent_list) {
    List assoc_list;
    List delta_qos_list = NULL;
    ListIterator itr = NULL;
    MYSQL_RES *result = NULL;
    ...
    if (!is_admin && (private_data & PRIVATE_DATA_USERS)) {
        int set = 0;
        query = xstrdup_printf("select lft from \"%s_%s\" where user='%s'", cluster_name, assoc_table, user->name);
        if (user->coord_accts) {
            slurmdb_coord_rec_t *coord = NULL;
            itr = list_iterator_create(user->coord_accts);
            while ((coord = list_next(itr))) {
                xstrfmtcat(query, " || acct='%s'", coord->name);
            }
            list_iterator_destroy(itr);
        }
        if (debug_flags & DEBUG_FLAG_DB_ASSOC)
        ...
        while ((row = mysql_fetch_row(result))) {
            if (set) {
                xstrfmtcat(extra, " || (%s between t1.lft and t1.rgt)", row[0]);
            } else {
                set = 1;
                xstrfmtcat(extra, " && ((%s between t1.lft and t1.rgt)", row[0]);
            }
    ...
    query = xstrdup_printf("select distinct %s from \"%s_%s\" as t1%s%s "
                           "order by lft;", fields, cluster_name, assoc_table, qos_extra, extra);
    xfree(qos_extra);
    xfree(extra);
    if (debug_flags & DEBUG_FLAG_DB_ASSOC)
        DB_DEBUG(mysql_conn->conn, "query\n%s", query);
    ...
    while ((row = mysql_fetch_row(result))) {
        ...
        if (!without_parent_info && row[ASSOC_REQ_PARENT][0]) {
        ...
        if (!without_parent_info && parent_acct &&
            (!last_acct || !last_cluster || xstrcmp(parent_acct, last_acct) || xstrcmp(cluster_name, last_cluster))) {
            query = xstrdup_printf("call get_parent_limits('%s', "
                                   "'%s', '%s', %u); %s", assoc_table, parent_acct, cluster_name, without_parent_limits,
                                   get_parent_limits_select);
            debug4("%d(%s:%d) query\n%s", mysql_conn->conn, THIS_FILE, __LINE__, query);
            if (!(result2 = mysql_db_query_ret(mysql_conn, query, 1))) {
                xfree(query);
            ...
            if (!without_parent_limits) {
                if (row2[ASSOC2_REQ_DEF_QOS])
                    parent_def_qos_id = slurm_atoul(row2[ASSOC2_REQ_DEF_QOS]);
                else
                    parent_def_qos_id = 0;
                ...
                if (row2[ASSOC2_REQ_MJA])
                    parent_mja = slurm_atoul(row2[ASSOC2_REQ_MJA]);
                else
                    parent_mja = INFINITE;

                if (row2[ASSOC2_REQ_MPT])
                    parent_mpt = slurm_atoul(row2[ASSOC2_REQ_MPT]);
                else
                    parent_mpt = INFINITE;
                ...
                if (row2[ASSOC2_REQ_MWPJ])
                    parent_mwpj = slurm_atoul(row2[ASSOC2_REQ_MWPJ]);
                else
                    parent_mwpj = INFINITE;


                if (row2[ASSOC2_REQ_PRIO])
                    parent_prio = slurm_atoul(row2[ASSOC2_REQ_PRIO]);
                else
                    parent_prio = INFINITE;

                xfree(parent_mtpj);
                if (row2[ASSOC2_REQ_MTPJ][0])
                    parent_mtpj = xstrdup(row2[ASSOC2_REQ_MTPJ]);

                xfree(parent_mtpn);
                if (row2[ASSOC2_REQ_MTPN][0])
                    parent_mtpn = xstrdup(row2[ASSOC2_REQ_MTPN]);

                xfree(parent_mtmpj);
                if (row2[ASSOC2_REQ_MTMPJ][0])
                    parent_mtmpj = xstrdup(row2[ASSOC2_REQ_MTMPJ]);

                xfree(parent_mtrm);
                if (row2[ASSOC2_REQ_MTRM][0])
                    parent_mtrm = xstrdup(row2[ASSOC2_REQ_MTRM]);

                xfree(parent_qos);
                if (row2[ASSOC2_REQ_QOS][0])
                    parent_qos = xstrdup(row2[ASSOC2_REQ_QOS]);

                xfree(parent_delta_qos);
                if (row2[ASSOC2_REQ_DELTA_QOS][0])
                    parent_delta_qos = xstrdup(row2[ASSOC2_REQ_DELTA_QOS]);
            }
        ...
        if (row[ASSOC_REQ_MJA])
            assoc->max_jobs_accrue = slurm_atoul(row[ASSOC_REQ_MJA]);
        else
            assoc->max_jobs_accrue = parent_mja;

        if (row[ASSOC_REQ_MPT])
            assoc->min_prio_thresh = slurm_atoul(row[ASSOC_REQ_MPT]);
        else
		 ...
		 * TRES.
		 */
        slurmdb_combine_tres_strings(&assoc->max_tres_pj, parent_mtpj, TRES_STR_FLAG_SORT_ID);
        slurmdb_combine_tres_strings(&assoc->max_tres_pn, parent_mtpn, TRES_STR_FLAG_SORT_ID);
        slurmdb_combine_tres_strings(&assoc->max_tres_mins_pj, parent_mtmpj, TRES_STR_FLAG_SORT_ID);
        slurmdb_combine_tres_strings(&assoc->max_tres_run_mins, parent_mtrm, TRES_STR_FLAG_SORT_ID);
        ...
        if (row[ASSOC_REQ_QOS][0])
			...
			*/
            ...
            if (parent_delta_qos)
                slurm_addto_char_list(delta_qos_list, parent_delta_qos + 1);

            /* now add the associations */
            if (row[ASSOC_REQ_DELTA_QOS][0])
                slurm_addto_char_list(delta_qos_list, row[ASSOC_REQ_DELTA_QOS] + 1);
        }
		...
		*/
        if (with_raw_qos && list_count(delta_qos_list)) {
            ...
            ListIterator curr_qos_itr = list_iterator_create(assoc->qos_list);
            ListIterator new_qos_itr = list_iterator_create(delta_qos_list);
    ...
    if (with_usage && assoc_list && list_count(assoc_list))
        get_usage_for_list(mysql_conn, DBD_GET_ASSOC_USAGE, assoc_list, cluster_name, assoc_cond->usage_start,
...
extern int as_mysql_get_modified_lfts(mysql_conn_t *mysql_conn, char *cluster_name, uint32_t start_lft) {
    MYSQL_RES *result = NULL;
    MYSQL_ROW row;
    char *query = xstrdup_printf("select id_assoc, lft from \"%s_%s\" where lft > %u "
                                 "&& deleted = 0", cluster_name, assoc_table, start_lft);
    if (debug_flags & DEBUG_FLAG_DB_ASSOC)
        DB_DEBUG(mysql_conn->conn, "query\n%s", query);
    if (!(result = mysql_db_query_ret(mysql_conn, query, 0))) {
        xfree(query);
    ...
    while ((row = mysql_fetch_row(result))) {
        ...
        assoc->cluster = xstrdup(cluster_name);
        if (addto_update_list(mysql_conn->update_list, SLURMDB_MODIFY_ASSOC, assoc) != SLURM_SUCCESS)
...
extern int as_mysql_add_assocs(mysql_conn_t *mysql_conn, uint32_t uid, List assoc_list) {
    ListIterator itr = NULL;
    int rc = SLURM_SUCCESS;
    int i = 0;
    ...
    if (!is_user_min_admin_level(mysql_conn, uid, SLURMDB_ADMIN_OPERATOR)) {
        ...
        if (!coord) {
            error("Coordinator %s(%d) tried to add associations "
                  "where they were not allowed", user.name, user.uid);
            return ESLURM_ACCESS_DENIED;
        }
        is_coord = true;
    }
    ...
    itr = list_iterator_create(assoc_list);
    while ((object = list_next(itr))) {
        if (!object->cluster || !object->cluster[0] || !object->acct || !object->acct[0]) {
        ...
        if (is_coord && (object->is_def == 1)) {
            error("Coordinator %s(%d) tried to change the default account of user %s to account %s", user_name, uid,
        ...
        if (is_coord &&
            _check_coord_qos(mysql_conn, object->cluster, object->acct, user_name, object->qos_list) == SLURM_ERROR) {
            assoc_mgr_lock_t locks = {NO_LOCK, NO_LOCK, READ_LOCK, NO_LOCK, NO_LOCK, NO_LOCK, NO_LOCK};
            char *requested_qos;

            assoc_mgr_lock(&locks);
            requested_qos = get_qos_complete_str(assoc_mgr_qos_list, object->qos_list);
            assoc_mgr_unlock(&locks);
            error("Coordinator %s(%d) does not have the "
                  "access to all the qos requested (%s), "
                  "so they can't add to account "
                  "%s with it.", user_name, uid, requested_qos, object->acct);
            xfree(requested_qos);
            rc = ESLURM_ACCESS_DENIED;
            break;
        }
		...
		*/
        ...
        xstrfmtcat(vals, "%ld, %ld, '%s'", now, now, object->acct);
        xstrfmtcat(update, "where acct='%s'", object->acct);

        xstrfmtcat(extra, ", mod_time=%ld, acct='%s'", now, object->acct);
        if (!object->user) {
            xstrcat(cols, ", parent_acct");
            xstrfmtcat(vals, ", '%s'", parent);
            xstrfmtcat(extra, ", parent_acct='%s', user=''", parent);
        ...
        if ((rc = setup_assoc_limits(object, &cols, &vals, &extra, QOS_LEVEL_NONE, 1))) {
        ...
        xstrfmtcat(query, "select distinct %s from \"%s_%s\" %s order by lft "
                          "FOR UPDATE;", tmp_char, object->cluster, assoc_table, update);
        xfree(tmp_char);
        if (debug_flags & DEBUG_FLAG_DB_ASSOC)
            DB_DEBUG(mysql_conn->conn, "query\n%s", query);
        if (!(result = mysql_db_query_ret(mysql_conn, query, 0))) {
        ...
        if (!(row = mysql_fetch_row(result))) {
			 ...
			 * while.
			 */
            if (!old_parent || !old_cluster || xstrcasecmp(parent, old_parent) ||
                xstrcasecmp(object->cluster, old_cluster)) {
                char *sel_query = xstrdup_printf("SELECT lft FROM \"%s_%s\" WHERE "
                                                 "acct = '%s' and user = '' "
                                                 "order by lft;", object->cluster, assoc_table, parent);
                MYSQL_RES *sel_result = NULL;

                if (incr) {
                    char *up_query = xstrdup_printf("UPDATE \"%s_%s\" SET "
                                                    "rgt = rgt+%d "
                                                    ...
                                                    "UPDATE \"%s_%s\" SET "
                                                    "deleted = 0 "
                                                    "WHERE deleted = 2;", old_cluster, assoc_table, incr, my_left,
                                                    old_cluster, assoc_table, incr, my_left, old_cluster, assoc_table);
                    if (debug_flags & DEBUG_FLAG_DB_ASSOC)
                        DB_DEBUG(mysql_conn->conn, "query\n%s", up_query);
                ...
                if (debug_flags & DEBUG_FLAG_DB_ASSOC)
                    DB_DEBUG(mysql_conn->conn, "query\n%s", sel_query);
                if (!(sel_result = mysql_db_query_ret(mysql_conn, sel_query, 0))) {
                    xfree(cols);
                    xfree(vals);
                    xfree(update);
                    xfree(extra);
            ...
            xstrfmtcat(query, "insert into \"%s_%s\" "
                              "(%s, lft, rgt, deleted) "
                              "values (%s, %d, %d, 2);", object->cluster, assoc_table, cols, vals, my_left + (incr - 1),
                       my_left + incr);
...
/* 				   vals); */
            ...
            debug("This account %s was added already", object->acct);
            xfree(cols);
            xfree(vals);
            xfree(update);
            mysql_free_result(result);
            ...
            if (object->parent_acct && xstrcasecmp(object->parent_acct, row[AASSOC_PACCT])) {

                /* We need to move the parent! */
                if (_move_parent(mysql_conn, uid, &lft, &rgt, object->cluster, row[AASSOC_ID], row[AASSOC_PACCT],
                                 object->parent_acct, now) == SLURM_ERROR)
                    continue;
                moved_parent = 1;
            } else {
                object->lft = lft;
            ...
            xstrfmtcat(query, "update \"%s_%s\" set deleted=0, "
                              "id_assoc=LAST_INSERT_ID(id_assoc)%s %s;", object->cluster, assoc_table, extra, update);
        }
        mysql_free_result(result);
		 ...
		 * happened */
        if (!moved_parent &&
            (!last_parent || !last_cluster || xstrcmp(parent, last_parent) || xstrcmp(object->cluster, last_cluster))) {
            uint32_t tmp32 = 0;
            if ((tmp32 = _get_parent_id(mysql_conn, parent, object->cluster))) {
                my_par_id = tmp32;
        ...
        if (addto_update_list(mysql_conn->update_list, SLURMDB_ADD_ASSOC, object) == SLURM_SUCCESS) {
            list_remove(itr);
        }
        ...
        if (xstrcmp(object->cluster, tmp_cluster_name)) {
            ...
            if (txn_query)
                xstrfmtcat(txn_query, ", (%ld, %d, 'id_assoc=%d', "
                                      "'%s', '%s', '%s')", now, DBD_ADD_ASSOCS, assoc_id, user_name, tmp_extra,
                           object->cluster);
            else
                xstrfmtcat(txn_query, "insert into %s "
                                      "(timestamp, action, name, actor, "
                                      "info, cluster) values (%ld, %d, "
                                      "'id_assoc=%d', '%s', '%s', '%s')", txn_table, now, DBD_ADD_ASSOCS, assoc_id,
                           user_name, tmp_extra, object->cluster);
            xfree(tmp_extra);
        }
        xfree(extra);
    }
    ...
    if (incr) {
        char *up_query = xstrdup_printf("UPDATE \"%s_%s\" SET rgt = rgt+%d "
                                        "WHERE rgt > %d && deleted < 2;"
                                        "UPDATE \"%s_%s\" SET lft = lft+%d "
                                        "WHERE lft > %d "
                                        "&& deleted < 2;"
                                        "UPDATE \"%s_%s\" SET deleted = 0 "
                                        "WHERE deleted = 2;", old_cluster, assoc_table, incr, my_left, old_cluster,
                                        assoc_table, incr, my_left, old_cluster, assoc_table);
        if (debug_flags & DEBUG_FLAG_DB_ASSOC)
            DB_DEBUG(mysql_conn->conn, "query\n%s", up_query);
        rc = mysql_db_query(mysql_conn, up_query);
        xfree(up_query);
    ...
    while ((update_object = list_next(itr))) {
        if (!update_object->objects || !list_count(update_object->objects))
            continue;
    ...
    if (update_object && update_object->objects && list_count(update_object->objects))
    ...
    if (assoc_list_tmp) {
        ...
        if (!moved_parent) {
            ...
            while ((cluster_name = list_next(itr))) {
                ...
                while ((object = list_next(itr2))) {
                    if (object->lft < smallest_lft && !xstrcmp(object->cluster, cluster_name))
        ...
        list_iterator_reset(itr2);
        while ((object = list_next(itr2))) {
            if ((object->is_def != 1) || !object->cluster || !object->acct || !object->user)
                continue;

            if ((rc = _reset_default_assoc(mysql_conn, object, &query, moved_parent ? 0 : 1)) != SLURM_SUCCESS) {
                xfree(query);
                goto end_it;
            }
        }
    ...
    if (rc != SLURM_ERROR) {
        ...
        if (txn_query) {
            xstrcat(txn_query, ";");
            debug4("%d(%s:%d) query\n%s", mysql_conn->conn, THIS_FILE, __LINE__, txn_query);
        ...
        if (moved_parent) {
            ...
            if (!(assoc_list_tmp = as_mysql_get_assocs(mysql_conn, uid, NULL))) {
...
extern List as_mysql_modify_assocs(mysql_conn_t *mysql_conn, uint32_t uid, slurmdb_assoc_cond_t *assoc_cond,
    ...
    if (!(is_admin = is_user_min_admin_level(mysql_conn, uid, SLURMDB_ADMIN_OPERATOR))) {
        if (is_user_any_coord(mysql_conn, &user)) {
            goto is_same_user;
        } else if (assoc_cond->user_list && (list_count(assoc_cond->user_list) == 1)) {
            uid_t pw_uid;
            char *name;
    ...
    if ((assoc_cond->qos_list && list_count(assoc_cond->qos_list)) || assoc_cond->with_sub_accts)
        prefix = "t2";
    ...
    if ((rc = setup_assoc_limits(assoc, &tmp_char1, &tmp_char2, &vals, QOS_LEVEL_MODIFY, 0))) {
        xfree(tmp_char1);
        xfree(tmp_char2);
        xfree(vals);
        xfree(extra);
    ...
    while ((cluster_name = list_next(itr))) {
        ...
        xstrfmtcat(query, "select distinct %s "
                          "from \"%s_%s\" as t1%s%s "
                          "order by lft FOR UPDATE;", object, cluster_name, assoc_table, qos_extra, extra);
        xfree(qos_extra);
        if (debug_flags & DEBUG_FLAG_DB_ASSOC)
            DB_DEBUG(mysql_conn->conn, "query\n%s", query);
        if (!(result = mysql_db_query_ret(mysql_conn, query, 0))) {
        ...
        xfree(query);
        rc = _process_modify_assoc_results(mysql_conn, result, assoc, &user, cluster_name, vals, is_admin, same_user,
                                           ret_list);
        mysql_free_result(result);
...
extern List as_mysql_remove_assocs(mysql_conn_t *mysql_conn, uint32_t uid, slurmdb_assoc_cond_t *assoc_cond) {
    ListIterator itr = NULL;
    List ret_list = NULL;
    int rc = SLURM_SUCCESS;
    ...
    while ((cluster_name = list_next(itr))) {
        ...
        query = xstrdup_printf("select distinct t1.lft, t1.rgt from "
                               ...
                               "lft FOR UPDATE;", cluster_name, assoc_table, qos_extra, extra);
        xfree(qos_extra);
        if (debug_flags & DEBUG_FLAG_DB_ASSOC)
            DB_DEBUG(mysql_conn->conn, "query\n%s", query);
        if (!(result = mysql_db_query_ret(mysql_conn, query, 0))) {
        ...
        while ((row = mysql_fetch_row(result))) {
            if (name_char)
                xstrfmtcat(name_char, " || lft between %s and %s", row[0], row[1]);
            else
                xstrfmtcat(name_char, "lft between %s and %s", row[0], row[1]);
        }
        mysql_free_result(result);
        ...
        query = xstrdup_printf("select distinct %s "
                               "from \"%s_%s\" where (%s) "
                               "and deleted = 0 order by lft;", object, cluster_name, assoc_table, name_char);
        if (debug_flags & DEBUG_FLAG_DB_ASSOC)
            DB_DEBUG(mysql_conn->conn, "query\n%s", query);
        if (!(result = mysql_db_query_ret(mysql_conn, query, 0))) {
            xfree(query);
        ...
        rc = _process_remove_assoc_results(mysql_conn, result, &user, cluster_name, name_char, is_admin, ret_list,
                                           &jobs_running);
        xfree(name_char);
        mysql_free_result(result);
...
extern List as_mysql_get_assocs(mysql_conn_t *mysql_conn, uid_t uid, slurmdb_assoc_cond_t *assoc_cond) {
    //DEF_TIMERS;
    char *extra = NULL;
    char *tmp = NULL;
    ...
    if (private_data & PRIVATE_DATA_USERS) {
        if (!(is_admin = is_user_min_admin_level(mysql_conn, uid, SLURMDB_ADMIN_OPERATOR))) {
            /* Fill in the user with any accounts they may
			   ...
			   _cluster_get_assocs.
			*/
            assoc_mgr_fill_in_user(mysql_conn, &user, 1, NULL, false);
        }
    ...
    while ((cluster_name = list_next(itr))) {
        int rc;
        if ((rc = _cluster_get_assocs(mysql_conn, &user, assoc_cond, cluster_name, tmp, extra, is_admin, assoc_list)) !=
            SLURM_SUCCESS) {
            FREE_NULL_LIST(assoc_list);
            assoc_list = NULL;
            break;
...
extern int as_mysql_reset_lft_rgt(mysql_conn_t *mysql_conn, uid_t uid, List cluster_list) {
    List assoc_list = NULL;
    ...
    while ((cluster_name = list_next(itr))) {
        ...
        if ((rc = _cluster_get_assocs(mysql_conn, &user, &assoc_cond, cluster_name, tmp, " deleted=1 || deleted=0",
                                      is_admin, assoc_list)) != SLURM_SUCCESS) {
        ...
        if (!list_count(assoc_list)) {
            info("Cluster %s has no associations, nothing to reset", cluster_name);
            FREE_NULL_LIST(assoc_list);
            continue;
        }
        ...
        xstrfmtcat(query, "insert into \"%s_%s\" "
                          ...
                          "id_assoc=LAST_INSERT_ID(id_assoc), mod_time=%ld;", tmp_cluster_name, assoc_table, now, now,
                   now);
        ...
        if (as_mysql_add_assocs(mysql_conn, uid, assoc_list) != SLURM_SUCCESS)
            goto endit;
        ...
        query = xstrdup_printf("update \"%s_%s\" t1 left outer join \"%s_%s\" t2 on "
                               "t1.id_assoc = t2.id_assoc set t1.lft = t2.lft, "
                               "t1.rgt = t2.rgt, t1.mod_time = t2.mod_time;", cluster_name, assoc_table,
        ...
        if (!(update_object = list_find_first(mysql_conn->update_list, slurmdb_find_update_object_in_list, &type))) {
            ...
            update_object->objects = list_create(slurmdb_destroy_assoc_rec);
        }

        /* set this up to get the associations with parent_limits */
        assoc_cond.without_parent_limits = 0;
        if ((rc = _cluster_get_assocs(mysql_conn, &user, &assoc_cond, cluster_name, tmp, " deleted=0", is_admin,
        ...
        query = xstrdup_printf("drop table \"%s_%s\";", tmp_cluster_name, assoc_table);
        rc = mysql_db_query(mysql_conn, query);
        xfree(query);
        if (rc != SLURM_SUCCESS) {
            error("problem with update query");
