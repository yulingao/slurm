...
enum {
    JASSOC_JOB, JASSOC_ACCT, JASSOC_USER, JASSOC_PART, JASSOC_COUNT
};
...
static int _set_qos_cnt(mysql_conn_t *mysql_conn) {
    ...
    assoc_mgr_lock_t locks = {NO_LOCK, NO_LOCK, WRITE_LOCK, NO_LOCK, NO_LOCK, NO_LOCK, NO_LOCK};
...
static void _process_running_jobs_result(char *cluster_name, MYSQL_RES *result, List ret_list) {
    MYSQL_ROW row;
    char *object;
    ...
    while ((row = mysql_fetch_row(result))) {
        if (!row[JASSOC_USER][0]) {
            ...
            error("How did we get a job running on an association "
                  ...
                  "'%s' acct '%s'?", row[JASSOC_JOB], cluster_name, row[JASSOC_ACCT]);
            continue;
        }
        object = xstrdup_printf("JobID = %-10s C = %-10s A = %-10s U = %-9s", row[JASSOC_JOB], cluster_name,
                                row[JASSOC_ACCT], row[JASSOC_USER]);
 ...
 */
static bool _check_jobs_before_remove(mysql_conn_t *mysql_conn, char *cluster_name, char *assoc_char, List ret_list,
    ...
    static char *jassoc_req_inx[] = {"t0.id_job", "t1.acct", "t1.user", "t1.partition"};
    if (ret_list) {
        ...
        query = xstrdup_printf("select distinct %s "
                               ...
                               "and t0.time_end=0 && t0.state<%d;", object, cluster_name, job_table, cluster_name,
                               assoc_table, cluster_name, assoc_table, assoc_char, JOB_COMPLETE);
        xfree(object);
    } else {
        query = xstrdup_printf("select t0.id_assoc from \"%s_%s\" as t0, "
                               ...
                               "where t1.lft between "
                               "t2.lft and t2.rgt && (%s) "
                               "and t0.id_assoc=t1.id_assoc limit 1;", cluster_name, job_table, cluster_name,
                               assoc_table, cluster_name, assoc_table, assoc_char);
    }
...
static bool
_check_jobs_before_remove_assoc(mysql_conn_t *mysql_conn, char *cluster_name, char *assoc_char, List ret_list,
                                bool *already_flushed) {
    char *query = NULL, *object = NULL;
    bool rc = 0;
    int i;
    ...
    /* if this changes you will need to edit the corresponding
	 * enum above in the global settings */
    static char *jassoc_req_inx[] = {"t1.id_job", "t2.acct", "t2.user", "t2.partition"};
    ...
    if (ret_list) {
        ...
        query = xstrdup_printf("select %s "
                               ...
                               "and t1.time_end=0 && t1.state<%d;", object, cluster_name, job_table, cluster_name,
                               assoc_table, assoc_char, JOB_COMPLETE);
        xfree(object);
    } else {
        query = xstrdup_printf("select t1.id_assoc from \"%s_%s\" as t1, "
                               "\"%s_%s\" as t2 where (%s) "
                               "and t1.id_assoc=t2.id_assoc limit 1;", cluster_name, job_table, cluster_name,
 ...
 * like qos or wckey */
static bool
_check_jobs_before_remove_without_assoctable(mysql_conn_t *mysql_conn, char *cluster_name, char *where_char) {
    char *query = NULL;
    bool rc = 0;
    MYSQL_RES *result = NULL;

    query = xstrdup_printf("select id_assoc from \"%s_%s\" "
                           "where (%s) limit 1;", cluster_name, job_table, where_char);
...
static int _as_mysql_acct_check_tables(mysql_conn_t *mysql_conn) {
    storage_field_t acct_coord_table_fields[] = {{"creation_time", "bigint unsigned not null"},
    ...
    storage_field_t acct_table_fields[] = {{"creation_time", "bigint unsigned not null"},
                                           {"mod_time",      "bigint unsigned default 0 not null"},
                                           {"deleted",       "tinyint default 0"},
    ...
    storage_field_t tres_table_fields[] = {{"creation_time", "bigint unsigned not null"},
    ...
    storage_field_t cluster_table_fields[] = {{"creation_time",    "bigint unsigned not null"},
                                              {"mod_time",         "bigint unsigned default 0 not null"},
                                              {"deleted",          "tinyint default 0"},
                                              {"name",             "tinytext not null"},
    ...
    storage_field_t clus_res_table_fields[] = {{"creation_time",   "bigint unsigned not null"},
                                               {"mod_time",        "bigint unsigned default 0 not null"},
                                               {"deleted",         "tinyint default 0"},
    ...
    storage_field_t convert_version_table_fields[] = {{"mod_time", "bigint unsigned default 0 not null"},
                                                      {"version",  "int default 0"},
                                                      {NULL, NULL}};

    storage_field_t federation_table_fields[] = {{"creation_time", "int unsigned not null"},
    ...
    storage_field_t qos_table_fields[] = {{"creation_time",             "bigint unsigned not null"},
                                          {"mod_time",                  "bigint unsigned default 0 not null"},
                                          {"deleted",                   "tinyint default 0"},
                                          {"id",                        "int not null auto_increment"},
                                          {"name",                      "tinytext not null"},
    ...
    storage_field_t res_table_fields[] = {{"creation_time", "bigint unsigned not null"},
                                          {"mod_time",      "bigint unsigned default 0 not null"},
                                          {"deleted",       "tinyint default 0"},
                                          {"id",            "int not null auto_increment"},
                                          {"name",          "tinytext not null"},
    ...
    storage_field_t txn_table_fields[] = {{"id",        "int not null auto_increment"},
    ...
    storage_field_t user_table_fields[] = {{"creation_time", "bigint unsigned not null"},
                                           {"mod_time",      "bigint unsigned default 0 not null"},
                                           {"deleted",       "tinyint default 0"},
                                           {"name",          "tinytext not null"},
	 ...
	 */
    char *get_parent_proc = "drop procedure if exists get_parent_limits; "
    ...
    /* 	"|| @my_acct = '' END REPEAT; " */
    /* 	"END;"; */
    char *get_coord_qos = "drop procedure if exists get_coord_qos; "
    ...
    if (mysql_db_create_table(mysql_conn, convert_version_table, convert_version_table_fields,
                              ", primary key (version))") == SLURM_ERROR)
        return SLURM_ERROR;

    /* Make the cluster table first since we build other tables
	   built off this one */
    if (mysql_db_create_table(mysql_conn, cluster_table, cluster_table_fields, ", primary key (name(42)))") ==
        SLURM_ERROR)
        return SLURM_ERROR;
	...
	*/
    if (mysql_db_create_table(mysql_conn, txn_table, txn_table_fields, ", primary key (id))") == SLURM_ERROR)
        return SLURM_ERROR;

    if (mysql_db_create_table(mysql_conn, tres_table, tres_table_fields, ", primary key (id), "
                                                                         "unique index udex (type(42), name(42))) "
                                                                         "auto_increment=1001") == SLURM_ERROR)
        return SLURM_ERROR;
    ...
    if (!backup_dbd) {
		...
		*/
        query = xstrdup_printf("insert into %s (creation_time, id, deleted, type) values "
                               "(%ld, %d, 0, 'cpu'), "
                               "(%ld, %d, 0, 'mem'), "
                               "(%ld, %d, 0, 'energy'), "
                               "(%ld, %d, 0, 'node'), "
                               ...
                               tres_table, now, TRES_CPU, now, TRES_MEM, now, TRES_ENERGY, now, TRES_NODE, now,
                               TRES_BILLING, now, TRES_VMEM, now, TRES_PAGES, now, TRES_OFFSET);
        ...
        query = xstrdup_printf("insert into %s (creation_time, id, deleted, type, name) values "
                               "(%ld, %d, 0, 'fs', 'disk') "
                               "on duplicate key update deleted=VALUES(deleted), type=VALUES(type), name=VALUES(name), id=VALUES(id);",
                               tres_table, now, TRES_FS_DISK);
        if (debug_flags & DEBUG_FLAG_DB_TRES)
            DB_DEBUG(mysql_conn->conn, "%s", query);
        rc = mysql_db_query(mysql_conn, query);
        xfree(query);
	...
	*/
    if (!(as_mysql_total_cluster_list = _get_cluster_names(mysql_conn, 1))) {
        error("issue getting total contents of %s", cluster_table);
        slurm_mutex_unlock(&as_mysql_cluster_list_lock);
        return SLURM_ERROR;
    }

    if ((rc = as_mysql_convert_tables_pre_create(mysql_conn)) != SLURM_SUCCESS) {
        slurm_mutex_unlock(&as_mysql_cluster_list_lock);
    ...
    while ((cluster_name = list_next(itr))) {
        if ((rc = create_cluster_tables(mysql_conn, cluster_name)) != SLURM_SUCCESS)
            break;
    }
    ...
    if (mysql_db_create_table(mysql_conn, acct_coord_table, acct_coord_table_fields,
                              ", primary key (acct(42), user(42)), "
                              "key user (user(42)))") == SLURM_ERROR)
        return SLURM_ERROR;

    if (mysql_db_create_table(mysql_conn, acct_table, acct_table_fields, ", primary key (name(42)))") == SLURM_ERROR)
        return SLURM_ERROR;

    if (mysql_db_create_table(mysql_conn, res_table, res_table_fields, ", primary key (id), "
                                                                       "unique index udex (name(42), server(42), type))") ==
        SLURM_ERROR)
        return SLURM_ERROR;

    if (mysql_db_create_table(mysql_conn, clus_res_table, clus_res_table_fields, ", primary key (res_id, cluster(42)), "
                                                                                 "unique index udex (res_id, cluster(42)))") ==
        SLURM_ERROR)
        return SLURM_ERROR;


    if (mysql_db_create_table(mysql_conn, qos_table, qos_table_fields, ", primary key (id), "
                                                                       "unique index udex (name(42)))") == SLURM_ERROR)
        return SLURM_ERROR;
    else {
        int qos_id = 0;
        if (slurmdbd_conf && slurmdbd_conf->default_qos) {
            ...
            char *qos = NULL;
            ListIterator itr = NULL;
            slurm_addto_char_list(char_list, slurmdbd_conf->default_qos);
			...
			*/
            ...
            while ((qos = list_next(itr))) {
                query = xstrdup_printf("insert into %s "
                                       ...
                                       "id=LAST_INSERT_ID(id), deleted=0;", qos_table, now, now, qos);
            ...
            query = xstrdup_printf("insert into %s "
                                   ...
                                   "'Normal QOS default') "
                                   "on duplicate key update "
                                   "id=LAST_INSERT_ID(id), deleted=0;", qos_table, now, now);
            if (debug_flags & DEBUG_FLAG_DB_QOS)
                DB_DEBUG(mysql_conn->conn, "%s", query);
            qos_id = (int) mysql_db_insert_ret_id(mysql_conn, query);
            if (!qos_id)
    ...
    if (mysql_db_create_table(mysql_conn, user_table, user_table_fields, ", primary key (name(42)))") == SLURM_ERROR)
        return SLURM_ERROR;

    if (mysql_db_create_table(mysql_conn, federation_table, federation_table_fields, ", primary key (name(42)))") ==
        SLURM_ERROR)
        return SLURM_ERROR;
	 ...
	 * really be deleted.
	 */
    query = xstrdup_printf("insert into %s (creation_time, mod_time, name, "
                           "admin_level) values (%ld, %ld, 'root', %d) "
                           "on duplicate key update name='root';", user_table, (long) now, (long) now,
                           SLURMDB_ADMIN_SUPER_USER);
    xstrfmtcat(query, "insert into %s (creation_time, mod_time, name, "
                      "description, organization) values (%ld, %ld, 'root', "
                      "'default root account', 'root') on duplicate key "
                      "update name='root';", acct_table, (long) now, (long) now);
...
extern int check_connection(mysql_conn_t *mysql_conn) {
    if (!mysql_conn) {
        ...
        if (mysql_db_get_db_connection(mysql_conn, mysql_db_name, mysql_db_info) != SLURM_SUCCESS) {
...
extern int last_affected_rows(mysql_conn_t *mysql_conn) {
    ...
    do {
        ...
        if (result)
            mysql_free_result(result);
        else if (mysql_field_count(mysql_conn->db_conn) == 0) {
            status = mysql_affected_rows(mysql_conn->db_conn);
            if (status > 0)
                rows = status;
        }
        if ((status = mysql_next_result(mysql_conn->db_conn)) > 0)
            if (debug_flags & DEBUG_FLAG_DB_ASSOC)
                DB_DEBUG(mysql_conn->conn, "Could not execute statement\n");
    } while (status == 0);
...
extern int create_cluster_assoc_table(mysql_conn_t *mysql_conn, char *cluster_name) {
    storage_field_t assoc_table_fields[] = {{"creation_time",     "bigint unsigned not null"},
                                            {"mod_time",          "bigint unsigned default 0 not null"},
                                            {"deleted",           "tinyint default 0 not null"},
    ...
    snprintf(table_name, sizeof(table_name), "\"%s_%s\"", cluster_name, assoc_table);
    if (mysql_db_create_table(mysql_conn, table_name, assoc_table_fields, ", primary key (id_assoc), "
...
extern int create_cluster_tables(mysql_conn_t *mysql_conn, char *cluster_name) {
    storage_field_t cluster_usage_table_fields[] = {{"creation_time", "bigint unsigned not null"},
                                                    {"mod_time",      "bigint unsigned default 0 not null"},
                                                    {"deleted",       "tinyint default 0 not null"},
    ...
    storage_field_t event_table_fields[] = {{"time_start",    "bigint unsigned not null"},
    ...
    storage_field_t id_usage_table_fields[] = {{"creation_time", "bigint unsigned not null"},
                                               {"mod_time",      "bigint unsigned default 0 not null"},
                                               {"deleted",       "tinyint default 0 not null"},
    ...
    storage_field_t job_table_fields[] = {{"job_db_inx",         "bigint unsigned not null auto_increment"},
                                          {"mod_time",           "bigint unsigned default 0 not null"},
                                          {"deleted",            "tinyint default 0 not null"},
                                          {"account",            "tinytext"},
                                          {"admin_comment",      "text"},
    ...
    storage_field_t last_ran_table_fields[] = {{"hourly_rollup",  "bigint unsigned default 0 not null"},
                                               {"daily_rollup",   "bigint unsigned default 0 not null"},
                                               {"monthly_rollup", "bigint unsigned default 0 not null"},
                                               {NULL, NULL}};

    storage_field_t resv_table_fields[] = {{"id_resv",     "int unsigned default 0 not null"},
                                           {"deleted",     "tinyint default 0 not null"},
    ...
    storage_field_t step_table_fields[] = {{"job_db_inx",                "bigint unsigned not null"},
                                           {"deleted",                   "tinyint default 0 not null"},
    ...
    storage_field_t suspend_table_fields[] = {{"job_db_inx", "bigint unsigned not null"},
                                              {"id_assoc",   "int not null"},
                                              {"time_start", "bigint unsigned default 0 not null"},
                                              {"time_end",   "bigint unsigned default 0 not null"},
                                              {NULL, NULL}};

    storage_field_t wckey_table_fields[] = {{"creation_time", "bigint unsigned not null"},
                                            {"mod_time",      "bigint unsigned default 0 not null"},
                                            {"deleted",       "tinyint default 0 not null"},
                                            {"is_def",        "tinyint default 0 not null"},
    ...
    if (create_cluster_assoc_table(mysql_conn, cluster_name) == SLURM_ERROR)
        return SLURM_ERROR;

    snprintf(table_name, sizeof(table_name), "\"%s_%s\"", cluster_name, assoc_day_table);

    if (mysql_db_create_table(mysql_conn, table_name, id_usage_table_fields,
                              ", primary key (id, id_tres, time_start))") == SLURM_ERROR)
        return SLURM_ERROR;

    snprintf(table_name, sizeof(table_name), "\"%s_%s\"", cluster_name, assoc_hour_table);

    if (mysql_db_create_table(mysql_conn, table_name, id_usage_table_fields,
                              ", primary key (id, id_tres, time_start))") == SLURM_ERROR)
        return SLURM_ERROR;

    snprintf(table_name, sizeof(table_name), "\"%s_%s\"", cluster_name, assoc_month_table);

    if (mysql_db_create_table(mysql_conn, table_name, id_usage_table_fields,
                              ", primary key (id, id_tres, time_start))") == SLURM_ERROR)
        return SLURM_ERROR;

    snprintf(table_name, sizeof(table_name), "\"%s_%s\"", cluster_name, cluster_day_table);

    if (mysql_db_create_table(mysql_conn, table_name, cluster_usage_table_fields,
                              ", primary key (id_tres, time_start))") == SLURM_ERROR)
        return SLURM_ERROR;

    snprintf(table_name, sizeof(table_name), "\"%s_%s\"", cluster_name, cluster_hour_table);

    if (mysql_db_create_table(mysql_conn, table_name, cluster_usage_table_fields,
                              ", primary key (id_tres, time_start))") == SLURM_ERROR)
        return SLURM_ERROR;

    snprintf(table_name, sizeof(table_name), "\"%s_%s\"", cluster_name, cluster_month_table);

    if (mysql_db_create_table(mysql_conn, table_name, cluster_usage_table_fields,
                              ", primary key (id_tres, time_start))") == SLURM_ERROR)
        return SLURM_ERROR;

    snprintf(table_name, sizeof(table_name), "\"%s_%s\"", cluster_name, event_table);

    if (mysql_db_create_table(mysql_conn, table_name, event_table_fields,
                              ", primary key (node_name(42), time_start))") == SLURM_ERROR)
        return SLURM_ERROR;

    snprintf(table_name, sizeof(table_name), "\"%s_%s\"", cluster_name, job_table);
    /*
    ...
    if (mysql_db_create_table(mysql_conn, table_name, job_table_fields, ", primary key (job_db_inx), "
                                                                        ...
                                                                        "time_eligible))") == SLURM_ERROR)
        return SLURM_ERROR;

    snprintf(table_name, sizeof(table_name), "\"%s_%s\"", cluster_name, last_ran_table);
    if (mysql_db_create_table(mysql_conn, table_name, last_ran_table_fields, ", primary key (hourly_rollup, "
                                                                             "daily_rollup, monthly_rollup))") ==
        SLURM_ERROR)
        return SLURM_ERROR;

    snprintf(table_name, sizeof(table_name), "\"%s_%s\"", cluster_name, resv_table);
    if (mysql_db_create_table(mysql_conn, table_name, resv_table_fields, ", primary key (id_resv, time_start))") ==
        SLURM_ERROR)
        return SLURM_ERROR;

    snprintf(table_name, sizeof(table_name), "\"%s_%s\"", cluster_name, step_table);
    if (mysql_db_create_table(mysql_conn, table_name, step_table_fields, ", primary key (job_db_inx, id_step))") ==
        SLURM_ERROR)
        return SLURM_ERROR;

    snprintf(table_name, sizeof(table_name), "\"%s_%s\"", cluster_name, suspend_table);
    if (mysql_db_create_table(mysql_conn, table_name, suspend_table_fields, ", primary key (job_db_inx, time_start), "
                                                                            "key job_db_inx_times (job_db_inx, "
                                                                            "time_start, time_end))") == SLURM_ERROR)
        return SLURM_ERROR;

    snprintf(table_name, sizeof(table_name), "\"%s_%s\"", cluster_name, wckey_table);
    if (mysql_db_create_table(mysql_conn, table_name, wckey_table_fields, ", primary key (id_wckey), "
                                                                          " unique index udex (wckey_name(42), "
                                                                          "user(42)))") == SLURM_ERROR)
        return SLURM_ERROR;

    snprintf(table_name, sizeof(table_name), "\"%s_%s\"", cluster_name, wckey_day_table);

    if (mysql_db_create_table(mysql_conn, table_name, id_usage_table_fields,
                              ", primary key (id, id_tres, time_start))") == SLURM_ERROR)
        return SLURM_ERROR;

    snprintf(table_name, sizeof(table_name), "\"%s_%s\"", cluster_name, wckey_hour_table);

    if (mysql_db_create_table(mysql_conn, table_name, id_usage_table_fields,
                              ", primary key (id, id_tres, time_start))") == SLURM_ERROR)
        return SLURM_ERROR;

    snprintf(table_name, sizeof(table_name), "\"%s_%s\"", cluster_name, wckey_month_table);
...
extern int remove_cluster_tables(mysql_conn_t *mysql_conn, char *cluster_name) {
    ...
    query = xstrdup_printf("select id_assoc from \"%s_%s\" limit 1;", cluster_name, assoc_table);
    ...
    mysql_free_result(result);
    xstrfmtcat(mysql_conn->pre_commit_query, "drop table \"%s_%s\", \"%s_%s\", \"%s_%s\", "
                                             "\"%s_%s\", \"%s_%s\", \"%s_%s\", \"%s_%s\", "
                                             "\"%s_%s\", \"%s_%s\", \"%s_%s\", \"%s_%s\", "
                                             "\"%s_%s\", \"%s_%s\", \"%s_%s\", \"%s_%s\", "
                                             "\"%s_%s\", \"%s_%s\";", cluster_name, assoc_table, cluster_name,
               assoc_day_table, cluster_name, assoc_hour_table, cluster_name, assoc_month_table, cluster_name,
               cluster_day_table, cluster_name, cluster_hour_table, cluster_name, cluster_month_table, cluster_name,
               event_table, cluster_name, job_table, cluster_name, last_ran_table, cluster_name, resv_table,
               cluster_name, step_table, cluster_name, suspend_table, cluster_name, wckey_table, cluster_name,
               wckey_day_table, cluster_name, wckey_hour_table, cluster_name, wckey_month_table);
...
extern int setup_assoc_limits(slurmdb_assoc_rec_t *assoc, char **cols, char **vals, char **extra, qos_level_t qos_level,
                              bool for_add) {
    uint32_t tres_str_flags =
            TRES_STR_FLAG_REMOVE | TRES_STR_FLAG_SORT_ID | TRES_STR_FLAG_SIMPLE | TRES_STR_FLAG_NO_NULL;

    assoc_mgr_lock_t locks = {NO_LOCK, NO_LOCK, READ_LOCK, NO_LOCK, NO_LOCK, NO_LOCK, NO_LOCK};
    if (!assoc)
        return SLURM_ERROR;
    ...
    if (assoc->shares_raw == INFINITE) {
        ...
        assoc->shares_raw = 1;
    } else if ((assoc->shares_raw != NO_VAL) && (int32_t) assoc->shares_raw >= 0) {
        xstrcat(*cols, ", shares");
    ...
    } else if ((assoc->grp_jobs != NO_VAL) && ((int32_t) assoc->grp_jobs >= 0)) {
        xstrcat(*cols, ", grp_jobs");
    ...
    } else if ((assoc->grp_jobs_accrue != NO_VAL) && ((int32_t) assoc->grp_jobs_accrue >= 0)) {
        xstrcat(*cols, ", grp_jobs_accrue");
        xstrfmtcat(*vals, ", %u", assoc->grp_jobs_accrue);
        xstrfmtcat(*extra, ", grp_jobs_accrue=%u", assoc->grp_jobs_accrue);
    }
    ...
    } else if ((assoc->grp_submit_jobs != NO_VAL) && ((int32_t) assoc->grp_submit_jobs >= 0)) {
        xstrcat(*cols, ", grp_submit_jobs");
    ...
    } else if ((assoc->grp_wall != NO_VAL) && ((int32_t) assoc->grp_wall >= 0)) {
        xstrcat(*cols, ", grp_wall");
    ...
    if ((assoc->is_def == 1) && ((qos_level == QOS_LEVEL_MODIFY) || (assoc->user && assoc->cluster && assoc->acct))) {
        xstrcat(*cols, ", is_def");
        xstrcat(*vals, ", 1");
    ...
    } else if ((assoc->max_jobs != NO_VAL) && ((int32_t) assoc->max_jobs >= 0)) {
        xstrcat(*cols, ", max_jobs");
    ...
    } else if ((assoc->max_jobs_accrue != NO_VAL) && ((int32_t) assoc->max_jobs_accrue >= 0)) {
        xstrcat(*cols, ", max_jobs_accrue");
        xstrfmtcat(*vals, ", %u", assoc->max_jobs_accrue);
        xstrfmtcat(*extra, ", max_jobs_accrue=%u", assoc->max_jobs_accrue);
    }
    ...
    } else if ((assoc->min_prio_thresh != NO_VAL) && ((int32_t) assoc->min_prio_thresh >= 0)) {
        xstrcat(*cols, ", min_prio_thresh");
        xstrfmtcat(*vals, ", %u", assoc->min_prio_thresh);
        xstrfmtcat(*extra, ", min_prio_thresh=%u", assoc->min_prio_thresh);
    }
    ...
    } else if ((assoc->max_submit_jobs != NO_VAL) && ((int32_t) assoc->max_submit_jobs >= 0)) {
        xstrcat(*cols, ", max_submit_jobs");
        xstrfmtcat(*vals, ", %u", assoc->max_submit_jobs);
        xstrfmtcat(*extra, ", max_submit_jobs=%u", assoc->max_submit_jobs);
    }
    ...
    } else if ((assoc->max_wall_pj != NO_VAL) && ((int32_t) assoc->max_wall_pj >= 0)) {
        xstrcat(*cols, ", max_wall_pj");
    ...
    } else if ((assoc->priority != NO_VAL) && ((int32_t) assoc->priority >= 0)) {
        xstrcat(*cols, ", priority");
    ...
    } else if ((assoc->def_qos_id != NO_VAL) && ((int32_t) assoc->def_qos_id > 0)) {
        assoc_mgr_lock(&locks);
        if (!list_find_first(assoc_mgr_qos_list, slurmdb_find_qos_in_list, &(assoc->def_qos_id))) {
            assoc_mgr_unlock(&locks);
    ...
    if (assoc->grp_tres) {
        ...
        slurmdb_combine_tres_strings(&assoc->grp_tres, NULL, tres_str_flags);
    ...
    if (assoc->grp_tres_mins) {
        ...
        slurmdb_combine_tres_strings(&assoc->grp_tres_mins, NULL, tres_str_flags);
        xstrfmtcat(*vals, ", '%s'", assoc->grp_tres_mins);
        xstrfmtcat(*extra, ", grp_tres_mins='%s'", assoc->grp_tres_mins);
    }
    ...
    if (assoc->grp_tres_run_mins) {
        ...
        slurmdb_combine_tres_strings(&assoc->grp_tres_run_mins, NULL, tres_str_flags);
        xstrfmtcat(*vals, ", '%s'", assoc->grp_tres_run_mins);
        xstrfmtcat(*extra, ", grp_tres_run_mins='%s'", assoc->grp_tres_run_mins);
    }
    ...
    if (assoc->max_tres_pj) {
        ...
        slurmdb_combine_tres_strings(&assoc->max_tres_pj, NULL, tres_str_flags);
    ...
    if (assoc->max_tres_pn) {
        ...
        slurmdb_combine_tres_strings(&assoc->max_tres_pn, NULL, tres_str_flags);
    ...
    if (assoc->max_tres_mins_pj) {
        ...
        slurmdb_combine_tres_strings(&assoc->max_tres_mins_pj, NULL, tres_str_flags);
        xstrfmtcat(*vals, ", '%s'", assoc->max_tres_mins_pj);
        xstrfmtcat(*extra, ", max_tres_mins_pj='%s'", assoc->max_tres_mins_pj);
    }
    ...
    if (assoc->max_tres_run_mins) {
        ...
        slurmdb_combine_tres_strings(&assoc->max_tres_run_mins, NULL, tres_str_flags);
        xstrfmtcat(*vals, ", '%s'", assoc->max_tres_run_mins);
        xstrfmtcat(*extra, ", max_tres_run_mins='%s'", assoc->max_tres_run_mins);
    }
...
modify_common(mysql_conn_t *mysql_conn, uint16_t type, time_t now, char *user_name, char *table, char *cond_char,
    ...
    if ((table == cluster_table) || (table == acct_coord_table) || (table == acct_table) || (table == qos_table) ||
        (table == txn_table) || (table == user_table) || (table == res_table) || (table == clus_res_table) ||
    ...
    if (cluster_centric) {
        ...
        xstrfmtcat(query, "update \"%s_%s\" set mod_time=%ld%s "
                          "where deleted=0 && %s;", cluster_name, table, now, vals, cond_char);
        xstrfmtcat(query, "insert into %s "
                          "(timestamp, action, name, cluster, actor, info) "
                          "values (%ld, %d, '%s', '%s', '%s', '%s');", txn_table, now, type, tmp_cond_char,
                   cluster_name, user_name, tmp_vals);
    } else {
        xstrfmtcat(query, "update %s set mod_time=%ld%s "
                          "where deleted=0 && %s;", table, now, vals, cond_char);
        xstrfmtcat(query, "insert into %s "
                          "(timestamp, action, name, actor, info) "
                          "values (%ld, %d, '%s', '%s', '%s');", txn_table, now, type, tmp_cond_char, user_name,
                   tmp_vals);
    }
    xfree(tmp_cond_char);
    xfree(tmp_vals);
...
remove_common(mysql_conn_t *mysql_conn, uint16_t type, time_t now, char *user_name, char *table, char *name_char,
	 ...
	 */
    if ((table == acct_coord_table) || (table == res_table) || (table == clus_res_table) ||
        (table == federation_table)) {
        ...
        if (cluster_name)
            has_jobs = _check_jobs_before_remove_without_assoctable(mysql_conn, cluster_name, assoc_char);
    } else if (table != assoc_table) {
        /* first check to see if we are running jobs now */
        if (_check_jobs_before_remove(mysql_conn, cluster_name, assoc_char, ret_list, jobs_running) || (*jobs_running))
            return SLURM_SUCCESS;

        has_jobs = _check_jobs_before_remove(mysql_conn, cluster_name, assoc_char, NULL, NULL);
    } else {
        /* first check to see if we are running jobs now */
        if (_check_jobs_before_remove_assoc(mysql_conn, cluster_name, name_char, ret_list, jobs_running) ||
            (*jobs_running))
            return SLURM_SUCCESS;

        /* now check to see if any jobs were ever run. */
        has_jobs = _check_jobs_before_remove_assoc(mysql_conn, cluster_name, name_char, NULL, NULL);
    }
    ...
    if (!has_jobs && table != assoc_table) {
        if (cluster_centric) {
            query = xstrdup_printf("delete from \"%s_%s\" where "
                                   "creation_time>%ld && (%s);", cluster_name, table, day_old, name_char);
        } else {
            query = xstrdup_printf("delete from %s where "
                                   "creation_time>%ld && (%s);", table, day_old, name_char);
        }
    }

    if (table != assoc_table) {
        if (cluster_centric) {
            xstrfmtcat(query, "update \"%s_%s\" set mod_time=%ld, "
                              "deleted=1 where deleted=0 && (%s);", cluster_name, table, now, name_char);
        } else if (table == federation_table) {
            xstrfmtcat(query, "update %s set "
                              "mod_time=%ld, deleted=1, "
                              "flags=DEFAULT "
                              "where deleted=0 && (%s);", federation_table, now, name_char);
            ...
            xstrfmtcat(query, "update %s set "
                              ...
                              "usage_thres=DEFAULT "
                              "where deleted=0 && (%s);", qos_table, now, name_char);
        } else {
            xstrfmtcat(query, "update %s set mod_time=%ld, deleted=1 "
                              "where deleted=0 && (%s);", table, now, name_char);
        }
    }
	...
	*/
    ...
    if (cluster_centric)
        xstrfmtcat(query, "insert into %s (timestamp, action, name, "
                          "actor, cluster) values "
                          "(%ld, %d, '%s', '%s', '%s');", txn_table, now, type, tmp_name_char, user_name, cluster_name);
    else
        xstrfmtcat(query, "insert into %s (timestamp, action, name, actor) "
                          "values (%ld, %d, '%s', '%s');", txn_table, now, type, tmp_name_char, user_name);
    ...
    if (rc != SLURM_SUCCESS) {
        reset_mysql_conn(mysql_conn);
        return SLURM_ERROR;
    } else if ((table == acct_coord_table) || (table == wckey_table) || (table == clus_res_table) ||
               (table == res_table) || (table == federation_table) || (table == qos_table))
        return SLURM_SUCCESS;
    ...
    if (table != assoc_table) {
        ...
        query = xstrdup_printf("select distinct t1.id_assoc "
                               ...
                               "&& t2.deleted=0;", cluster_name, assoc_table, cluster_name, assoc_table, assoc_char);
	 ...
	 */
    xstrfmtcat(query, "update \"%s_%s\" set mod_time=%ld, deleted=1 where (%s);"
                      "update \"%s_%s\" set mod_time=%ld, deleted=1 where (%s);"
                      "update \"%s_%s\" set mod_time=%ld, deleted=1 where (%s);", cluster_name, assoc_day_table, now,
               loc_usage_id_char, cluster_name, assoc_hour_table, now, loc_usage_id_char, cluster_name,
               assoc_month_table, now, loc_usage_id_char);
    xfree(loc_usage_id_char);

    if (debug_flags & DEBUG_FLAG_DB_ASSOC)
        DB_DEBUG(mysql_conn->conn, "query\n%s %zu", query, strlen(query));
    rc = mysql_db_query(mysql_conn, query);
    xfree(query);
    if (rc != SLURM_SUCCESS) {
        reset_mysql_conn(mysql_conn);
    ...
    query = xstrdup_printf("select id_assoc from \"%s_%s\" as t1 where "
                           "creation_time>%ld && (%s);", cluster_name, assoc_table, day_old, loc_assoc_char);
    ...
    while ((row = mysql_fetch_row(result))) {
        ...
        /* we have to do this one at a time since the lft's and rgt's
		   ...
		   in the association. */
        xstrfmtcat(query, "SELECT lft, rgt, (rgt - lft + 1) "
                          "FROM \"%s_%s\" WHERE id_assoc = %s;", cluster_name, assoc_table, row[0]);
        if (debug_flags & DEBUG_FLAG_DB_ASSOC)
            DB_DEBUG(mysql_conn->conn, "query\n%s", query);
        ...
        xstrfmtcat(query, "delete quick from \"%s_%s\" where "
                          "lft between %s AND %s;", cluster_name, assoc_table, row2[0], row2[1]);

        xstrfmtcat(query, "UPDATE \"%s_%s\" SET rgt = rgt - %s WHERE rgt > %s;"
                          "UPDATE \"%s_%s\" SET "
                          "lft = lft - %s WHERE lft > %s;", cluster_name, assoc_table, row2[2], row2[1], cluster_name,
	...
	*/
    if (rc == SLURM_SUCCESS)
        rc = as_mysql_get_modified_lfts(mysql_conn, cluster_name, smallest_lft);
    ...
    query = xstrdup_printf("update \"%s_%s\" as t1 set "
                           ...
                           "priority=DEFAULT "
                           "where (%s);", cluster_name, assoc_table, now, loc_assoc_char);
    ...
    *out = xstrdup(mod);
    ...
    if (xstrcmp(*out, cur)) {
        if (vals) {
			 ...
			 */
            xstrfmtcat(*vals, ", %s = "
                              "if (%s=%u, '%s', %s)", name, assoc ? "id_assoc" : "id", id, *out, name);
        ...
        if (cur_par)
            slurmdb_combine_tres_strings(out, cur_par, tres_str_flags);
    } else
        xfree(*out);
}

static int _get_database_variable(mysql_conn_t *mysql_conn, const char *variable_name, uint64_t *value) {
    MYSQL_ROW row = NULL;
    MYSQL_RES *result = NULL;
    char *err_check = NULL;
    char *query;

    query = xstrdup_printf("show variables like \'%s\';", variable_name);
...
extern int init(void) {
    ...
    if (slurmdbd_conf->dbd_backup) {
        ...
        if (!xstrcmp(node_name_short, slurmdbd_conf->dbd_backup) ||
            !xstrcmp(node_name_long, slurmdbd_conf->dbd_backup) || !xstrcmp(slurmdbd_conf->dbd_backup, "localhost"))
            backup_dbd = true;
    }
    ...
    while (mysql_db_get_db_connection(mysql_conn, mysql_db_name, mysql_db_info) != SLURM_SUCCESS) {
...
extern void *
acct_storage_p_get_connection(const slurm_trigger_callbacks_t *cb, int conn_num, uint16_t *persist_conn_flags,
                              bool rollback, char *cluster_name) {
    mysql_conn_t *mysql_conn = NULL;

    debug2("acct_storage_p_get_connection: request new connection %d", rollback);

    if (!(mysql_conn = create_mysql_conn(conn_num, rollback, cluster_name))) {
...
extern int acct_storage_p_commit(mysql_conn_t *mysql_conn, bool commit) {
    ...
    if (mysql_conn->rollback) {
        if (!commit) {
            ...
            if (mysql_conn->pre_commit_query) {
                if (debug_flags & DEBUG_FLAG_DB_ASSOC)
                    DB_DEBUG(mysql_conn->conn, "query\n%s", mysql_conn->pre_commit_query);
                rc = mysql_db_query(mysql_conn, mysql_conn->pre_commit_query);
            }
    ...
    if (commit && list_count(mysql_conn->update_list)) {
        ...
        xstrfmtcat(query, "select control_host, control_port, "
                          "name, rpc_version "
                          "from %s where deleted=0 && control_port != 0", cluster_table);
        if (!(result = mysql_db_query_ret(mysql_conn, query, 0))) {
            xfree(query);
            goto skip;
        }
        xfree(query);
        while ((row = mysql_fetch_row(result))) {
            (void) slurmdb_send_accounting_update(mysql_conn->update_list, row[2], row[0], slurm_atoul(row[1]),
                                                  slurm_atoul(row[3]));
        }
        mysql_free_result(result);
        skip:
                ...
                case SLURMDB_REMOVE_CLUSTER:
                    ...
                    while ((rem_cluster = list_next(itr3))) {
                        while ((cluster_name = list_next(itr2))) {
                            if (!xstrcmp(cluster_name, rem_cluster)) {
...
extern int acct_storage_p_add_users(mysql_conn_t *mysql_conn, uint32_t uid, List user_list) {
    return as_mysql_add_users(mysql_conn, uid, user_list);
}

extern int
acct_storage_p_add_coord(mysql_conn_t *mysql_conn, uint32_t uid, List acct_list, slurmdb_user_cond_t *user_cond) {
    return as_mysql_add_coord(mysql_conn, uid, acct_list, user_cond);
}

extern int acct_storage_p_add_accts(mysql_conn_t *mysql_conn, uint32_t uid, List acct_list) {
    return as_mysql_add_accts(mysql_conn, uid, acct_list);
}

extern int acct_storage_p_add_clusters(mysql_conn_t *mysql_conn, uint32_t uid, List cluster_list) {
    return as_mysql_add_clusters(mysql_conn, uid, cluster_list);
}

extern int acct_storage_p_add_federations(mysql_conn_t *mysql_conn, uint32_t uid, List federation_list) {
    return as_mysql_add_federations(mysql_conn, uid, federation_list);
}

extern int acct_storage_p_add_tres(mysql_conn_t *mysql_conn, uint32_t uid, List tres_list_in) {
...
extern int acct_storage_p_add_qos(mysql_conn_t *mysql_conn, uint32_t uid, List qos_list) {
    return as_mysql_add_qos(mysql_conn, uid, qos_list);
}

extern int acct_storage_p_add_res(mysql_conn_t *mysql_conn, uint32_t uid, List res_list) {
...
extern int acct_storage_p_add_reservation(mysql_conn_t *mysql_conn, slurmdb_reservation_rec_t *resv) {
    return as_mysql_add_resv(mysql_conn, resv);
}

extern List acct_storage_p_modify_users(mysql_conn_t *mysql_conn, uint32_t uid, slurmdb_user_cond_t *user_cond,
                                        slurmdb_user_rec_t *user) {
    return as_mysql_modify_users(mysql_conn, uid, user_cond, user);
}

extern List acct_storage_p_modify_accts(mysql_conn_t *mysql_conn, uint32_t uid, slurmdb_account_cond_t *acct_cond,
                                        slurmdb_account_rec_t *acct) {
    return as_mysql_modify_accts(mysql_conn, uid, acct_cond, acct);
}

extern List acct_storage_p_modify_clusters(mysql_conn_t *mysql_conn, uint32_t uid, slurmdb_cluster_cond_t *cluster_cond,
                                           slurmdb_cluster_rec_t *cluster) {
    return as_mysql_modify_clusters(mysql_conn, uid, cluster_cond, cluster);
}

extern List acct_storage_p_modify_assocs(mysql_conn_t *mysql_conn, uint32_t uid, slurmdb_assoc_cond_t *assoc_cond,
                                         slurmdb_assoc_rec_t *assoc) {
    return as_mysql_modify_assocs(mysql_conn, uid, assoc_cond, assoc);
}

extern List
acct_storage_p_modify_federations(mysql_conn_t *mysql_conn, uint32_t uid, slurmdb_federation_cond_t *fed_cond,
                                  slurmdb_federation_rec_t *fed) {
    return as_mysql_modify_federations(mysql_conn, uid, fed_cond, fed);
}

extern List acct_storage_p_modify_job(mysql_conn_t *mysql_conn, uint32_t uid, slurmdb_job_modify_cond_t *job_cond,
                                      slurmdb_job_rec_t *job) {
    return as_mysql_modify_job(mysql_conn, uid, job_cond, job);
}

extern List acct_storage_p_modify_qos(mysql_conn_t *mysql_conn, uint32_t uid, slurmdb_qos_cond_t *qos_cond,
                                      slurmdb_qos_rec_t *qos) {
    return as_mysql_modify_qos(mysql_conn, uid, qos_cond, qos);
}

extern List acct_storage_p_modify_res(mysql_conn_t *mysql_conn, uint32_t uid, slurmdb_res_cond_t *res_cond,
                                      slurmdb_res_rec_t *res) {
    return as_mysql_modify_res(mysql_conn, uid, res_cond, res);
}

extern List acct_storage_p_modify_wckeys(mysql_conn_t *mysql_conn, uint32_t uid, slurmdb_wckey_cond_t *wckey_cond,
                                         slurmdb_wckey_rec_t *wckey) {
    return as_mysql_modify_wckeys(mysql_conn, uid, wckey_cond, wckey);
}

extern int acct_storage_p_modify_reservation(mysql_conn_t *mysql_conn, slurmdb_reservation_rec_t *resv) {
    return as_mysql_modify_resv(mysql_conn, resv);
}

extern List acct_storage_p_remove_users(mysql_conn_t *mysql_conn, uint32_t uid, slurmdb_user_cond_t *user_cond) {
...
extern List acct_storage_p_remove_accts(mysql_conn_t *mysql_conn, uint32_t uid, slurmdb_account_cond_t *acct_cond) {
    return as_mysql_remove_accts(mysql_conn, uid, acct_cond);
}

extern List
acct_storage_p_remove_clusters(mysql_conn_t *mysql_conn, uint32_t uid, slurmdb_cluster_cond_t *cluster_cond) {
    return as_mysql_remove_clusters(mysql_conn, uid, cluster_cond);
}

extern List acct_storage_p_remove_assocs(mysql_conn_t *mysql_conn, uint32_t uid, slurmdb_assoc_cond_t *assoc_cond) {
    return as_mysql_remove_assocs(mysql_conn, uid, assoc_cond);
}

extern List
acct_storage_p_remove_federations(mysql_conn_t *mysql_conn, uint32_t uid, slurmdb_federation_cond_t *fed_cond) {
    return as_mysql_remove_federations(mysql_conn, uid, fed_cond);
}

extern List acct_storage_p_remove_qos(mysql_conn_t *mysql_conn, uint32_t uid, slurmdb_qos_cond_t *qos_cond) {
    return as_mysql_remove_qos(mysql_conn, uid, qos_cond);
}

extern List acct_storage_p_remove_res(mysql_conn_t *mysql_conn, uint32_t uid, slurmdb_res_cond_t *res_cond) {
...
extern int acct_storage_p_remove_reservation(mysql_conn_t *mysql_conn, slurmdb_reservation_rec_t *resv) {
    return as_mysql_remove_resv(mysql_conn, resv);
}

extern List acct_storage_p_get_users(mysql_conn_t *mysql_conn, uid_t uid, slurmdb_user_cond_t *user_cond) {
    return as_mysql_get_users(mysql_conn, uid, user_cond);
}

extern List acct_storage_p_get_accts(mysql_conn_t *mysql_conn, uid_t uid, slurmdb_account_cond_t *acct_cond) {
    return as_mysql_get_accts(mysql_conn, uid, acct_cond);
}

extern List acct_storage_p_get_clusters(mysql_conn_t *mysql_conn, uid_t uid, slurmdb_cluster_cond_t *cluster_cond) {
    return as_mysql_get_clusters(mysql_conn, uid, cluster_cond);
}

extern List acct_storage_p_get_federations(mysql_conn_t *mysql_conn, uid_t uid, slurmdb_federation_cond_t *fed_cond) {
    return as_mysql_get_federations(mysql_conn, uid, fed_cond);
}

extern List acct_storage_p_get_tres(mysql_conn_t *mysql_conn, uid_t uid, slurmdb_tres_cond_t *tres_cond) {
...
extern List acct_storage_p_get_events(mysql_conn_t *mysql_conn, uint32_t uid, slurmdb_event_cond_t *event_cond) {
    return as_mysql_get_cluster_events(mysql_conn, uid, event_cond);
}

extern List acct_storage_p_get_problems(mysql_conn_t *mysql_conn, uint32_t uid, slurmdb_assoc_cond_t *assoc_cond) {
    List ret_list = NULL;
    ...
    if (as_mysql_acct_no_assocs(mysql_conn, assoc_cond, ret_list) != SLURM_SUCCESS)
        goto end_it;

    if (as_mysql_acct_no_users(mysql_conn, assoc_cond, ret_list) != SLURM_SUCCESS)
        goto end_it;
...
extern List acct_storage_p_get_qos(mysql_conn_t *mysql_conn, uid_t uid, slurmdb_qos_cond_t *qos_cond) {
    return as_mysql_get_qos(mysql_conn, uid, qos_cond);
}

extern List acct_storage_p_get_res(mysql_conn_t *mysql_conn, uid_t uid, slurmdb_res_cond_t *res_cond) {
...
extern List
acct_storage_p_get_reservations(mysql_conn_t *mysql_conn, uid_t uid, slurmdb_reservation_cond_t *resv_cond) {
    return as_mysql_get_resvs(mysql_conn, uid, resv_cond);
}

extern List acct_storage_p_get_txn(mysql_conn_t *mysql_conn, uid_t uid, slurmdb_txn_cond_t *txn_cond) {
...
extern int
acct_storage_p_roll_usage(mysql_conn_t *mysql_conn, time_t sent_start, time_t sent_end, uint16_t archive_data,
                          rollup_stats_t *rollup_stats) {
    return as_mysql_roll_usage(mysql_conn, sent_start, sent_end, archive_data, rollup_stats);
}
...
extern int
clusteracct_storage_p_node_down(mysql_conn_t *mysql_conn, struct node_record *node_ptr, time_t event_time, char *reason,
                                uint32_t reason_uid) {
    return as_mysql_node_down(mysql_conn, node_ptr, event_time, reason, reason_uid);
}

extern int clusteracct_storage_p_node_up(mysql_conn_t *mysql_conn, struct node_record *node_ptr, time_t event_time) {
 ...
 */
extern int clusteracct_storage_p_register_ctld(mysql_conn_t *mysql_conn, uint16_t port) {
    return as_mysql_register_ctld(mysql_conn, mysql_conn->cluster_name, port);
}

extern uint16_t clusteracct_storage_p_register_disconn_ctld(mysql_conn_t *mysql_conn, char *control_host) {
    ...
    if (!mysql_conn->cluster_name) {
        ...
        error("%s:%d no control host for cluster %s", THIS_FILE, __LINE__, mysql_conn->cluster_name);
        return control_port;
    }

    query = xstrdup_printf("select last_port from %s where name='%s';", cluster_table, mysql_conn->cluster_name);
    if (!(result = mysql_db_query_ret(mysql_conn, query, 0))) {
        xfree(query);
        error("register_disconn_ctld: no result given for cluster %s", mysql_conn->cluster_name);
        return control_port;
    }
    xfree(query);
    ...
    if ((row = mysql_fetch_row(result))) {
		...
		*/
        query = xstrdup_printf("update %s set control_host='%s', "
                               "control_port=%u where name='%s';", cluster_table, control_host, control_port,
...
extern int clusteracct_storage_p_fini_ctld(mysql_conn_t *mysql_conn, slurmdb_cluster_rec_t *cluster_rec) {
    if (check_connection(mysql_conn) != SLURM_SUCCESS)
        return ESLURM_DB_CONNECTION;
...
extern int
clusteracct_storage_p_cluster_tres(mysql_conn_t *mysql_conn, char *cluster_nodes, char *tres_str_in, time_t event_time,
                                   uint16_t rpc_version) {
    return as_mysql_cluster_tres(mysql_conn, cluster_nodes, &tres_str_in, event_time, rpc_version);
}

/*
 * load into the storage the start of a job
 */
extern int jobacct_storage_p_job_start(mysql_conn_t *mysql_conn, struct job_record *job_ptr) {
 ...
 */
extern int jobacct_storage_p_job_complete(mysql_conn_t *mysql_conn, struct job_record *job_ptr) {
 ...
 */
extern int jobacct_storage_p_step_start(mysql_conn_t *mysql_conn, struct step_record *step_ptr) {
 ...
 */
extern int jobacct_storage_p_step_complete(mysql_conn_t *mysql_conn, struct step_record *step_ptr) {
 ...
 */
extern int jobacct_storage_p_suspend(mysql_conn_t *mysql_conn, struct job_record *job_ptr) {
 ...
 */
extern List jobacct_storage_p_get_jobs_cond(mysql_conn_t *mysql_conn, uid_t uid, slurmdb_job_cond_t *job_cond) {
    List job_list = NULL;
 ...
 */
extern int jobacct_storage_p_archive(mysql_conn_t *mysql_conn, slurmdb_archive_cond_t *arch_cond) {
    int rc;
 ...
 */
extern int jobacct_storage_p_archive_load(mysql_conn_t *mysql_conn, slurmdb_archive_rec_t *arch_rec) {
    if (check_connection(mysql_conn) != SLURM_SUCCESS)
        return ESLURM_DB_CONNECTION;
...
extern int acct_storage_p_update_shares_used(mysql_conn_t *mysql_conn, List shares_used) {
    /* No plans to have the database hold the used shares */
    return SLURM_SUCCESS;
}

extern int acct_storage_p_flush_jobs_on_cluster(mysql_conn_t *mysql_conn, time_t event_time) {
...
extern int acct_storage_p_reset_lft_rgt(mysql_conn_t *mysql_conn, uid_t uid, List cluster_list) {
    if (check_connection(mysql_conn) != SLURM_SUCCESS)
        return ESLURM_DB_CONNECTION;
