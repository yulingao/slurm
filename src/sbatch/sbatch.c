...
int main(int argc, char **argv) {
    ...
    slurm_conf_init(NULL); /* slurm环境初始化 */
    ...
    /*
    当sbatch退出时，务必调用spank_fini
    Be sure to call spank_fini when sbatch exits

    atexit函数，功能：注册终止函数(即main执行结束后调用的函数)
     */
    ...
    pack_argc = argc - sbopt.script_argc; /* pack参数的数量 = 输入参数的数量 - sbatch脚本里面参数的数量 */
    pack_argv = argv;
    for (pack_inx = 0; !pack_fini; pack_inx++) /* 配置异构作业的环境和各个参数 */
    {
        ...
        } else if (!more_packs) /* 如果没有更多的包，打包完成*/
        {
            pack_fini = true;
        }
        ...
        注意，这里的处理与salloc/srun中的处理不同。它将被拼接到作业脚本中，而不是将文件内容作为job_des_msg_t中的burst_buffer字段发送。
        ...
        if (opt.get_user_env_time < 0) {
            /*
            Moab不会传播用户的资源限制，所以slurmd在获取用户的默认环境变量的同时确定这些值
        ...
        _set_prio_process_env(); /* 设置作业提交时的调度优先级（nice值） */
        _set_spank_env();
        _set_submit_dir_env(); /* 设置提交作业的目录和节点名*/
        _set_umask_env();      /* 设置作业提交时的umask掩码 */
        ...
        slurm_init_job_desc_msg(desc);              /* 使用默认值初始化job的参数 */
        if (_fill_job_desc_from_opts(desc) == -1) /* 将options中的job参数加到job上 */
            exit(error_exit);
        if (!first_desc)
            first_desc = desc;
        if (pack_inx || !pack_fini) {
            set_env_from_opts(&opt, &first_desc->environment, pack_inx); /* 设置环境变量 */
        } else
    ...
    /*
    *如果可以在多个集群上运行，请找到最早的运行时间并在那里运行它
    ...
    /* test-only，作业不真正执行，仅仅测试脚本是否可以执行，并给出预计开始执行的时间 */
    if (sbopt.test_only) {
        if (job_req_list)
    ...
    while (true) {
        ...
        if (errno == ESLURM_ERROR_ON_DESC_TO_RECORD_COPY) {
            ...
            msg = NULL;
        if ((msg == NULL) || (retries >= MAX_RETRIES)) /* 没有消息而且大于最大尝试次数 */
        {
        ...
        if (retries)
            ...
            info("%s", msg); /*不是错误，启动节点*/ /* Not an error, powering up nodes */
        else
            error("%s", msg);
        slurm_free_submit_response_response_msg(resp);
        sleep(++retries); /* 挂起retries秒 */
    }
...
/*将“burst_buffer_file”的内容插入“script_body”*/
...
/* 
在当前状态下设置SLURM_SUBMIT_DIR和SLURM_SUBMIT_HOST环境变量
 ...
 * 设置内部SLURM_PRIO_PROCESS环境变量，以支持传播用户nice值和“propagation eprioprocess”配置关键字。
 * 
 *
...
/*
获取脚本缓冲区
如果“filename”为NULL，则从标准输入读取批处理脚本。
