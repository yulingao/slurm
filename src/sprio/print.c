...
int print_jobs_array(List jobs, List format) {
    ...
    if (jobs) {
        sort_job_list(jobs);
        list_for_each(jobs, (ListForF) print_job_from_format, (void *) format);
    }
...
double get_priority_from_factors(priority_factors_object_t *prio_factors) {
    int i = 0;
    double priority = prio_factors->priority_age + prio_factors->priority_assoc + prio_factors->priority_fs +
                      prio_factors->priority_js + prio_factors->priority_part + prio_factors->priority_qos +
...
int _print_job_job_id(priority_factors_object_t *job, int width, bool right, char *suffix) {
...
int _print_job_prefix(priority_factors_object_t *job, int width, bool right, char *suffix) {
    if (suffix)
        printf("%s", suffix);
    return SLURM_SUCCESS;
}

int _print_age_priority_normalized(priority_factors_object_t *job, int width, bool right, char *suffix) {
    if (job == NULL)    /* Print the Header instead */
...
int _print_age_priority_weighted(priority_factors_object_t *job, int width, bool right, char *suffix) {
    if (job == NULL)    /* Print the Header instead */
        _print_str("AGE", width, right, true);
    else if (job == (priority_factors_object_t * ) - 1)
        _print_int(weight_age, width, right, true);
...
int _print_assoc_priority_normalized(priority_factors_object_t *job, int width, bool right, char *suffix) {
    if (job == NULL)    /* Print the Header instead */
...
int _print_assoc_priority_weighted(priority_factors_object_t *job, int width, bool right, char *suffix) {
    if (job == NULL)    /* Print the Header instead */
        _print_str("ASSOC", width, right, true);
    else if (job == (priority_factors_object_t * ) - 1)
        _print_int(weight_assoc, width, right, true);
...
int _print_fs_priority_normalized(priority_factors_object_t *job, int width, bool right, char *suffix) {
    if (job == NULL)    /* Print the Header instead */
...
int _print_fs_priority_weighted(priority_factors_object_t *job, int width, bool right, char *suffix) {
    if (job == NULL)    /* Print the Header instead */
        _print_str("FAIRSHARE", width, right, true);
    else if (job == (priority_factors_object_t * ) - 1)
        _print_int(weight_fs, width, right, true);
...
int _print_job_priority_normalized(priority_factors_object_t *job, int width, bool right, char *suffix) {
...
int _print_job_priority_weighted(priority_factors_object_t *job, int width, bool right, char *suffix) {
    char temp[FORMAT_STRING_SIZE];
    if (job == NULL)    /* Print the Header instead */
        _print_str("PRIORITY", width, right, true);
    else if (job == (priority_factors_object_t * ) - 1)
...
int _print_js_priority_normalized(priority_factors_object_t *job, int width, bool right, char *suffix) {
    if (job == NULL)    /* Print the Header instead */
...
int _print_js_priority_weighted(priority_factors_object_t *job, int width, bool right, char *suffix) {
    if (job == NULL)    /* Print the Header instead */
        _print_str("JOBSIZE", width, right, true);
    else if (job == (priority_factors_object_t * ) - 1)
        _print_int(weight_js, width, right, true);
...
int _print_part_priority_normalized(priority_factors_object_t *job, int width, bool right, char *suffix) {
    if (job == NULL)    /* Print the Header instead */
...
int _print_part_priority_weighted(priority_factors_object_t *job, int width, bool right, char *suffix) {
    if (job == NULL)    /* Print the Header instead */
        _print_str("PARTITION", width, right, true);
    else if (job == (priority_factors_object_t * ) - 1)
        _print_int(weight_part, width, right, true);
...
int _print_partition(priority_factors_object_t *job, int width, bool right, char *suffix) {
    if (job == NULL)    /* Print the Header instead */
        _print_str("PARTITION", width, right, true);
    else if (job == (priority_factors_object_t * ) - 1)
        _print_str("", width, right, true);
...
int _print_qos_priority_normalized(priority_factors_object_t *job, int width, bool right, char *suffix) {
    if (job == NULL)    /* Print the Header instead */
...
int _print_qos_priority_weighted(priority_factors_object_t *job, int width, bool right, char *suffix) {
    if (job == NULL)    /* Print the Header instead */
        _print_str("QOS", width, right, true);
    else if (job == (priority_factors_object_t * ) - 1)
        _print_int(weight_qos, width, right, true);
...
int _print_site_priority(priority_factors_object_t *job, int width, bool right, char *suffix) {
    if (job == NULL)    /* Print the Header instead */
        ...
        _print_int((int64_t) job->priority_site - NICE_OFFSET, width, right, true);
    if (suffix)
        printf("%s", suffix);
    return SLURM_SUCCESS;
}

int _print_job_nice(priority_factors_object_t *job, int width, bool right, char *suffix) {
    if (job == NULL)    /* Print the Header instead */
...
int _print_job_user_name(priority_factors_object_t *job, int width, bool right, char *suffix) {
    if (job == NULL)    /* Print the Header instead */
...
int _print_tres_normalized(priority_factors_object_t *job, int width, bool right, char *suffix) {
    if (job == NULL) {    /* Print the Header instead */
        ...
        for (i = 0; i < job->tres_cnt; i++) {
            ...
            if (values[0])
                xstrcat(values, ",");
            xstrfmtcat(values, "%s=%.2f", job->tres_names[i], job->priority_tres[i] / job->tres_weights[i]);
        }
...
int _print_tres_weighted(priority_factors_object_t *job, int width, bool right, char *suffix) {
    if (job == NULL) {    /* Print the Header instead */
        _print_str("TRES", width, right, true);
    } else if (job == (priority_factors_object_t * ) - 1)
        ...
        for (i = 0; i < job->tres_cnt; i++) {
            ...
            if (values[0])
                xstrcat(values, ",");
            xstrfmtcat(values, "%s=%.0f", job->tres_names[i], job->priority_tres[i]);
        }
