        ...
        for (rel_node_inx = 0; rel_node_inx < job_resrcs->nhosts;
            ...
            if (xstrcmp(tmp1, tmp2) ||
                ...
                if (rel_node_inx < job_ptr->gres_detail_cnt) {
                    gres_last = job_ptr->gres_detail_str[rel_node_inx];
                } else {
                    gres_last = "";
                }
                last_mem_alloc_ptr = job_resrcs->memory_allocated;
...
/*
 *slurm_load_jobs  -发出RPC以获取所有作业配置
 *自update_time以来更改的信息
 *IN update_time  -当前配置数据的时间
 *IN /OUT job_info_msg_pptr  -存储作业配置指针的位置
 *IN show_flags  -作业过滤选项：0，SHOW_ALL，SHOW_DETAIL或SHOW_LOCAL
 *出错时RET 0或-1
 *注意：使用slurm_free_job_info_msg释放响应
...
extern int32_t islurm_get_rem_time__(uint32_t *jobid) {
    ...
    if ((jobid == NULL) || (slurm_get_end_time(*jobid, &end_time) != SLURM_SUCCESS))
        return 0;
...
slurm_get_end_time(uint32_t jobid, time_t *end_time_ptr) {
    ...
    if ((jobid == jobid_cache) && (difftime(now, last_test_time) < 60)) {
