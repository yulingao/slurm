...
typedef enum {
    CLUSTER_REPORT_UA, CLUSTER_REPORT_AU, CLUSTER_REPORT_UW, CLUSTER_REPORT_WU
} cluster_report_t;
...
static void _process_ua(List user_list, slurmdb_assoc_rec_t *assoc) {
    ...
    */
    ...
    while ((slurmdb_report_user = list_next(itr))) {
        if (!xstrcmp(slurmdb_report_user->name, assoc->user) && !xstrcmp(slurmdb_report_user->acct, assoc->acct))
            break;
    }
    ...
    if (!slurmdb_report_user) {
        ...
        */
        slurmdb_report_user = xmalloc(sizeof(slurmdb_report_user_rec_t));
    ...
    slurmdb_transfer_acct_list_2_tres(assoc->accounting_list, &slurmdb_report_user->tres_list);
}

static void _process_au(List assoc_list, slurmdb_assoc_rec_t *assoc) {
    slurmdb_report_assoc_rec_t *slurmdb_report_assoc = xmalloc(sizeof(slurmdb_report_assoc_rec_t));
    ...
    /* get the amount of time this assoc used
       during the time we are looking at */
    slurmdb_transfer_acct_list_2_tres(assoc->accounting_list, &slurmdb_report_assoc->tres_list);
}
...
static void _process_uw(List user_list, slurmdb_wckey_rec_t *wckey) {
    ...
    */
    ...
    /* get the amount of time this wckey used
       during the time we are looking at */
    slurmdb_transfer_acct_list_2_tres(wckey->accounting_list, &slurmdb_report_user->tres_list);
}

static void _process_wu(List assoc_list, slurmdb_wckey_rec_t *wckey) {
    slurmdb_report_assoc_rec_t *slurmdb_report_assoc = NULL, *parent_assoc = NULL;
    ListIterator itr = NULL;

    /* find the parent */
    itr = list_iterator_create(assoc_list);
    while ((parent_assoc = list_next(itr))) {
        if (!parent_assoc->user && !xstrcmp(parent_assoc->acct, wckey->name))
            break;
    }
    list_iterator_destroy(itr);
    if (!parent_assoc) {
        ...
        list_append(assoc_list, parent_assoc);
    ...
    /* get the amount of time this wckey used
       during the time we are looking at */
    slurmdb_transfer_acct_list_2_tres(wckey->accounting_list, &slurmdb_report_assoc->tres_list);
    slurmdb_transfer_acct_list_2_tres(wckey->accounting_list, &parent_assoc->tres_list);
}

static void
_process_assoc_type(ListIterator itr, slurmdb_report_cluster_rec_t *slurmdb_report_cluster, char *cluster_name,
                    cluster_report_t type) {
    slurmdb_assoc_rec_t *assoc = NULL;

    /* now add the associations of interest here by user */
    while ((assoc = list_next(itr))) {
        if (!assoc->accounting_list || !list_count(assoc->accounting_list) ||
        ...
        if (type == CLUSTER_REPORT_UA)
            _process_ua(slurmdb_report_cluster->user_list, assoc);
        else if (type == CLUSTER_REPORT_AU)
            _process_au(slurmdb_report_cluster->assoc_list, assoc);
...
static void
_process_wckey_type(ListIterator itr, slurmdb_report_cluster_rec_t *slurmdb_report_cluster, char *cluster_name,
                    cluster_report_t type) {
    slurmdb_wckey_rec_t *wckey = NULL;

    /* now add the wckeyiations of interest here by user */
    while ((wckey = list_next(itr))) {
        if (!wckey->accounting_list || !list_count(wckey->accounting_list) ||
        ...
        if (type == CLUSTER_REPORT_UW)
            _process_uw(slurmdb_report_cluster->user_list, wckey);
        else if (type == CLUSTER_REPORT_WU)
            _process_wu(slurmdb_report_cluster->assoc_list, wckey);

        list_delete_item(itr);
    }
}

static List _process_util_by_report(void *db_conn, char *calling_name, void *cond, cluster_report_t type) {
    ListIterator itr = NULL;
    ...
    if ((type == CLUSTER_REPORT_UA) || (type == CLUSTER_REPORT_AU)) {
        ...
        cluster_cond.cluster_list = ((slurmdb_assoc_cond_t *) cond)->cluster_list;
        ...
        cluster_cond.cluster_list = ((slurmdb_wckey_cond_t *) cond)->cluster_list;
    } else {
    ...
    */
    ...
    cluster_list = acct_storage_g_get_clusters(db_conn, my_uid, &cluster_cond);
    ...
    if ((type == CLUSTER_REPORT_UA) || (type == CLUSTER_REPORT_AU)) {
        ...
        type_list = acct_storage_g_get_assocs(db_conn, my_uid, cond);
    } else if ((type == CLUSTER_REPORT_UW) || (type == CLUSTER_REPORT_WU)) {
        ((slurmdb_wckey_cond_t *) cond)->usage_start = start_time;
        ((slurmdb_wckey_cond_t *) cond)->usage_end = end_time;
        type_list = acct_storage_g_get_wckeys(db_conn, my_uid, cond);
    }
    ...
    if ((type == CLUSTER_REPORT_UA) || (type == CLUSTER_REPORT_AU)) {
        ...
        type_list = slurmdb_get_hierarchical_sorted_assoc_list(first_list, true);
    }
    ...
    while ((cluster = list_next(itr))) {
        ...
        if ((type == CLUSTER_REPORT_UA) || (type == CLUSTER_REPORT_UW))
            slurmdb_report_cluster->user_list = list_create(slurmdb_destroy_report_user_rec);
        else if ((type == CLUSTER_REPORT_AU) || (type == CLUSTER_REPORT_WU))
            slurmdb_report_cluster->assoc_list = list_create(slurmdb_destroy_report_assoc_rec);

        if ((type == CLUSTER_REPORT_UA) || (type == CLUSTER_REPORT_AU))
            _process_assoc_type(type_itr, slurmdb_report_cluster, cluster->name, type);
        else if ((type == CLUSTER_REPORT_UW) || (type == CLUSTER_REPORT_WU))
            _process_wckey_type(type_itr, slurmdb_report_cluster, cluster->name, type);
...
extern List slurmdb_report_cluster_account_by_user(void *db_conn, slurmdb_assoc_cond_t *assoc_cond) {
    return _process_util_by_report(db_conn, "slurmdb_report_cluster_account_by_user", assoc_cond, CLUSTER_REPORT_AU);
}

extern List slurmdb_report_cluster_user_by_account(void *db_conn, slurmdb_assoc_cond_t *assoc_cond) {
    return _process_util_by_report(db_conn, "slurmdb_report_cluster_user_by_account", assoc_cond, CLUSTER_REPORT_UA);
}

extern List slurmdb_report_cluster_wckey_by_user(void *db_conn, slurmdb_wckey_cond_t *wckey_cond) {
    return _process_util_by_report(db_conn, "slurmdb_report_cluster_wckey_by_user", wckey_cond, CLUSTER_REPORT_WU);
}

extern List slurmdb_report_cluster_user_by_wckey(void *db_conn, slurmdb_wckey_cond_t *wckey_cond) {
    return _process_util_by_report(db_conn, "slurmdb_report_cluster_user_by_wckey", wckey_cond, CLUSTER_REPORT_UW);
}
