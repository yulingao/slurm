...
static int _local_send_recv_rc_msgs(const char *nodelist, slurm_msg_type_t type, void *data) {
    List ret_list = NULL;
    int temp_rc = 0, rc = 0;
...
static int _signal_batch_script_step(const resource_allocation_response_msg_t *allocation, uint32_t signal) {
    slurm_msg_t msg;
    ...
    if (!name) {
        error("_signal_batch_script_step: "
              "can't get the first name out of %s", allocation->node_list);
        return -1;
    }
    ...
    if (slurm_conf_get_addr(name, &msg.address) == SLURM_ERROR) {
        error("_signal_batch_script_step: "
              "can't find address for host %s, check slurm.conf", name);
        free(name);
        return -1;
    }
    free(name);
...
static int
_signal_job_step(const job_step_info_t *step, const resource_allocation_response_msg_t *allocation, uint16_t signal) {
    signal_tasks_msg_t rpc;
    int rc = SLURM_SUCCESS;
    ...
    rc = _local_send_recv_rc_msgs(allocation->node_list, REQUEST_SIGNAL_TASKS, &rpc);
    return rc;
}

static int _terminate_batch_script_step(const resource_allocation_response_msg_t *allocation) {
    slurm_msg_t msg;
    signal_tasks_msg_t rpc;
    int rc = SLURM_SUCCESS;
    int i;
 ...
 */
static int _terminate_job_step(const job_step_info_t *step, const resource_allocation_response_msg_t *allocation) {
    signal_tasks_msg_t rpc;
    int rc = SLURM_SUCCESS;
    ...
    rpc.job_step_id = step->step_id;
    rpc.signal = (uint16_t) -1; /* not used by slurmd */
    rc = _local_send_recv_rc_msgs(allocation->node_list, REQUEST_TERMINATE_TASKS, &rpc);
...
extern int slurm_signal_job(uint32_t job_id, uint16_t signal) {
    ...
    rc = _local_send_recv_rc_msgs(alloc_info->node_list, REQUEST_SIGNAL_TASKS, &rpc);
 ...
 * RET SLURM_SUCCESS on success, otherwise return SLURM_ERROR with errno set
 */
extern int slurm_signal_job_step(uint32_t job_id, uint32_t step_id, uint32_t signal) {
    resource_allocation_response_msg_t *alloc_info = NULL;
    ...
    for (i = 0; i < step_info->job_step_count; i++) {
        if ((step_info->job_steps[i].job_id == job_id) && (step_info->job_steps[i].step_id == step_id)) {
            rc = _signal_job_step(&step_info->job_steps[i], alloc_info, signal);
            save_errno = rc;
            break;
        }
    }
...
extern int slurm_terminate_job_step(uint32_t job_id, uint32_t step_id) {
    ...
    for (i = 0; i < step_info->job_step_count; i++) {
        if ((step_info->job_steps[i].job_id == job_id) && (step_info->job_steps[i].step_id == step_id)) {
            rc = _terminate_job_step(&step_info->job_steps[i], alloc_info);
            save_errno = errno;
            break;
        }
    }
