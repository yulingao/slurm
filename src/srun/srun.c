 ...
 *	not open the reserved port. It was already open by some other process. */
/* 此退出代码表示启动的Open MPI任务可以
 *不打开保留端口。它已经被其他一些过程打开了。 */
...
int srun(int ac, char **av) {
    ...
    slurm_conf_init(NULL);                         /* 初始化slurm配置环境 */
    debug_level = _slurm_debug_env_val();         // 得到slurm环境的debug等级
    logopt.stderr_level += debug_level;             // debug等级
    log_init(xbasename(av[0]), logopt, 0, NULL); // 初始化日志
    _set_exit_code();                             // 设置退出码
...
static void _set_exit_code(void) {
    ...
    eg:
    printf("PATH : %s\n", getenv("PATH"));
    输出：PATH : /sbin:/usr/sbin:/bin:/usr/bin:/usr/local/bin
     */
...
static void _setup_env_working_cluster(void) {
    ...
    /* Format is cluster_name:address:port:rpc[:plugin_id_select]
        格式是集群名:地址:端口:rpc[:plugin_id_select]
    ...
    *rpc_ptr++ = '\0';
    ...
    if (xstrcmp(slurmctld_conf.cluster_name, working_env)) {
        ...
        strtol函数会将参数nptr字符串根据参数base来转换成 长整型数(long)，参数base范围从2至36，base代表代用的进制方式。
         */
