	...
	slurm_opt_t *opt_local)
{
	...
	ai->ntasks_per_socket = (opt_local->ntasks_per_socket != NO_VAL) ? opt_local->ntasks_per_socket : resp->ntasks_per_socket;
...
static void _handle_intr(srun_job_t *job)
{
	...
	{ /* second Ctrl-C in half as many seconds */
						  ...
						  int pack_offset)
{
	...
	if (resp->env_size)
	{ /* Used to set Burst Buffer environment */
							  ...
							  slurm_opt_t *opt_local)
{
		...
		((srun_opt->relative + opt_local->min_nodes) > resp->node_cnt))
	{
		...
		{ /* SLURM_JOB_NUM_NODES option used */
