...
static slurm_opt_t *_get_first_opt(int pack_offset)
{
	...
	while ((opt_local = list_next(opt_iter)))
	{
		...
		xassert(srun_opt);
		if (srun_opt->pack_grp_bits && (pack_offset >= 0) && (pack_offset < bit_size(srun_opt->pack_grp_bits)) && bit_test(srun_opt->pack_grp_bits, pack_offset))
			break;
	}
