...
static int _set_cond(int *start, int argc, char **argv, slurmdb_account_cond_t *acct_cond, List format_list) {
    int i;
    int a_set = 0;
    int u_set = 0;
    int end = 0;
    ...
    for (i = (*start); i < argc; i++) {
        ...
        if (!xstrncasecmp(argv[i], "Set", MAX(command_len, 3))) {
            ...
            continue;
                   ...
                   !xstrncasecmp(argv[i], "Accounts", MAX(command_len, 1)) ||
                   !xstrncasecmp(argv[i], "Acct", MAX(command_len, 4))) {
            if (!assoc_cond->acct_list) {
                assoc_cond->acct_list = list_create(slurm_destroy_char);
            }
            if (slurm_addto_char_list(assoc_cond->acct_list, argv[i] + end))
                u_set = 1;
            ...
            if (slurm_addto_char_list(acct_cond->description_list, argv[i] + end))
                u_set = 1;
        } else if (!xstrncasecmp(argv[i], "Format", MAX(command_len, 1))) {
            if (format_list)
                slurm_addto_char_list(format_list, argv[i] + end);
        } else if (!xstrncasecmp(argv[i], "Organizations", MAX(command_len, 1))) {
            if (!acct_cond->organization_list) {
                acct_cond->organization_list = list_create(slurm_destroy_char);
            }
            if (slurm_addto_char_list(acct_cond->organization_list, argv[i] + end))
                u_set = 1;
...
static int _set_rec(int *start, int argc, char **argv, List acct_list, List cluster_list, slurmdb_account_rec_t *acct,
    ...
    for (i = (*start); i < argc; i++) {
        ...
        if (!xstrncasecmp(argv[i], "Where", MAX(command_len, 5))) {
            ...
            continue;
        } else if (!end || !xstrncasecmp(argv[i], "Accounts", MAX(command_len, 1)) ||
            ...
            if (acct_list)
                ...
                exit_code = 1;
                fprintf(stderr, " Can't modify the name "
                                "of an account\n");
            }
            ...
            u_set = 1;
        } else if (!xstrncasecmp(argv[i], "Organization", MAX(command_len, 1))) {
...
static int _isdefault(int cond_set, List acct_list, List assoc_list) {
    ...
    if (!acct_list || !list_count(acct_list) || !assoc_list || !list_count(assoc_list))
        return rc;
...
extern int sacctmgr_add_account(int argc, char **argv) {
    ...
    slurmdb_assoc_rec_t *start_assoc = xmalloc(sizeof(slurmdb_assoc_rec_t));
    ...
    for (i = 0; i < argc; i++) {
        ...
        if (!xstrncasecmp(argv[i], "Where", MAX(command_len, 5)) || !xstrncasecmp(argv[i], "Set", MAX(command_len, 3)))
            i++;
        limit_set += _set_rec(&i, argc, argv, name_list, cluster_list, start_acct, start_assoc);
    }
    if (exit_code) {
    ...
    if (!name_list || !list_count(name_list)) {
        ...
        local_account_list = slurmdb_accounts_get(db_conn, &account_cond);
    }
    ...
    if (!cluster_list || !list_count(cluster_list)) {
        ...
        List tmp_list = slurmdb_clusters_get(db_conn, NULL);
        if (!tmp_list) {
            exit_code = 1;
        ...
        if (!list_count(tmp_list)) {
            exit_code = 1;
            fprintf(stderr, "  Can't add accounts, no cluster "
                            "defined yet.\n"
    ...
    } else if (sacctmgr_validate_cluster_list(cluster_list) != SLURM_SUCCESS) {
        slurmdb_destroy_assoc_rec(start_assoc);
        slurmdb_destroy_account_rec(start_acct);
        FREE_NULL_LIST(local_account_list);
    ...
    while ((name = list_next(itr))) {
        ...
        if (!sacctmgr_find_account_from_list(local_account_list, name)) {
            ...
            if (start_acct->description)
                acct->description = xstrdup(start_acct->description);
            else
                acct->description = xstrdup(name);

            if (start_acct->organization)
                acct->organization = xstrdup(start_acct->organization);
            else if (xstrcmp(start_assoc->parent_acct, "root"))
                acct->organization = xstrdup(start_assoc->parent_acct);
            else
        ...
        while ((cluster = list_next(itr_c))) {
            if (sacctmgr_find_account_base_assoc_from_list(local_assoc_list, name, cluster)) {
                //printf(" already have this assoc\n");
                continue;
            }
            if (!sacctmgr_find_account_base_assoc_from_list(local_assoc_list, start_assoc->parent_acct, cluster)) {
                exit_code = 1;
    ...
    if (rc == SLURM_SUCCESS) {
        ...
        exit_code = 1;
        fprintf(stderr, " error: Problem adding account associations: %s\n", slurm_strerror(rc));
        rc = SLURM_ERROR;
    }
...
extern int sacctmgr_list_account(int argc, char **argv) {
    int rc = SLURM_SUCCESS;
    slurmdb_account_cond_t *acct_cond = xmalloc(sizeof(slurmdb_account_cond_t));
    List acct_list;
    ...
    if (exit_code) {
        ...
        if (acct_cond->with_assocs)
            slurm_addto_char_list(format_list, "Cl,ParentN,U,Share,Priority,"
                                               "GrpJ,GrpN,"
    ...
    while ((acct = list_next(itr))) {
        if (acct->assoc_list) {
            ListIterator itr3 = list_iterator_create(acct->assoc_list);
            while ((assoc = list_next(itr3))) {
                int curr_inx = 1;
                while ((field = list_next(itr2))) {
                    switch (field->type) {
                        case PRINT_ACCT:
                            field->print_routine(field, acct->name, (curr_inx == field_count));
                            break;
                        case PRINT_COORDS:
                            field->print_routine(field, acct->coordinators, (curr_inx == field_count));
                            break;
                        case PRINT_DESC:
                            field->print_routine(field, acct->description, (curr_inx == field_count));
                            break;
                        case PRINT_ORG:
                            field->print_routine(field, acct->organization, (curr_inx == field_count));
                            break;
                        default:
...
extern int sacctmgr_modify_account(int argc, char **argv) {
    ...
    for (i = 0; i < argc; i++) {
        ...
        if (!xstrncasecmp(argv[i], "Where", MAX(command_len, 5))) {
            ...
            i++;
            prev_set = _set_rec(&i, argc, argv, NULL, NULL, acct, assoc);
            rec_set |= prev_set;
        } else {
            prev_set = _set_cond(&i, argc, argv, acct_cond, NULL);
            cond_set |= prev_set;
    ...
    if (rec_set & 1) { // process the account changes
        ...
        ret_list = slurmdb_accounts_modify(db_conn, acct_cond, acct);
        if (ret_list && list_count(ret_list)) {
            char *object = NULL;
            ListIterator itr = list_iterator_create(ret_list);
    ...
    if (rec_set == 3 || rec_set == 2) { // process the association changes
        ...
        if (assoc->parent_acct) {
            slurmdb_account_rec_t *acct_rec = sacctmgr_find_account(assoc->parent_acct);
            if (!acct_rec) {
                exit_code = 1;
                fprintf(stderr, " Parent Account %s doesn't exist.\n", assoc->parent_acct);
                rc = SLURM_ERROR;
                goto assoc_end;
            }
        }

        ret_list = slurmdb_associations_modify(db_conn, acct_cond->assoc_cond, assoc);

        if (ret_list && list_count(ret_list)) {
            set = 1;
            if (assoc->def_qos_id != NO_VAL)
                set = sacctmgr_check_default_qos(assoc->def_qos_id, acct_cond->assoc_cond);
            else if (assoc->qos_list)
                set = sacctmgr_check_default_qos(-1, acct_cond->assoc_cond);
...
extern int sacctmgr_delete_account(int argc, char **argv) {
    ...
    local_assoc_list = slurmdb_associations_get(db_conn, acct_cond->assoc_cond);
    acct_cond->assoc_cond->only_defs = 0;

    notice_thread_init();
    if (cond_set == 1) {
        ret_list = slurmdb_accounts_remove(db_conn, acct_cond);
    } else if (cond_set & 2) {
        ret_list = slurmdb_associations_remove(db_conn, acct_cond->assoc_cond);
    }
    rc = errno;
    notice_thread_fini();
    slurmdb_destroy_account_cond(acct_cond);
