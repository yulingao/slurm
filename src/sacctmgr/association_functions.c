...
static int _set_cond(int *start, int argc, char **argv, slurmdb_assoc_cond_t *assoc_cond, List format_list) {
    ...
    for (i = (*start); i < argc; i++) {
        ...
        if (!end && !xstrncasecmp(argv[i], "OnlyDefaults", MAX(command_len, 2))) {
        ...
        } else if (!end && !xstrncasecmp(argv[i], "WithSubAccounts", MAX(command_len, 5))) {
            assoc_cond->with_sub_accts = 1;
        } else if (!end && !xstrncasecmp(argv[i], "WOPInfo", MAX(command_len, 4))) {
            ...
            continue;
                   ...
                   !xstrncasecmp(argv[i], "Associations", MAX(command_len, 2))) {
        ...
        } else if (!(set = sacctmgr_set_assoc_cond(assoc_cond, argv[i], argv[i] + end, command_len, option)) ||
                   exit_code) {
            exit_code = 1;
...
extern bool sacctmgr_check_default_qos(uint32_t qos_id, slurmdb_assoc_cond_t *assoc_cond) {
    ...
    while ((assoc = list_next(itr))) {
        ...
        if (!qos) {
            char *name = slurmdb_qos_str(g_qos_list, assoc->def_qos_id);
            if (!assoc->user) {
                // see if this isn't a user
                object = xstrdup_printf("  DefQOS = %-10s C = %-10s A = %-20s ", name, assoc->cluster, assoc->acct);
            } else if (assoc->partition) {
                // see if there is a partition name
                object = xstrdup_printf("  DefQOS = %-10s C = %-10s A = %-20s "
                                        "U = %-9s P = %s", name, assoc->cluster, assoc->acct, assoc->user,
                                        assoc->partition);
            } else {
                object = xstrdup_printf("  DefQOS = %-10s C = %-10s A = %-20s "
                                        "U = %-9s", name, assoc->cluster, assoc->acct, assoc->user);
            }

            if (!no_access_list)
                no_access_list = list_create(slurm_destroy_char);
    ...
    if (!no_access_list)
        return true;
    fprintf(stderr, " These associations don't have access to their "
                    "default qos.\n");
    fprintf(stderr, " Please give them access before they the default "
                    "can be set to this.\n");
...
sacctmgr_set_assoc_cond(slurmdb_assoc_cond_t *assoc_cond, char *type, char *value, int command_len, int option) {
    ...
    if (!xstrncasecmp(type, "Accounts", MAX(command_len, 2)) || !xstrncasecmp(type, "Acct", MAX(command_len, 4))) {
        ...
        if (slurmdb_addto_qos_char_list(assoc_cond->def_qos_id_list, g_qos_list, value, 0))
            set = 1;
        else
            exit_code = 1;
        ...
        if (!assoc_cond->parent_acct_list)
            assoc_cond->parent_acct_list = list_create(slurm_destroy_char);
        ...
        if (slurmdb_addto_qos_char_list(assoc_cond->qos_list, g_qos_list, value, option))
            set = 1;
    } else if (!xstrncasecmp(type, "Users", MAX(command_len, 1))) {
        if (!assoc_cond->user_list)
            assoc_cond->user_list = list_create(slurm_destroy_char);
        if (slurm_addto_char_list_with_case(assoc_cond->user_list, value, user_case_norm))
            set = 1;
    }
...
extern int sacctmgr_set_assoc_rec(slurmdb_assoc_rec_t *assoc, char *type, char *value, int command_len, int option) {
    int set = 0;
    ...
    if (!xstrncasecmp(type, "DefaultQOS", MAX(command_len, 8))) {
        ...
        if (assoc->def_qos_id == NO_VAL) {
            fprintf(stderr, "You gave a bad default qos '%s'.  "
                            "Use 'list qos' to get "
                            "complete list.\n", value);
            exit_code = 1;
        }
        ...
        if (!xstrncasecmp(value, "parent", 6)) {
            ...
            set = 1;
        } else if (get_uint(value, &assoc->shares_raw, "FairShare") == SLURM_SUCCESS) {
            set = 1;
        }
    } else if (!xstrncasecmp(type, "GrpCPUMins", MAX(command_len, 7))) {
        if (get_uint64(value, &tmp64, "GrpCPUMins") == SLURM_SUCCESS) {
            set = 1;
            tmp_char = xstrdup_printf("%d=%"
            PRIu64, TRES_CPU, tmp64);

            slurmdb_combine_tres_strings(&assoc->grp_tres_mins, tmp_char, tres_flags);
            xfree(tmp_char);
        }
    } else if (!xstrncasecmp(type, "GrpCPURunMins", MAX(command_len, 7))) {
        if (get_uint64(value, &tmp64, "GrpCPURunMins") == SLURM_SUCCESS) {
            set = 1;
            tmp_char = xstrdup_printf("%d=%"
            PRIu64, TRES_CPU, tmp64);
            slurmdb_combine_tres_strings(&assoc->grp_tres_run_mins, tmp_char, tres_flags);
            xfree(tmp_char);
        }
    } else if (!xstrncasecmp(type, "GrpCpus", MAX(command_len, 7))) {
        if (get_uint64(value, &tmp64, "GrpCpus") == SLURM_SUCCESS) {
            set = 1;
            tmp_char = xstrdup_printf("%d=%"
            PRIu64, TRES_CPU, tmp64);
            slurmdb_combine_tres_strings(&assoc->grp_tres, tmp_char, tres_flags);
            xfree(tmp_char);
        }
    } else if (!xstrncasecmp(type, "GrpJobs", MAX(command_len, 4))) {
        if (get_uint(value, &assoc->grp_jobs, "GrpJobs") == SLURM_SUCCESS)
            set = 1;
    } else if (!xstrncasecmp(type, "GrpJobsAccrue", MAX(command_len, 8))) {
        if (get_uint(value, &assoc->grp_jobs_accrue, "GrpJobsAccrue") == SLURM_SUCCESS)
            set = 1;
    } else if (!xstrncasecmp(type, "GrpMemory", MAX(command_len, 4))) {
        if (get_uint64(value, &tmp64, "GrpMemory") == SLURM_SUCCESS) {
            set = 1;
            tmp_char = xstrdup_printf("%d=%"
        ...
        if (get_uint64(value, &tmp64, "GrpNodes") == SLURM_SUCCESS) {
            set = 1;
            tmp_char = xstrdup_printf("%d=%"
    ...
    } else if (!xstrncasecmp(type, "GrpSubmitJobs", MAX(command_len, 4))) {
        if (get_uint(value, &assoc->grp_submit_jobs, "GrpSubmitJobs") == SLURM_SUCCESS)
            set = 1;
    ...
    } else if (!xstrncasecmp(type, "MaxCPUMinsPerJob", MAX(command_len, 7))) {
        if (get_uint64(value, &tmp64, "MaxCPUMinsPerJob") == SLURM_SUCCESS) {
            set = 1;
            tmp_char = xstrdup_printf("%d=%"
            PRIu64, TRES_CPU, tmp64);
            slurmdb_combine_tres_strings(&assoc->max_tres_mins_pj, tmp_char, tres_flags);
            xfree(tmp_char);
        }
    } else if (!xstrncasecmp(type, "MaxCPURunMins", MAX(command_len, 7))) {
        if (get_uint64(value, &tmp64, "MaxCPURunMins") == SLURM_SUCCESS) {
            set = 1;
            tmp_char = xstrdup_printf("%d=%"
            PRIu64, TRES_CPU, tmp64);
            slurmdb_combine_tres_strings(&assoc->max_tres_run_mins, tmp_char, tres_flags);
            xfree(tmp_char);
        }
    } else if (!xstrncasecmp(type, "MaxCpusPerJob", MAX(command_len, 7))) {
        if (get_uint64(value, &tmp64, "MaxCpusPerJob") == SLURM_SUCCESS) {
            set = 1;
            tmp_char = xstrdup_printf("%d=%"
            PRIu64, TRES_CPU, tmp64);
            slurmdb_combine_tres_strings(&assoc->max_tres_pj, tmp_char, tres_flags);
            xfree(tmp_char);
        }
    } else if (!xstrncasecmp(type, "MaxJobs", MAX(command_len, 4))) {
        if (get_uint(value, &assoc->max_jobs, "MaxJobs") == SLURM_SUCCESS)
            set = 1;
    } else if (!xstrncasecmp(type, "MaxJobsAccrue", MAX(command_len, 8))) {
        if (get_uint(value, &assoc->max_jobs_accrue, "MaxJobsAccrue") == SLURM_SUCCESS)
            set = 1;
    } else if (!xstrncasecmp(type, "MaxNodesPerJob", MAX(command_len, 4))) {
        if (get_uint64(value, &tmp64, "MaxNodes") == SLURM_SUCCESS) {
            set = 1;
            tmp_char = xstrdup_printf("%d=%"
            PRIu64, TRES_NODE, tmp64);
            slurmdb_combine_tres_strings(&assoc->max_tres_pj, tmp_char, tres_flags);
    ...
    } else if (!xstrncasecmp(type, "MinPrioThresh", MAX(command_len, 4))) {
        if (get_uint(value, &assoc->min_prio_thresh, "MinPrioThresh") == SLURM_SUCCESS)
            set = 1;
    } else if (!xstrncasecmp(type, "MaxSubmitJobs", MAX(command_len, 4))) {
        if (get_uint(value, &assoc->max_submit_jobs, "MaxSubmitJobs") == SLURM_SUCCESS)
            set = 1;
        ...
        if ((tmp_char = slurmdb_format_tres_str(value, g_tres_list, 1))) {
            slurmdb_combine_tres_strings(&assoc->max_tres_pn, tmp_char, tres_flags);
            set = 1;
            xfree(tmp_char);
        }
    } else if (!xstrncasecmp(type, "MaxTRESMinsPerJob", MAX(command_len, 8))) {
        sacctmgr_initialize_g_tres_list();
    ...
    } else if (!xstrncasecmp(type, "MaxWallDurationPerJob", MAX(command_len, 4))) {
        mins = time_str2mins(value);
        if (mins != NO_VAL) {
        ...
        if (get_uint(value, &assoc->priority, "Priority") == SLURM_SUCCESS)
            set = 1;
    } else if (!xstrncasecmp(type, "QosLevel", MAX(command_len, 1))) {
        if (!assoc->qos_list)
        ...
        if (slurmdb_addto_qos_char_list(assoc->qos_list, g_qos_list, value, option))
            set = 1;
    }
...
extern void sacctmgr_print_assoc_rec(slurmdb_assoc_rec_t *assoc, print_field_t *field, List tree_list, bool last) {
    ...
    switch (field->type) {
        ...
        case PRINT_FAIRSHARE:
            ...
            break;
        case PRINT_GRPCM:
            field->print_routine(field, slurmdb_find_tres_count_in_string(assoc->grp_tres_mins, TRES_CPU), last);
            break;
        case PRINT_GRPCRM:
            field->print_routine(field, slurmdb_find_tres_count_in_string(assoc->grp_tres_run_mins, TRES_CPU), last);
            break;
        case PRINT_GRPC:
            field->print_routine(field, slurmdb_find_tres_count_in_string(assoc->grp_tres, TRES_CPU), last);
            break;
        case PRINT_GRPTM:
        ...
        case PRINT_GRPJA:
            ...
            break;
        case PRINT_GRPMEM:
            field->print_routine(field, slurmdb_find_tres_count_in_string(assoc->grp_tres, TRES_MEM), last);
            break;
        case PRINT_GRPN:
            field->print_routine(field, slurmdb_find_tres_count_in_string(assoc->grp_tres, TRES_NODE), last);
            break;
        case PRINT_GRPS:
        ...
        case PRINT_LFT:
            ...
            break;
        case PRINT_MAXCM:
            field->print_routine(field, slurmdb_find_tres_count_in_string(assoc->max_tres_mins_pj, TRES_CPU), last);
            break;
        case PRINT_MAXCRM:
            field->print_routine(field, slurmdb_find_tres_count_in_string(assoc->max_tres_run_mins, TRES_CPU), last);
            break;
        case PRINT_MAXC:
            field->print_routine(field, slurmdb_find_tres_count_in_string(assoc->max_tres_pj, TRES_CPU), last);
            break;
        case PRINT_MAXTM:
        ...
        case PRINT_MINPT:
            ...
            break;
        case PRINT_MAXN:
            field->print_routine(field, slurmdb_find_tres_count_in_string(assoc->max_tres_pj, TRES_NODE), last);
            break;
        case PRINT_MAXS:
...
extern int sacctmgr_list_assoc(int argc, char **argv) {
    ...
    if (exit_code) {
        ...
        if (!assoc_cond->without_parent_limits)
            slurm_addto_char_list(format_list, "Share,Priority,GrpJ,GrpTRES,"
    ...
    while ((assoc = list_next(itr))) {
        ...
        while ((field = list_next(itr2))) {
            sacctmgr_print_assoc_rec(assoc, field, tree_list, (curr_inx == field_count));
            curr_inx++;
        }
