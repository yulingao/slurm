...
/* check signature */
#define _assert_bitstr_valid(name)                                                                \
	do                                                                                            \
	{                                                                                             \
		assert((name) != NULL);                                                                   \
		assert(_bitstr_magic(name) == BITSTR_MAGIC || _bitstr_magic(name) == BITSTR_MAGIC_STACK); \
	} while (0)

/* check bit position */
#define _assert_bit_valid(name, bit)        \
	do                                      \
	{                                       \
		assert((bit) >= 0);                 \
...
bit_noc(bitstr_t *b, int32_t n, int32_t seed)
{
	...
	for (bit = seed; bit < _bitstr_bits(b); bit++)
	{ /* start at offset */
		if (bit_test(b, bit))
		{ /* fail */
			cnt = 0;
	...
	*str = '\0';
	for (bit = 0; bit < _bitstr_bits(b);)
	{
		...
		if (bit_test(b, bit))
		{
			...
			size = strlen(str);
			if (bit == start)
			{ /* add single bit position */
				ret = snprintf(str + size, len - size,
							   "%" BITSTR_FMT ",", start);
			}
			else
			{ /* add bit position range */
				ret = snprintf(str + size, len - size,
