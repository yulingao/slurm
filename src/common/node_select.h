...
typedef struct slurm_select_ops {
    ...
    int (*node_ranking)(struct node_record *node_ptr, int node_cnt);

    int (*node_init)(struct node_record *node_ptr, int node_cnt);

    int (*block_init)(List block_list);

    int (*job_test)(struct job_record *job_ptr, bitstr_t *bitmap, uint32_t min_nodes, uint32_t max_nodes,
                    uint32_t req_nodes, uint16_t mode, List preeemptee_candidates, List *preemptee_job_list,
    ...
    int (*job_expand)(struct job_record *from_job_ptr, struct job_record *to_job_ptr);

    int (*job_resized)(struct job_record *job_ptr, struct node_record *node_ptr);

    int (*job_signal)(struct job_record *job_ptr, int signal);

    int (*job_mem_confirm)(struct job_record *job_ptr);

    int (*job_fini)(struct job_record *job_ptr);

    int (*job_suspend)(struct job_record *job_ptr, bool indf_susp);

    int (*job_resume)(struct job_record *job_ptr, bool indf_susp);

    bitstr_t *(*step_pick_nodes)(struct job_record *job_ptr, select_jobinfo_t *step_jobinfo, uint32_t node_count,
                                 bitstr_t **avail_nodes);

    int (*step_start)(struct step_record *step_ptr);

    int (*step_finish)(struct step_record *step_ptr, bool killing_step);

    int (*nodeinfo_pack)(select_nodeinfo_t *nodeinfo, Buf buffer, uint16_t protocol_version);

    int (*nodeinfo_unpack)(select_nodeinfo_t **nodeinfo, Buf buffer, uint16_t protocol_version);
    ...
    int
    (*nodeinfo_get)(select_nodeinfo_t *nodeinfo, enum select_nodedata_type dinfo, enum node_states state, void *data);

    select_jobinfo_t *(*jobinfo_alloc)(void);

    int (*jobinfo_free)(select_jobinfo_t *jobinfo);

    int (*jobinfo_set)(select_jobinfo_t *jobinfo, enum select_jobdata_type data_type, void *data);

    int (*jobinfo_get)(select_jobinfo_t *jobinfo, enum select_jobdata_type data_type, void *data);

    select_jobinfo_t *(*jobinfo_copy)(select_jobinfo_t *jobinfo);

    int (*jobinfo_pack)(select_jobinfo_t *jobinfo, Buf buffer, uint16_t protocol_version);

    int (*jobinfo_unpack)(select_jobinfo_t **jobinfo_pptr, Buf buffer, uint16_t protocol_version);

    char *(*jobinfo_sprint)(select_jobinfo_t *jobinfo, char *buf, size_t size, int mode);

    char *(*jobinfo_xstrdup)(select_jobinfo_t *jobinfo, int mode);

    int (*get_info_from_plugin)(enum select_plugindata_info dinfo, struct job_record *job_ptr, void *data);
    ...
    bitstr_t *
    (*resv_test)(resv_desc_msg_t *resv_desc_ptr, uint32_t node_cnt, bitstr_t *avail_bitmap, bitstr_t **core_bitmap);
} slurm_select_ops_t;
