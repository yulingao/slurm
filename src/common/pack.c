...
/*
 * create_mmap_buf - 从提供的文件创建一个mmap()'d只读缓冲区。
 * 
 * create an mmap()'d read-only buffer from the supplied file.
 */
...
void packmem(char *valp, uint32_t size_val, Buf buffer) {
    ...
    if (remaining_buf(buffer) < (sizeof(ns) + size_val)) {
        if ((buffer->size + size_val + BUF_SIZE) > MAX_BUF_SIZE) {
            error("%s: Buffer size limit exceeded (%u > %u)", __func__, (buffer->size + size_val + BUF_SIZE),
                  MAX_BUF_SIZE);
            return;
        }
...
int unpackmem_ptr(char **valp, uint32_t *size_valp, Buf buffer) {
    ...
    *size_valp = ntohl(ns);
    ...
    if (*size_valp > MAX_ARRAY_LEN_LARGE) {
        error("%s: Buffer to be unpacked is too large (%u > %u)", __func__, *size_valp, MAX_ARRAY_LEN_LARGE);
        return SLURM_ERROR;
    } else if (*size_valp > 0) {
...
int unpackmem_malloc(char **valp, uint32_t *size_valp, Buf buffer) {
    ...
    *size_valp = ntohl(ns);
    ...
    if (*size_valp > MAX_ARRAY_LEN_SMALL) {
        error("%s: Buffer to be unpacked is too large (%u > %u)", __func__, *size_valp, MAX_ARRAY_LEN_SMALL);
        return SLURM_ERROR;
    } else if (*size_valp > 0) {
        if (remaining_buf(buffer) < *size_valp)
            return SLURM_ERROR;
...
int unpackstr_array(char ***valp, uint32_t *size_valp, Buf buffer) {
    ...
    *size_valp = ntohl(ns);
    ...
    if (*size_valp > MAX_ARRAY_LEN_MEDIUM) {
        error("%s: Buffer to be unpacked is too large (%u > %u)", __func__, *size_valp, MAX_ARRAY_LEN_MEDIUM);
        return SLURM_ERROR;
    } else if (*size_valp > 0) {
