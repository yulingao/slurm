...
#define
sched_getaffinity sched_getaffinity_np
#endif
...
typedef struct slurm_gres_ops {
    int (*node_config_load)(List gres_conf_list, node_config_load_t *node_conf);

    void (*job_set_env)(char ***job_env_ptr, void *gres_ptr, int node_inx);

    void (*step_set_env)(char ***job_env_ptr, void *gres_ptr, char *tres_freq, int local_proc_id);

    void (*step_reset_env)(char ***job_env_ptr, void *gres_ptr, bitstr_t *usable_gres);
...
static void _build_node_gres_str(List *gres_list, char **gres_str, int cores_per_sock, int sock_per_node);

static uint32_t **
_build_tasks_per_node_sock(struct job_resources *job_res, uint8_t overcommit, gres_mc_data_t *tres_mc_ptr,
...
static void _free_tasks_per_node_sock(uint32_t **tasks_per_node_socket, int node_cnt);

static void _get_gres_cnt(gres_node_state_t *gres_data, char *orig_config, char *gres_name, char *gres_name_colon,
                          int gres_name_colon_len);

static uint32_t _get_task_cnt_node(uint32_t **tasks_per_node_socket, int node_inx, int sock_cnt);

static uint64_t _get_tot_gres_cnt(uint32_t plugin_id, uint64_t *topo_cnt, int *config_type_cnt);
...
static int
_job_alloc(void *job_gres_data, void *node_gres_data, int node_cnt, int node_index, int node_offset, char *gres_name,
           uint32_t job_id, char *node_name, bitstr_t *core_bitmap, uint32_t plugin_id, uint32_t user_id);

static void _job_core_filter(void *job_gres_data, void *node_gres_data, bool use_total_gres, bitstr_t *core_bitmap,
                             int core_start_bit, int core_end_bit, char *gres_name, char *node_name,
                             uint32_t plugin_id);

static int _job_dealloc(void *job_gres_data, void *node_gres_data, int node_offset, char *gres_name, uint32_t job_id,
                        char *node_name, bool old_job, uint32_t plugin_id, uint32_t user_id, bool job_fini);
...
static uint32_t
_job_test(void *job_gres_data, void *node_gres_data, bool use_total_gres, bitstr_t *core_bitmap, int core_start_bit,
          int core_end_bit, bool *topo_set, uint32_t job_id, char *node_name, char *gres_name, uint32_t plugin_id);

static int _load_gres_plugin(char *plugin_name, slurm_gres_context_t *plugin_context);
...
static int
_node_config_init(char *node_name, char *orig_config, slurm_gres_context_t *context_ptr, gres_state_t *gres_ptr);

static char *_node_gres_used(void *gres_data, char *gres_name);

static int
_node_reconfig(char *node_name, char *new_gres, char **gres_str, gres_state_t *gres_ptr, uint16_t fast_schedule,
               slurm_gres_context_t *context_ptr, bool *updated_gpu_cnt);

static int
_node_reconfig_test(char *node_name, char *new_gres, gres_state_t *gres_ptr, slurm_gres_context_t *context_ptr);
...
static int
_parse_gres_config(void **dest, slurm_parser_enum_t type, const char *key, const char *value, const char *line,
                   char **leftover);

static int
_parse_gres_config2(void **dest, slurm_parser_enum_t type, const char *key, const char *value, const char *line,
                    char **leftover);
...
static int
_step_alloc(void *step_gres_data, void *job_gres_data, uint32_t plugin_id, int node_offset, bool first_step_node,
            char *gres_name, uint32_t job_id, uint32_t step_id, uint16_t tasks_on_node, uint32_t rem_nodes);

static int _step_dealloc(void *step_gres_data, void *job_gres_data, char *gres_name, uint32_t job_id, uint32_t step_id);

static void *_step_state_dup(void *gres_data);

static void *_step_state_dup2(void *gres_data, int node_index);

static void _step_state_log(void *gres_data, uint32_t job_id, uint32_t step_id, char *gres_name);

static uint64_t
_step_test(void *step_gres_data, void *job_gres_data, int node_offset, bool first_step_node, uint16_t cpus_per_task,
           int max_rem_nodes, bool ignore_alloc, char *gres_name, uint32_t job_id, uint32_t step_id,
           uint32_t plugin_id);

static void _sync_node_mps_to_gpu(gres_state_t *mps_gres_ptr, gres_state_t *gpu_gres_ptr);
...
static void _validate_gres_node_cores(gres_node_state_t *node_gres_ptr, int cpus_ctld, char *node_name);

static int _valid_gres_type(char *gres_name, gres_node_state_t *gres_data, uint16_t fast_schedule, char **reason_down);
...
static int _gres_find_job_by_key(void *x, void *key) {
    ...
    if ((state_ptr->plugin_id == job_key->plugin_id) &&
        ((job_key->type_id == NO_VAL) || (gres_data_ptr->type_id == job_key->type_id)))
        return 1;
    return 0;
}
...
static int _gres_find_step_by_key(void *x, void *key) {
    ...
    if ((state_ptr->plugin_id == step_key->plugin_id) && (gres_data_ptr->type_id == step_key->type_id))
        return 1;
    return 0;
}
...
static int _gres_job_find_name(void *x, void *key) {
    ...
    return _gres_find_name_internal(gres_data_ptr->type_name, (char *) key, state_ptr->plugin_id);
}

static int _gres_step_find_name(void *x, void *key) {
    gres_state_t *state_ptr = (gres_state_t *) x;
    gres_step_state_t *gres_data_ptr = (gres_step_state_t *) state_ptr->gres_data;
    return _gres_find_name_internal(gres_data_ptr->type_name, (char *) key, state_ptr->plugin_id);
}
...
/*
	 ...
	 */
    static const char *syms[] = {"node_config_load", "job_set_env", "step_set_env", "step_reset_env", "send_stepd",
                                 "recv_stepd", "job_info", "step_info", "get_devices", "step_hardware_init",
                                 "step_hardware_fini", "epilog_build_env", "epilog_set_env"};
...
/* Find the correct plugin */
    ...
    plugin_context->cur_plugin = plugin_load_and_link(plugin_context->gres_type, n_syms, syms,
                                                      (void **) &plugin_context->ops);
    if (plugin_context->cur_plugin != PLUGIN_INVALID_HANDLE)
        return SLURM_SUCCESS;

    if (errno != EPLUGIN_NOTFOUND) {
        error("Couldn't load specified plugin name for %s: %s", plugin_context->gres_type, plugin_strerror(errno));
        return SLURM_ERROR;
    }
...
/* Get plugin list */
    ...
    plugin_context->cur_plugin = plugrack_use_by_type(plugin_context->plugin_list, plugin_context->gres_type);
    if (plugin_context->cur_plugin == PLUGIN_INVALID_HANDLE) {
        debug("Cannot find plugin of type %s, just track gres counts", plugin_context->gres_type);
        return SLURM_ERROR;
    }

/* Dereference the API. */
    if (plugin_get_syms(plugin_context->cur_plugin, n_syms, syms, (void **) &plugin_context->ops) < n_syms) {
        error("Incomplete %s plugin detected", plugin_context->gres_type);
        return SLURM_ERROR;
    }
...
/* Ensure that "gres/mps" follows "gres/gpu" */
    ...
    while (one_name) {
        ...
        if (i < gres_context_cnt) {
            error("Duplicate plugin %s ignored", gres_context[i].gres_type);
        } else {
            xrealloc(gres_context, (sizeof(slurm_gres_context_t) * (gres_context_cnt + 1)));
            (void) _load_gres_plugin(one_name, gres_context + gres_context_cnt);
/*
			 * Ignore return code.
			 * Proceed to support gres even without the plugin
			 */
            gres_context[gres_context_cnt].gres_name = xstrdup(one_name);
            gres_context[gres_context_cnt].plugin_id = gres_plugin_build_id(one_name);
            gres_context_cnt++;
        }
        one_name = strtok_r(NULL, ",", &last);
    }
...
/* Ensure that plugin_id is valid and unique */
    for (i = 0; i < gres_context_cnt; i++) {
        for (j = i + 1; j < gres_context_cnt; j++) {
            if (gres_context[i].plugin_id != gres_context[j].plugin_id)
                continue;
            fatal("Gres: Duplicate plugin_id %u for %s and %s, "
                  "change gres name for one of them", gres_context[i].plugin_id, gres_context[i].gres_type,
                  gres_context[j].gres_type);
        }
        xassert(gres_context[i].gres_name);

        gres_context[i].gres_name_colon = xstrdup_printf("%s:", gres_context[i].gres_name);
        gres_context[i].gres_name_colon_len = strlen(gres_context[i].gres_name_colon);
    }
    init_run = true;

    if ((select_plugin_type == NO_VAL) &&
        (select_g_get_info_from_plugin(SELECT_CR_PLUGIN, NULL, &select_plugin_type) != SLURM_SUCCESS)) {
        select_plugin_type = NO_VAL;    /* error */
    }
    if (have_mps && run_in_daemon("slurmctld") && (select_plugin_type != SELECT_TYPE_CONS_TRES)) {
...
/* Ignore return code. Support gres even without the plugin */
    ...
    gres_context[gres_context_cnt].plugin_id = gres_plugin_build_id(gres_name);
    gres_context_cnt++;
    fini:
    slurm_mutex_unlock(&gres_context_lock);
}
...
/* Logging may not be initialized at this point */
            error("Invalid GRES configured on node %s: %s", nodes, tok);
        }
        xfree(name);
...
extern int gres_plugin_reconfig(void) {
    ...
    if (xstrcmp(plugin_names, gres_plugin_list))
        plugin_change = true;
    else plugin_change = false;
    slurm_mutex_unlock(&gres_context_lock);

    if (plugin_change) {
        error("GresPlugins changed from %s to %s ignored", gres_plugin_list, plugin_names);
...
static int _log_gres_slurmd_conf(void *x, void *arg) {
    ...
    if (!gres_debug) {
        verbose("Gres Name=%s Type=%s Count=%"
        PRIu64, p->name, p->type_name, p->count);
        return 0;
    }
    ...
    if (p->cpus && (index != -1)) {
        info("Gres Name=%s Type=%s Count=%"
        PRIu64
        " Index=%d ID=%u "
        "File=%s Cores=%s CoreCnt=%u %s", p->name, p->type_name, p->count, index, p->plugin_id, p->file, p->cpus, p->cpu_cnt, links);
    } else if (index != -1) {
        info("Gres Name=%s Type=%s Count=%"
        PRIu64
        " Index=%d ID=%u File=%s %s", p->name, p->type_name, p->count, index, p->plugin_id, p->file, links);
    } else if (p->file) {
        info("Gres Name=%s Type=%s Count=%"
        PRIu64
        " ID=%u File=%s %s", p->name, p->type_name, p->count, p->plugin_id, p->file, links);
    } else {
        info("Gres Name=%s Type=%s Count=%"
        PRIu64
        " ID=%u %s", p->name, p->type_name, p->count, p->plugin_id, links);
    }
    xfree(links);
...
static int _validate_file(char *path_name, char *gres_name) {
    ...
    while ((one_name = hostlist_shift(hl))) {
        if (slash) {
            ...
            xstrfmtcat(formatted_path, "%s/%s", root_path, one_name);
...
static void _validate_links(gres_slurmd_conf_t *p) {
    ...
    while (tok) {
        ...
        if ((val < -2) || (val > GRES_MAX_LINK) || (val == LONG_MIN) || (end_ptr[0] != '\0')) {
            error("gres.conf: Ignoring invalid Link (%s) for Name=%s", tok, p->name);
            xfree(p->links);
            break;
        }
        tok = strtok_r(NULL, ",", &save_ptr);
...
_parse_gres_config(void **dest, slurm_parser_enum_t type, const char *key, const char *value, const char *line,
                   char **leftover) {
    static s_p_options_t _gres_options[] = {{"Count", S_P_STRING},    /* Number of Gres available */
    ...
    if (cores_flag || cpus_flag) {
        ...
        if (xcpuinfo_ops.xcpuinfo_abs_to_mac) {
            i = (xcpuinfo_ops.xcpuinfo_abs_to_mac)(p->cpus, &local_cpus);
/*
			 * Only executed by slurmstepd and we don't want
			 * fatal here. Ignore bad Core/CPU configuration.
			 */
            if (i != SLURM_SUCCESS) {
                error("Invalid GRES data for %s, %s=%s", p->name, type_str, p->cpus);
            }
        } else {
        ...
        if (i == SLURM_SUCCESS) {
            ...
            if ((bit_size(p->cpus_bitmap) == 0) || bit_unfmt(p->cpus_bitmap, local_cpus) != 0) {
                fatal("Invalid GRES data for %s, %s=%s (only %u CPUs are available)", p->name, type_str, p->cpus,
                      gres_cpu_cnt);
            }
        }
        xfree(local_cpus);
    ...
    if (s_p_get_string(&p->file, "File", tbl) || s_p_get_string(&p->file, "Files", tbl)) {
        p->count = _validate_file(p->file, p->name);
        p->config_flags |= GRES_CONF_HAS_FILE;
    }

    if (s_p_get_string(&p->links, "Link", tbl) || s_p_get_string(&p->links, "Links", tbl)) {
        _validate_links(p);
    }
    ...
    if (s_p_get_string(&tmp_str, "Count", tbl)) {
        ...
        if ((tmp_uint64 == LONG_MIN) || (tmp_uint64 == LONG_MAX)) {
            fatal("Invalid GRES record for %s, invalid count %s", p->name, tmp_str);
        }
...
/*
		 ...
		 */
        if (p->count && (p->count != tmp_uint64) && !_multi_count_per_file(p->name)) {
            fatal("Invalid GRES record for %s, count does not match File value", p->name);
        }
        if (tmp_uint64 >= NO_VAL64) {
            fatal("GRES %s has invalid count value %"
            PRIu64, p->name, tmp_uint64);
        }
        p->count = tmp_uint64;
        xfree(tmp_str);
    } else if (p->count == 0)
...
static void _validate_config(slurm_gres_context_t *context_ptr) {
    ...
    while ((gres_slurmd_conf = (gres_slurmd_conf_t *) list_next(iter))) {
        ...
        } else if ((new_has_file && !orig_has_file) || (!new_has_file && orig_has_file)) {
            fatal("gres.conf for %s, some records have \"File\" specification while others do not",
                  context_ptr->gres_name);
        }
        orig_has_type = gres_slurmd_conf->config_flags & GRES_CONF_HAS_TYPE;
        if (new_has_type == -1) {
            if (gres_slurmd_conf->config_flags & GRES_CONF_HAS_TYPE) {
                new_has_type = 1;
            } else new_has_type = 0;
        } else if ((new_has_type && !orig_has_type) || (!new_has_type && orig_has_type)) {
            fatal("gres.conf for %s, some records have \"Type=\" specification while others do not",
                  context_ptr->gres_name);
        }
        if ((new_has_file == 0) && (new_has_type == 0) && (rec_count > 1)) {
            fatal("gres.conf duplicate records for %s", context_ptr->gres_name);
        }
...
static void _add_gres_config(List gres_conf_list, gres_state_t *gres_ptr, slurm_gres_context_t *gres_context,
 ...
 */
static void _merge_config(node_config_load_t *node_conf, List gres_conf_list, List slurm_conf_list) {
    int i;
...
/* Determine if configuration already gathered from gres.conf */
        if (list_find_first(gres_conf_list, _find_gres_conf_by_plugin_id, &gres_context[i])) {
            continue;
        }
        ...
        if (slurm_conf_list) {
            ...
            while ((gres_ptr = (gres_state_t *) list_next(iter))) {
                if (gres_ptr->plugin_id != gres_context[i].plugin_id)
                    continue;
                found = true;
                _add_gres_config(gres_conf_list, gres_ptr, &gres_context[i], node_conf);
            }
            list_iterator_destroy(iter);
            if (found)
                continue;
...
/* Add GRES record with zero count */
        _add_gres_config(gres_conf_list, NULL, &gres_context[i], node_conf);
    }
}
 ...
 */
extern int gres_plugin_node_config_load(uint32_t cpu_cnt, char *node_name, List gres_list, void *xcpuinfo_abs_to_mac,
                                        void *xcpuinfo_mac_to_abs) {
    static s_p_options_t _gres_options[] = {{"AutoDetect", S_P_STRING},
    ...
    node_config_load_t node_conf = {.cpu_cnt = cpu_cnt, .xcpuinfo_mac_to_abs = xcpuinfo_mac_to_abs};
    ...
    for (i = 0; i < gres_context_cnt; i++) {
        ...
        if (gres_context[i].ops.node_config_load == NULL)
            continue;    /* No plugin */
        rc2 = (*(gres_context[i].ops.node_config_load))(gres_conf_list, &node_conf);
        if (rc == SLURM_SUCCESS)
            rc = rc2;
    }
...
extern int gres_plugin_node_config_unpack(Buf buffer, char *node_name) {
    ...
    for (i = 0; i < rec_cnt; i++) {
        ...
        if (slurm_get_debug_flags() & DEBUG_FLAG_GRES) {
            ...
            PRIu64
            " Links:%s", node_name, tmp_name, tmp_type, gres_flags2str(
                    config_flags), tmp_cpus, cpu_cnt, count64, tmp_links);
        }
...
/*
				 ...
				 * gres_plugin_init()
				 */
                error("%s: gres/%s duplicate plugin ID with %s, unable to process", __func__, tmp_name,
            ...
            if (orig_has_file && !new_has_file && count64) {
                error("%s: gres/%s lacks \"File=\" parameter for node %s", __func__, tmp_name, node_name);
                config_flags |= GRES_CONF_HAS_FILE;
            }
...
/*
                ...
                PRIu64
                ") for node %s, "
                "resetting value to %d", __func__, tmp_name, count64, node_name, MAX_GRES_BITMAP);
                count64 = MAX_GRES_BITMAP;
            }
            new_has_type = config_flags & GRES_CONF_HAS_TYPE;
            orig_has_type = gres_context[j].config_flags & GRES_CONF_HAS_TYPE;
            if (orig_has_type && !new_has_type && count64) {
                error("%s: gres/%s lacks \"Type\" parameter for node %s", __func__, tmp_name, node_name);
                config_flags |= GRES_CONF_HAS_TYPE;
            }
...
/*
			 ...
			 */
            error("%s: No plugin configured to process GRES data from node %s (Name:%s Type:%s PluginID:%u Count:%"
            PRIu64
            ")", __func__, node_name, tmp_name, tmp_type, plugin_id, count64);
            xfree(tmp_cpus);
            xfree(tmp_links);
            xfree(tmp_name);
            xfree(tmp_type);
...
static void _add_gres_type(char *type, gres_node_state_t *gres_data, uint64_t tmp_gres_cnt) {
    int i;
    uint32_t type_id;
    ...
    if (i >= gres_data->type_cnt) {
        gres_data->type_cnt++;
        gres_data->type_cnt_alloc = xrealloc(gres_data->type_cnt_alloc, sizeof(uint64_t) * gres_data->type_cnt);
        gres_data->type_cnt_avail = xrealloc(gres_data->type_cnt_avail, sizeof(uint64_t) * gres_data->type_cnt);
        gres_data->type_id = xrealloc(gres_data->type_id, sizeof(uint32_t) * gres_data->type_cnt);
        gres_data->type_name = xrealloc(gres_data->type_name, sizeof(char *) * gres_data->type_cnt);
        gres_data->type_cnt_avail[i] += tmp_gres_cnt;
...
/*
            ...
            while (sub_tok) {
                _add_gres_type(sub_tok, gres_data, tmp_gres_cnt);
                sub_tok = strtok_r(NULL, ":", &last_sub_tok);
            }
        }
    ...
    for (i = 0; i < gres_data->type_cnt; i++) {
        ...
        if (gres_data->type_cnt) {
            for (j = 0; j < gres_data->type_cnt; j++) {
                if (gres_data->type_id[i] == gres_data->type_id[j])
                    model_cnt += gres_data->type_cnt_avail[j];
            }
        } else {
        ...
        if (fast_schedule >= 2) {
            ...
            if (reason_down) {
                xstrfmtcat(*reason_down, "%s:%s count too low "
                                         "(%"
                PRIu64
                " < %"
                PRIu64
                ")", gres_name, gres_data->type_name[i], model_cnt, gres_data->type_cnt_avail[i]);
            }
            return -1;
        }
    }
...
/* If the resource isn't configured for use with this node */
    ...
    _get_gres_cnt(gres_data, orig_config, context_ptr->gres_name, context_ptr->gres_name_colon,
                  context_ptr->gres_name_colon_len);

    context_ptr->total_cnt += gres_data->gres_cnt_config;

/* Use count from recovered state, if higher */
    gres_data->gres_cnt_avail = MAX(gres_data->gres_cnt_avail, gres_data->gres_cnt_config);
    if ((gres_data->gres_bit_alloc != NULL) && (gres_data->gres_cnt_avail > bit_size(gres_data->gres_bit_alloc)) &&
        !_shared_gres(context_ptr->plugin_id)) {
        gres_data->gres_bit_alloc = bit_realloc(gres_data->gres_bit_alloc, gres_data->gres_cnt_avail);
    }
 ...
 */
extern int gres_plugin_init_node_config(char *node_name, char *orig_config, List *gres_list) {
    int i, rc, rc2;
...
/* Find or create gres_state entry on the list */
        ...
        rc2 = _node_config_init(node_name, orig_config, &gres_context[i], gres_ptr);
        if (rc == SLURM_SUCCESS)
            rc = rc2;
    }
    slurm_mutex_unlock(&gres_context_lock);
 ...
 */
extern int gres_gresid_to_gresname(uint32_t gres_id, char *gres_name, int gres_name_len) {
    int rc = SLURM_SUCCESS;
    int found = 0;
    int i;
...
/*
    ...
    for (i = 0; i < gres_context_cnt; ++i) {
        if (gres_id == gres_context[i].plugin_id) {
            strlcpy(gres_name, gres_context[i].gres_name, gres_name_len);
            found = 1;
            break;
        }
    }
...
static void _links_str2array(char *links, char *node_name, gres_node_state_t *gres_data, int gres_inx, int gres_cnt) {
    ...
    if (gres_inx >= gres_data->link_len) {
        error("%s: Invalid GRES index (%d >= %d)", __func__, gres_inx, gres_cnt);
        return;
    }

    start_ptr = links;
    while (1) {
        gres_data->links_cnt[gres_inx][i] = strtol(start_ptr, &end_ptr, 10);
        if (gres_data->links_cnt[gres_inx][i] < -2) {
            error("%s: Invalid GRES Links value (%s) on node %s:"
                  "Link value '%d' < -2", __func__, links, node_name, gres_data->links_cnt[gres_inx][i]);
        ...
        if (end_ptr[0] != ',') {
            error("%s: Invalid GRES Links value (%s) on node %s:"
                  "end_ptr[0]='%c' != ','", __func__, links, node_name, end_ptr[0]);
            return;
        }
        if (++i >= gres_data->link_len) {
            error("%s: Invalid GRES Links value (%s) on node %s:"
                  "i=%d >= link_len=%d", __func__, links, node_name, i, gres_data->link_len);
            return;
        }
        start_ptr = end_ptr + 1;
    }
...
static bool _valid_gres_types(char *gres_name, gres_node_state_t *gres_data, char **reason_down) {
    bool rc = true;
    ...
    for (type_inx = 0; type_inx < gres_data->type_cnt; type_inx++) {
        ...
        for (topo_inx = 0; topo_inx < gres_data->topo_cnt; topo_inx++) {
            if (gres_data->topo_type_id[topo_inx] != gres_data->type_id[type_inx])
                continue;
            gres_sum = gres_cnt_found + gres_data->topo_gres_cnt_avail[topo_inx];
            if (gres_sum > gres_data->type_cnt_avail[type_inx]) {
                gres_data->topo_gres_cnt_avail[topo_inx] -= (gres_sum - gres_data->type_cnt_avail[type_inx]);
            }
            gres_cnt_found += gres_data->topo_gres_cnt_avail[topo_inx];
        }
    ...
    if (!rc && reason_down && (*reason_down == NULL)) {
        ...
        " < %"
        PRIu64
        ")", gres_name, gres_data->type_name[type_inx], gres_cnt_found, gres_data->type_cnt_avail[type_inx]);
    }

    return rc;
}

static void _gres_bit_alloc_resize(gres_node_state_t *gres_data, uint64_t gres_bits) {
    if (!gres_bits) {
    ...
    if (!gres_data->gres_bit_alloc)
        ...
        gres_data->gres_bit_alloc = bit_realloc(gres_data->gres_bit_alloc, gres_bits);
}

static int
_node_config_validate(char *node_name, char *orig_config, char **new_config, gres_state_t *gres_ptr, int cpu_cnt,
                      int core_cnt, int sock_cnt, uint16_t fast_schedule, char **reason_down,
    ...
    gres_cnt = _get_tot_gres_cnt(context_ptr->plugin_id, &topo_cnt, &config_type_cnt);
    if ((gres_data->gres_cnt_config > gres_cnt) && (fast_schedule == 1)) {
        if (reason_down && (*reason_down == NULL)) {
            xstrfmtcat(*reason_down, "%s count reported lower than configured "
                                     "(%"
            PRIu64
            " < %"
            PRIu64
            ")", context_ptr->gres_type, gres_cnt, gres_data->gres_cnt_config);
        }
        rc = EINVAL;
    }
    if ((gres_cnt > gres_data->gres_cnt_config) && (fast_schedule != 0)) {
        ...
        " > %"
        PRIu64
        ")", __func__, context_ptr->gres_type, node_name, gres_cnt, gres_data->gres_cnt_config);
        gres_cnt = gres_data->gres_cnt_config;
    }
    if (gres_data->gres_cnt_found != gres_cnt) {
        if (gres_data->gres_cnt_found != NO_VAL64) {
            info("%s: %s: Count changed on node %s (%"
            PRIu64
            " != %"
            PRIu64
            ")", __func__, context_ptr->gres_type, node_name, gres_data->gres_cnt_found, gres_cnt);
        }
        if ((gres_data->gres_cnt_found != NO_VAL64) && (gres_data->gres_cnt_alloc != 0)) {
            if (reason_down && (*reason_down == NULL)) {
                xstrfmtcat(*reason_down, "%s count changed and jobs are using them "
                                         "(%"
                PRIu64
                " != %"
                PRIu64
                ")", context_ptr->gres_type, gres_data->gres_cnt_found, gres_cnt);
            }
            rc = EINVAL;
        } else {
...
/*
    ...
    if ((gres_cnt > gres_data->gres_cnt_config) && (fast_schedule == 2)) {
        info("%s: %s: count on node %s inconsistent with slurmctld count (%"
        PRIu64
        " != %"
        PRIu64
        ")", __func__, context_ptr->gres_type, node_name, gres_cnt, gres_data->gres_cnt_config);
...
/*
		 ...
		 */
        rebuild_topo = true;
        gres_data->topo_gres_cnt_alloc = xrealloc(gres_data->topo_gres_cnt_alloc, topo_cnt * sizeof(uint64_t));
        gres_data->topo_gres_cnt_avail = xrealloc(gres_data->topo_gres_cnt_avail, topo_cnt * sizeof(uint64_t));
        for (i = 0; i < gres_data->topo_cnt; i++) {
            ...
            if (gres_data->topo_core_bitmap) {
                FREE_NULL_BITMAP(gres_data->topo_core_bitmap[i]);
            }
            xfree(gres_data->topo_type_name[i]);
        }
        gres_data->topo_gres_bitmap = xrealloc(gres_data->topo_gres_bitmap, topo_cnt * sizeof(bitstr_t * ));
        gres_data->topo_core_bitmap = xrealloc(gres_data->topo_core_bitmap, topo_cnt * sizeof(bitstr_t * ));
        gres_data->topo_type_id = xrealloc(gres_data->topo_type_id, topo_cnt * sizeof(uint32_t));
        gres_data->topo_type_name = xrealloc(gres_data->topo_type_name, topo_cnt * sizeof(char *));
        if (gres_data->gres_bit_alloc)
            gres_data->gres_bit_alloc = bit_realloc(gres_data->gres_bit_alloc, dev_cnt);
        gres_data->topo_cnt = topo_cnt;
...
/*
    ...
    if (rebuild_topo) {
        ...
        while ((gres_slurmd_conf = (gres_slurmd_conf_t *) list_next(iter))) {
            if (gres_slurmd_conf->plugin_id != context_ptr->plugin_id)
                continue;
            if ((gres_data->gres_bit_alloc) && !_shared_gres(context_ptr->plugin_id))
                gres_data->topo_gres_cnt_alloc[i] = 0;
            gres_data->topo_gres_cnt_avail[i] = gres_slurmd_conf->count;
            if (gres_slurmd_conf->cpus) {
                bitstr_t *tmp_bitmap;
                tmp_bitmap = bit_alloc(gres_slurmd_conf->cpu_cnt);
                bit_unfmt(tmp_bitmap, gres_slurmd_conf->cpus);
                if (gres_slurmd_conf->cpu_cnt == core_cnt) {
                    gres_data->topo_core_bitmap[i] = tmp_bitmap;
                    tmp_bitmap = NULL; /* Nothing to free */
                } else if (gres_slurmd_conf->cpu_cnt == cpu_cnt) {
/* Translate CPU to core bitmap */
                    int cpus_per_core = cpu_cnt / core_cnt;
                    int j, core_inx;
                    gres_data->topo_core_bitmap[i] = bit_alloc(core_cnt);
                    for (j = 0; j < cpu_cnt; j++) {
                        if (!bit_test(tmp_bitmap, j))
                            continue;
                        core_inx = j / cpus_per_core;
                        bit_set(gres_data->topo_core_bitmap[i], core_inx);
                    }
                } else if (i == 0) {
                    error("%s: %s: invalid GRES cpu count (%u) on node %s", __func__, context_ptr->gres_type,
                          gres_slurmd_conf->cpu_cnt, node_name);
                }
                FREE_NULL_BITMAP(tmp_bitmap);
                cpus_config = core_cnt;
            } else if (cpus_config && !cpu_config_err) {
                cpu_config_err = true;
                error("%s: %s: has CPUs configured for only some of the records on node %s", __func__,
                      context_ptr->gres_type, node_name);
            }

            if (gres_slurmd_conf->links) {
                if (gres_data->links_cnt && (gres_data->link_len != gres_cnt)) {
/* Size changed, need to rebuild */
                ...
                if (!gres_data->links_cnt) {
                    ...
                    gres_data->links_cnt = xcalloc(gres_cnt, sizeof(int *));
                    for (j = 0; j < gres_cnt; j++) {
                        gres_data->links_cnt[j] = xcalloc(gres_cnt, sizeof(int));
                    }
                }
            }
...
/* If running jobs recovered then already set */
                if (!gres_data->topo_gres_bitmap[i]) {
                    gres_data->topo_gres_bitmap[i] = bit_alloc(dev_cnt);
                    bit_set(gres_data->topo_gres_bitmap[i], gres_inx);
                }
                gres_inx++;
            } else if (dev_cnt == 0) {
/*
...
/* Ignore excess GRES on node */
                    ...
                    bit_set(gres_data->topo_gres_bitmap[i], gres_inx);
                    if (gres_data->gres_bit_alloc && bit_test(gres_data->gres_bit_alloc, gres_inx)) {
/* Set by recovered job */
                        gres_data->topo_gres_cnt_alloc[i]++;
                    }
                    _links_str2array(gres_slurmd_conf->links, node_name, gres_data, gres_inx, gres_cnt);
                    gres_inx++;
                }
            }
            gres_data->topo_type_id[i] = gres_plugin_build_id(gres_slurmd_conf->type_name);
            gres_data->topo_type_name[i] = xstrdup(gres_slurmd_conf->type_name);
            i++;
            ...
            while ((gres_slurmd_conf = (gres_slurmd_conf_t *) list_next(iter))) {
                ...
                for (j = 0; j < i; j++) {
                    if (gres_data->topo_core_bitmap[j])
                        continue;
                    gres_data->topo_core_bitmap[j] = bit_alloc(cpus_config);
                    bit_set_all(gres_data->topo_core_bitmap[j]);
                }
            }
            list_iterator_destroy(iter);
        }
...
/* Add GRES Type information as needed */
        ...
        while ((gres_slurmd_conf = (gres_slurmd_conf_t *) list_next(iter))) {
            if (gres_slurmd_conf->plugin_id != context_ptr->plugin_id)
                continue;
            type_id = gres_plugin_build_id(gres_slurmd_conf->type_name);
            for (i = 0; i < gres_data->type_cnt; i++) {
...
/* Update count as needed */
                gres_data->type_cnt_avail[i] = gres_slurmd_conf->count;
            } else {
                _add_gres_type(gres_slurmd_conf->type_name, gres_data, gres_slurmd_conf->count);
            }
...
/* This should have been filled in by _node_config_init() */
        ...
        if (_shared_gres(context_ptr->plugin_id)) {
            ...
            if (gres_data->gres_cnt_avail > MAX_GRES_BITMAP) {
                error("%s: %s has \"File\" plus very large \"Count\" "
                      "(%"
                PRIu64
                ") for node %s, resetting value to %u", __func__, context_ptr->gres_type, gres_data->gres_cnt_avail, node_name, MAX_GRES_BITMAP);
    ...
    if ((config_type_cnt > 1) && (fast_schedule > 0) &&
        ...
        rc = EINVAL;
    } else if ((fast_schedule == 1) && (gres_data->gres_cnt_found < gres_data->gres_cnt_config)) {
        if (reason_down && (*reason_down == NULL)) {
            xstrfmtcat(*reason_down, "%s count too low (%"
            PRIu64
            " < %"
            PRIu64
            ")", context_ptr->gres_type, gres_data->gres_cnt_found, gres_data->gres_cnt_config);
        }
        rc = EINVAL;
    } else if (_valid_gres_type(context_ptr->gres_type, gres_data, fast_schedule, reason_down)) {
        rc = EINVAL;
    } else if ((fast_schedule == 2) && gres_data->topo_cnt &&
               (gres_data->gres_cnt_found != gres_data->gres_cnt_config)) {
        error("%s on node %s configured for %"
        PRIu64
        " resources but "
        "%"
        PRIu64
        " found, ignoring topology support", context_ptr->gres_type, node_name, gres_data->gres_cnt_config, gres_data->gres_cnt_found);
        if (gres_data->topo_core_bitmap) {
            for (i = 0; i < gres_data->topo_cnt; i++) {
                if (gres_data->topo_core_bitmap) {
                    FREE_NULL_BITMAP(gres_data->topo_core_bitmap[i]);
 ...
 */
extern int gres_plugin_node_config_validate(char *node_name, char *orig_config, char **new_config, List *gres_list,
                                            int threads_per_core, int cores_per_sock, int sock_cnt,
...
/* Find or create gres_state entry on the list */
        ...
        rc2 = _node_config_validate(node_name, orig_config, new_config, gres_ptr, cpu_cnt, core_cnt, sock_cnt,
                                    fast_schedule, reason_down, &gres_context[i]);
        rc = MAX(rc, rc2);
...
static void _gres_scale_value(uint64_t gres_size, uint64_t *gres_scaled, char **suffix) {
 ...
 */
extern void
gres_plugin_node_feature(char *node_name, char *gres_name, uint64_t gres_size, char **new_config, List *gres_list) {
    ...
    if (*new_config) {
        ...
        while (tok) {
            if (!strncmp(tok, gres_name, gres_name_len) &&
                ((tok[gres_name_len] == ':') || (tok[gres_name_len] == '\0'))) {
/* Skip this record */
            } else {
                xstrfmtcat(new_gres, "%s%s", sep, tok);
                sep = ",";
    ...
    xstrfmtcat(new_gres, "%s%s:%"
    PRIu64
    "%s", sep, gres_name, gres_scaled, suffix);
    xfree(*new_config);
    *new_config = new_gres;
    ...
    if (gres_context_cnt > 0) {
        ...
        if (gres_size >= gres_node_ptr->gres_cnt_alloc) {
            gres_node_ptr->gres_cnt_avail = gres_size - gres_node_ptr->gres_cnt_alloc;
        } else {
            error("%s: Changed size count of GRES %s from %"
            PRIu64
            " to %"
            PRIu64
            ", resource over allocated", __func__, gres_name, gres_node_ptr->gres_cnt_avail, gres_size);
...
_node_reconfig_test(char *node_name, char *new_gres, gres_state_t *gres_ptr, slurm_gres_context_t *context_ptr) {
    ...
    _get_gres_cnt(new_gres_data, new_gres, context_ptr->gres_name, context_ptr->gres_name_colon,
                  context_ptr->gres_name_colon_len);
    if ((new_gres_data->gres_cnt_config != 0) && (new_gres_data->gres_cnt_config != orig_gres_data->gres_cnt_config)) {
        error("Attempt to change gres/%s Count on node %s from %"
        PRIu64
        " to %"
        PRIu64
        " invalid with File configuration", context_ptr->gres_name, node_name, orig_gres_data->gres_cnt_config, new_gres_data->gres_cnt_config);
...
_node_reconfig(char *node_name, char *new_gres, char **gres_str, gres_state_t *gres_ptr, uint16_t fast_schedule,
    ...
    *updated_gpu_cnt = false;
    ...
    _get_gres_cnt(gres_data, new_gres, context_ptr->gres_name, context_ptr->gres_name_colon,
                  context_ptr->gres_name_colon_len);
...
/* Update count */
    ...
    if (context_ptr->config_flags & GRES_CONF_HAS_FILE) {
        ...
        else gres_bits = gres_data->gres_cnt_avail;

        _gres_bit_alloc_resize(gres_data, gres_bits);
    } else if (gres_data->gres_bit_alloc && !_shared_gres(context_ptr->plugin_id)) {
/*
        ...
        if (gres_bits != bit_size(gres_data->gres_bit_alloc)) {
            info("gres/%s count changed on node %s to %"
            PRIu64, context_ptr->gres_name, node_name, gres_bits);
            if (_sharing_gres(context_ptr->plugin_id))
                *updated_gpu_cnt = true;
            gres_data->gres_bit_alloc = bit_realloc(gres_data->gres_bit_alloc, gres_bits);
            for (i = 0; i < gres_data->topo_cnt; i++) {
                if (gres_data->topo_gres_bitmap && gres_data->topo_gres_bitmap[i] &&
                    (gres_bits != bit_size(gres_data->topo_gres_bitmap[i]))) {
                    gres_data->topo_gres_bitmap[i] = bit_realloc(gres_data->topo_gres_bitmap[i], gres_bits);
                }
            }
        }
    }
...
/* No gres/mps on this node */
    ...
    if (!mps_gres_data->gres_bit_alloc) {
        ...
        mps_gres_data->gres_bit_alloc = bit_realloc(mps_gres_data->gres_bit_alloc, gpu_cnt);
    }

/* Add any additional required gres/mps topo records */
    if (mps_gres_data->topo_cnt) {
        mps_gres_data->topo_core_bitmap = xrealloc(mps_gres_data->topo_core_bitmap, sizeof(bitstr_t * ) * gpu_cnt);
        mps_gres_data->topo_gres_bitmap = xrealloc(mps_gres_data->topo_gres_bitmap, sizeof(bitstr_t * ) * gpu_cnt);
        mps_gres_data->topo_gres_cnt_alloc = xrealloc(mps_gres_data->topo_gres_cnt_alloc, sizeof(uint64_t) * gpu_cnt);
        mps_gres_data->topo_gres_cnt_avail = xrealloc(mps_gres_data->topo_gres_cnt_avail, sizeof(uint64_t) * gpu_cnt);
        mps_gres_data->topo_type_id = xrealloc(mps_gres_data->topo_type_id, sizeof(uint32_t) * gpu_cnt);
        mps_gres_data->topo_type_name = xrealloc(mps_gres_data->topo_type_name, sizeof(char *) * gpu_cnt);
    } else {
        mps_gres_data->topo_core_bitmap = xcalloc(gpu_cnt, sizeof(bitstr_t * ));
        mps_gres_data->topo_gres_bitmap = xcalloc(gpu_cnt, sizeof(bitstr_t * ));
        mps_gres_data->topo_gres_cnt_alloc = xcalloc(gpu_cnt, sizeof(uint64_t));
        mps_gres_data->topo_gres_cnt_avail = xcalloc(gpu_cnt, sizeof(uint64_t));
        mps_gres_data->topo_type_id = xcalloc(gpu_cnt, sizeof(uint32_t));
        mps_gres_data->topo_type_name = xcalloc(gpu_cnt, sizeof(char *));
    }
...
/*
    ...
    if (mps_alloc >= mps_gres_data->gres_cnt_avail)
        mps_rem = 0;
    else mps_rem = mps_gres_data->gres_cnt_avail - mps_alloc;
    ...
    for (i = 0; i < mps_gres_data->topo_cnt; i++) {
        if (mps_gres_data->topo_gres_bitmap && mps_gres_data->topo_gres_bitmap[i] &&
            (gpu_cnt != bit_size(mps_gres_data->topo_gres_bitmap[i]))) {
            mps_gres_data->topo_gres_bitmap[i] = bit_realloc(mps_gres_data->topo_gres_bitmap[i], gpu_cnt);
        }
    }
}

/* Convert core bitmap into socket string, xfree return value */
static char *_core_bitmap2str(bitstr_t *core_map, int cores_per_sock, int sock_per_node) {
    ...
    for (s = 0; s < sock_per_node; s++) {
        ...
        for (c = 0; c < cores_per_sock; c++) {
            if (core_offset > max_core) {
                error("%s: bad core offset (%d >= %d)", __func__, core_offset, max_core);
                break;
            }
...
/* Find gres_state entry on the list */
        ...
        if (gres_node_state->topo_cnt && gres_node_state->gres_cnt_avail) {
            done_topo = bit_alloc(gres_node_state->topo_cnt);
            for (i = 0; i < gres_node_state->topo_cnt; i++) {
                if (bit_test(done_topo, i))
                    continue;
                bit_set(done_topo, i);
                gres_sum = gres_node_state->topo_gres_cnt_avail[i];
                if (gres_node_state->topo_core_bitmap[i]) {
                    core_map = bit_copy(gres_node_state->topo_core_bitmap[i]);
                } else core_map = NULL;
                for (j = 0; j < gres_node_state->topo_cnt; j++) {
                    if (gres_node_state->topo_type_id[i] != gres_node_state->topo_type_id[j])
                        continue;
                    if (bit_test(done_topo, j))
                        continue;
                    bit_set(done_topo, j);
                    gres_sum += gres_node_state->topo_gres_cnt_avail[j];
                    if (core_map && gres_node_state->topo_core_bitmap[j]) {
                        bit_or(core_map, gres_node_state->topo_core_bitmap[j]);
                    } else if (gres_node_state->topo_core_bitmap[j]) {
                        core_map = bit_copy(gres_node_state->topo_core_bitmap[j]);
                    }
                }
                if (core_map) {
                    sock_info = _core_bitmap2str(core_map, cores_per_sock, sock_per_node);
                    bit_free(core_map);
                    sock_str = sock_info;
                } else sock_str = "";
                suffix = _get_suffix(&gres_sum);
                if (gres_node_state->topo_type_name[i]) {
                    xstrfmtcat(*gres_str, "%s%s:%s:%"
                    PRIu64
                    "%s%s", sep, gres_context[c].gres_name, gres_node_state->topo_type_name[i], gres_sum, suffix, sock_str);
                } else {
                    xstrfmtcat(*gres_str, "%s%s:%"
                    PRIu64
                    "%s%s", sep, gres_context[c].gres_name, gres_sum, suffix, sock_str);
                }
                xfree(sock_info);
                sep = ",";
            }
            bit_free(done_topo);
        } else if (gres_node_state->type_cnt && gres_node_state->gres_cnt_avail) {
            for (i = 0; i < gres_node_state->type_cnt; i++) {
                gres_sum = gres_node_state->type_cnt_avail[i];
                suffix = _get_suffix(&gres_sum);
                xstrfmtcat(*gres_str, "%s%s:%s:%"
                PRIu64
                "%s", sep, gres_context[c].gres_name, gres_node_state->type_name[i], gres_sum, suffix);
                sep = ",";
            }
            ...
            xstrfmtcat(*gres_str, "%s%s:%"
            PRIu64
            "%s", sep, gres_context[c].gres_name, gres_sum, suffix);
            sep = ",";
        }
    }
}
 ...
 */
extern int
gres_plugin_node_reconfig(char *node_name, char *new_gres, char **gres_str, List *gres_list, uint16_t fast_schedule,
...
/* Find gres_state entry on the list */
        ...
        rc = _node_reconfig_test(node_name, new_gres, gres_ptr, &gres_context[i]);
    }
...
/* Now update the GRES counts */
    for (i = 0; (rc == SLURM_SUCCESS) && (i < gres_context_cnt); i++) {
        ...
        if (gres_ptr_array[i] == NULL)
            continue;
        rc = _node_reconfig(node_name, new_gres, gres_str, gres_ptr_array[i], fast_schedule, &gres_context[i],
                            &updated_gpu_cnt);
        if (updated_gpu_cnt)
 ...
 * IN node_name - name of the node for which the gres information applies
 */
extern int gres_plugin_node_state_pack(List gres_list, Buf buffer, char *node_name) {
    int rc = SLURM_SUCCESS;
 ...
 * IN node_name - name of the node for which the gres information applies
 */
extern int gres_plugin_node_state_unpack(List *gres_list, Buf buffer, char *node_name, uint16_t protocol_version) {
    int i, rc;
    ...
    while ((rc == SLURM_SUCCESS) && (rec_cnt)) {
        ...
        if (i >= gres_context_cnt) {
            error("%s: no plugin configured to unpack data type %u from node %s", __func__, plugin_id, node_name);
/*
        ...
        if (gres_bitmap_size) {
            gres_node_ptr->gres_bit_alloc = bit_alloc(gres_bitmap_size);
        }
        gres_ptr = xmalloc(sizeof(gres_state_t));
        gres_ptr->plugin_id = gres_context[i].plugin_id;
...
static void *_node_state_dup(void *gres_data) {
    ...
    if (gres_ptr->links_cnt && gres_ptr->link_len) {
        new_gres->links_cnt = xcalloc(gres_ptr->link_len, sizeof(int *));
    ...
    if (gres_ptr->topo_cnt) {
        ...
        new_gres->topo_core_bitmap = xcalloc(gres_ptr->topo_cnt, sizeof(bitstr_t * ));
        new_gres->topo_gres_bitmap = xcalloc(gres_ptr->topo_cnt, sizeof(bitstr_t * ));
        new_gres->topo_gres_cnt_alloc = xcalloc(gres_ptr->topo_cnt, sizeof(uint64_t));
        new_gres->topo_gres_cnt_avail = xcalloc(gres_ptr->topo_cnt, sizeof(uint64_t));
        new_gres->topo_type_id = xcalloc(gres_ptr->topo_cnt, sizeof(uint32_t));
        new_gres->topo_type_name = xcalloc(gres_ptr->topo_cnt, sizeof(char *));
        for (i = 0; i < gres_ptr->topo_cnt; i++) {
            if (gres_ptr->topo_core_bitmap[i]) {
                new_gres->topo_core_bitmap[i] = bit_copy(gres_ptr->topo_core_bitmap[i]);
            }
            new_gres->topo_gres_bitmap[i] = bit_copy(gres_ptr->topo_gres_bitmap[i]);
            new_gres->topo_gres_cnt_alloc[i] = gres_ptr->topo_gres_cnt_alloc[i];
            new_gres->topo_gres_cnt_avail[i] = gres_ptr->topo_gres_cnt_avail[i];
            new_gres->topo_type_id[i] = gres_ptr->topo_type_id[i];
            new_gres->topo_type_name[i] = xstrdup(gres_ptr->topo_type_name[i]);
        }
    }

    if (gres_ptr->type_cnt) {
        new_gres->type_cnt = gres_ptr->type_cnt;
        new_gres->type_cnt_alloc = xcalloc(gres_ptr->type_cnt, sizeof(uint64_t));
        new_gres->type_cnt_avail = xcalloc(gres_ptr->type_cnt, sizeof(uint64_t));
        new_gres->type_id = xcalloc(gres_ptr->type_cnt, sizeof(uint32_t));
        new_gres->type_name = xcalloc(gres_ptr->type_cnt, sizeof(char *));
...
extern List gres_plugin_node_state_dup(List gres_list) {
    ...
    while ((gres_ptr = (gres_state_t *) list_next(gres_iter))) {
        ...
        if (i >= gres_context_cnt) {
            error("Could not find plugin id %u to dup node record", gres_ptr->plugin_id);
        }
    }
    list_iterator_destroy(gres_iter);
    slurm_mutex_unlock(&gres_context_lock);
...
static char *_node_gres_used(void *gres_data, char *gres_name) {
    ...
    if ((gres_node_ptr->topo_cnt != 0) && (gres_node_ptr->no_consume == false)) {
        ...
        for (i = 0; i < gres_node_ptr->topo_cnt; i++) {
            ...
            if (gres_node_ptr->topo_gres_bitmap[i]) {
                topo_gres_bitmap = bit_copy(gres_node_ptr->topo_gres_bitmap[i]);
            }
            for (j = i + 1; j < gres_node_ptr->topo_cnt; j++) {
                if (bit_test(topo_printed, j))
                    continue;
                if (gres_node_ptr->topo_type_id[i] != gres_node_ptr->topo_type_id[j])
                    continue;
                bit_set(topo_printed, j);
                if (gres_node_ptr->topo_gres_bitmap[j]) {
                    if (!topo_gres_bitmap) {
                        topo_gres_bitmap = bit_copy(gres_node_ptr->topo_gres_bitmap[j]);
                    } else if (bit_size(topo_gres_bitmap) == bit_size(gres_node_ptr->topo_gres_bitmap[j])) {
                        bit_or(topo_gres_bitmap, gres_node_ptr->topo_gres_bitmap[j]);
                    }
                }
            }
            if (gres_node_ptr->gres_bit_alloc && topo_gres_bitmap &&
                (bit_size(topo_gres_bitmap) == bit_size(gres_node_ptr->gres_bit_alloc))) {
                bit_and(topo_gres_bitmap, gres_node_ptr->gres_bit_alloc);
                gres_alloc_cnt = bit_set_count(topo_gres_bitmap);
            }
            if (gres_alloc_cnt > 0) {
                bit_fmt(tmp_str, sizeof(tmp_str), topo_gres_bitmap);
                gres_alloc_idx = tmp_str;
            } else {
                gres_alloc_idx = "N/A";
            }
            xstrfmtcat(gres_node_ptr->gres_used, "%s%s:%s:%"
            PRIu64
            "(IDX:%s)", sep, gres_name, gres_node_ptr->topo_type_name[i], gres_alloc_cnt, gres_alloc_idx);
            sep = ",";
            FREE_NULL_BITMAP(topo_gres_bitmap);
        }
        FREE_NULL_BITMAP(topo_printed);
    } else if (gres_node_ptr->gres_used) { ;    /* Used cached value */
    } else if (gres_node_ptr->type_cnt == 0) {
        if (gres_node_ptr->no_consume) {
            xstrfmtcat(gres_node_ptr->gres_used, "%s:0", gres_name);
        } else {
            xstrfmtcat(gres_node_ptr->gres_used, "%s:%"
            PRIu64, gres_name, gres_node_ptr->gres_cnt_alloc);
        }
    } else {
        for (i = 0; i < gres_node_ptr->type_cnt; i++) {
            if (gres_node_ptr->no_consume) {
                xstrfmtcat(gres_node_ptr->gres_used, "%s%s:%s:0", sep, gres_name, gres_node_ptr->type_name[i]);
            } else {
                xstrfmtcat(gres_node_ptr->gres_used, "%s%s:%s:%"
                PRIu64, sep, gres_name, gres_node_ptr->type_name[i], gres_node_ptr->type_cnt_alloc[i]);
            }
            sep = ",";
        }
    }
...
static void _node_state_log(void *gres_data, char *node_name, char *gres_name) {
    ...
    if (gres_node_ptr->gres_cnt_found == NO_VAL64) {
        ...
        PRIu64, gres_node_ptr->gres_cnt_found);
    }

    if (gres_node_ptr->no_consume) {
        info("  gres_cnt found:%s configured:%"
        PRIu64
        " "
        "avail:%"
        PRIu64
        " no_consume", tmp_str, gres_node_ptr->gres_cnt_config, gres_node_ptr->gres_cnt_avail);
    } else {
        info("  gres_cnt found:%s configured:%"
        PRIu64
        " "
        ...
        " alloc:%"
        PRIu64
        "", tmp_str, gres_node_ptr->gres_cnt_config, gres_node_ptr->gres_cnt_avail, gres_node_ptr->gres_cnt_alloc);
    }

    if (gres_node_ptr->gres_bit_alloc) {
        bit_fmt(tmp_str, sizeof(tmp_str), gres_node_ptr->gres_bit_alloc);
        info("  gres_bit_alloc:%s of %d", tmp_str, (int) bit_size(gres_node_ptr->gres_bit_alloc));
    } else {
    ...
    if (gres_node_ptr->links_cnt && gres_node_ptr->link_len) {
        for (i = 0; i < gres_node_ptr->link_len; i++) {
            ...
            for (j = 0; j < gres_node_ptr->link_len; j++) {
                xstrfmtcat(buf, "%s%d", sep, gres_node_ptr->links_cnt[i][j]);
                sep = ", ";
            }
    ...
    for (i = 0; i < gres_node_ptr->topo_cnt; i++) {
        info("  topo[%d]:%s(%u)", i, gres_node_ptr->topo_type_name[i], gres_node_ptr->topo_type_id[i]);
        if (gres_node_ptr->topo_core_bitmap[i]) {
            bit_fmt(tmp_str, sizeof(tmp_str), gres_node_ptr->topo_core_bitmap[i]);
            info("   topo_core_bitmap[%d]:%s of %d", i, tmp_str, (int) bit_size(gres_node_ptr->topo_core_bitmap[i]));
        } else info("   topo_core_bitmap[%d]:NULL", i);
        if (gres_node_ptr->topo_gres_bitmap[i]) {
            bit_fmt(tmp_str, sizeof(tmp_str), gres_node_ptr->topo_gres_bitmap[i]);
            info("   topo_gres_bitmap[%d]:%s of %d", i, tmp_str, (int) bit_size(gres_node_ptr->topo_gres_bitmap[i]));
        } else info("   topo_gres_bitmap[%d]:NULL", i);
        info("   topo_gres_cnt_alloc[%d]:%"
        PRIu64
        "", i, gres_node_ptr->topo_gres_cnt_alloc[i]);
        info("   topo_gres_cnt_avail[%d]:%"
        PRIu64
        "", i, gres_node_ptr->topo_gres_cnt_avail[i]);
    }

    for (i = 0; i < gres_node_ptr->type_cnt; i++) {
        info("  type[%d]:%s(%u)", i, gres_node_ptr->type_name[i], gres_node_ptr->type_id[i]);
        info("   type_cnt_alloc[%d]:%"
        PRIu64, i, gres_node_ptr->type_cnt_alloc[i]);
        info("   type_cnt_avail[%d]:%"
        PRIu64, i, gres_node_ptr->type_cnt_avail[i]);
    }
}
...
extern void gres_plugin_node_state_log(List gres_list, char *node_name) {
    ...
    while ((gres_ptr = (gres_state_t *) list_next(gres_iter))) {
        for (i = 0; i < gres_context_cnt; i++) {
            if (gres_ptr->plugin_id != gres_context[i].plugin_id)
                continue;
            _node_state_log(gres_ptr->gres_data, node_name, gres_context[i].gres_name);
            break;
        }
    }
    list_iterator_destroy(gres_iter);
...
extern char *gres_get_node_used(List gres_list) {
    ...
    while ((gres_ptr = (gres_state_t *) list_next(gres_iter))) {
        for (i = 0; i < gres_context_cnt; i++) {
            if (gres_ptr->plugin_id != gres_context[i].plugin_id)
                continue;
            tmp = _node_gres_used(gres_ptr->gres_data, gres_context[i].gres_name);
            if (!tmp)
                continue;
            if (gres_used)
                xstrcat(gres_used, ",");
...
/* Find or create gres_state entry on the list */
            ...
            break;
        } else if (!xstrncmp(name, gres_context[i].gres_name_colon, gres_context[i].gres_name_colon_len)) {
            int type;
            uint32_t type_id;
            char *type_str = NULL;
 ...
 */
static int
_test_gres_cnt(gres_job_state_t *job_gres_data, uint32_t *num_tasks, uint32_t *min_nodes, uint32_t *max_nodes,
               uint16_t *ntasks_per_node, uint16_t *ntasks_per_socket, uint16_t *sockets_per_node,
...
/* Ensure gres_per_job >= gres_per_node >= gres_per_socket */
    if (job_gres_data->gres_per_job &&
        ((job_gres_data->gres_per_node && (job_gres_data->gres_per_node > job_gres_data->gres_per_job)) ||
         (job_gres_data->gres_per_task && (job_gres_data->gres_per_task > job_gres_data->gres_per_job)) ||
         (job_gres_data->gres_per_socket && (job_gres_data->gres_per_socket > job_gres_data->gres_per_job))))
        return -1;

/* Ensure gres_per_job >= gres_per_task */
    if (job_gres_data->gres_per_node &&
        ((job_gres_data->gres_per_task && (job_gres_data->gres_per_task > job_gres_data->gres_per_node)) ||
         (job_gres_data->gres_per_socket && (job_gres_data->gres_per_socket > job_gres_data->gres_per_node))))
        return -1;
...
/* gres_per_job not multiple of gres_per_node */
        ...
        req_nodes = job_gres_data->gres_per_job / job_gres_data->gres_per_node;
...
/* gres_per_node not multiple of gres_per_socket */
        ...
        req_sockets = job_gres_data->gres_per_node / job_gres_data->gres_per_socket;
        if (*sockets_per_node == NO_VAL16)
...
/* gres_per_job not multiple of gres_per_task */
        ...
        req_tasks = job_gres_data->gres_per_job / job_gres_data->gres_per_task;
        if (*num_tasks == NO_VAL)
            *num_tasks = req_tasks;
...
/* gres_per_node not multiple of gres_per_task */
        ...
        req_tasks_per_node = job_gres_data->gres_per_node / job_gres_data->gres_per_task;
        if ((*ntasks_per_node == NO_VAL16) || (*ntasks_per_node == 0))
...
/* gres_per_socket not multiple of gres_per_task */
        ...
        req_tasks_per_socket = job_gres_data->gres_per_socket / job_gres_data->gres_per_task;
        if ((*ntasks_per_socket == NO_VAL16) || (*ntasks_per_socket == 0))
 ...
 */
static int
_get_next_gres(char *in_val, char **type_ptr, int *context_inx_ptr, uint64_t *cnt, uint16_t *flags, char **save_ptr) {
    ...
    *flags = 0;
    ...
    next:
    if (*save_ptr[0] == '\0') {    /* Empty input token */
        *save_ptr = NULL;
        goto fini;
    }
...
/* Bad format (e.g. "gpu:tesla:" or "gpu::1") */
        ...
        if (!_is_valid_number(sep2, &value)) {
            debug("%s: Invalid count value GRES %s:%s:%s", __func__, name, type, sep2);
            rc = ESLURM_INVALID_GRES;
            goto fini;
        }
...
/* We have no type and implicit count of 1 */
    ...
    for (i = 0; i < gres_context_cnt; i++) {
        if (!xstrcmp(name, gres_context[i].gres_name) ||
            !xstrncmp(name, gres_context[i].gres_name_colon, gres_context[i].gres_name_colon_len))
    ...
    if (rc != SLURM_SUCCESS) {
        *save_ptr = NULL;
        if (rc == ESLURM_INVALID_GRES) {
            info("%s: Invalid GRES job specification %s", __func__, in_val);
        }
        xfree(type);
        *type_ptr = NULL;
    } else {
 ...
 */
static gres_job_state_t *_get_next_job_gres(char *in_val, uint64_t *cnt, List gres_list, char **save_ptr, int *rc) {
    ...
    if ((my_rc = _get_next_gres(in_val, &type, &context_inx, cnt, &flags, &prev_save_ptr)) || (context_inx == NO_VAL)) {
        prev_save_ptr = NULL;
        goto fini;
    }
...
/* Find the job GRES record */
    ...
    gres_ptr = list_find_first(gres_list, _gres_find_job_by_key, &job_search_key);

    if (gres_ptr) {
        job_gres_data = gres_ptr->gres_data;
    } else {
        job_gres_data = xmalloc(sizeof(gres_job_state_t));
        job_gres_data->gres_name = xstrdup(gres_context[context_inx].gres_name);
...
static bool _generic_job_state(gres_job_state_t *job_state) {
    if (job_state->gres_per_job || job_state->gres_per_node || job_state->gres_per_socket || job_state->gres_per_task)
        return false;
    return true;
}
 ...
 */
extern int gres_plugin_job_state_validate(char *cpus_per_tres, char *tres_freq, char *tres_per_job, char *tres_per_node,
                                          char *tres_per_socket, char *tres_per_task, char *mem_per_tres,
                                          uint32_t *num_tasks, uint32_t *min_nodes, uint32_t *max_nodes,
                                          uint16_t *ntasks_per_node, uint16_t *ntasks_per_socket,
                                          uint16_t *sockets_per_node, uint16_t *cpus_per_task, List *gres_list) {
    ...
    if (!cpus_per_tres && !tres_per_job && !tres_per_node && !tres_per_socket && !tres_per_task && !mem_per_tres)
        return SLURM_SUCCESS;

    if (tres_per_task && (*num_tasks == NO_VAL) && (*min_nodes != NO_VAL) && (*min_nodes == *max_nodes)) {
/* Implicitly set task count */
    ...
    if ((select_plugin_type != SELECT_TYPE_CONS_TRES) &&
        (cpus_per_tres || tres_per_job || tres_per_socket || tres_per_task || mem_per_tres))
...
/*
    ...
    if (cpus_per_tres) {
        ...
        while ((job_gres_data = _get_next_job_gres(in_val, &cnt, *gres_list, &save_ptr, &rc))) {
    ...
    if (tres_per_job) {
        ...
        while ((job_gres_data = _get_next_job_gres(in_val, &cnt, *gres_list, &save_ptr, &rc))) {
            ...
            in_val = NULL;
            job_gres_data->total_gres = MAX(job_gres_data->total_gres, cnt);
        }
    }
    if (tres_per_node) {
    ...
    if (tres_per_socket) {
        ...
        while ((job_gres_data = _get_next_job_gres(in_val, &cnt, *gres_list, &save_ptr, &rc))) {
            ...
            in_val = NULL;
            if ((*min_nodes != NO_VAL) && (*sockets_per_node != NO_VAL16)) {
                cnt *= (*min_nodes * *sockets_per_node);
            } else if ((*num_tasks != NO_VAL) && (*ntasks_per_socket != NO_VAL16)) {
                cnt *= ((*num_tasks + *ntasks_per_socket - 1) / *ntasks_per_socket);
            }
        }
    }
    if (tres_per_task) {
...
/*
    ...
    while ((gres_state = (gres_state_t *) list_next(iter))) {
        ...
        if (_test_gres_cnt(job_gres_data, num_tasks, min_nodes, max_nodes, ntasks_per_node, ntasks_per_socket,
                           sockets_per_node, cpus_per_task) != 0) {
            rc = ESLURM_INVALID_GRES;
            break;
        }
...
/*
			 ...
			 */
            if (job_gres_data->gres_per_job && (*max_nodes != 1)) {
                rc = ESLURM_INVALID_GRES;
                break;
            }
            if (job_gres_data->gres_per_socket && (*sockets_per_node != 1)) {
                rc = ESLURM_INVALID_GRES;
                break;
            }
    ...
    if (have_gres_mps && (rc == SLURM_SUCCESS) && tres_freq && strstr(tres_freq, "gpu")) {
        rc = ESLURM_INVALID_GRES;
    }
    ...
    if (overlap_merge) {    /* Merge generic data if possible */
        ...
        for (i = 0; i < over_count; i++) {
            if (!over_list[i].with_model || !over_list[i].without_model_state)
                continue;
            if (!_generic_job_state(over_list[i].without_model_state)) {
                rc = ESLURM_INVALID_GRES_TYPE;
                break;
            }
/* Propagate generic parameters */
            cpus_per_gres = over_list[i].without_model_state->cpus_per_gres;
            mem_per_gres = over_list[i].without_model_state->mem_per_gres;
            list_iterator_reset(iter);
            while ((gres_state = (gres_state_t *) list_next(iter))) {
                job_gres_data = (gres_job_state_t *) gres_state->gres_data;
                if (over_list[i].plugin_id != gres_state->plugin_id)
                    continue;
                if (job_gres_data == over_list[i].without_model_state) {
                    list_remove(iter);
                    continue;
                }
                if (job_gres_data->cpus_per_gres == 0) {
                    job_gres_data->cpus_per_gres = cpus_per_gres;
                }
                if (job_gres_data->mem_per_gres == 0) {
                    job_gres_data->mem_per_gres = mem_per_gres;
                }
            }
        }
    }
...
extern int gres_plugin_job_revalidate(List gres_list) {
    ...
    while ((gres_state = (gres_state_t *) list_next(iter))) {
        job_gres_data = (gres_job_state_t *) gres_state->gres_data;
        if (job_gres_data->gres_per_job || job_gres_data->gres_per_socket || job_gres_data->gres_per_task) {
...
static bool _job_has_gres_bits(List job_gres_list) {
    ...
    while ((gres_ptr = (gres_state_t *) list_next(job_gres_iter))) {
        ...
        for (i = 0; i < job_gres_ptr->node_cnt; i++) {
            if (job_gres_ptr->gres_bit_alloc && job_gres_ptr->gres_bit_alloc[i]) {
                rc = true;
                break;
            }
        }
 ...
 */
static bool
_validate_node_gres_cnt(uint32_t job_id, List job_gres_list, int node_inx, List node_gres_list, char *node_name) {
    ListIterator job_gres_iter;
    gres_state_t *gres_ptr;
    gres_job_state_t *job_gres_ptr;
    bool rc = true;
    ...
    while ((gres_ptr = (gres_state_t *) list_next(job_gres_iter))) {
        ...
        if (!job_gres_ptr || !job_gres_ptr->gres_bit_alloc)
            continue;
        if ((node_inx >= job_gres_ptr->node_cnt) || !job_gres_ptr->gres_bit_alloc[node_inx])
            continue;
        job_gres_cnt = bit_size(job_gres_ptr->gres_bit_alloc[node_inx]);
        node_gres_cnt = _get_node_gres_cnt(node_gres_list, gres_ptr->plugin_id);
        if (job_gres_cnt != node_gres_cnt) {
            error("%s: Killing job %u: gres/%s count mismatch on node "
                  "%s (%d != %d)", __func__, job_id, job_gres_ptr->gres_name, node_name, job_gres_cnt, node_gres_cnt);
            rc = false;
            break;
        }
    }
 ...
 * RET SLURM_SUCCESS or ESLURM_INVALID_GRES
 */
extern int gres_plugin_job_revalidate2(uint32_t job_id, List job_gres_list, bitstr_t *node_bitmap) {
    struct node_record *node_ptr;
    int rc = SLURM_SUCCESS;
    int i_first, i_last, i;
    int node_inx = -1;

    if (!job_gres_list || !node_bitmap || !_job_has_gres_bits(job_gres_list))
        return SLURM_SUCCESS;
    ...
    for (i = i_first; i <= i_last; i++) {
        ...
        node_inx++;
        if (!_validate_node_gres_cnt(job_id, job_gres_list, node_inx, node_ptr->gres_list, node_ptr->name)) {
            rc = ESLURM_INVALID_GRES;
            break;
        }
    }
...
static int _find_sock_by_job_gres(void *x, void *key) {
    ...
    if ((sock_data->plugin_id == job_gres_state->plugin_id) && (sock_data->type_id == job_data->type_id))
        return 1;
    return 0;
}
...
extern bool gres_plugin_job_sched_test(List job_gres_list, uint32_t job_id) {
    ...
    while ((job_gres_state = (gres_state_t *) list_next(iter))) {
        job_data = (gres_job_state_t *) job_gres_state->gres_data;
        if (job_data->gres_per_job && (job_data->gres_per_job > job_data->total_gres)) {
            rc = false;
            break;
        }
    }
 ...
 */
extern bool gres_plugin_job_sched_test2(List job_gres_list, List sock_gres_list, uint32_t job_id) {
    ListIterator iter;
    gres_state_t *job_gres_state;
    gres_job_state_t *job_data;
    ...
    while ((job_gres_state = (gres_state_t *) list_next(iter))) {
        job_data = (gres_job_state_t *) job_gres_state->gres_data;
        if ((job_data->gres_per_job == 0) || (job_data->gres_per_job < job_data->total_gres))
            continue;
        sock_data = list_find_first(sock_gres_list, _find_sock_by_job_gres, job_gres_state);
        if (!sock_data || (job_data->gres_per_job > (job_data->total_gres + sock_data->total_cnt))) {
            rc = false;
            break;
        }
    }
 ...
 */
extern void gres_plugin_job_sched_add(List job_gres_list, List sock_gres_list, uint16_t avail_cpus) {
    ListIterator iter;
    gres_state_t *job_gres_state;
    gres_job_state_t *job_data;
    sock_gres_t *sock_data;
    ...
    while ((job_gres_state = (gres_state_t *) list_next(iter))) {
        ...
        } else gres_limit = sock_data->total_cnt;
 ...
 * IN sock_gres_list - Per socket GRES availability on this node (sock_gres_t)
 */
extern void gres_plugin_job_sched_consec(List *consec_gres, List job_gres_list, List sock_gres_list) {
    ListIterator iter;
    gres_state_t *job_gres_state;
    gres_job_state_t *job_data;
    ...
    while ((job_gres_state = (gres_state_t *) list_next(iter))) {
        ...
        consec_data = list_find_first(*consec_gres, _find_sock_by_job_gres, job_gres_state);
        if (!consec_data) {
 ...
 */
extern bool gres_plugin_job_sched_sufficient(List job_gres_list, List sock_gres_list) {
    ListIterator iter;
    gres_state_t *job_gres_state;
    gres_job_state_t *job_data;
    sock_gres_t *sock_data;
    ...
    while ((job_gres_state = (gres_state_t *) list_next(iter))) {
        ...
        if ((job_data->total_gres + sock_data->total_cnt) < job_data->gres_per_job) {
            rc = false;
            break;
        }
    }
...
extern char *gres_plugin_job_sched_str(List sock_gres_list, List job_gres_list) {
    ...
    while ((sock_data = (sock_gres_t *) list_next(iter))) {
        job_gres_state = list_find_first(job_gres_list, _find_job_by_sock_gres, sock_data);
        if (!job_gres_state) {    /* Should never happen */
            error("%s: Could not find job GRES for type %u:%u", __func__, sock_data->plugin_id, sock_data->type_id);
            continue;
        }
        job_data = (gres_job_state_t *) job_gres_state->gres_data;
        if (out_str)
        ...
        if (job_data->type_name) {
            xstrfmtcat(out_str, "%s%s:%s:%"
            PRIu64, sep, job_data->gres_name, job_data->type_name, sock_data->total_cnt);
        } else {
            xstrfmtcat(out_str, "%s%s:%"
            PRIu64, sep, job_data->gres_name, sock_data->total_cnt);
        }
    }
    list_iterator_destroy(iter);
...
static void *_job_state_dup(void *gres_data) {
    ...
    if (gres_ptr->gres_cnt_node_alloc) {
        ...
        memcpy(new_gres_ptr->gres_cnt_node_alloc, gres_ptr->gres_cnt_node_alloc, i);
    }
    if (gres_ptr->gres_bit_alloc) {
        new_gres_ptr->gres_bit_alloc = xcalloc(gres_ptr->node_cnt, sizeof(bitstr_t * ));
        for (i = 0; i < gres_ptr->node_cnt; i++) {
            if (gres_ptr->gres_bit_alloc[i] == NULL)
                continue;
            new_gres_ptr->gres_bit_alloc[i] = bit_copy(gres_ptr->gres_bit_alloc[i]);
        }
    }
    return new_gres_ptr;
}
...
static void *_job_state_dup2(void *gres_data, int node_index) {
    ...
    if (gres_ptr->gres_cnt_node_alloc) {
        ...
        new_gres_ptr->gres_cnt_node_alloc[0] = gres_ptr->gres_cnt_node_alloc[node_index];
    }
    if (gres_ptr->gres_bit_alloc && gres_ptr->gres_bit_alloc[node_index]) {
        new_gres_ptr->gres_bit_alloc = xmalloc(sizeof(bitstr_t * ));
        new_gres_ptr->gres_bit_alloc[0] = bit_copy(gres_ptr->gres_bit_alloc[node_index]);
    }
    return new_gres_ptr;
}
...
extern List gres_plugin_job_state_extract(List gres_list, int node_index) {
    ...
    while ((gres_ptr = (gres_state_t *) list_next(gres_iter))) {
        if (node_index == -1)
            ...
            new_gres_data = _job_state_dup2(gres_ptr->gres_data, node_index);
        }
 ...
 */
extern int
gres_plugin_job_state_pack(List gres_list, Buf buffer, uint32_t job_id, bool details, uint16_t protocol_version) {
    int i, rc = SLURM_SUCCESS;
    uint32_t top_offset, tail_offset;
    uint32_t magic = GRES_MAGIC;
    uint16_t rec_cnt = 0;
    ...
    while ((gres_ptr = (gres_state_t *) list_next(gres_iter))) {
        ...
        if (protocol_version >= SLURM_19_05_PROTOCOL_VERSION) {
            ...
            if (gres_job_ptr->gres_cnt_node_alloc) {
                ...
                pack64_array(gres_job_ptr->gres_cnt_node_alloc, gres_job_ptr->node_cnt, buffer);
            } else {
            ...
            if (gres_job_ptr->gres_bit_alloc) {
                ...
                for (i = 0; i < gres_job_ptr->node_cnt; i++) {
                    pack_bit_str_hex(gres_job_ptr->gres_bit_alloc[i], buffer);
                }
            } else {
                pack8((uint8_t) 0, buffer);
            }
            if (details && gres_job_ptr->gres_bit_step_alloc) {
                pack8((uint8_t) 1, buffer);
                for (i = 0; i < gres_job_ptr->node_cnt; i++) {
                    pack_bit_str_hex(gres_job_ptr->gres_bit_step_alloc[i], buffer);
                }
            } else {
                pack8((uint8_t) 0, buffer);
            }
            if (details && gres_job_ptr->gres_cnt_step_alloc) {
                pack8((uint8_t) 1, buffer);
                for (i = 0; i < gres_job_ptr->node_cnt; i++) {
                    pack64(gres_job_ptr->gres_cnt_step_alloc[i], buffer);
                }
            } else {
                pack8((uint8_t) 0, buffer);
            }
 ...
 * IN job_id - job's ID
 */
extern int gres_plugin_job_state_unpack(List *gres_list, Buf buffer, uint32_t job_id, uint16_t protocol_version) {
    int i = 0, rc;
    ...
    while ((rc == SLURM_SUCCESS) && (rec_cnt)) {
        ...
        if (protocol_version >= SLURM_19_05_PROTOCOL_VERSION) {
            ...
            safe_unpackstr_xmalloc(&gres_job_ptr->type_name, &utmp32, buffer);
            gres_job_ptr->type_id = gres_plugin_build_id(gres_job_ptr->type_name);
            ...
            if (has_more) {
                safe_unpack64_array(&gres_job_ptr->gres_cnt_node_alloc, &utmp32, buffer);
            }

            safe_unpack8(&has_more, buffer);
            if (has_more) {
                safe_xcalloc(gres_job_ptr->gres_bit_alloc, gres_job_ptr->node_cnt, sizeof(bitstr_t * ));
                for (i = 0; i < gres_job_ptr->node_cnt; i++) {
                    unpack_bit_str_hex(&gres_job_ptr->gres_bit_alloc[i], buffer);
                }
            }
            safe_unpack8(&has_more, buffer);
            if (has_more) {
                safe_xcalloc(gres_job_ptr->gres_bit_step_alloc, gres_job_ptr->node_cnt, sizeof(bitstr_t * ));
                for (i = 0; i < gres_job_ptr->node_cnt; i++) {
                    unpack_bit_str_hex(&gres_job_ptr->gres_bit_step_alloc[i], buffer);
                }
            }
            safe_unpack8(&has_more, buffer);
            if (has_more) {
                safe_xcalloc(gres_job_ptr->gres_cnt_step_alloc, gres_job_ptr->node_cnt, sizeof(uint64_t));
                for (i = 0; i < gres_job_ptr->node_cnt; i++) {
                    safe_unpack64(&gres_job_ptr->gres_cnt_step_alloc[i], buffer);
                }
            }
        } else if (protocol_version >= SLURM_MIN_PROTOCOL_VERSION) {
            safe_unpack32(&magic, buffer);
...
/*
			 ...
			 * Not a fatal error, skip over the data.
			 */
            error("%s: no plugin configured to unpack data type %u from job %u", __func__, plugin_id, job_id);
 ...
 * IN/OUT buffer - location to write state to
 */
extern int gres_plugin_job_alloc_pack(List gres_list, Buf buffer, uint16_t protocol_version) {
    int i, rc = SLURM_SUCCESS;
    uint32_t top_offset, tail_offset;
    uint32_t magic = GRES_MAGIC;
    uint16_t rec_cnt = 0;
 ...
 * IN/OUT buffer - location to read state from
 */
extern int gres_plugin_job_alloc_unpack(List *gres_list, Buf buffer, uint16_t protocol_version) {
    int i = 0, rc;
    ...
    while ((rc == SLURM_SUCCESS) && (rec_cnt)) {
        ...
        for (i = 0; i < gres_context_cnt; i++) {
            if (gres_context[i].plugin_id == gres_job_ptr->plugin_id)
                break;
        }
        if (i >= gres_context_cnt) {
/*
			 ...
			 * Not a fatal error, skip over the data.
			 */
            error("%s: no plugin configured to unpack data type %u", __func__, gres_job_ptr->plugin_id);
...
extern List gres_plugin_epilog_build_env(List job_gres_list, char *node_list) {
    ...
    while ((gres_ptr = list_next(gres_iter))) {
        ...
        if (i >= gres_context_cnt) {
            error("%s: gres not found in context.  This should never happen", __func__);
            continue;
        }

        if (!gres_context[i].ops.epilog_build_env)
            continue;    /* No plugin to call */
        epilog_info = (*(gres_context[i].ops.epilog_build_env))(gres_ptr->gres_data);
        if (!epilog_info)
 ...
 */
extern void gres_plugin_epilog_set_env(char ***epilog_env_ptr, List epilog_gres_list, int node_inx) {
    int i;
    ...
    *epilog_env_ptr = NULL;
    ...
    while ((epilog_info = list_next(epilog_iter))) {
        ...
        if (i >= gres_context_cnt) {
            error("%s: GRES ID %u not found in context", __func__, epilog_info->plugin_id);
            continue;
        }

        if (!gres_context[i].ops.epilog_set_env)
            continue;    /* No plugin to call */
        (*(gres_context[i].ops.epilog_set_env))(epilog_env_ptr, epilog_info, node_inx);
    }
...
static void _validate_gres_node_cores(gres_node_state_t *node_gres_ptr, int cores_ctld, char *node_name) {
    int i, cores_slurmd;
    bitstr_t *new_core_bitmap;
    ...
    for (i = 0; i < node_gres_ptr->topo_cnt; i++) {
        ...
        if (cores_slurmd == cores_ctld)
            continue;
        if (log_mismatch) {
            debug("Rebuilding node %s gres core bitmap (%d != %d)", node_name, cores_slurmd, cores_ctld);
            log_mismatch = false;
        }
        new_core_bitmap = _core_bitmap_rebuild(node_gres_ptr->topo_core_bitmap[i], cores_ctld);
...
static void _job_core_filter(void *job_gres_data, void *node_gres_data, bool use_total_gres, bitstr_t *core_bitmap,
    ...
    if (!use_total_gres && (plugin_id == mps_plugin_id) && (node_gres_ptr->gres_cnt_alloc != 0)) {
...
/* Determine which specific cores can be used */
    ...
    for (i = 0; i < node_gres_ptr->topo_cnt; i++) {
        if (node_gres_ptr->topo_gres_cnt_avail[i] == 0)
            continue;
        if (!use_total_gres && (node_gres_ptr->topo_gres_cnt_alloc[i] >= node_gres_ptr->topo_gres_cnt_avail[i]))
            continue;
        if (use_busy_dev && (node_gres_ptr->topo_gres_cnt_alloc[i] == 0))
            continue;
        if (job_gres_ptr->type_name &&
            (!node_gres_ptr->topo_type_name[i] || (job_gres_ptr->type_id != node_gres_ptr->topo_type_id[i])))
            continue;
        if (!node_gres_ptr->topo_core_bitmap[i]) {
...
/* We must use the ONE already active GRES of this type */
    ...
    if (job_gres_ptr->gres_per_node && node_gres_ptr->topo_cnt && *topo_set) {
/*
        ...
        if (core_bitmap) {
            core_ctld = core_end_bit - core_start_bit + 1;
            if (core_ctld < 1) {
                error("gres/%s: job %u cores on node %s < 1", gres_name, job_id, node_name);
                return (uint32_t) 0;
            }
            _validate_gres_node_cores(node_gres_ptr, core_ctld, node_name);
        }
        for (i = 0; i < node_gres_ptr->topo_cnt; i++) {
            if (job_gres_ptr->type_name &&
                (!node_gres_ptr->topo_type_name[i] || (node_gres_ptr->topo_type_id[i] != job_gres_ptr->type_id)))
                continue;
            if (use_busy_dev && (node_gres_ptr->topo_gres_cnt_alloc[i] == 0))
                continue;
            if (!node_gres_ptr->topo_core_bitmap[i]) {
                gres_avail += node_gres_ptr->topo_gres_cnt_avail[i];
                if (!use_total_gres) {
                    gres_avail -= node_gres_ptr->topo_gres_cnt_alloc[i];
                }
                if (shared_gres)
            ...
            core_ctld = bit_size(node_gres_ptr->topo_core_bitmap[i]);
            for (j = 0; j < core_ctld; j++) {
                if (core_bitmap && !bit_test(core_bitmap, core_start_bit + j))
                    continue;
                if (!bit_test(node_gres_ptr->topo_core_bitmap[i], j))
...
/* Need to determine which specific cores can be used */
        ...
        for (i = 0; i < node_gres_ptr->topo_cnt; i++) {
            ...
            if (job_gres_ptr->type_name &&
                ...
                continue;
            if (!node_gres_ptr->topo_core_bitmap[i]) {
                cores_avail[i] = core_end_bit - core_start_bit + 1;
                continue;
            }
...
/* Pick the topology entries with the most cores available */
        ...
        while (gres_avail < job_gres_ptr->gres_per_node) {
            ...
            for (j = 0; j < node_gres_ptr->topo_cnt; j++) {
                if ((gres_avail == 0) || (cores_avail[j] == 0) || !node_gres_ptr->topo_core_bitmap[j]) {
                    cores_addnt[j] = cores_avail[j];
                } else {
                    cores_addnt[j] =
                            cores_avail[j] - bit_overlap(alloc_core_bitmap, node_gres_ptr->topo_core_bitmap[j]);
                }
            ...
            if (!use_total_gres && (gres_tmp >= node_gres_ptr->topo_gres_cnt_alloc[top_inx])) {
                gres_tmp -= node_gres_ptr->topo_gres_cnt_alloc[top_inx];
            } else if (!use_total_gres) {
                gres_tmp = 0;
            }
            if (gres_tmp == 0) {
                error("gres/%s: topology allocation error on node %s", gres_name, node_name);
                break;
            }
...
/*
                ...
                bit_or(alloc_core_bitmap, node_gres_ptr->topo_core_bitmap[top_inx]);
                if (core_bitmap)
                    bit_and(alloc_core_bitmap, avail_core_bitmap);
            } else {
                bit_and(alloc_core_bitmap, node_gres_ptr->topo_core_bitmap[top_inx]);
            }
            if (shared_gres) {
...
/*
        ...
        if (shared_gres && (top_inx >= 0) && (gres_avail >= job_gres_ptr->gres_per_node)) {
            ...
            *topo_set = true;
            for (i = 0; i < core_ctld; i++) {
                if (!bit_test(alloc_core_bitmap, i)) {
                    bit_clear(core_bitmap, core_start_bit + i);
                }
            }
        }
        ...
        for (i = 0; i < node_gres_ptr->type_cnt; i++) {
            if (node_gres_ptr->type_name[i] && (node_gres_ptr->type_id[i] == job_gres_ptr->type_id))
                break;
        }
 ...
 */
extern void
gres_plugin_job_core_filter(List job_gres_list, List node_gres_list, bool use_total_gres, bitstr_t *core_bitmap,
                            int core_start_bit, int core_end_bit, char *node_name) {
    int i;
...
/* node lack resources required by the job */
        ...
        for (i = 0; i < gres_context_cnt; i++) {
            if (job_gres_ptr->plugin_id != gres_context[i].plugin_id)
                continue;
            _job_core_filter(job_gres_ptr->gres_data, node_gres_ptr->gres_data, use_total_gres, core_bitmap,
                             core_start_bit, core_end_bit, gres_context[i].gres_name, node_name,
 ...
 */
extern uint32_t
gres_plugin_job_test(List job_gres_list, List node_gres_list, bool use_total_gres, bitstr_t *core_bitmap,
                     int core_start_bit, int core_end_bit, uint32_t job_id, char *node_name) {
    int i;
...
/* node lack resources required by the job */
        ...
        for (i = 0; i < gres_context_cnt; i++) {
            if (job_gres_ptr->plugin_id != gres_context[i].plugin_id)
                continue;
            tmp_cnt = _job_test(job_gres_ptr->gres_data, node_gres_ptr->gres_data, use_total_gres, core_bitmap,
                                core_start_bit, core_end_bit, &topo_set, job_id, node_name, gres_context[i].gres_name,
                                gres_context[i].plugin_id);
            if (tmp_cnt != NO_VAL) {
                if (core_cnt == NO_VAL)
                    core_cnt = tmp_cnt;
                else core_cnt = MIN(tmp_cnt, core_cnt);
            }
            break;
        }
        if (core_cnt == 0)
        ...
        if (sock_inx < 0) {
            if (sock_gres->cnt_any_sock) {
                if (sock_gres->type_name) {
                    xstrfmtcat(gres_str, "%s%s:%s:%"
                    PRIu64, sep, sock_gres->gres_name, sock_gres->type_name, sock_gres->cnt_any_sock);
                } else {
                    xstrfmtcat(gres_str, "%s%s:%"
                    PRIu64, sep, sock_gres->gres_name, sock_gres->cnt_any_sock);
                }
                sep = " ";
            }
            continue;
        }
        if (!sock_gres->cnt_by_sock || (sock_gres->cnt_by_sock[sock_inx] == 0))
            continue;
        if (sock_gres->type_name) {
            xstrfmtcat(gres_str, "%s%s:%s:%"
            PRIu64, sep, sock_gres->gres_name, sock_gres->type_name, sock_gres->cnt_by_sock[sock_inx]);
        } else {
            xstrfmtcat(gres_str, "%s%s:%"
            PRIu64, sep, sock_gres->gres_name, sock_gres->cnt_by_sock[sock_inx]);
        }
        sep = " ";
    }
    list_iterator_destroy(iter);
 ...
 */
static sock_gres_t *
_build_sock_gres_by_topo(gres_job_state_t *job_gres_ptr, gres_node_state_t *node_gres_ptr, bool use_total_gres,
                         bitstr_t *core_bitmap, uint16_t sockets, uint16_t cores_per_sock, uint32_t job_id,
                         char *node_name, bool enforce_binding, uint32_t s_p_n, bitstr_t **req_sock_map,
                         uint32_t main_plugin_id, uint32_t alt_plugin_id, gres_node_state_t *alt_node_gres_ptr,
    ...
    if (!use_total_gres && (main_plugin_id == mps_plugin_id) && (node_gres_ptr->gres_cnt_alloc != 0)) {
/* We must use the ONE already active GRES of this type */
        use_busy_dev = true;
    }
    ...
    for (i = 0; i < node_gres_ptr->topo_cnt; i++) {
        ...
        if (job_gres_ptr->type_name && (job_gres_ptr->type_id != node_gres_ptr->topo_type_id[i]))
            continue;    /* Wrong type_model */
        if (use_busy_dev && (node_gres_ptr->topo_gres_cnt_alloc[i] == 0))
            continue;
        if (!use_total_gres && !node_gres_ptr->no_consume &&
            (node_gres_ptr->topo_gres_cnt_alloc[i] >= node_gres_ptr->topo_gres_cnt_avail[i])) {
            continue;    /* No GRES remaining */
        }

        if (!use_total_gres && !node_gres_ptr->no_consume) {
            avail_gres = node_gres_ptr->topo_gres_cnt_avail[i] - node_gres_ptr->topo_gres_cnt_alloc[i];
        } else {
...
/*
		 ...
		 */
        if (alt_node_gres_ptr && alt_node_gres_ptr->gres_bit_alloc && node_gres_ptr->topo_gres_bitmap[i]) {
            c = bit_overlap(node_gres_ptr->topo_gres_bitmap[i], alt_node_gres_ptr->gres_bit_alloc);
...
/* gres/mps can only use one GPU per node */
        if ((main_plugin_id == mps_plugin_id) && (avail_gres > sock_gres->max_node_gres))
...
/*
        ...
        if (node_gres_ptr->topo_core_bitmap && node_gres_ptr->topo_core_bitmap[i]) {
        ...
        if (!node_gres_ptr->topo_core_bitmap || !node_gres_ptr->topo_core_bitmap[i] || use_all_sockets) {
/*
            ...
            if (!sock_gres->bits_any_sock) {
                sock_gres->bits_any_sock = bit_copy(node_gres_ptr->topo_gres_bitmap[i]);
            } else {
                bit_or(sock_gres->bits_any_sock, node_gres_ptr->topo_gres_bitmap[i]);
            }
            match = true;
            continue;
        }
...
/* Constrained by core */
        ...
        if (node_gres_ptr->topo_core_bitmap[i]) {
            tot_cores = MIN(tot_cores, bit_size(node_gres_ptr->topo_core_bitmap[i]));
        }
...
/* No available cores on this socket */
            ...
            for (c = 0; c < cores_per_sock; c++) {
                ...
                if (node_gres_ptr->topo_core_bitmap[i] && !bit_test(node_gres_ptr->topo_core_bitmap[i], j))
                    continue;
                if (!node_gres_ptr->topo_gres_bitmap[i]) {
                    error("%s: topo_gres_bitmap NULL on node %s", __func__, node_name);
                    continue;
                }
                if (!sock_gres->bits_by_sock[s]) {
                    sock_gres->bits_by_sock[s] = bit_copy(node_gres_ptr->topo_gres_bitmap[i]);
                } else {
                    bit_or(sock_gres->bits_by_sock[s], node_gres_ptr->topo_gres_bitmap[i]);
                }
...
/*
		 ...
		 */
        for (s = 0; s < sockets; s++) {
            if (sock_gres->cnt_by_sock[s] < job_gres_ptr->gres_per_socket) {
/* Insufficient GRES, clear count */
            ...
            } else if (sock_gres->cnt_by_sock[s] > job_gres_ptr->gres_per_socket) {
/* Excess GRES, reduce count */
                i = sock_gres->cnt_by_sock[s] - job_gres_ptr->gres_per_socket;
                sock_gres->cnt_by_sock[s] = job_gres_ptr->gres_per_socket;
        ...
        while (avail_sock > s_p_n) {
            ...
            for (s = 0; s < sockets; s++) {
                ...
                if ((low_gres_sock_inx == -1) ||
                    (sock_gres->cnt_by_sock[s] < sock_gres->cnt_by_sock[low_gres_sock_inx]))
                    low_gres_sock_inx = s;
            }
            if (low_gres_sock_inx == -1)
                break;
...
/*
    ...
    if (match && core_bitmap && (s_p_n == NO_VAL) && (add_gres > 0) && job_gres_ptr->gres_per_node) {
        ...
        for (s = 0; s < sockets; s++) {
            ...
            for (c = 0; c < cores_per_sock; c++) {
                ...
                avail_sock++;
                avail_sock_flag[s] = true;
                if ((best_sock_inx == -1) || (sock_gres->cnt_by_sock[s] > sock_gres->cnt_by_sock[best_sock_inx])) {
                    best_sock_inx = s;
                }
                break;
            }
...
/* Find next best socket */
            best_sock_inx = -1;
            for (s = 0; s < sockets; s++) {
                if ((sock_gres->cnt_by_sock[s] == 0) || !avail_sock_flag[s])
                    continue;
                if ((best_sock_inx == -1) || (sock_gres->cnt_by_sock[s] > sock_gres->cnt_by_sock[best_sock_inx])) {
                    best_sock_inx = s;
                }
 ...
 */
static sock_gres_t *
_build_sock_gres_by_type(gres_job_state_t *job_gres_ptr, gres_node_state_t *node_gres_ptr, bool use_total_gres,
                         bitstr_t *core_bitmap, uint16_t sockets, uint16_t cores_per_sock, uint32_t job_id,
                         char *node_name) {
    int i;
    sock_gres_t *sock_gres;
    ...
    sock_gres = xmalloc(sizeof(sock_gres_t));
    for (i = 0; i < node_gres_ptr->type_cnt; i++) {
        if (job_gres_ptr->type_name && (job_gres_ptr->type_id != node_gres_ptr->type_id[i]))
            continue;    /* Wrong type_model */
        if (!use_total_gres && (node_gres_ptr->type_cnt_alloc[i] >= node_gres_ptr->type_cnt_avail[i])) {
            continue;    /* No GRES remaining */
        } else if (!use_total_gres) {
            avail_gres = node_gres_ptr->type_cnt_avail[i] - node_gres_ptr->type_cnt_alloc[i];
        } else {
 ...
 */
static sock_gres_t *
_build_sock_gres_basic(gres_job_state_t *job_gres_ptr, gres_node_state_t *node_gres_ptr, bool use_total_gres,
                       bitstr_t *core_bitmap, uint16_t sockets, uint16_t cores_per_sock, uint32_t job_id,
                       char *node_name) {
    sock_gres_t *sock_gres;
    uint64_t avail_gres, min_gres = 1;
    ...
    if (job_gres_ptr->type_name)
        return NULL;
    if (!use_total_gres && (node_gres_ptr->gres_cnt_alloc >= node_gres_ptr->gres_cnt_avail))
    ...
    if (job_gres_ptr->gres_per_task)
        min_gres = MAX(min_gres, job_gres_ptr->gres_per_task);
    if (!use_total_gres) {
        avail_gres = node_gres_ptr->gres_cnt_avail - node_gres_ptr->gres_cnt_alloc;
    } else avail_gres = node_gres_ptr->gres_cnt_avail;
    if (avail_gres < min_gres)
...
static void _sock_gres_log(List sock_gres_list, char *node_name) {
    ...
    iter = list_iterator_create(sock_gres_list);
    while ((sock_gres = (sock_gres_t *) list_next(iter))) {
        info("Gres:%s Type:%s TotalCnt:%"
        PRIu64
        " MaxNodeGres:%"
        PRIu64, sock_gres->gres_name, sock_gres->type_name, sock_gres->total_cnt, sock_gres->max_node_gres);
        if (sock_gres->bits_any_sock) {
        ...
        info("  Sock[ANY]Cnt:%"
        PRIu64
        " Bits:%s of %d", sock_gres->cnt_any_sock, tmp, len);
        ...
        for (i = 0; i < sock_gres->sock_cnt; i++) {
            ...
            info("  Sock[%d]Cnt:%"
            PRIu64
            " Bits:%s of %d", i, sock_gres->cnt_by_sock[i], tmp, len);
        }
    }
    list_iterator_destroy(iter);
}
 ...
 *	release memory.
 */
extern List gres_plugin_job_test2(List job_gres_list, List node_gres_list, bool use_total_gres, bitstr_t *core_bitmap,
                                  uint16_t sockets, uint16_t cores_per_sock, uint32_t job_id, char *node_name,
                                  bool enforce_binding, uint32_t s_p_n, bitstr_t **req_sock_map, uint32_t user_id,
...
/* node lack GRES of type required by the job */
        ...
        if (job_data_ptr->gres_per_job && !job_data_ptr->gres_per_socket)
            local_s_p_n = s_p_n;    /* Maximize GRES per node */
        else local_s_p_n = NO_VAL;    /* No need to optimize socket */
        if (core_bitmap && (bit_set_count(core_bitmap) == 0)) {
            ...
            if (alt_plugin_id) {
                node_gres_iter = list_iterator_create(node_gres_list);
                while ((node_gres_ptr = (gres_state_t *) list_next(node_gres_iter))) {
                    if (node_gres_ptr->plugin_id == alt_plugin_id)
                        break;
                }
                list_iterator_destroy(node_gres_iter);
            }
            if (alt_plugin_id && node_gres_ptr) {
                alt_node_data_ptr = (gres_node_state_t *) node_gres_ptr->gres_data;
            } else {
/* GRES of interest not on this node */
                alt_plugin_id = 0;
            }
            sock_gres = _build_sock_gres_by_topo(job_data_ptr, node_data_ptr, use_total_gres, core_bitmap, sockets,
                                                 cores_per_sock, job_id, node_name, enforce_binding, local_s_p_n,
                                                 req_sock_map, job_gres_ptr->plugin_id, alt_plugin_id,
                                                 alt_node_data_ptr, user_id, node_inx);
        } else if (node_data_ptr->type_cnt) {
            sock_gres = _build_sock_gres_by_type(job_data_ptr, node_data_ptr, use_total_gres, core_bitmap, sockets,
                                                 cores_per_sock, job_id, node_name);
        } else {
            sock_gres = _build_sock_gres_basic(job_data_ptr, node_data_ptr, use_total_gres, core_bitmap, sockets,
                                               cores_per_sock, job_id, node_name);
        }
        if (!sock_gres) {
...
static bool *_build_avail_cores_by_sock(bitstr_t *core_bitmap, uint16_t sockets, uint16_t cores_per_sock) {
 ...
 */
extern int
gres_plugin_job_core_filter2(List sock_gres_list, uint64_t avail_mem, uint16_t max_cpus, bool enforce_binding,
                             bitstr_t *core_bitmap, uint16_t sockets, uint16_t cores_per_sock, uint16_t cpus_per_core,
                             uint32_t sock_per_node, uint16_t task_per_node, uint16_t *avail_gpus,
                             uint16_t *near_gpus) {
    ...
    *avail_gpus = 0;
    *near_gpus = 0;
    if (!core_bitmap || !sock_gres_list || (list_count(sock_gres_list) == 0))
        return rc;
    ...
    while ((sock_gres = (sock_gres_t *) list_next(sock_gres_iter))) {
        ...
        else cpus_per_gres = sock_gres->job_specs->def_cpus_per_gres;
        if (cpus_per_gres) {
            max_gres = max_cpus / cpus_per_gres;
            if ((max_gres == 0) || (sock_gres->job_specs->gres_per_node > max_gres) ||
...
/* Insufficient CPUs for any GRES */
        ...
        else mem_per_gres = sock_gres->job_specs->def_mem_per_gres;
        ...
        if (sock_gres->cnt_by_sock || enforce_binding) {
            if (!avail_cores_by_sock) {
                avail_cores_by_sock = _build_avail_cores_by_sock(core_bitmap, sockets, cores_per_sock);
            }
        }
/*
        ...
        if (sock_gres->job_specs && sock_gres->job_specs->gres_per_node) {
            if ((sock_gres->max_node_gres == 0) || (sock_gres->max_node_gres > sock_gres->job_specs->gres_per_node)) {
                sock_gres->max_node_gres = sock_gres->job_specs->gres_per_node;
            }
        }
        if (cpus_per_gres) {
            int cpu_cnt;
        ...
        if (_sharing_gres(sock_gres->plugin_id)) {
            *avail_gpus += sock_gres->total_cnt;
            if (sock_gres->max_node_gres && (sock_gres->max_node_gres < near_gres_cnt))
 ...
 */
extern void
gres_plugin_job_core_filter3(gres_mc_data_t *mc_ptr, List sock_gres_list, uint16_t sockets, uint16_t cores_per_socket,
                             uint16_t cpus_per_core, uint16_t *avail_cpus, uint32_t *min_tasks_this_node,
                             uint32_t *max_tasks_this_node, int rem_nodes, bool enforce_binding, bool first_pass,
                             bitstr_t *avail_core) {
    ...
    while ((sock_gres = (sock_gres_t *) list_next(sock_gres_iter))) {
        ...
        if (!job_specs)
            continue;
        if (job_specs->gres_per_job && (job_specs->total_gres < job_specs->gres_per_job)) {
            rem_gres = job_specs->gres_per_job - job_specs->total_gres;
        }
...
/*
        ...
        if (sock_gres->max_node_gres) {
            if (rem_gres && (rem_gres < sock_gres->max_node_gres))
                max_gres = rem_gres;
            else max_gres = sock_gres->max_node_gres;
        }
        rem_nodes = MAX(rem_nodes, 1);
        rem_sockets = MAX(1, mc_ptr->sockets_per_node);
        if (max_gres &&
            ((job_specs->gres_per_node > max_gres) || ((job_specs->gres_per_socket * rem_sockets) > max_gres))) {
            *max_tasks_this_node = 0;
            break;
        }
        if (job_specs->gres_per_node && job_specs->gres_per_task) {
            max_tasks = job_specs->gres_per_node / job_specs->gres_per_task;
            if ((max_tasks == 0) || (max_tasks > *max_tasks_this_node) || (max_tasks < *min_tasks_this_node)) {
                *max_tasks_this_node = 0;
                break;
            }
            if ((*max_tasks_this_node == NO_VAL) || (*max_tasks_this_node > max_tasks))
                *max_tasks_this_node = max_gres;
            if (*min_tasks_this_node < max_gres)
                *min_tasks_this_node = max_gres;
        }

        min_core_cnt = MAX(*min_tasks_this_node, 1) * MAX(mc_ptr->cpus_per_task, 1);
        min_core_cnt = (min_core_cnt + cpus_per_core - 1) / cpus_per_core;
...
/* Test for sufficient gres_per_socket */
            ...
            } else cnt_avail_sock = 0;
            tot_gres_sock = sock_gres->cnt_any_sock + cnt_avail_sock;
            if ((job_specs->gres_per_socket > tot_gres_sock) || (tot_gres_sock == 0)) {
/* Insufficient GRES on this socket */
                if (sock_gres->cnt_by_sock) {
                    sock_gres->total_cnt -= sock_gres->cnt_by_sock[s];
                    sock_gres->cnt_by_sock[s] = 0;
                }
                if (first_pass && (tot_core_cnt > min_core_cnt)) {
                    for (c = 0; c < cores_per_socket; c++) {
                        i = (s * cores_per_socket) + c;
                        if (!bit_test(avail_core, i))
                            continue;
...
/* Test for available cores on this socket */
            if ((enforce_binding || first_pass) && (avail_cores_per_sock[s] == 0))
                continue;
            ...
            req_sock[s] = true;
            sock_cnt++;
            if (job_specs->gres_per_node && (cnt_avail_total >= job_specs->gres_per_node))
...
/*
		 ...
		 */
        *min_tasks_this_node = (*min_tasks_this_node / task_cnt_incr) * task_cnt_incr;
        *max_tasks_this_node = (*max_tasks_this_node / task_cnt_incr) * task_cnt_incr;
...
/*
        ...
        if (mc_ptr->cpus_per_task) {
            req_cores *= ((mc_ptr->cpus_per_task + cpus_per_core - 1) / cpus_per_core);
        }
        if (cpus_per_gres) {
            if (job_specs->gres_per_node) {
                ...
                i = job_specs->gres_per_task * *max_tasks_this_node;
...
/*
        ...
        while (avail_cores_tot > req_cores) {
            ...
            for (s = 0; s < sockets; s++) {
                if (avail_cores_tot == req_cores)
                    break;
                if (!req_sock[s] || (avail_cores_per_sock[s] == 0))
                    continue;
                if ((full_socket == -1) || (avail_cores_per_sock[full_socket] < avail_cores_per_sock[s])) {
                    full_socket = s;
                }
            }
            if (full_socket == -1)
    ...
    if ((mc_ptr->cpus_per_task > 1) || ((select_type_param & CR_ONE_TASK_PER_CORE) == 0)) {
/*
		 ...
		 */
        *avail_cpus = MIN(*avail_cpus, *max_tasks_this_node * mc_ptr->cpus_per_task);
    }
}
...
extern uint32_t gres_plugin_get_task_limit(List sock_gres_list) {
    ...
    while ((sock_gres = (sock_gres_t *) list_next(sock_gres_iter))) {
        ...
        if (sock_gres->job_specs->gres_per_task == 0)
            continue;
        task_limit = sock_gres->total_cnt / sock_gres->job_specs->gres_per_task;
 ...
 * RET socket count
 */
static int _get_sock_cnt(struct job_resources *job_res, int node_inx, int job_node_inx) {
    int core_offset, used_sock_cnt = 0;
    uint16_t sock_cnt = 0, cores_per_socket_cnt = 0;
    int c, i, rc, s;

    rc = get_job_resources_cnt(job_res, job_node_inx, &sock_cnt, &cores_per_socket_cnt);
    if (rc != SLURM_SUCCESS) {
 ...
 */
static int
_set_job_bits1(struct job_resources *job_res, int node_inx, int job_node_inx, int rem_nodes, sock_gres_t *sock_gres,
               uint32_t job_id, gres_mc_data_t *tres_mc_ptr, uint16_t cpus_per_core) {
    ...
    rc = get_job_resources_cnt(job_res, job_node_inx, &sock_cnt, &cores_per_socket_cnt);
    if (rc != SLURM_SUCCESS) {
        error("%s: Invalid socket/core count for job %u on node %d", __func__, job_id, node_inx);
        return rc;
    }
    core_offset = get_job_resources_offset(job_res, job_node_inx, 0, 0);
    if (core_offset < 0) {
        error("%s: Invalid core offset for job %u on node %d", __func__, job_id, node_inx);
        return rc;
    }
    i = sock_gres->sock_cnt;
    if ((i != 0) && (i != sock_cnt)) {
        error("%s: Inconsistent socket count (%d != %d) for job %u on node %d", __func__, i, sock_cnt, job_id,
              node_inx);
    ...
    max_gres = job_specs->gres_per_job - job_specs->total_gres - (rem_nodes - 1);
    ...
    if (job_specs->cpus_per_gres) {
        max_gres = MIN(max_gres, ((total_cores * cpus_per_core) / job_specs->cpus_per_gres));
    }
    if ((max_gres > 1) && (node_specs->link_len == gres_cnt))
        pick_gres = NO_VAL16;
    else pick_gres = max_gres;
/*
    ...
    for (s = -1;    /* Socket == - 1 if GRES avail from any socket */
        ...
        if ((s >= 0) && !cores_on_sock[s])
            continue;
        for (g = 0; ((g < gres_cnt) && (alloc_gres_cnt < pick_gres)); g++) {
            if ((s == -1) && sock_gres->bits_any_sock && !bit_test(sock_gres->bits_any_sock, g))
                continue;   /* GRES not avail any socket */
            if ((s >= 0) && sock_gres->bits_by_sock[s] && !bit_test(sock_gres->bits_by_sock[s], g))
                continue;   /* GRES not on this socket */
            if (bit_test(node_specs->gres_bit_alloc, g) || bit_test(job_specs->gres_bit_select[node_inx], g))
    ...
    if (alloc_gres_cnt == 0) {
        for (s = 0; ((s < sock_cnt) && (alloc_gres_cnt == 0)); s++) {
            ...
            for (g = 0; g < gres_cnt; g++) {
                if (sock_gres->bits_by_sock && sock_gres->bits_by_sock[s] && !bit_test(sock_gres->bits_by_sock[s], g))
                    continue;   /* GRES not on this socket */
                if (bit_test(node_specs->gres_bit_alloc, g) || bit_test(job_specs->gres_bit_select[node_inx], g))
                    continue;   /* Already allocated GRES */
                bit_set(job_specs->gres_bit_select[node_inx], g);
    ...
    if (alloc_gres_cnt == 0) {
        error("%s: job %u failed to find any available GRES on node %d", __func__, job_id, node_inx);
    }
    ...
    if (alloc_gres_cnt > max_gres) {
        ...
        for (s = 0; s < gres_cnt; s++) {
            ...
            for (g = s + 1; g < gres_cnt; g++) {
                if (!bit_test(job_specs->gres_bit_select[node_inx], g))
                    continue;
                if (node_specs->links_cnt[s][g] <= best_link_cnt)
                    continue;
        ...
        while ((alloc_gres_cnt > max_gres) && (best_link_cnt != -1)) {
            ...
            for (g = 0; g < gres_cnt; g++) {
                ...
                if (!bit_test(job_specs->gres_bit_select[node_inx], g))
                    continue;
                if (node_specs->links_cnt[best_inx][g] >= worst_link_cnt)
                    continue;
                worst_link_cnt = node_specs->links_cnt[best_inx][g];
                worst_inx = g;
            }
            if (worst_inx == -1) {
            ...
            bit_clear(job_specs->gres_bit_select[node_inx], worst_inx);
            job_specs->gres_cnt_node_select[node_inx]--;
            alloc_gres_cnt--;
            job_specs->total_gres--;
        }
 ...
 */
static int
_set_job_bits2(struct job_resources *job_res, int node_inx, int job_node_inx, sock_gres_t *sock_gres, uint32_t job_id,
    ...
    if (!job_specs->gres_bit_select || !job_specs->gres_bit_select[node_inx]) {
        error("%s: gres_bit_select NULL for job %u on node %d", __func__, job_id, node_inx);
        return SLURM_ERROR;
    }
    rc = get_job_resources_cnt(job_res, job_node_inx, &sock_cnt, &cores_per_socket_cnt);
    if (rc != SLURM_SUCCESS) {
...
/*
    ...
    xassert(job_res->core_bitmap);
    gres_cnt = bit_size(job_specs->gres_bit_select[node_inx]);
    if ((job_specs->gres_per_job > job_specs->total_gres) && (node_specs->link_len == gres_cnt)) {
        for (g = 0; g < gres_cnt; g++) {
            if (!bit_test(job_specs->gres_bit_select[node_inx], g))
                continue;
            best_inx = g;
            for (s = 0; s < gres_cnt; s++) {
                best_link_cnt = MAX(node_specs->links_cnt[s][g], best_link_cnt);
            }
            break;
        }
    }
...
/*
	 ...
	 */
    for (l = best_link_cnt; ((l >= 0) && (job_specs->gres_per_job > job_specs->total_gres)); l--) {
        for (s = -1;   /* Socket == - 1 if GRES avail from any socket */
             ((s < sock_cnt) && (job_specs->gres_per_job > job_specs->total_gres)); s++) {
            for (g = 0; ((g < gres_cnt) && (job_specs->gres_per_job > job_specs->total_gres)); g++) {
                if ((l > 0) && (node_specs->links_cnt[best_inx][g] < l))
                    continue;   /* Want better link count */
                if ((s == -1) && sock_gres->bits_any_sock && !bit_test(sock_gres->bits_any_sock, g))
                    continue;  /* GRES not avail any sock */
                if ((s >= 0) && sock_gres->bits_by_sock && sock_gres->bits_by_sock[s] &&
 ...
 */
static void
_set_node_bits(struct job_resources *job_res, int node_inx, int job_node_inx, sock_gres_t *sock_gres, uint32_t job_id,
               gres_mc_data_t *tres_mc_ptr) {
    int core_offset, gres_cnt;
    uint16_t sock_cnt = 0, cores_per_socket_cnt = 0;
    int c, i, g, l, rc, s;
...
/*
    ...
    for (s = -1;    /* Socket == - 1 if GRES avail from any socket */
         ((s < sock_cnt) && (alloc_gres_cnt < job_specs->gres_per_node)); s++) {
        if ((s >= 0) && !used_sock[s])
            continue;
        for (g = 0; g < gres_cnt; g++) {
            if ((s == -1) && sock_gres->bits_any_sock && !bit_test(sock_gres->bits_any_sock, g))
...
/*
	 ...
	 */
    for (l = best_link_cnt; ((l >= 0) && (alloc_gres_cnt < job_specs->gres_per_node)); l--) {
        for (s = -1;   /* Socket == - 1 if GRES avail from any socket */
             ((s < sock_cnt) && (alloc_gres_cnt < job_specs->gres_per_node)); s++) {
            if ((s >= 0) && !used_sock[s])
                continue;
...
/*
	 ...
	 */
    for (l = best_link_cnt; ((l >= 0) && (alloc_gres_cnt < job_specs->gres_per_node)); l--) {
        for (s = 0; ((s < sock_cnt) && (alloc_gres_cnt < job_specs->gres_per_node)); s++) {
            if (used_sock[s])
                continue;
            for (g = 0; g < gres_cnt; g++) {
                if (links_cnt && (links_cnt[g] < l))
 ...
 * tres_mc_ptr IN - job's multi-core options
 */
static void _pick_specific_topo(struct job_resources *job_res, int node_inx, int job_node_inx, sock_gres_t *sock_gres,
    ...
    if ((sock_gres->plugin_id == mps_plugin_id) && (node_specs->gres_cnt_alloc != 0)) {
/* We must use the ONE already active GRES of this type */
        use_busy_dev = true;
    }
...
/*
    ...
    for (s = -1;    /* Socket == - 1 if GRES avail from any socket */
        ...
        for (t = 0; t < node_specs->topo_cnt; t++) {
            if (use_busy_dev && (node_specs->topo_gres_cnt_alloc[t] == 0))
                continue;
            if (node_specs->topo_gres_cnt_alloc && node_specs->topo_gres_cnt_avail &&
                ((node_specs->topo_gres_cnt_avail[t] - node_specs->topo_gres_cnt_alloc[t]) < gres_per_bit))
                continue;    /* Insufficient resources */
            if ((s == -1) && sock_gres->bits_any_sock && !bit_test(sock_gres->bits_any_sock, t))
                continue;  /* GRES not avail any socket */
            if ((s >= 0) && sock_gres->bits_by_sock && sock_gres->bits_by_sock[s] &&
...
/* Select available GRES with sufficient resources */
    for (t = 0; (t < node_specs->topo_cnt) && (alloc_gres_cnt == 0); t++) {
        if (use_busy_dev && (node_specs->topo_gres_cnt_alloc[t] == 0))
            continue;
        if (node_specs->topo_gres_cnt_alloc && node_specs->topo_gres_cnt_avail && node_specs->topo_gres_cnt_avail[t] &&
            ((node_specs->topo_gres_cnt_avail[t] - node_specs->topo_gres_cnt_alloc[t]) < gres_per_bit))
            continue;    /* Insufficient resources */
        bit_set(job_specs->gres_bit_select[node_inx], t);
        job_specs->gres_cnt_node_select[node_inx] += gres_per_bit;
 ...
 */
static void
_set_sock_bits(struct job_resources *job_res, int node_inx, int job_node_inx, sock_gres_t *sock_gres, uint32_t job_id,
               gres_mc_data_t *tres_mc_ptr) {
    int core_offset, gres_cnt;
    uint16_t sock_cnt = 0, cores_per_socket_cnt = 0;
    int c, i, g, l, rc, s;
    ...
    if (tres_mc_ptr && tres_mc_ptr->sockets_per_node && (tres_mc_ptr->sockets_per_node != used_sock_cnt) &&
...
/* Somehow we have too few sockets in job allocation */
            error("%s: Inconsistent requested/allocated socket count "
                  "(%d > %d) for job %u on node %d", __func__, tres_mc_ptr->sockets_per_node, used_sock_cnt, job_id,
                  node_inx);
            for (s = 0; s < sock_cnt; s++) {
                if (used_sock[s] || !sock_gres->bits_by_sock[s])
                    continue;
/* Determine currently free GRES by socket */
                used_sock[s] = bit_set_count(sock_gres->bits_by_sock[s]) -
                               bit_overlap(sock_gres->bits_by_sock[s], node_specs->gres_bit_alloc);
                if ((used_sock[s] == 0) || (used_sock[s] < job_specs->gres_per_socket)) {
                    used_sock[s] = 0;
                } else if (++used_sock_cnt == tres_mc_ptr->sockets_per_node) {
                    break;
                }
            }
        } else {
...
/* Exclude sockets with low GRES counts */
            while (tres_mc_ptr->sockets_per_node > used_sock_cnt) {
                ...
                for (s = sock_cnt - 1; s >= 0; s--) {
                    if (used_sock[s] == 0)
                        continue;
                    if ((low_sock_inx == -1) || (used_sock[s] < used_sock[low_sock_inx]))
                        low_sock_inx = s;
                }
                if (low_sock_inx == -1)
                    break;
...
/*
    ...
    for (s = 0; s < sock_cnt; s++) {
        if (!used_sock[s])
            continue;
        i = 0;
        for (l = best_link_cnt; ((l >= 0) && (i < job_specs->gres_per_socket)); l--) {
            for (g = 0; g < gres_cnt; g++) {
                if (sock_gres->bits_by_sock && sock_gres->bits_by_sock[s] && !bit_test(sock_gres->bits_by_sock[s], g))
                    continue;  /* GRES not on this socket */
                if (node_specs->gres_bit_alloc && bit_test(node_specs->gres_bit_alloc, g))
                    continue;   /* Already allocated GRES */
                if (job_specs->gres_bit_select[node_inx] && bit_test(job_specs->gres_bit_select[node_inx], g))
                    continue;   /* Already allocated GRES */
                bit_set(job_specs->gres_bit_select[node_inx], g);
                job_specs->gres_cnt_node_select[node_inx]++;
        ...
        if ((i < job_specs->gres_per_socket) && sock_gres->bits_any_sock) {
/* Add GRES unconstrained by socket as needed */
            for (g = 0; g < gres_cnt; g++) {
                if (sock_gres->bits_any_sock && !bit_test(sock_gres->bits_any_sock, g))
                    continue;  /* GRES not on this socket */
                if (node_specs->gres_bit_alloc && bit_test(node_specs->gres_bit_alloc, g))
                    continue;   /* Already allocated GRES */
                if (job_specs->gres_bit_select[node_inx] && bit_test(job_specs->gres_bit_select[node_inx], g))
 ...
 */
static void
_set_task_bits(struct job_resources *job_res, int node_inx, int job_node_inx, sock_gres_t *sock_gres, uint32_t job_id,
               gres_mc_data_t *tres_mc_ptr, uint32_t **tasks_per_node_socket) {
    uint16_t sock_cnt = 0;
    int gres_cnt, g, l, s;
    gres_job_state_t *job_specs;
    gres_node_state_t *node_specs;
...
/* First pick GRES for acitve sockets */
    for (s = -1;    /* Socket == - 1 if GRES avail from any socket */
         s < sock_cnt; s++) {
        if ((s > 0) && (!tasks_per_node_socket[node_inx] || (tasks_per_node_socket[node_inx][s] == 0)))
            continue;
...
/*
	 ...
	 */
    total_gres_goal = total_tasks * job_specs->gres_per_task;
    for (l = best_link_cnt; ((l >= 0) && (total_gres_cnt < total_gres_goal)); l--) {
        for (s = -1;   /* Socket == - 1 if GRES avail from any socket */
             ((s < sock_cnt) && (total_gres_cnt < total_gres_goal)); s++) {
            for (g = 0; ((g < gres_cnt) && (total_gres_cnt < total_gres_goal)); g++) {
                if (links_cnt && (links_cnt[g] < l))
                    continue;
                if ((s == -1) && sock_gres->bits_any_sock && !bit_test(sock_gres->bits_any_sock, g))
                    continue;  /* GRES not avail any sock */
...
/* Something bad happened on task layout for this GRES type */
        ...
        " < %"
        PRIu64
        ")", __func__, sock_gres->gres_name, job_id, node_inx, total_gres_cnt, total_gres_goal);
    }
}
...
_build_tasks_per_node_sock(struct job_resources *job_res, uint8_t overcommit, gres_mc_data_t *tres_mc_ptr,
    ...
    for (i = i_first; i <= i_last; i++) {
        ...
        if (!bit_test(job_res->node_bitmap, i))
            continue;
        if (get_job_resources_cnt(job_res, job_node_inx, &sock_cnt, &cores_per_socket_cnt)) {
...
/*
            ...
            cpus_per_node = get_job_resources_cpus(job_res, job_node_inx);
            if (cpus_per_node < 1) {
                error("%s: failed to get cpus_per_node count", __func__);
/* Set default of 1 task on socket 0 */
                tasks_per_node_socket[i][0] = 1;
                rem_tasks--;
                continue;
        ...
        for (s = 0; s < sock_cnt; s++) {
            ...
            for (c = 0; c < cores_per_socket_cnt; c++) {
                ...
                if (tres_mc_ptr->ntasks_per_core) {
                    ...
                    if (tpc < 1) {
                        tpc = 1;
                        skip_cores = cpus_per_task / cpus_per_core;
...
/* Start with 1 task per core */
                ...
                if (task_per_node_limit) {
                    if (tasks_per_node > task_per_node_limit) {
                        ...
                        tasks_per_node_socket[i][s] -= excess_tasks;
...
/* NOTE: No support for ntasks_per_board */
                if (tres_mc_ptr->ntasks_per_socket) {
                    if (tasks_per_socket > tres_mc_ptr->ntasks_per_socket) {
                        excess_tasks = tasks_per_socket - tres_mc_ptr->ntasks_per_socket;
                        tasks_per_node_socket[i][s] -= excess_tasks;
                        rem_tasks += excess_tasks;
                    }
                    if (tasks_per_socket >= tres_mc_ptr->ntasks_per_socket) {
...
static uint64_t _get_job_cnt(sock_gres_t *sock_gres, gres_node_state_t *node_specs, int rem_node_cnt) {
...
/* Ensure at least one GRES per node on remaining nodes */
    max_gres = job_specs->gres_per_job - job_specs->total_gres - (rem_node_cnt - 1);
 ...
 */
extern int
gres_plugin_job_core_filter4(List *sock_gres_list, uint32_t job_id, struct job_resources *job_res, uint8_t overcommit,
                             gres_mc_data_t *tres_mc_ptr, struct node_record *node_table_ptr) {
    ListIterator sock_gres_iter;
    sock_gres_t *sock_gres;
    gres_job_state_t *job_specs;
    gres_node_state_t *node_specs;
    ...
    for (i = i_first; i <= i_last; i++) {
        if (!bit_test(job_res->node_bitmap, i))
            continue;
        sock_gres_iter = list_iterator_create(sock_gres_list[++node_inx]);
        while ((sock_gres = (sock_gres_t *) list_next(sock_gres_iter))) {
            job_specs = sock_gres->job_specs;
            node_specs = sock_gres->node_specs;
            ...
            if (job_specs->gres_per_task &&    /* Data needed */
                ...
                tasks_per_node_socket = _build_tasks_per_node_sock(job_res, overcommit, tres_mc_ptr, node_table_ptr);
            }
            ...
            if (!job_specs->gres_cnt_node_select) {
                job_specs->gres_cnt_node_select = xcalloc(node_cnt, sizeof(uint64_t));
            }
...
/* No topology, just set a count */
                if (job_specs->gres_per_node) {
                    job_specs->gres_cnt_node_select[i] = job_specs->gres_per_node;
                } else if (job_specs->gres_per_socket) {
                    job_specs->gres_cnt_node_select[i] = job_specs->gres_per_socket;
                    job_specs->gres_cnt_node_select[i] *= _get_sock_cnt(job_res, i, node_inx);
                } else if (job_specs->gres_per_task) {
                    job_specs->gres_cnt_node_select[i] = job_specs->gres_per_task;
                    job_specs->gres_cnt_node_select[i] *= _get_task_cnt_node(tasks_per_node_socket, i,
                                                                             node_table_ptr[i].sockets);
                } else if (job_specs->gres_per_job) {
                    job_specs->gres_cnt_node_select[i] = _get_job_cnt(sock_gres, node_specs, rem_node_cnt--);
                }
                job_specs->total_gres += job_specs->gres_cnt_node_select[i];
                continue;
            }

/* Working with topology, need to pick specific GRES */
            if (!job_specs->gres_bit_select) {
                job_specs->gres_bit_select = xcalloc(node_cnt, sizeof(bitstr_t * ));
            }
            ...
            if (job_specs->gres_per_node && _shared_gres(sock_gres->plugin_id)) {
/* gres/mps: select specific topo bit for job */
                _pick_specific_topo(job_res, i, node_inx, sock_gres, job_id, tres_mc_ptr);
            } else if (job_specs->gres_per_node) {
                _set_node_bits(job_res, i, node_inx, sock_gres, job_id, tres_mc_ptr);
            } else if (job_specs->gres_per_socket) {
                _set_sock_bits(job_res, i, node_inx, sock_gres, job_id, tres_mc_ptr);
            } else if (job_specs->gres_per_task) {
                _set_task_bits(job_res, i, node_inx, sock_gres, job_id, tres_mc_ptr, tasks_per_node_socket);
            } else if (job_specs->gres_per_job) {
                uint16_t cpus_per_core;
                cpus_per_core =
                        node_table_ptr->cpus / node_table_ptr->boards / node_table_ptr->sockets / node_table_ptr->cores;
                job_fini = _set_job_bits1(job_res, i, node_inx, rem_node_cnt--, sock_gres, job_id, tres_mc_ptr,
                                          cpus_per_core);
            } else {
                error("%s job %u job_spec lacks GRES counter", __func__, job_id);
            }
            if (job_fini == -1) {
/*
...
/*
        ...
        for (i = i_first; i <= i_last; i++) {
            ...
            while ((sock_gres = (sock_gres_t *) list_next(sock_gres_iter))) {
                ...
                if (!job_specs || !node_specs)
                    continue;
                job_fini = _set_job_bits2(job_res, i, node_inx, sock_gres, job_id, tres_mc_ptr);
                if (job_fini == 1)
                    break;
            }
            list_iterator_destroy(sock_gres_iter);
        ...
        if (job_fini == 0) {
            error("%s job %u failed to satisfy gres-per-job counter", __func__, job_id);
        }
    }
...
extern uint64_t gres_plugin_job_mem_max(List job_gres_list) {
    ...
    while ((job_gres_ptr = (gres_state_t *) list_next(job_gres_iter))) {
        ...
        else mem_per_gres = job_data_ptr->def_mem_per_gres;
 ...
 */
extern bool gres_plugin_job_mem_set(List job_gres_list, job_resources_t *job_res) {
    ListIterator job_gres_iter;
    gres_state_t *job_gres_ptr;
    gres_job_state_t *job_data_ptr;
    ...
    while ((job_gres_ptr = (gres_state_t *) list_next(job_gres_iter))) {
        ...
        for (i = i_first; i <= i_last; i++) {
            ...
            if (first_set) {
                ...
                job_res->memory_allocated[node_off] = MAX(job_res->memory_allocated[node_off], mem_size);
            }
        }
        first_set = false;
    }
 ...
 */
extern int
gres_plugin_job_min_cpus(uint32_t node_count, uint32_t sockets_per_node, uint32_t task_count, List job_gres_list) {
    ListIterator job_gres_iter;
    gres_state_t *job_gres_ptr;
    gres_job_state_t *job_data_ptr;
    int tmp, min_cpus = 0;
    ...
    while ((job_gres_ptr = (gres_state_t *) list_next(job_gres_iter))) {
        ...
        else cpus_per_gres = job_data_ptr->def_cpus_per_gres;
        if (cpus_per_gres == 0)
            continue;
        if (job_data_ptr->gres_per_job) {
            total_gres = job_data_ptr->gres_per_job;
        } else if (job_data_ptr->gres_per_node) {
            total_gres = job_data_ptr->gres_per_node * node_count;
        } else if (job_data_ptr->gres_per_socket) {
            total_gres = job_data_ptr->gres_per_socket * node_count * sockets_per_node;
 ...
 * RET count of required CPUs for the job
 */
extern int gres_plugin_job_min_cpu_node(uint32_t sockets_per_node, uint32_t tasks_per_node, List job_gres_list) {
    ListIterator job_gres_iter;
    gres_state_t *job_gres_ptr;
    gres_job_state_t *job_data_ptr;
    int tmp, min_cpus = 0;
    ...
    while ((job_gres_ptr = (gres_state_t *) list_next(job_gres_iter))) {
        ...
        if (job_data_ptr->gres_per_node) {
            ...
            total_gres = job_data_ptr->gres_per_socket * sockets_per_node;
        } else if (job_data_ptr->gres_per_task) {
            total_gres = job_data_ptr->gres_per_task * tasks_per_node;
        } else total_gres = 1;
        tmp = cpus_per_gres * total_gres;
        min_cpus = MAX(min_cpus, tmp);
    }
 ...
 */
static bool
_cores_on_gres(bitstr_t *core_bitmap, bitstr_t *alloc_core_bitmap, gres_node_state_t *node_gres_ptr, int gres_inx,
               gres_job_state_t *job_gres_ptr) {
    int i, avail_cores;
    ...
    for (i = 0; i < node_gres_ptr->topo_cnt; i++) {
        ...
        if (!node_gres_ptr->topo_core_bitmap[i])
            return true;
        if (bit_size(node_gres_ptr->topo_core_bitmap[i]) != bit_size(core_bitmap))
            break;
        avail_cores = bit_overlap(node_gres_ptr->topo_core_bitmap[i], core_bitmap);
        if (avail_cores && alloc_core_bitmap) {
            avail_cores -= bit_overlap(node_gres_ptr->topo_core_bitmap[i], alloc_core_bitmap);
            if (avail_cores) {
                bit_or(alloc_core_bitmap, node_gres_ptr->topo_core_bitmap[i]);
            }
        }
        if (avail_cores)
            return true;
...
/*
    ...
    if ((plugin_id == mps_plugin_id) && (node_gres_ptr->gres_cnt_alloc != 0)) {
/* We must use the ONE already active GRES of this type */
        use_busy_dev = true;
    }
    ...
    if (job_gres_ptr->node_cnt == 0) {
        ...
        if (job_gres_ptr->gres_bit_alloc) {
            error("gres/%s: job %u node_cnt==0 and gres_bit_alloc is set", gres_name, job_id);
...
/*
	 ...
	 */
        ...
        debug2("gres/%s: job %u node_cnt is now larger than it was when allocated from %u to %d", gres_name, job_id,
               job_gres_ptr->node_cnt, node_cnt);
        if (node_offset >= job_gres_ptr->node_cnt)
            return SLURM_ERROR;
    } else if (job_gres_ptr->node_cnt > node_cnt) {
        debug2("gres/%s: job %u node_cnt is now smaller than it was when allocated %u to %d", gres_name, job_id,
               job_gres_ptr->node_cnt, node_cnt);
    }

    if (!job_gres_ptr->gres_bit_alloc) {
        job_gres_ptr->gres_bit_alloc = xcalloc(node_cnt, sizeof(bitstr_t * ));
    }
    if (!job_gres_ptr->gres_cnt_node_alloc) {
        job_gres_ptr->gres_cnt_node_alloc = xcalloc(node_cnt, sizeof(uint64_t));
    }
...
/* Using pre-selected GRES */
        } else if (job_gres_ptr->gres_cnt_node_select && job_gres_ptr->gres_cnt_node_select[node_index]) {
            gres_cnt = job_gres_ptr->gres_cnt_node_select[node_index];
        } else if (job_gres_ptr->gres_bit_select && job_gres_ptr->gres_bit_select[node_index]) {
            gres_cnt = bit_set_count(job_gres_ptr->gres_bit_select[node_index]);
            gres_cnt *= gres_per_bit;
        } else {
            error("gres/%s: job %u node %s no resources selected", gres_name, job_id, node_name);
            return SLURM_ERROR;
        }
    } else {
...
/*
    ...
    if (i > node_gres_ptr->gres_cnt_avail) {
        ...
        " > %"
        PRIu64
        ")", gres_name, job_id, node_name, i - node_gres_ptr->gres_cnt_avail, i, node_gres_ptr->gres_cnt_avail);
/* proceed with request, give job what is available */
    }

    if (!node_offset && job_gres_ptr->gres_cnt_step_alloc) {
        uint64_t *tmp = xcalloc(job_gres_ptr->node_cnt, sizeof(uint64_t));
        memcpy(tmp, job_gres_ptr->gres_cnt_step_alloc, sizeof(uint64_t) * MIN(node_cnt, job_gres_ptr->node_cnt));
        xfree(job_gres_ptr->gres_cnt_step_alloc);
        job_gres_ptr->gres_cnt_step_alloc = tmp;
    }
    if (job_gres_ptr->gres_cnt_step_alloc == NULL) {
        job_gres_ptr->gres_cnt_step_alloc = xcalloc(job_gres_ptr->node_cnt, sizeof(uint64_t));
    }
...
/*
        ...
        if (node_gres_ptr->gres_bit_alloc == NULL) {
            node_gres_ptr->gres_bit_alloc = bit_copy(job_gres_ptr->gres_bit_alloc[node_offset]);
            node_gres_ptr->gres_cnt_alloc += bit_set_count(node_gres_ptr->gres_bit_alloc);
            node_gres_ptr->gres_cnt_alloc *= gres_per_bit;
        } else if (node_gres_ptr->gres_bit_alloc) {
            gres_cnt = (int64_t) MIN(bit_size(node_gres_ptr->gres_bit_alloc),
            ...
            for (i = 0; i < gres_cnt; i++) {
                if (bit_test(job_gres_ptr->gres_bit_alloc[node_offset], i) &&
                    (shared_gres || !bit_test(node_gres_ptr->gres_bit_alloc, i))) {
                    bit_set(node_gres_ptr->gres_bit_alloc, i);
                    node_gres_ptr->gres_cnt_alloc += gres_per_bit;
                }
            }
        }
    } else if (job_gres_ptr->total_node_cnt && job_gres_ptr->gres_bit_select &&
               job_gres_ptr->gres_bit_select[node_index] && job_gres_ptr->gres_cnt_node_select) {
/* Specific GRES already selected, update the node record */
        ...
        if (sz1 > sz2) {
            error("gres/%s: job %u node %s gres bitmap size bad (%d > %d)", gres_name, job_id, node_name, sz1, sz2);
            job_gres_ptr->gres_bit_select[node_index] = bit_realloc(job_gres_ptr->gres_bit_select[node_index], sz2);
            job_mod = true;
        } else if (sz1 < sz2) {
            error("gres/%s: job %u node %s gres bitmap size bad (%d < %d)", gres_name, job_id, node_name, sz1, sz2);
            job_gres_ptr->gres_bit_select[node_index] = bit_realloc(job_gres_ptr->gres_bit_select[node_index], sz2);
        ...
        if (!shared_gres && bit_overlap(job_gres_ptr->gres_bit_select[node_index], node_gres_ptr->gres_bit_alloc)) {
            error("gres/%s: job %u node %s gres bitmap overlap", gres_name, job_id, node_name);
            bit_and_not(job_gres_ptr->gres_bit_select[node_index], node_gres_ptr->gres_bit_alloc);
        }
        job_gres_ptr->gres_bit_alloc[node_offset] = bit_copy(job_gres_ptr->gres_bit_select[node_index]);
        job_gres_ptr->gres_cnt_node_alloc[node_offset] = job_gres_ptr->gres_cnt_node_select[node_index];
        if (!node_gres_ptr->gres_bit_alloc) {
            node_gres_ptr->gres_bit_alloc = bit_copy(job_gres_ptr->gres_bit_alloc[node_offset]);
        } else {
            bit_or(node_gres_ptr->gres_bit_alloc, job_gres_ptr->gres_bit_alloc[node_offset]);
        }
        if (job_mod) {
            node_gres_ptr->gres_cnt_alloc = bit_set_count(node_gres_ptr->gres_bit_alloc);
            node_gres_ptr->gres_cnt_alloc *= gres_per_bit;
        ...
        job_gres_ptr->gres_bit_alloc[node_offset] = bit_alloc(node_gres_ptr->gres_cnt_avail);
        ...
        if (i < node_gres_ptr->gres_cnt_avail) {
            ...
            " < %"
            PRIu64
            ")", gres_name, node_name, i, node_gres_ptr->gres_cnt_avail);
...
/* Pass 1: Allocate GRES overlapping all allocated cores */
        for (i = 0; i < node_gres_ptr->gres_cnt_avail && gres_cnt > 0; i++) {
            if (bit_test(node_gres_ptr->gres_bit_alloc, i))
                continue;
            if (!_cores_on_gres(core_bitmap, alloc_core_bitmap, node_gres_ptr, i, job_gres_ptr))
                continue;
            bit_set(node_gres_ptr->gres_bit_alloc, i);
...
/* Pass 2: Allocate GRES overlapping any allocated cores */
        for (i = 0; i < node_gres_ptr->gres_cnt_avail && gres_cnt > 0; i++) {
            if (bit_test(node_gres_ptr->gres_bit_alloc, i))
                continue;
            if (!_cores_on_gres(core_bitmap, NULL, node_gres_ptr, i, job_gres_ptr))
                continue;
            bit_set(node_gres_ptr->gres_bit_alloc, i);
            bit_set(job_gres_ptr->gres_bit_alloc[node_offset], i);
            node_gres_ptr->gres_cnt_alloc += gres_per_bit;
        ...
        if (gres_cnt) {
            verbose("gres/%s topology sub-optimal for job %u", gres_name, job_id);
        }
/* Pass 3: Allocate any available GRES */
    ...
    if (job_gres_ptr->gres_bit_alloc[node_offset] && node_gres_ptr->topo_gres_bitmap &&
...
/* Avoid abort on bit_overlap below */
                error("gres/%s %s mismatch for node %s (%d != %d)", gres_name, log_type, node_name, sz1, sz2);
                log_cnt_err = false;
            }
            if (sz1 != sz2)
                continue;    /* See error above */
            gres_cnt = bit_overlap(job_gres_ptr->gres_bit_alloc[node_offset], node_gres_ptr->topo_gres_bitmap[i]);
            gres_cnt *= gres_per_bit;
            node_gres_ptr->topo_gres_cnt_alloc[i] += gres_cnt;
            if ((node_gres_ptr->type_cnt == 0) || (node_gres_ptr->topo_type_name == NULL) ||
                (node_gres_ptr->topo_type_name[i] == NULL))
                continue;
            for (j = 0; j < node_gres_ptr->type_cnt; j++) {
                if (!node_gres_ptr->type_name[j] || (node_gres_ptr->topo_type_id[i] != node_gres_ptr->type_id[j]))
                    continue;
        ...
        if (!node_gres_ptr->topo_gres_cnt_alloc) {
            node_gres_ptr->topo_gres_cnt_alloc = xcalloc(len, sizeof(uint64_t));
        } else {
...
/*
			 ...
			 */
            node_gres_ptr->topo_cnt = bit_size(job_gres_ptr->gres_bit_alloc[node_offset]);
            node_gres_ptr->topo_core_bitmap = xcalloc(node_gres_ptr->topo_cnt, sizeof(bitstr_t * ));
            node_gres_ptr->topo_gres_bitmap = xcalloc(node_gres_ptr->topo_cnt, sizeof(bitstr_t * ));
            node_gres_ptr->topo_gres_cnt_alloc = xcalloc(node_gres_ptr->topo_cnt, sizeof(uint64_t));
            node_gres_ptr->topo_gres_cnt_avail = xcalloc(node_gres_ptr->topo_cnt, sizeof(uint64_t));
            node_gres_ptr->topo_type_id = xcalloc(node_gres_ptr->topo_cnt, sizeof(uint32_t));
            node_gres_ptr->topo_type_name = xcalloc(node_gres_ptr->topo_cnt, sizeof(char *));
            for (i = 0; i < node_gres_ptr->topo_cnt; i++) {
                node_gres_ptr->topo_gres_bitmap[i] = bit_alloc(node_gres_ptr->topo_cnt);
...
/*
			 ...
			 */
            for (j = 0; j < node_gres_ptr->topo_cnt; j++) {
                if (use_busy_dev && (node_gres_ptr->topo_gres_cnt_alloc[j] == 0))
                    continue;
                if (node_gres_ptr->topo_gres_bitmap && node_gres_ptr->topo_gres_bitmap[j] &&
                    bit_test(node_gres_ptr->topo_gres_bitmap[j], i)) {
                    node_gres_ptr->topo_gres_cnt_alloc[i] += gres_per_bit;
                    gres_cnt += gres_per_bit;
                }
            }
            if ((node_gres_ptr->type_cnt == 0) || (node_gres_ptr->topo_type_name == NULL) ||
...
/*
			 ...
			 */
            _add_gres_type(job_gres_ptr->type_name, node_gres_ptr, 0);
            for (j = 0; j < node_gres_ptr->type_cnt; j++) {
                if (job_gres_ptr->type_id != node_gres_ptr->type_id[j])
                    continue;
                node_gres_ptr->type_cnt_alloc[j] += job_gres_ptr->gres_per_node;
                break;
            }
        }
    }
    ...
    if (!type_array_updated && job_gres_ptr->type_name) {
        ...
        for (j = 0; j < node_gres_ptr->type_cnt; j++) {
            ...
            if (job_gres_ptr->type_id != node_gres_ptr->type_id[j])
                continue;
            k = node_gres_ptr->type_cnt_avail[j] - node_gres_ptr->type_cnt_alloc[j];
            k = MIN(gres_cnt, k);
 ...
 * RET SLURM_SUCCESS or error code
 */
extern int gres_plugin_job_alloc(List job_gres_list, List node_gres_list, int node_cnt, int node_index, int node_offset,
                                 uint32_t job_id, char *node_name, bitstr_t *core_bitmap, uint32_t user_id) {
    int i, rc, rc2;
    ListIterator job_gres_iter, node_gres_iter;
    gres_state_t *job_gres_ptr, *node_gres_ptr;
    ...
    if (job_gres_list == NULL)
        return SLURM_SUCCESS;
    if (node_gres_list == NULL) {
        error("%s: job %u has gres specification while node %s has none", __func__, job_id, node_name);
        return SLURM_ERROR;
    }
    ...
    while ((job_gres_ptr = (gres_state_t *) list_next(job_gres_iter))) {
        ...
        if (i >= gres_context_cnt) {
            error("%s: no plugin configured for data type %u for job %u and node %s", __func__, job_gres_ptr->plugin_id,
                  job_id, node_name);
/* A likely sign that GresPlugins has changed */
            continue;
        }
        ...
        list_iterator_destroy(node_gres_iter);
        if (node_gres_ptr == NULL) {
            error("%s: job %u allocated gres/%s on node %s lacking that gres", __func__, job_id,
                  gres_context[i].gres_name, node_name);
            continue;
        }

        rc2 = _job_alloc(job_gres_ptr->gres_data, node_gres_ptr->gres_data, node_cnt, node_index, node_offset,
                         gres_context[i].gres_name, job_id, node_name, core_bitmap, job_gres_ptr->plugin_id, user_id);
...
/*
    ...
    if (job_gres_ptr->node_cnt <= node_offset) {
        error("gres/%s: job %u dealloc of node %s bad node_offset %d "
              "count is %u", gres_name, job_id, node_name, node_offset, job_gres_ptr->node_cnt);
        return SLURM_ERROR;
    }
    ...
    xfree(node_gres_ptr->gres_used);    /* Clear cache */
    if (node_gres_ptr->gres_bit_alloc && job_gres_ptr->gres_bit_alloc && job_gres_ptr->gres_bit_alloc[node_offset]) {
        len = bit_size(job_gres_ptr->gres_bit_alloc[node_offset]);
        i = bit_size(node_gres_ptr->gres_bit_alloc);
        if (i != len) {
            error("gres/%s: job %u and node %s bitmap sizes differ "
                  "(%d != %d)", gres_name, job_id, node_name, len, i);
            len = MIN(len, i);
...
/*
            ...
            if (node_gres_ptr->gres_cnt_alloc >= gres_per_bit) {
                ...
                error("gres/%s: job %u dealloc node %s GRES count underflow (%"
                PRIu64
                " < %"
                PRIu64
                ")", gres_name, job_id, node_name, node_gres_ptr->gres_cnt_alloc, gres_per_bit);
    ...
    if (gres_cnt && (node_gres_ptr->gres_cnt_alloc >= gres_cnt))
        ...
        " < %"
        PRIu64
        ")", gres_name, job_id, node_name, node_gres_ptr->gres_cnt_alloc, gres_cnt);
        node_gres_ptr->gres_cnt_alloc = 0;
    }

    if (job_gres_ptr->gres_bit_alloc && job_gres_ptr->gres_bit_alloc[node_offset] && node_gres_ptr->topo_gres_bitmap &&
        node_gres_ptr->topo_gres_cnt_alloc) {
        for (i = 0; i < node_gres_ptr->topo_cnt; i++) {
            sz1 = bit_size(job_gres_ptr->gres_bit_alloc[node_offset]);
            sz2 = bit_size(node_gres_ptr->topo_gres_bitmap[i]);
            if (sz1 != sz2)
                continue;
            gres_cnt = (uint64_t) bit_overlap(job_gres_ptr->gres_bit_alloc[node_offset],
            ...
            if (node_gres_ptr->topo_gres_cnt_alloc[i] >= gres_cnt) {
                ...
                " %"
                PRIu64
                ")", gres_name, job_id, node_name, node_gres_ptr->topo_gres_cnt_alloc[i], gres_cnt);
                node_gres_ptr->topo_gres_cnt_alloc[i] = 0;
            }
            if ((node_gres_ptr->type_cnt == 0) || (node_gres_ptr->topo_type_name == NULL) ||
                (node_gres_ptr->topo_type_name[i] == NULL))
            ...
            for (j = 0; j < node_gres_ptr->type_cnt; j++) {
                if (!node_gres_ptr->type_name[j] || (node_gres_ptr->topo_type_id[i] != node_gres_ptr->type_id[j]))
                    continue;
                if (node_gres_ptr->type_cnt_alloc[j] >= gres_cnt) {
                    node_gres_ptr->type_cnt_alloc[j] -= gres_cnt;
                } else if (old_job) {
                    ...
                    " %"
                    PRIu64
                    ")", gres_name, job_id, node_name, node_gres_ptr->type_name[j], node_gres_ptr->type_cnt_alloc[j], gres_cnt);
                    node_gres_ptr->type_cnt_alloc[j] = 0;
                }
            }
        }
        type_array_updated = true;
    } else if (job_gres_ptr->gres_bit_alloc && job_gres_ptr->gres_bit_alloc[node_offset] &&
               node_gres_ptr->topo_gres_cnt_alloc) {
/* Avoid crash if configuration inconsistent */
        len = MIN(node_gres_ptr->gres_cnt_config, bit_size(job_gres_ptr->gres_bit_alloc[node_offset]));
        for (i = 0; i < len; i++) {
            if (!bit_test(job_gres_ptr->gres_bit_alloc[node_offset], i) || !node_gres_ptr->topo_gres_cnt_alloc[i])
                continue;
            if (node_gres_ptr->topo_gres_cnt_alloc[i] >= gres_per_bit) {
                node_gres_ptr->topo_gres_cnt_alloc[i] -= gres_per_bit;
            } else {
                ...
                " %"
                PRIu64
                ")", gres_name, job_id, node_name, i, node_gres_ptr->topo_gres_cnt_alloc[i], gres_per_bit);
                node_gres_ptr->topo_gres_cnt_alloc[i] = 0;
            }
            if ((node_gres_ptr->type_cnt == 0) || (node_gres_ptr->topo_type_name == NULL) ||
                (node_gres_ptr->topo_type_name[i] == NULL))
            ...
            for (j = 0; j < node_gres_ptr->type_cnt; j++) {
                if (!node_gres_ptr->type_name[j] || (node_gres_ptr->topo_type_id[i] != node_gres_ptr->type_id[j]))
                    continue;
                if (node_gres_ptr->type_cnt_alloc[j] >= gres_per_bit) {
                    node_gres_ptr->type_cnt_alloc[j] -= gres_per_bit;
                } else {
                    error("gres/%s: job %u dealloc node %s "
                    ...
                    " %"
                    PRIu64
                    ")", gres_name, job_id, node_name, node_gres_ptr->type_name[j], node_gres_ptr->type_cnt_alloc[j], gres_per_bit);
                    node_gres_ptr->type_cnt_alloc[j] = 0;
                }
            }
        }
 ...
 */
extern int
gres_plugin_job_dealloc(List job_gres_list, List node_gres_list, int node_offset, uint32_t job_id, char *node_name,
                        bool old_job, uint32_t user_id, bool job_fini) {
    int i, rc, rc2;
    ListIterator job_gres_iter, node_gres_iter;
    gres_state_t *job_gres_ptr, *node_gres_ptr;
    char *gres_name = NULL;
...
/* A likely sign that GresPlugins has changed */
            gres_name = "UNKNOWN";
        } else gres_name = gres_context[i].gres_name;
        ...
        list_iterator_destroy(node_gres_iter);
        if (node_gres_ptr == NULL) {
            error("%s: node %s lacks gres/%s for job %u", __func__, node_name, gres_name, job_id);
            continue;
        }

        rc2 = _job_dealloc(job_gres_ptr->gres_data, node_gres_ptr->gres_data, node_offset, gres_name, job_id, node_name,
                           old_job, job_gres_ptr->plugin_id, user_id, job_fini);
        if (rc2 != SLURM_SUCCESS)
            rc = rc2;
    }
    list_iterator_destroy(job_gres_iter);
 ...
 */
extern void gres_plugin_job_merge(List from_job_gres_list, bitstr_t *from_job_node_bitmap, List to_job_gres_list,
...
/*
        ...
        if (select_type && (strstr(select_type, "cons_tres") || (strstr(select_type, "cray_aries") &&
                                                                 (slurm_get_select_type_param() &
                                                                  CR_OTHER_CONS_TRES)))) {
            select_hetero = 1;
        } else select_hetero = 0;
        xfree(select_type);
    }

    (void) gres_plugin_init();
    new_node_cnt = bit_set_count(from_job_node_bitmap) + bit_set_count(to_job_node_bitmap) -
                   bit_overlap(from_job_node_bitmap, to_job_node_bitmap);
    i_first = MIN(bit_ffs(from_job_node_bitmap), bit_ffs(to_job_node_bitmap));
    i_first = MAX(i_first, 0);
    i_last = MAX(bit_fls(from_job_node_bitmap), bit_fls(to_job_node_bitmap));
    if (i_last == -1) {
...
/* Step one - Expand the gres data structures in "to" job */
    ...
    while ((gres_ptr = (gres_state_t *) list_next(gres_iter))) {
        ...
        new_gres_cnt_node_alloc = xcalloc(new_node_cnt, sizeof(uint64_t));
        new_gres_bit_step_alloc = xcalloc(new_node_cnt, sizeof(bitstr_t * ));
        new_gres_cnt_step_alloc = xcalloc(new_node_cnt, sizeof(uint64_t));
        ...
        for (i = i_first; i <= i_last; i++) {
            ...
            if (to_match) {
                if (gres_job_ptr->gres_bit_alloc) {
                    new_gres_bit_alloc[new_inx] = gres_job_ptr->gres_bit_alloc[to_inx];
                }
                if (gres_job_ptr->gres_cnt_node_alloc) {
                    new_gres_cnt_node_alloc[new_inx] = gres_job_ptr->gres_cnt_node_alloc[to_inx];
                }
                if (gres_job_ptr->gres_bit_step_alloc) {
                    new_gres_bit_step_alloc[new_inx] = gres_job_ptr->gres_bit_step_alloc[to_inx];
                }
                if (gres_job_ptr->gres_cnt_step_alloc) {
                    new_gres_cnt_step_alloc[new_inx] = gres_job_ptr->gres_cnt_step_alloc[to_inx];
                }
            }
        }
...
/*
    ...
    while ((gres_ptr = (gres_state_t *) list_next(gres_iter))) {
        gres_job_ptr = (gres_job_state_t *) gres_ptr->gres_data;
        gres_ptr2 = list_find_first(to_job_gres_list, _gres_find_id, &gres_ptr->plugin_id);
        if (gres_ptr2) {
            ...
            gres_job_ptr2->gres_name = xstrdup(gres_job_ptr->gres_name);
            gres_job_ptr2->cpus_per_gres = gres_job_ptr->cpus_per_gres;
            gres_job_ptr2->gres_per_job = gres_job_ptr->gres_per_job;
            gres_job_ptr2->gres_per_job = gres_job_ptr->gres_per_job;
            gres_job_ptr2->gres_per_socket = gres_job_ptr->gres_per_socket;
            gres_job_ptr2->gres_per_task = gres_job_ptr->gres_per_task;
            gres_job_ptr2->mem_per_gres = gres_job_ptr->mem_per_gres;
            gres_job_ptr2->node_cnt = new_node_cnt;
            gres_job_ptr2->gres_bit_alloc = xcalloc(new_node_cnt, sizeof(bitstr_t * ));
            gres_job_ptr2->gres_cnt_node_alloc = xcalloc(new_node_cnt, sizeof(uint64_t));
            gres_job_ptr2->gres_bit_step_alloc = xcalloc(new_node_cnt, sizeof(bitstr_t * ));
            gres_job_ptr2->gres_cnt_step_alloc = xcalloc(new_node_cnt, sizeof(uint64_t));
        ...
        for (i = i_first; i <= i_last; i++) {
            ...
            if (from_match) {
                if (!gres_job_ptr->gres_bit_alloc) { ;
                } else if (select_hetero && gres_job_ptr2->gres_bit_alloc[new_inx] && gres_job_ptr->gres_bit_alloc &&
                           gres_job_ptr->gres_bit_alloc[new_inx]) {
/* Merge job's GRES bitmaps */
                    bit_or(gres_job_ptr2->gres_bit_alloc[new_inx], gres_job_ptr->gres_bit_alloc[from_inx]);
                } else if (gres_job_ptr2->gres_bit_alloc[new_inx]) {
/* Keep original job's GRES bitmap */
                } else {
                    gres_job_ptr2->gres_bit_alloc[new_inx] = gres_job_ptr->gres_bit_alloc[from_inx];
                    gres_job_ptr->gres_bit_alloc[from_inx] = NULL;
                }
                if (!gres_job_ptr->gres_bit_alloc) { ;
                } else if (select_hetero && gres_job_ptr2->gres_cnt_node_alloc[new_inx] &&
                           gres_job_ptr->gres_cnt_node_alloc && gres_job_ptr->gres_cnt_node_alloc[new_inx]) {
                    gres_job_ptr2->gres_cnt_node_alloc[new_inx] += gres_job_ptr->gres_cnt_node_alloc[from_inx];
                } else if (gres_job_ptr2->gres_cnt_node_alloc[new_inx]) {
/* Keep original job's GRES bitmap */
                } else {
                    gres_job_ptr2->gres_cnt_node_alloc[new_inx] = gres_job_ptr->gres_cnt_node_alloc[from_inx];
                    gres_job_ptr->gres_cnt_node_alloc[from_inx] = 0;
                }
                if (gres_job_ptr->gres_cnt_step_alloc && gres_job_ptr->gres_cnt_step_alloc[from_inx]) {
 ...
 * IN node_inx - zero origin node index
 */
extern void gres_plugin_job_set_env(char ***job_env_ptr, List job_gres_list, int node_inx) {
    int i;
    ListIterator gres_iter;
    gres_state_t *gres_ptr = NULL;
    bool found;
    ...
    for (i = 0; i < gres_context_cnt; i++) {
        ...
        if (job_gres_list) {
            ...
            while ((gres_ptr = (gres_state_t *) list_next(gres_iter))) {
                if (gres_ptr->plugin_id != gres_context[i].plugin_id)
                    continue;
                (*(gres_context[i].ops.job_set_env))(job_env_ptr, gres_ptr->gres_data, node_inx);
                found = true;
            }
            list_iterator_destroy(gres_iter);
        }
/*
		 ...
		 */
        if (!found) {
            (*(gres_context[i].ops.job_set_env))(job_env_ptr, NULL, node_inx);
        }
    }
    slurm_mutex_unlock(&gres_context_lock);
}
 ...
 */
extern void gres_plugin_job_set_defs(List job_gres_list, char *gres_name, uint64_t cpu_per_gpu, uint64_t mem_per_gpu) {
    uint32_t plugin_id;
    ListIterator gres_iter;
    gres_state_t *gres_ptr = NULL;
    gres_job_state_t *job_gres_data;
...
static void _job_state_log(void *gres_data, uint32_t job_id, uint32_t plugin_id) {
    ...
    info("gres:%s(%u) type:%s(%u) job:%u flags:%s state", gres_ptr->gres_name, plugin_id, gres_ptr->type_name,
    ...
    if (gres_ptr->gres_per_job)
        info("  gres_per_job:%"
    PRIu64, gres_ptr->gres_per_job);
    if (gres_ptr->gres_per_node) {
        info("  gres_per_node:%"
        PRIu64
        " node_cnt:%u", gres_ptr->gres_per_node, gres_ptr->node_cnt);
    }
    if (gres_ptr->gres_per_socket)
        info("  gres_per_socket:%"
    PRIu64, gres_ptr->gres_per_socket);
    if (gres_ptr->gres_per_task)
        info("  gres_per_task:%"
    PRIu64, gres_ptr->gres_per_task);
    if (gres_ptr->mem_per_gres)
        info("  mem_per_gres:%"
    PRIu64, gres_ptr->mem_per_gres);
    else if (gres_ptr->def_mem_per_gres)
        info("  def_mem_per_gres:%"
    PRIu64, gres_ptr->def_mem_per_gres);
    ...
    for (i = 0; i < gres_ptr->node_cnt; i++) {
        if (gres_ptr->gres_cnt_node_alloc && gres_ptr->gres_cnt_node_alloc[i]) {
            info("  gres_cnt_node_alloc[%d]:%"
            PRIu64, i, gres_ptr->gres_cnt_node_alloc[i]);
        ...
        if (gres_ptr->gres_bit_alloc && gres_ptr->gres_bit_alloc[i]) {
            bit_fmt(tmp_str, sizeof(tmp_str), gres_ptr->gres_bit_alloc[i]);
            info("  gres_bit_alloc[%d]:%s of %d", i, tmp_str, (int) bit_size(gres_ptr->gres_bit_alloc[i]));
        } else if (gres_ptr->gres_bit_alloc)
            info("  gres_bit_alloc[%d]:NULL", i);

        if (gres_ptr->gres_bit_step_alloc && gres_ptr->gres_bit_step_alloc[i]) {
            bit_fmt(tmp_str, sizeof(tmp_str), gres_ptr->gres_bit_step_alloc[i]);
            info("  gres_bit_step_alloc[%d]:%s of %d", i, tmp_str, (int) bit_size(gres_ptr->gres_bit_step_alloc[i]));
        } else if (gres_ptr->gres_bit_step_alloc)
            info("  gres_bit_step_alloc[%d]:NULL", i);

        if (gres_ptr->gres_cnt_step_alloc) {
            info("  gres_cnt_step_alloc[%d]:%"
            PRIu64
            "", i, gres_ptr->gres_cnt_step_alloc[i]);
        }
    }
...
/*
    ...
    for (i = 0; i < gres_ptr->total_node_cnt; i++) {
        if (gres_ptr->gres_cnt_node_select && gres_ptr->gres_cnt_node_select[i]) {
            info("  gres_cnt_node_select[%d]:%"
            PRIu64, i, gres_ptr->gres_cnt_node_select[i]);
        }
        if (gres_ptr->gres_bit_select && gres_ptr->gres_bit_select[i]) {
            bit_fmt(tmp_str, sizeof(tmp_str), gres_ptr->gres_bit_select[i]);
            info("  gres_bit_select[%d]:%s of %d", i, tmp_str, (int) bit_size(gres_ptr->gres_bit_select[i]));
        }
    }
}
 ...
 */
extern uint64_t gres_plugin_get_job_value_by_type(List job_gres_list, char *gres_name_type) {
    uint64_t gres_val;
    ...
    while ((job_gres_ptr = (gres_state_t *) list_next(job_gres_iter))) {
        if (job_gres_ptr->plugin_id == gres_name_type_id) {
            gres_val = ((gres_job_state_t * )(job_gres_ptr->gres_data))->gres_per_node;
            break;
        }
    }
    list_iterator_destroy(job_gres_iter);
...
extern void gres_plugin_job_state_log(List gres_list, uint32_t job_id) {
    ...
    gres_iter = list_iterator_create(gres_list);
    while ((gres_ptr = (gres_state_t *) list_next(gres_iter))) {
        _job_state_log(gres_ptr->gres_data, job_id, gres_ptr->plugin_id);
    }
    list_iterator_destroy(gres_iter);
    slurm_mutex_unlock(&gres_context_lock);
}
...
/*
    ...
    while ((gres_ptr = list_next(gres_itr))) {
        ...
        if (is_job) {
            gres_job_state_t *gres_data_ptr = (gres_job_state_t *) gres_ptr->gres_data;
            local_bit_alloc = gres_data_ptr->gres_bit_alloc;
            node_cnt = gres_data_ptr->node_cnt;
        } else {
            gres_step_state_t *gres_data_ptr = (gres_step_state_t *) gres_ptr->gres_data;
            local_bit_alloc = gres_data_ptr->gres_bit_alloc;
            node_cnt = gres_data_ptr->node_cnt;
        }

        if ((node_cnt != 1) || !local_bit_alloc || !local_bit_alloc[0] || !gres_context[j].ops.get_devices)
            continue;
        ...
        if (!gres_devices) {
            ...
            continue;
        } else if ((int) bit_size(local_bit_alloc[0]) != list_count(gres_devices)) {
            error("We got %d gres devices when we were only told about %d.  This should never happen.",
                  list_count(gres_devices), (int) bit_size(local_bit_alloc[0]));
            continue;
...
_step_test(void *step_gres_data, void *job_gres_data, int node_offset, bool first_step_node, uint16_t cpus_per_task,
    ...
    if ((node_offset >= job_gres_ptr->node_cnt) && (job_gres_ptr->node_cnt != 0)) {    /* GRES is type no_consume */
        error("gres/%s: %s %u.%u node offset invalid (%d >= %u)", gres_name, __func__, job_id, step_id, node_offset,
    ...
    if (first_step_node) {
        ...
        else step_gres_ptr->total_gres = 0;
    }
    ...
    if (step_gres_ptr->gres_per_step && (step_gres_ptr->gres_per_step > step_gres_ptr->total_gres) &&
        (max_rem_nodes == 1)) {
        gres_cnt = step_gres_ptr->gres_per_step;
        if (ignore_alloc)
            gres_cnt -= step_gres_ptr->gross_gres;
        else gres_cnt -= step_gres_ptr->total_gres;
        min_gres = MAX(min_gres, gres_cnt);
    }

    if (!_shared_gres(plugin_id) && job_gres_ptr->gres_bit_alloc && job_gres_ptr->gres_bit_alloc[node_offset]) {
        gres_cnt = bit_set_count(job_gres_ptr->gres_bit_alloc[node_offset]);
        if (!ignore_alloc && job_gres_ptr->gres_bit_step_alloc && job_gres_ptr->gres_bit_step_alloc[node_offset]) {
            gres_cnt -= bit_set_count(job_gres_ptr->gres_bit_step_alloc[node_offset]);
        }
        if (min_gres > gres_cnt) {
            core_cnt = 0;
        } else if (step_gres_ptr->gres_per_task) {
            task_cnt = (gres_cnt + step_gres_ptr->gres_per_task - 1) / step_gres_ptr->gres_per_task;
            core_cnt = task_cnt * cpus_per_task;
        } else core_cnt = NO_VAL64;
    } else if (job_gres_ptr->gres_cnt_node_alloc && job_gres_ptr->gres_cnt_step_alloc) {
        gres_cnt = job_gres_ptr->gres_cnt_node_alloc[node_offset];
        if (!ignore_alloc) {
            gres_cnt -= job_gres_ptr->gres_cnt_step_alloc[node_offset];
        }
        if (min_gres > gres_cnt) {
            core_cnt = 0;
        } else if (step_gres_ptr->gres_per_task) {
        ...
        debug3("gres/%s: %s %u.%u gres_bit_alloc and gres_cnt_node_alloc are NULL", gres_name, __func__, job_id,
               step_id);
        gres_cnt = 0;
        core_cnt = NO_VAL64;
    }
    if (core_cnt != 0) {
        ...
        else step_gres_ptr->total_gres += gres_cnt;
    }
 ...
 */
static gres_step_state_t *_get_next_step_gres(char *in_val, uint64_t *cnt, List gres_list, char **save_ptr, int *rc) {
    static char *prev_save_ptr = NULL;
    int context_inx = NO_VAL, my_rc = SLURM_SUCCESS;
...
/* Find the step GRES record */
    ...
    gres_ptr = list_find_first(gres_list, _gres_find_step_by_key, &step_search_key);
...
static void _validate_step_counts(List step_gres_list, List job_gres_list, int *rc) {
    ListIterator iter;
    ...
    while ((step_gres_ptr = (gres_state_t *) list_next(iter))) {
        ...
        else job_search_key.type_id = step_gres_data->type_id;
        job_gres_ptr = list_find_first(job_gres_list, _gres_find_job_by_key, &job_search_key);
        ...
        if (cpus_per_gres && step_gres_data->cpus_per_gres && (cpus_per_gres < step_gres_data->cpus_per_gres)) {
            *rc = ESLURM_INVALID_GRES;
            break;
        }
        if (job_gres_data->gres_per_job && step_gres_data->gres_per_step &&
            (job_gres_data->gres_per_job < step_gres_data->gres_per_step)) {
            *rc = ESLURM_INVALID_GRES;
            break;
        }
        if (job_gres_data->gres_per_node && step_gres_data->gres_per_node &&
            (job_gres_data->gres_per_node < step_gres_data->gres_per_node)) {
            *rc = ESLURM_INVALID_GRES;
            break;
        }
        if (job_gres_data->gres_per_socket && step_gres_data->gres_per_socket &&
            (job_gres_data->gres_per_socket < step_gres_data->gres_per_socket)) {
            *rc = ESLURM_INVALID_GRES;
            break;
        }
        if (job_gres_data->gres_per_task && step_gres_data->gres_per_task &&
            (job_gres_data->gres_per_task < step_gres_data->gres_per_task)) {
            *rc = ESLURM_INVALID_GRES;
            break;
        }
        if (job_gres_data->mem_per_gres)
            mem_per_gres = job_gres_data->mem_per_gres;
        else mem_per_gres = job_gres_data->def_mem_per_gres;
        if (mem_per_gres && step_gres_data->mem_per_gres && (mem_per_gres < step_gres_data->mem_per_gres)) {
            *rc = ESLURM_INVALID_GRES;
            break;
        }
 ...
 */
extern int
gres_plugin_step_state_validate(char *cpus_per_tres, char *tres_per_step, char *tres_per_node, char *tres_per_socket,
                                char *tres_per_task, char *mem_per_tres, List *step_gres_list, List job_gres_list,
    ...
    *step_gres_list = NULL;
    ...
    if (cpus_per_tres) {
        char *in_val = cpus_per_tres, *save_ptr = NULL;
        while ((step_gres_data = _get_next_step_gres(in_val, &cnt, new_step_list, &save_ptr, &rc))) {
    ...
    if (list_count(new_step_list) == 0) {
        ...
        if (rc == SLURM_SUCCESS)
            _validate_step_counts(new_step_list, job_gres_list, &rc);
        if (rc == SLURM_SUCCESS)
            *step_gres_list = new_step_list;
        else FREE_NULL_LIST(new_step_list);
    }
    slurm_mutex_unlock(&gres_context_lock);
    return rc;
}
...
static void *_step_state_dup2(void *gres_data, int node_index) {
    ...
    if ((node_index < gres_ptr->node_cnt) && gres_ptr->gres_bit_alloc && gres_ptr->gres_bit_alloc[node_index]) {
        new_gres_ptr->gres_bit_alloc = xmalloc(sizeof(bitstr_t * ));
        new_gres_ptr->gres_bit_alloc[0] = bit_copy(gres_ptr->gres_bit_alloc[node_index]);
    }
    return new_gres_ptr;
...
List gres_plugin_step_state_extract(List gres_list, int node_index) {
    ...
    while ((gres_ptr = (gres_state_t *) list_next(gres_iter))) {
        if (node_index == -1)
            ...
            new_gres_data = _step_state_dup2(gres_ptr->gres_data, node_index);
        }
        if (new_gres_list == NULL) {
 ...
 */
void gres_plugin_step_state_rebase(List gres_list, bitstr_t *orig_job_node_bitmap, bitstr_t *new_job_node_bitmap) {
    ListIterator gres_iter;
    gres_state_t *gres_ptr;
    ...
    while ((gres_ptr = (gres_state_t *) list_next(gres_iter))) {
        ...
        i_first = MIN(bit_ffs(orig_job_node_bitmap), bit_ffs(new_job_node_bitmap));
        i_first = MAX(i_first, 0);
        i_last = MAX(bit_fls(orig_job_node_bitmap), bit_fls(new_job_node_bitmap));
        if (i_last == -1) {
        ...
        for (i = i_first; i <= i_last; i++) {
            ...
            if (old_match && new_match) {
                ...
                if (gres_step_ptr->gres_bit_alloc) {
                    ...
                    new_gres_bit_alloc[new_inx] = gres_step_ptr->gres_bit_alloc[old_inx];
                }
            } else if (old_match && gres_step_ptr->gres_bit_alloc && gres_step_ptr->gres_bit_alloc[old_inx]) {
/* Node removed from job allocation,
				 * release step's resources */
                bit_free(gres_step_ptr->gres_bit_alloc[old_inx]);
            }
        }
 ...
 */
extern int
gres_plugin_step_state_pack(List gres_list, Buf buffer, uint32_t job_id, uint32_t step_id, uint16_t protocol_version) {
    int i, rc = SLURM_SUCCESS;
    ...
    while ((gres_ptr = (gres_state_t *) list_next(gres_iter))) {
        ...
        if (protocol_version >= SLURM_19_05_PROTOCOL_VERSION) {
            ...
            if (gres_step_ptr->gres_cnt_node_alloc) {
                pack8((uint8_t) 1, buffer);
                pack64_array(gres_step_ptr->gres_cnt_node_alloc, gres_step_ptr->node_cnt, buffer);
            } else {
                pack8((uint8_t) 0, buffer);
            }
            if (gres_step_ptr->gres_bit_alloc) {
                ...
                for (i = 0; i < gres_step_ptr->node_cnt; i++)
                    pack_bit_str_hex(gres_step_ptr->gres_bit_alloc[i], buffer);
            } else {
                pack8((uint8_t) 0, buffer);
            }
            rec_cnt++;
 ...
 * IN job_id, step_id - job and step ID for logging
 */
extern int gres_plugin_step_state_unpack(List *gres_list, Buf buffer, uint32_t job_id, uint32_t step_id,
    ...
    while ((rc == SLURM_SUCCESS) && (rec_cnt)) {
        ...
        if (protocol_version >= SLURM_19_05_PROTOCOL_VERSION) {
            ...
            if (data_flag) {
                safe_unpack64_array(&gres_step_ptr->gres_cnt_node_alloc, &uint32_tmp, buffer);
            }
            safe_unpack8(&data_flag, buffer);
            if (data_flag) {
                gres_step_ptr->gres_bit_alloc = xcalloc(gres_step_ptr->node_cnt, sizeof(bitstr_t * ));
                for (i = 0; i < gres_step_ptr->node_cnt; i++) {
                    unpack_bit_str_hex(&gres_step_ptr->gres_bit_alloc[i], buffer);
                }
            }
        } else if (protocol_version >= SLURM_MIN_PROTOCOL_VERSION) {
            safe_unpack32(&magic, buffer);
...
/*
			 ...
			 * Not a fatal error, skip over the data.
			 */
            info("%s: no plugin configured to unpack data type %u from step %u.%u", __func__, plugin_id, job_id,
                 step_id);
...
extern uint64_t gres_plugin_step_count(List step_gres_list, char *gres_name) {
    ...
    for (i = 0; i < gres_context_cnt; i++) {
        ...
        while ((gres_ptr = (gres_state_t *) list_next(gres_iter))) {
            ...
            else gres_cnt += gres_step_ptr->gres_per_node;
        }
        list_iterator_destroy(gres_iter);
        break;
    }
...
// On NetBSD, cpuset_t is an opaque data type
    ...
    CPU_ZERO(&mask);
#ifdef
    __FreeBSD__ rc = cpuset_getaffinity(CPU_LEVEL_WHICH, CPU_WHICH_PID, -1, sizeof(mask), &mask);
#else
    ...
    iter = list_iterator_create(gres_conf_list);
    while ((gres_slurmd_conf = (gres_slurmd_conf_t *) list_next(iter))) {
        if (gres_slurmd_conf->plugin_id != gres_context[context_inx].plugin_id)
            continue;
        if ((gres_inx + gres_slurmd_conf->count) >= MAX_GRES_BITMAP) {
            error("GRES %s bitmap overflow ((%d + %"
            PRIu64
            ") >= %d)", gres_slurmd_conf->name, gres_inx, gres_slurmd_conf->count, MAX_GRES_BITMAP);
            continue;
        }
        if (!gres_slurmd_conf->cpus_bitmap) {
            bit_nset(usable_gres, gres_inx, gres_inx + gres_slurmd_conf->count - 1);
        } else {
...
#ifdef
    __NetBSD__ cpuset_destroy(mask);
#endif
 ...
 */
extern void gres_plugin_step_hardware_init(List step_gres_list, uint32_t node_id, char *settings) {
    int i;
    ListIterator iter;
    gres_state_t *gres_ptr;
    gres_step_state_t *gres_step_ptr;
    ...
    for (i = 0; i < gres_context_cnt; i++) {
        ...
        if (!gres_ptr || !gres_ptr->gres_data)
            continue;
        gres_step_ptr = (gres_step_state_t *) gres_ptr->gres_data;
        if ((gres_step_ptr->node_cnt != 1) || !gres_step_ptr->gres_bit_alloc || !gres_step_ptr->gres_bit_alloc[0])
            continue;
...
static bitstr_t *_get_gres_map(char *map_gres, int local_proc_id) {
    ...
    while (tok) {
        ...
        if ((local_proc_id >= task_offset) && (local_proc_id <= (task_offset + task_mult - 1))) {
 ...
 */
extern void
gres_plugin_step_set_env(char ***job_env_ptr, List step_gres_list, uint16_t accel_bind_type, char *tres_bind,
    ...
    for (i = 0; i < gres_context_cnt; i++) {
        ...
        if (bind_gpu || bind_mic || bind_nic || map_gpu || mask_gpu) {
            if (!xstrcmp(gres_context[i].gres_name, "gpu")) {
                if (map_gpu) {
                    usable_gres = _get_gres_map(map_gpu, local_proc_id);
                } else if (mask_gpu) {
                    usable_gres = _get_gres_mask(mask_gpu, local_proc_id);
                } else if (bind_gpu)
        ...
        if (step_gres_list) {
            ...
            while ((gres_ptr = (gres_state_t *) list_next(gres_iter))) {
                ...
                if (accel_bind_type || tres_bind) {
                    (*(gres_context[i].ops.step_reset_env))(job_env_ptr, gres_ptr->gres_data, usable_gres);
                } else {
                    (*(gres_context[i].ops.step_set_env))(job_env_ptr, gres_ptr->gres_data, tres_freq, local_proc_id);
                }
                found = true;
            }
            list_iterator_destroy(gres_iter);
        ...
        if (!found) { /* No data fond */
            if (accel_bind_type || tres_bind) {
                (*(gres_context[i].ops.step_reset_env))(job_env_ptr, NULL, NULL); /* Fixme */
            } else {
                (*(gres_context[i].ops.step_set_env))(job_env_ptr, NULL, NULL, /* Fixme */
                                                      0); /* Fixme */
            }
        }
...
static void _step_state_log(void *gres_data, uint32_t job_id, uint32_t step_id, char *gres_name) {
    ...
    xassert(gres_ptr);
    info("gres:%s type:%s(%u) step:%u.%u flags:%s state", gres_name, gres_ptr->type_name, gres_ptr->type_id, job_id,
         step_id, _gres_flags_str(gres_ptr->flags));
    if (gres_ptr->cpus_per_gres)
        info("  cpus_per_gres:%u", gres_ptr->cpus_per_gres);
    if (gres_ptr->gres_per_step)
        info("  gres_per_step:%"
    PRIu64, gres_ptr->gres_per_step);
    if (gres_ptr->gres_per_node) {
        info("  gres_per_node:%"
        PRIu64
        " node_cnt:%u", gres_ptr->gres_per_node, gres_ptr->node_cnt);
    ...
    if (gres_ptr->node_in_use == NULL)
        ...
        for (i = 0; i < gres_ptr->node_cnt; i++) {
            ...
            if (gres_ptr->gres_bit_alloc[i]) {
                bit_fmt(tmp_str, sizeof(tmp_str), gres_ptr->gres_bit_alloc[i]);
                info("  gres_bit_alloc[%d]:%s of %d", i, tmp_str, (int) bit_size(gres_ptr->gres_bit_alloc[i]));
            } else info("  gres_bit_alloc[%d]:NULL", i);
        }
    }
}
 ...
 * IN job_id - job's ID
 */
extern void gres_plugin_step_state_log(List gres_list, uint32_t job_id, uint32_t step_id) {
    int i;
    ListIterator gres_iter;
    gres_state_t *gres_ptr;
    ...
    while ((gres_ptr = (gres_state_t *) list_next(gres_iter))) {
        for (i = 0; i < gres_context_cnt; i++) {
            if (gres_ptr->plugin_id != gres_context[i].plugin_id)
                continue;
            _step_state_log(gres_ptr->gres_data, job_id, step_id, gres_context[i].gres_name);
            break;
        }
    }
    list_iterator_destroy(gres_iter);
 ...
 */
extern uint64_t gres_plugin_step_test(List step_gres_list, List job_gres_list, int node_offset, bool first_step_node,
                                      uint16_t cpus_per_task, int max_rem_nodes, bool ignore_alloc, uint32_t job_id,
                                      uint32_t step_id) {
    int i;
...
/* job lack resources required by the step */
        ...
        for (i = 0; i < gres_context_cnt; i++) {
            if (step_gres_ptr->plugin_id != gres_context[i].plugin_id)
                continue;
            tmp_cnt = _step_test(step_gres_ptr->gres_data, job_gres_ptr->gres_data, node_offset, first_step_node,
                                 cpus_per_task, max_rem_nodes, ignore_alloc, gres_context[i].gres_name, job_id, step_id,
...
_step_alloc(void *step_gres_data, void *job_gres_data, uint32_t plugin_id, int node_offset, bool first_step_node,
    ...
    if (node_offset >= job_gres_ptr->node_cnt) {
        error("gres/%s: %s for %u.%u, node offset invalid "
              "(%d >= %u)", gres_name, __func__, job_id, step_id, node_offset, job_gres_ptr->node_cnt);
        return SLURM_ERROR;
    }
    ...
    if (step_gres_ptr->gres_per_node) {
        ...
        gres_needed = step_gres_ptr->gres_per_step - step_gres_ptr->total_gres;
    } else if (step_gres_ptr->gres_per_step) {
/* Leave at least one GRES per remaining node */
        max_gres = step_gres_ptr->gres_per_step - step_gres_ptr->total_gres - (rem_nodes - 1);
        gres_needed = 1;
    } else {
/*
    ...
    if (!step_gres_ptr->gres_cnt_node_alloc) {
        step_gres_ptr->gres_cnt_node_alloc = xcalloc(step_gres_ptr->node_cnt, sizeof(uint64_t));
    }

    if (job_gres_ptr->gres_cnt_node_alloc && job_gres_ptr->gres_cnt_node_alloc[node_offset])
        gres_avail = job_gres_ptr->gres_cnt_node_alloc[node_offset];
    else if (job_gres_ptr->gres_bit_select && job_gres_ptr->gres_bit_select[node_offset])
        gres_avail = bit_set_count(job_gres_ptr->gres_bit_select[node_offset]);
    else if (job_gres_ptr->gres_cnt_node_alloc)
        gres_avail = job_gres_ptr->gres_cnt_node_alloc[node_offset];
    else gres_avail = job_gres_ptr->gres_per_node;
    if (gres_needed > gres_avail) {
        ...
        " > %"
        PRIu64
        ")", gres_name, __func__, job_id, step_id, node_offset, gres_needed, gres_avail);
        return SLURM_ERROR;
    }
    ...
    if (gres_needed > (gres_avail - job_gres_ptr->gres_cnt_step_alloc[node_offset])) {
        error("gres/%s: %s for %u.%u, step's > job's "
              "remaining for node %d (%"
        PRIu64
        " > "
        "(%"
        PRIu64
        " - %"
        PRIu64
        "))", gres_name, __func__, job_id, step_id, node_offset, gres_needed, gres_avail, job_gres_ptr->gres_cnt_step_alloc[node_offset]);
        return SLURM_ERROR;
    }
    ...
    if (step_gres_ptr->gres_cnt_node_alloc && (node_offset < step_gres_ptr->node_cnt))
    ...
    if ((job_gres_ptr->gres_bit_alloc == NULL) || (job_gres_ptr->gres_bit_alloc[node_offset] == NULL)) {
        debug3("gres/%s: %s gres_bit_alloc for %u.%u is NULL", gres_name, __func__, job_id, step_id);
        return SLURM_SUCCESS;
    }
    ...
    if (_shared_gres(plugin_id)) {
        ...
        if (job_gres_ptr->gres_bit_step_alloc && job_gres_ptr->gres_bit_step_alloc[node_offset]) {
            bit_and_not(gres_bit_alloc, job_gres_ptr->gres_bit_step_alloc[node_offset]);
        }
        for (i = 0; i < len; i++) {
            if (gres_needed > 0) {
                if (bit_test(gres_bit_alloc, i))
    ...
    if (gres_needed) {
        error("gres/%s: %s step %u.%u oversubscribed resources on node %d", gres_name, __func__, job_id, step_id,
              node_offset);
    }

    if (job_gres_ptr->gres_bit_step_alloc == NULL) {
        job_gres_ptr->gres_bit_step_alloc = xcalloc(job_gres_ptr->node_cnt, sizeof(bitstr_t * ));
    }
    if (job_gres_ptr->gres_bit_step_alloc[node_offset]) {
        bit_or(job_gres_ptr->gres_bit_step_alloc[node_offset], gres_bit_alloc);
    } else {
        job_gres_ptr->gres_bit_step_alloc[node_offset] = bit_copy(gres_bit_alloc);
    }
    if (step_gres_ptr->gres_bit_alloc == NULL) {
        step_gres_ptr->gres_bit_alloc = xcalloc(job_gres_ptr->node_cnt, sizeof(bitstr_t * ));
    }
    if (step_gres_ptr->gres_bit_alloc[node_offset]) {
        error("gres/%s: %s step %u.%u bit_alloc already exists", gres_name, __func__, job_id, step_id);
 ...
 * RET SLURM_SUCCESS or error code
 */
extern int gres_plugin_step_alloc(List step_gres_list, List job_gres_list, int node_offset, bool first_step_node,
                                  uint16_t tasks_on_node, uint32_t rem_nodes, uint32_t job_id, uint32_t step_id) {
    int i, rc, rc2;
    ...
    if (step_gres_list == NULL)
        return SLURM_SUCCESS;
    if (job_gres_list == NULL) {
        error("%s: step allocates GRES, but job %u has none", __func__, job_id);
        return SLURM_ERROR;
    }
    ...
    while ((step_gres_ptr = (gres_state_t *) list_next(step_gres_iter))) {
        ...
        if (i >= gres_context_cnt) {
            error("%s: could not find GRES plugin %u for step %u.%u", __func__, step_gres_ptr->plugin_id, job_id,
                  step_id);
            rc = ESLURM_INVALID_GRES;
            break;
        }
        ...
        job_gres_iter = list_iterator_create(job_gres_list);
        while ((job_gres_ptr = (gres_state_t *) list_next(job_gres_iter))) {
            gres_job_state_t *d_job_gres_ptr = (gres_job_state_t *) job_gres_ptr->gres_data;
            gres_step_state_t *d_step_gres_ptr = (gres_step_state_t *) step_gres_ptr->gres_data;
/*
			 ...
			 */
            if ((step_gres_ptr->plugin_id == job_gres_ptr->plugin_id) &&
                (!d_step_gres_ptr->type_name || (d_job_gres_ptr->type_id == d_step_gres_ptr->type_id)))
                break;
        }
        list_iterator_destroy(job_gres_iter);
        if (job_gres_ptr == NULL) {
            info("%s: job %u lacks gres/%s for step %u", __func__, job_id, gres_context[i].gres_name, step_id);
            rc = ESLURM_INVALID_GRES;
            break;
        }

        rc2 = _step_alloc(step_gres_ptr->gres_data, job_gres_ptr->gres_data, step_gres_ptr->plugin_id, node_offset,
                          first_step_node, gres_context[i].gres_name, job_id, step_id, tasks_on_node, rem_nodes);
        if (rc2 != SLURM_SUCCESS)
            rc = rc2;
    }
    list_iterator_destroy(step_gres_iter);
...
static int
_step_dealloc(void *step_gres_data, void *job_gres_data, char *gres_name, uint32_t job_id, uint32_t step_id) {
    ...
    if (step_gres_ptr->node_in_use == NULL) {
        error("gres/%s: %s step %u.%u dealloc, node_in_use is NULL", gres_name, __func__, job_id, step_id);
        return SLURM_ERROR;
    }
    ...
    for (i = 0; i < node_cnt; i++) {
        ...
        else gres_cnt = step_gres_ptr->gres_per_node;
        if (job_gres_ptr->gres_cnt_step_alloc) {
            if (job_gres_ptr->gres_cnt_step_alloc[i] >= gres_cnt) {
                job_gres_ptr->gres_cnt_step_alloc[i] -= gres_cnt;
            } else {
                error("gres/%s: %s step %u.%u dealloc count "
                      "underflow", gres_name, __func__, job_id, step_id);
                job_gres_ptr->gres_cnt_step_alloc[i] = 0;
            }
        }
        if ((step_gres_ptr->gres_bit_alloc == NULL) || (step_gres_ptr->gres_bit_alloc[i] == NULL))
            continue;
        ...
        if (len_j != len_s) {
            error("gres/%s: %s step %u.%u dealloc, bit_alloc[%d] "
                  "size mis-match (%d != %d)", gres_name, __func__, job_id, step_id, i, len_j, len_s);
            len_j = MIN(len_j, len_s);
        }
        for (j = 0; j < len_j; j++) {
            if (!bit_test(step_gres_ptr->gres_bit_alloc[i], j))
                continue;
            if (job_gres_ptr->gres_bit_step_alloc && job_gres_ptr->gres_bit_step_alloc[i]) {
                bit_clear(job_gres_ptr->gres_bit_step_alloc[i], j);
            }
        }
 ...
 * RET SLURM_SUCCESS or error code
 */
extern int gres_plugin_step_dealloc(List step_gres_list, List job_gres_list, uint32_t job_id, uint32_t step_id) {
    int i, rc, rc2;
    ListIterator step_gres_iter, job_gres_iter;
    gres_state_t *step_gres_ptr, *job_gres_ptr;
    ...
    if (step_gres_list == NULL)
        return SLURM_SUCCESS;
    if (job_gres_list == NULL) {
        error("%s: step deallocates gres, but job %u has none", __func__, job_id);
        return SLURM_ERROR;
    }
...
/*
        ...
        for (i = 0; i < gres_context_cnt; i++) {
            if (step_gres_ptr->plugin_id != gres_context[i].plugin_id)
                continue;
            rc2 = _step_dealloc(step_gres_ptr->gres_data, job_gres_ptr->gres_data, gres_context[i].gres_name, job_id,
                                step_id);
            if (rc2 != SLURM_SUCCESS)
                rc = rc2;
            break;
 ...
 * RET SLURM_SUCCESS or error code
 */
extern int gres_plugin_job_count(List gres_list, int arr_len, uint32_t *gres_count_ids, uint64_t *gres_count_vals) {
    ListIterator job_gres_iter;
    gres_state_t *job_gres_ptr;
    void *job_gres_data;
    int rc, ix = 0;
...
extern char *gres_plugin_job_alloc_count(List gres_list) {
    ...
    while ((job_gres_ptr = (gres_state_t *) list_next(job_gres_iter))) {
        ...
        gres_name = "UNKNOWN";
        for (i = 0; i < gres_context_cnt; i++) {
            if (gres_context[i].plugin_id != job_gres_ptr->plugin_id)
                continue;
            gres_name = gres_context[i].gres_name;
        }

        if (job_gres_state_ptr->type_name) {
            xstrfmtcat(gres_alloc, "%s%s:%s:%"
            PRIu64, sep, gres_name, job_gres_state_ptr->type_name, job_gres_state_ptr->total_gres);
        } else {
            xstrfmtcat(gres_alloc, "%s%s:%"
            PRIu64, sep, gres_name, job_gres_state_ptr->total_gres);
        }
        sep = ",";
    }
    list_iterator_destroy(job_gres_iter);
 ...
 */
extern int
gres_plugin_node_count(List gres_list, int arr_len, uint32_t *gres_count_ids, uint64_t *gres_count_vals, int val_type) {
    ListIterator node_gres_iter;
...
static int
_get_job_info(int gres_inx, gres_job_state_t *job_gres_data, uint32_t node_inx, enum gres_job_data_type data_type,
              void *data) {
...
/* Support here for plugin-specific data types */
        rc = (*(gres_context[gres_inx].ops.job_info))(job_gres_data, node_inx, data_type, data);
    }
 ...
 * RET - SLURM_SUCCESS or error code
 */
extern int gres_get_job_info(List job_gres_list, char *gres_name, uint32_t node_inx, enum gres_job_data_type data_type,
                             void *data) {
    ...
    while ((job_gres_ptr = (gres_state_t *) list_next(job_gres_iter))) {
        for (i = 0; i < gres_context_cnt; i++) {
            if (job_gres_ptr->plugin_id != plugin_id)
                continue;
            job_gres_data = (gres_job_state_t *) job_gres_ptr->gres_data;
            rc = _get_job_info(i, job_gres_data, node_inx, data_type, data);
            break;
        }
    }
    list_iterator_destroy(job_gres_iter);
 ...
 */
extern void gres_build_job_details(List job_gres_list, uint32_t *gres_detail_cnt, char ***gres_detail_str) {
    int i, j;
    ListIterator job_gres_iter;
    gres_state_t *job_gres_ptr;
    gres_job_state_t *job_gres_data;
        ...
        for (i = 0; i < gres_context_cnt; i++) {
            ...
            for (j = 0; j < my_gres_cnt; j++) {
                ...
                if (job_gres_data->gres_bit_alloc[j]) {
                    bit_fmt(tmp_str, sizeof(tmp_str), job_gres_data->gres_bit_alloc[j]);
                    xstrfmtcat(my_gres_details[j], "%s%s%s%s(IDX:%s)", sep1, gres_context[i].gres_name, sep2, type,
                               tmp_str);
                } else {
                    xstrfmtcat(my_gres_details[j], "%s%s%s%s(CNT:%"
                    PRIu64
                    ")", sep1, gres_context[i].gres_name, sep2, type, job_gres_data->gres_cnt_node_alloc[j]);
                }
            }
            break;
        }
...
/* Get generic GRES data types here. Call the plugin for others */
static int
_get_step_info(int gres_inx, gres_step_state_t *step_gres_data, uint32_t node_inx, enum gres_step_data_type data_type,
               void *data) {
    uint64_t *u64_data = (uint64_t *) data;
    bitstr_t **bit_data = (bitstr_t **) data;
    int rc = SLURM_SUCCESS;
...
/* Support here for plugin-specific data types */
        rc = (*(gres_context[gres_inx].ops.step_info))(step_gres_data, node_inx, data_type, data);
    }
 ...
 */
extern int
gres_get_step_info(List step_gres_list, char *gres_name, uint32_t node_inx, enum gres_step_data_type data_type,
                   void *data) {
    int i, rc = ESLURM_INVALID_GRES;
    uint32_t plugin_id;
    ...
    while ((step_gres_ptr = (gres_state_t *) list_next(step_gres_iter))) {
        for (i = 0; i < gres_context_cnt; i++) {
            if (step_gres_ptr->plugin_id != plugin_id)
                continue;
            step_gres_data = (gres_step_state_t *) step_gres_ptr->gres_data;
            rc = _get_step_info(i, step_gres_data, node_inx, data_type, data);
            break;
        }
    }
    list_iterator_destroy(step_gres_iter);
...
/* must be locked first before gres_contrex_lock!!! */
    ...
    while ((gres_state_ptr = list_next(itr))) {
        if (is_job) {
            gres_job_state_t *gres_data_ptr = (gres_job_state_t *) gres_state_ptr->gres_data;
            col_name = gres_data_ptr->type_name;
            count = gres_data_ptr->total_gres;
        } else {
            gres_step_state_t *gres_data_ptr = (gres_step_state_t *) gres_state_ptr->gres_data;
            col_name = gres_data_ptr->type_name;
            count = gres_data_ptr->total_gres;
        }

        for (i = 0; i < gres_context_cnt; i++) {
            if (gres_context[i].plugin_id == gres_state_ptr->plugin_id) {
        ...
        if (tres_rec && slurmdb_find_tres_count_in_string(tres_str, tres_rec->id) == INFINITE64)
/* New gres */
            xstrfmtcat(tres_str, "%s%u=%"
        PRIu64, tres_str ? "," : "", tres_rec->id, count);
...
/*
				 ...
				 */
                tres_req.name = xstrdup_printf("%s%s", gres_context[i].gres_name_colon, col_name);
                tres_rec = assoc_mgr_find_tres_rec(&tres_req);
                xfree(tres_req.name);
                if (tres_rec && slurmdb_find_tres_count_in_string(tres_str, tres_rec->id) == INFINITE64)
/* New GRES */
...
/*
				 ...
				 */
                tres_req.name = xstrdup_printf("%s", gres_context[i].gres_name);
...
static void _set_type_tres_cnt(gres_state_type_enum_t state_type, List gres_list, uint32_t node_cnt, uint64_t *tres_cnt,
                               bool locked) {
    ListIterator itr;
    gres_state_t *gres_state_ptr;
    static bool first_run = 1;
...
/* we only need to init this once */
    ...
    if (!gres_list || !tres_cnt || ((state_type == GRES_STATE_TYPE_JOB) && (!node_cnt || (node_cnt == NO_VAL))))
        return;
...
/* Initialize all GRES counters to zero. Increment them later. */
    for (i = 0; i < gres_context_cnt; i++) {
        ...
        if (tres_rec.name && ((tres_pos = assoc_mgr_find_tres_pos(&tres_rec, true)) != -1))
            tres_cnt[tres_pos] = 0;
    }
...
/* Get alloc count for main GRES. */
        switch (state_type) {
            ...
            case GRES_STATE_TYPE_NODE: {
                gres_node_state_t *gres_data_ptr = (gres_node_state_t *) gres_state_ptr->gres_data;
                count = gres_data_ptr->gres_cnt_alloc;
                break;
            }
            default:
                error("%s: unsupported state type %d", __func__, state_type);
                continue;
        }
/*
...
/*
        ...
        switch (state_type) {
            case GRES_STATE_TYPE_JOB: {
                ...
                col_name = gres_data_ptr->type_name;
                if (col_name) {
                    tres_rec.name = xstrdup_printf("%s%s", gres_context[i].gres_name_colon, col_name);
                    if ((tres_pos = assoc_mgr_find_tres_pos(&tres_rec, true)) != -1)
                        tres_cnt[tres_pos] = count;
                    xfree(tres_rec.name);
                } else if (!set_total) {
/*
				 ...
				 * accurate, it is better than nothing...
				 */
                    tres_rec.name = xstrdup_printf("%s", gres_context[i].gres_name);
                    if ((tres_pos = assoc_mgr_find_tres_pos2(&tres_rec, true)) != -1)
                        tres_cnt[tres_pos] = count;
                    xfree(tres_rec.name);
                }
                break;
...
extern void gres_set_job_tres_cnt(List gres_list, uint32_t node_cnt, uint64_t *tres_cnt, bool locked) {
    _set_type_tres_cnt(GRES_STATE_TYPE_JOB, gres_list, node_cnt, tres_cnt, locked);
}

extern void gres_set_node_tres_cnt(List gres_list, uint64_t *tres_cnt, bool locked) {
    _set_type_tres_cnt(GRES_STATE_TYPE_NODE, gres_list, 0, tres_cnt, locked);
}
...
extern char *gres_device_major(char *dev_path) {
    ...
    debug3("%s : %s major %d, minor %d", __func__, dev_path, loc_major, loc_minor);
 ...
 * records
 */
extern void add_gres_to_list(List gres_list, char *name, uint64_t device_cnt, int cpu_cnt, char *cpu_aff_abs_range,
...
/*
    ...
    else gpu_record = xmalloc(sizeof(gres_slurmd_conf_t));
    ...
    if (bit_unfmt(gpu_record->cpus_bitmap, cpu_aff_abs_range)) {
        ...
        error("    dst_bitmap_size: %"
        BITSTR_FMT, bit_size(gpu_record->cpus_bitmap));
