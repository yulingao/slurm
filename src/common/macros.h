...
/*
 *如果id为NULL，则将丢弃thread_id，而无需首先创建本地pthread_t对象。
 *
 *这仅适用于分离的线程 -如果您正在创建一个不需要保留id的附加线程，那么您应该真正将其分离。
 *
 *使这项工作的三元运算符是故意过度的，以避免编译器警告它始终解析为true，因为这是一个宏，优化过程将意识到本地范围中的变量将始终具有非零的内存地址
