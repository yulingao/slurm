...
typedef enum {
    PERSIST_TYPE_NONE = 0, PERSIST_TYPE_DBD, PERSIST_TYPE_FED, PERSIST_TYPE_HA_CTL, PERSIST_TYPE_HA_DBD,
} persist_conn_type_t;
...
typedef struct {
    ...
    int (*callback_proc)(void *arg, persist_msg_t *msg, Buf *out_buffer, uint32_t *uid);
...
extern Buf slurm_persist_make_rc_msg(slurm_persist_conn_t *persist_conn, uint32_t rc, char *comment, uint16_t ret_info);
