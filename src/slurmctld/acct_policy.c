...
enum {
    ACCT_POLICY_ADD_SUBMIT, ACCT_POLICY_REM_SUBMIT, ACCT_POLICY_JOB_BEGIN, ACCT_POLICY_JOB_FINI
};

typedef enum {
    TRES_USAGE_OKAY, TRES_USAGE_CUR_EXCEEDS_LIMIT, TRES_USAGE_REQ_EXCEEDS_LIMIT, TRES_USAGE_REQ_NOT_SAFE_WITH_USAGE
 ...
 */
static void _get_unique_job_node_cnt(struct job_record *job_ptr, bitstr_t *grp_node_bitmap, uint64_t *node_cnt) {
    xassert(node_cnt);
#if _DEBUG
                                                                                                                            char node_bitstr[64];
    ...
    if (job_ptr->job_resrcs && job_ptr->job_resrcs->node_bitmap && grp_node_bitmap) {
        uint64_t overlap_cnt = bit_overlap(job_ptr->job_resrcs->node_bitmap, grp_node_bitmap);
        if (overlap_cnt) {
            uint64_t init_cnt = bit_set_count(job_ptr->job_resrcs->node_bitmap);
            *node_cnt = init_cnt - overlap_cnt;
            if (slurmctld_conf.debug_flags & DEBUG_FLAG_TRES_NODE)
                info("%s: %pJ unique allocated node count changed from %"
            PRIu64
            " to %"
            PRIu64, __func__, job_ptr, init_cnt, *node_cnt);
        }
    } else if (job_ptr->details && job_ptr->details->req_node_bitmap && grp_node_bitmap) {
        uint64_t overlap_cnt = bit_overlap(job_ptr->details->req_node_bitmap, grp_node_bitmap);
        if (overlap_cnt <= *node_cnt) {
            *node_cnt -= overlap_cnt;
            ...
            PRIu64
            " to %"
            PRIu64, __func__, job_ptr, *node_cnt + overlap_cnt, *node_cnt);
        }
    }
}
 ...
 */
static void _add_usage_node_bitmap(struct job_record *job_ptr, bitstr_t **grp_node_bitmap, uint16_t **grp_node_job_cnt,
 ...
 * grp_used_tres[TRES_ARRAY_NODE] of an object (qos, assoc, etc).
 */
static void _rm_usage_node_bitmap(struct job_record *job_ptr, bitstr_t *grp_node_bitmap, uint16_t *grp_node_job_cnt,
                                  uint64_t *grp_used_tres) {
    int i, i_first, i_last;
...
static int _get_tres_state_reason(int tres_pos, int unk_reason) {
    switch (tres_pos) {
        ...
        default:
            ...
            else if (!xstrcmp("license", assoc_mgr_tres_array[tres_pos]->type))
                switch (unk_reason) {
                    case WAIT_ASSOC_GRP_UNK:
...
static bool _valid_job_assoc(struct job_record *job_ptr) {
    ...
    if ((job_ptr->assoc_ptr == NULL) || (job_ptr->assoc_ptr->id != job_ptr->assoc_id) ||
        ...
        if (assoc_mgr_fill_in_assoc(acct_db_conn, &assoc_rec, accounting_enforce, &job_ptr->assoc_ptr, false)) {
            info("%s: invalid account or partition for uid=%u %pJ", __func__, job_ptr->user_id, job_ptr);
            return false;
        }
...
static void
_qos_adjust_limit_usage(int type, struct job_record *job_ptr, slurmdb_qos_rec_t *qos_ptr, uint64_t *used_tres_run_secs,
                        uint32_t job_cnt) {
    ...
    used_limits_a = acct_policy_get_acct_used_limits(&qos_ptr->usage->acct_limit_list, job_ptr->assoc_ptr->acct);

    used_limits = acct_policy_get_user_used_limits(&qos_ptr->usage->user_limit_list, job_ptr->user_id);
    ...
    switch (type) {
        ...
        case ACCT_POLICY_REM_SUBMIT:
            if (qos_ptr->usage->grp_used_submit_jobs >= job_cnt)
                ...
                debug2("acct_policy_remove_job_submit: "
                       "grp_submit_jobs underflow for qos %s", qos_ptr->name);
            }
            ...
            if (used_limits->submit_jobs >= job_cnt)
                ...
                debug2("acct_policy_remove_job_submit: "
                       ...
                       "qos %s user %d", qos_ptr->name, used_limits->uid);
            }
            ...
            if (used_limits_a->submit_jobs >= job_cnt)
                ...
                debug2("acct_policy_remove_job_submit: "
                       "used_submit_jobs underflow for "
                       "qos %s account %s", qos_ptr->name, used_limits_a->acct);
            }
        ...
        case ACCT_POLICY_JOB_BEGIN:
			 ...
			 */
                ...
                qos_ptr->usage->grp_used_tres[i] += job_ptr->tres_alloc_cnt[i];
                qos_ptr->usage->grp_used_tres_run_secs[i] += used_tres_run_secs[i];
                debug2("acct_policy_job_begin: after adding %pJ, qos %s grp_used_tres_run_secs(%s) is %"
                PRIu64, job_ptr, qos_ptr->name, assoc_mgr_tres_name_array[i], qos_ptr->usage->grp_used_tres_run_secs[i]);
            }

            used_limits->jobs++;
            used_limits_a->jobs++;

            _add_usage_node_bitmap(job_ptr, &qos_ptr->usage->grp_node_bitmap, &qos_ptr->usage->grp_node_job_cnt,
                                   &qos_ptr->usage->grp_used_tres[TRES_ARRAY_NODE]);

            _add_usage_node_bitmap(job_ptr, &used_limits->node_bitmap, &used_limits->node_job_cnt,
                                   &used_limits->tres[TRES_ARRAY_NODE]);

            _add_usage_node_bitmap(job_ptr, &used_limits_a->node_bitmap, &used_limits_a->node_job_cnt,
		 ...
		 */
            ...
            for (i = 0; i < slurmctld_tres_cnt; i++) {
                if (i == TRES_ARRAY_ENERGY)
                    continue;
                if (job_ptr->tres_alloc_cnt[i] > qos_ptr->usage->grp_used_tres[i]) {
                    qos_ptr->usage->grp_used_tres[i] = 0;
                    debug2("acct_policy_job_fini: "
                           "grp_used_tres(%s) "
                           "underflow for QOS %s", assoc_mgr_tres_name_array[i], qos_ptr->name);
                } else
                    qos_ptr->usage->grp_used_tres[i] -= job_ptr->tres_alloc_cnt[i];
                ...
                if (job_ptr->tres_alloc_cnt[i] > used_limits->tres[i]) {
                    ...
                    debug2("acct_policy_job_fini: "
                           "used_limits->tres(%s) "
                           "underflow for qos %s user %u", assoc_mgr_tres_name_array[i], qos_ptr->name,
                           used_limits->uid);
                } else
                    used_limits->tres[i] -= job_ptr->tres_alloc_cnt[i];

                if (job_ptr->tres_alloc_cnt[i] > used_limits_a->tres[i]) {
                    used_limits_a->tres[i] = 0;
                    debug2("acct_policy_job_fini: "
                           "used_limits->tres(%s) "
                           "underflow for qos %s account %s", assoc_mgr_tres_name_array[i], qos_ptr->name,
                           used_limits_a->acct);
                } else
                    used_limits_a->tres[i] -= job_ptr->tres_alloc_cnt[i];
            }
            ...
            if (used_limits->jobs)
                ...
                debug2("acct_policy_job_fini: used_jobs "
                       "underflow for qos %s user %d", qos_ptr->name, used_limits->uid);

            if (used_limits_a->jobs)
                used_limits_a->jobs--;
            else
                debug2("acct_policy_job_fini: used_jobs "
                       "underflow for qos %s account %s", qos_ptr->name, used_limits_a->acct);

            _rm_usage_node_bitmap(job_ptr, qos_ptr->usage->grp_node_bitmap, qos_ptr->usage->grp_node_job_cnt,
                                  &qos_ptr->usage->grp_used_tres[TRES_ARRAY_NODE]);

            _rm_usage_node_bitmap(job_ptr, used_limits->node_bitmap, used_limits->node_job_cnt,
                                  &used_limits->tres[TRES_ARRAY_NODE]);

            _rm_usage_node_bitmap(job_ptr, used_limits_a->node_bitmap, used_limits_a->node_job_cnt,
                                  &used_limits_a->tres[TRES_ARRAY_NODE]);
            break;
        default:
...
static void _adjust_limit_usage(int type, struct job_record *job_ptr) {
    ...
    assoc_mgr_lock_t locks = {.assoc = WRITE_LOCK, .qos = WRITE_LOCK, .tres = READ_LOCK};
    ...
    if (!(accounting_enforce & ACCOUNTING_ENFORCE_LIMITS) || !_valid_job_assoc(job_ptr))
        return;
    ...
    if (type == ACCT_POLICY_JOB_FINI)
        ...
        for (i = 0; i < slurmctld_tres_cnt; i++) {
            if (i == TRES_ARRAY_ENERGY)
                continue;
            used_tres_run_secs[i] = job_ptr->tres_alloc_cnt[i] * time_limit_secs;
        }
    } else if (((type == ACCT_POLICY_ADD_SUBMIT) || (type == ACCT_POLICY_REM_SUBMIT)) && job_ptr->array_recs &&
	 ...
	 */
    if (((type == ACCT_POLICY_ADD_SUBMIT) || (type == ACCT_POLICY_REM_SUBMIT)) && job_ptr->part_ptr_list &&
        ...
        if (job_ptr->qos_ptr && (((slurmdb_qos_rec_t *) job_ptr->qos_ptr)->flags & QOS_FLAG_OVER_PART_QOS))
            job_first = true;

        if (job_first) {
            _qos_adjust_limit_usage(type, job_ptr, job_ptr->qos_ptr, used_tres_run_secs, job_cnt);
        ...
        while ((part_ptr = list_next(part_itr))) {
            ...
            if (!part_qos_list)
                part_qos_list = list_create(NULL);
            if (list_find_first(part_qos_list, _find_qos_part, part_ptr->qos_ptr))
                continue;
            list_push(part_qos_list, part_ptr->qos_ptr);
            _qos_adjust_limit_usage(type, job_ptr, part_ptr->qos_ptr, used_tres_run_secs, job_cnt);
        }
        list_iterator_destroy(part_itr);

        if (!job_first && (!part_qos_list || !list_find_first(part_qos_list, _find_qos_part, job_ptr->qos_ptr)))
            _qos_adjust_limit_usage(type, job_ptr, job_ptr->qos_ptr, used_tres_run_secs, job_cnt);
		 ...
		 */
        if ((type == ACCT_POLICY_JOB_BEGIN) && job_ptr->part_ptr_list) {
            ListIterator part_itr;
            struct part_record *part_ptr;
            ...
            if (job_ptr->qos_ptr)
                list_push(part_qos_list, job_ptr->qos_ptr);
            if (job_ptr->part_ptr && job_ptr->part_ptr->qos_ptr && job_ptr->qos_ptr != job_ptr->part_ptr->qos_ptr)
                list_push(part_qos_list, job_ptr->part_ptr->qos_ptr);
            ...
            while ((part_ptr = list_next(part_itr))) {
                ...
                if (list_find_first(part_qos_list, _find_qos_part, part_ptr->qos_ptr))
                    continue;
                _qos_adjust_limit_usage(ACCT_POLICY_REM_SUBMIT, job_ptr, part_ptr->qos_ptr, used_tres_run_secs,
                                        job_cnt);
            }
            list_iterator_destroy(part_itr);
            FREE_NULL_LIST(part_qos_list);
        ...
        _qos_adjust_limit_usage(type, job_ptr, qos_ptr_1, used_tres_run_secs, job_cnt);
        _qos_adjust_limit_usage(type, job_ptr, qos_ptr_2, used_tres_run_secs, job_cnt);
    }
        ...
        switch (type) {
            ...
            case ACCT_POLICY_REM_SUBMIT:
                if (assoc_ptr->usage->used_submit_jobs)
                    ...
                    debug2("acct_policy_remove_job_submit: "
                           "used_submit_jobs underflow for "
                           "account %s", assoc_ptr->acct);
                break;
            case ACCT_POLICY_JOB_BEGIN:
                assoc_ptr->usage->used_jobs++;
                _add_usage_node_bitmap(job_ptr, &assoc_ptr->usage->grp_node_bitmap, &assoc_ptr->usage->grp_node_job_cnt,
                ...
                for (i = 0; i < slurmctld_tres_cnt; i++) {
                    ...
                    if (i != TRES_ARRAY_NODE) {
                        assoc_ptr->usage->grp_used_tres[i] += job_ptr->tres_alloc_cnt[i];
                    }
                    assoc_ptr->usage->grp_used_tres_run_secs[i] += used_tres_run_secs[i];
                    debug2("acct_policy_job_begin: after adding %pJ, assoc %u(%s/%s/%s) grp_used_tres_run_secs(%s) is %"
                    PRIu64, job_ptr, assoc_ptr->id, assoc_ptr->acct, assoc_ptr->user, assoc_ptr->partition, assoc_mgr_tres_name_array[i], assoc_ptr->usage->grp_used_tres_run_secs[i]);
                }
                break;
            case ACCT_POLICY_JOB_FINI:
                ...
                _rm_usage_node_bitmap(job_ptr, assoc_ptr->usage->grp_node_bitmap, assoc_ptr->usage->grp_node_job_cnt,
                                      &assoc_ptr->usage->grp_used_tres[TRES_ARRAY_NODE]);
                for (i = 0; i < slurmctld_tres_cnt; i++) {
                    if ((i == TRES_ARRAY_ENERGY) || (i == TRES_ARRAY_NODE))
                        continue;
                    if (job_ptr->tres_alloc_cnt[i] > assoc_ptr->usage->grp_used_tres[i]) {
                        assoc_ptr->usage->grp_used_tres[i] = 0;
                        debug2("acct_policy_job_fini: "
                               "grp_used_tres(%s) "
                               "underflow for assoc "
                               "%u(%s/%s/%s)", assoc_mgr_tres_name_array[i], assoc_ptr->id, assoc_ptr->acct,
...
static void
_set_time_limit(uint32_t *time_limit, uint32_t part_max_time, uint32_t limit_max_time, uint16_t *limit_set_time) {
    if ((*time_limit) == NO_VAL) {
        ...
        if (limit_set_time)
            (*limit_set_time) = 1;
    } else if (limit_set_time && (*limit_set_time) && ((*time_limit) > limit_max_time))
        (*time_limit) = limit_max_time;
}

static void _qos_alter_job(struct job_record *job_ptr, slurmdb_qos_rec_t *qos_ptr, uint64_t *used_tres_run_secs,
    ...
    for (i = 0; i < slurmctld_tres_cnt; i++) {
        if (used_tres_run_secs[i] == new_used_tres_run_secs[i])
            continue;
        qos_ptr->usage->grp_used_tres_run_secs[i] -= used_tres_run_secs[i];
        qos_ptr->usage->grp_used_tres_run_secs[i] += new_used_tres_run_secs[i];
        debug2("altering %pJ QOS %s got %"
        PRIu64
        " just removed %"
        PRIu64
        " and added %"
        PRIu64, job_ptr, qos_ptr->name, qos_ptr->usage->grp_used_tres_run_secs[i], used_tres_run_secs[i], new_used_tres_run_secs[i]);
    }
}
 ...
 */
static bool
_validate_tres_limits_for_assoc(int *tres_pos, uint64_t *job_tres_array, uint64_t divisor, uint64_t *assoc_tres_array,
                                uint64_t *qos_tres_array, uint16_t *admin_set_limit_tres_array, bool strict_checking,
    ...
    for (i = 0; i < g_tres_count; i++) {
        ...
        if ((admin_set_limit_tres_array[i] == ADMIN_SET_LIMIT) || (qos_tres_array[i] != INFINITE64) ||
            (assoc_tres_array[i] == INFINITE64) || (!job_tres_array[i] && !update_call))
            continue;
 ...
 */
static bool
_validate_tres_limits_for_qos(int *tres_pos, uint64_t *job_tres_array, uint64_t divisor, uint64_t *grp_tres_array,
                              uint64_t *max_tres_array, uint64_t *out_grp_tres_array, uint64_t *out_max_tres_array,
                              uint16_t *admin_set_limit_tres_array, bool strict_checking, bool max_limit) {
    ...
    for (i = 0; i < g_tres_count; i++) {
        (*tres_pos) = i;
        if (grp_tres_array) {
            max_tres_limit = MIN(grp_tres_array[i], max_tres_array[i]);
            out_max_tres_limit = MIN(out_grp_tres_array[i], out_max_tres_array[i]);
        } else {
        ...
        if ((admin_set_limit_tres_array[i] == ADMIN_SET_LIMIT) || (out_max_tres_limit != INFINITE64) ||
            (max_tres_limit == INFINITE64) || (job_tres_array[i] && (job_tres_array[i] == NO_VAL64)))
            continue;
 ...
 */
static bool
_validate_time_limit(uint32_t *time_limit_in, uint32_t part_max_time, uint64_t tres_req_cnt, uint64_t max_limit,
                     void *out_max_limit, uint16_t *limit_set_time, bool strict_checking, bool is64) {
    uint32_t max_time_limit;
    uint64_t out_max_64 = *(uint64_t *) out_max_limit;
    uint32_t out_max_32 = *(uint32_t *) out_max_limit;

    if (!tres_req_cnt || (((*time_limit_in) != NO_VAL) && (!strict_checking || (*limit_set_time) == ADMIN_SET_LIMIT)))
        return true;

    if (is64) {
        if ((out_max_64 != INFINITE64) || (max_limit == INFINITE64) || (tres_req_cnt == NO_VAL64))
            return true;
    } else {
        if ((out_max_32 != INFINITE) || ((uint32_t) max_limit == INFINITE) || ((uint32_t) tres_req_cnt == NO_VAL))
            return true;
    }

    max_time_limit = (uint32_t)(max_limit / tres_req_cnt);

    _set_time_limit(time_limit_in, part_max_time, max_time_limit, limit_set_time);
 ...
 */
static bool
_validate_tres_time_limits(int *tres_pos, uint32_t *time_limit_in, uint32_t part_max_time, uint64_t *job_tres_array,
                           uint64_t *max_tres_array, uint64_t *out_max_tres_array, uint16_t *limit_set_time,
...
//	uint32_t max_time_limit;
    ...
    for (i = 0; i < g_tres_count; i++) {
        ...
        if (!_validate_time_limit(time_limit_in, part_max_time, job_tres_array[i], max_tres_array[i],
                                  &out_max_tres_array[i], limit_set_time, strict_checking, true))
            return false;
 ...
 */
static acct_policy_tres_usage_t
_validate_tres_usage_limits(int *tres_pos, uint64_t *tres_limit_array, uint64_t *out_tres_limit_array,
                            uint64_t *tres_req_cnt, uint64_t *tres_usage, uint64_t *curr_usage,
                            uint16_t *admin_limit_set, bool safe_limits, bool out_tres_limit_set) {
    int i;
    uint64_t usage = 0;
    ...
    for (i = 0; i < g_tres_count; i++) {
        ...
        if ((admin_limit_set && admin_limit_set[i] == ADMIN_SET_LIMIT) ||
            (out_tres_limit_array && out_tres_limit_array[i] != INFINITE64) || (tres_limit_array[i] == INFINITE64))
            continue;
        ...
        if (safe_limits) {
            ...
            if (curr_usage)
                usage = curr_usage[i];
            if (tres_usage && ((tres_req_cnt[i] + tres_usage[i]) > (tres_limit_array[i] - usage)))
                return TRES_USAGE_REQ_NOT_SAFE_WITH_USAGE;
        }
    }
 ...
 */
static acct_policy_tres_usage_t
_validate_tres_usage_limits_for_qos(int *tres_pos, uint64_t *tres_limit_array, uint64_t *out_tres_limit_array,
                                    uint64_t *tres_req_cnt, uint64_t *tres_usage, uint64_t *curr_usage,
                                    uint16_t *admin_limit_set, bool safe_limits) {
    return _validate_tres_usage_limits(tres_pos, tres_limit_array, out_tres_limit_array, tres_req_cnt, tres_usage,
                                       curr_usage, admin_limit_set, safe_limits, true);
}
 ...
 */
static acct_policy_tres_usage_t
_validate_tres_usage_limits_for_assoc(int *tres_pos, uint64_t *tres_limit_array, uint64_t *qos_tres_limit_array,
                                      uint64_t *tres_req_cnt, uint64_t *tres_usage, uint64_t *curr_usage,
                                      uint16_t *admin_limit_set, bool safe_limits) {
    return _validate_tres_usage_limits(tres_pos, tres_limit_array, qos_tres_limit_array, tres_req_cnt, tres_usage,
                                       curr_usage, admin_limit_set, safe_limits, false);
}

static int _qos_policy_validate(job_desc_msg_t *job_desc, slurmdb_assoc_rec_t *assoc_ptr, struct part_record *part_ptr,
                                slurmdb_qos_rec_t *qos_ptr, slurmdb_qos_rec_t *qos_out_ptr, uint32_t *reason,
                                acct_policy_limit_set_t *acct_policy_limit_set, bool update_call, char *user_name,
                                int job_cnt, bool strict_checking) {
    int rc = true;
    int tres_pos = 0;
    ...
    if (!_validate_tres_limits_for_qos(&tres_pos, job_desc->tres_req_cnt, 0, NULL, qos_ptr->max_tres_pa_ctld, NULL,
                                       qos_out_ptr->max_tres_pa_ctld, acct_policy_limit_set->tres, strict_checking,
                                       1)) {
        if (job_desc->tres_req_cnt[tres_pos] > qos_ptr->max_tres_pa_ctld[tres_pos]) {
            if (reason)
                *reason = _get_tres_state_reason(tres_pos, WAIT_QOS_MAX_UNK_PER_ACCT);

            debug2("job submit for user %s(%u): "
                   "min tres(%s) request %"
            PRIu64
            " exceeds "
            "per-acct max tres limit %"
            PRIu64
            " for qos '%s'", user_name, job_desc->user_id, assoc_mgr_tres_name_array[tres_pos], job_desc->tres_req_cnt[tres_pos], qos_ptr->max_tres_pa_ctld[tres_pos], qos_ptr->name);
            rc = false;
            goto end_it;
        }
    }

    if (!_validate_tres_limits_for_qos(&tres_pos, job_desc->tres_req_cnt, 0, qos_ptr->grp_tres_ctld,
                                       qos_ptr->max_tres_pu_ctld, qos_out_ptr->grp_tres_ctld,
                                       qos_out_ptr->max_tres_pu_ctld, acct_policy_limit_set->tres, strict_checking,
                                       1)) {
        if (job_desc->tres_req_cnt[tres_pos] > qos_ptr->max_tres_pu_ctld[tres_pos]) {
            if (reason)
                *reason = _get_tres_state_reason(tres_pos, WAIT_QOS_MAX_UNK_PER_USER);

            debug2("job submit for user %s(%u): "
                   "min tres(%s) request %"
            PRIu64
            " exceeds "
            "per-user max tres limit %"
            PRIu64
            " for qos '%s'", user_name, job_desc->user_id, assoc_mgr_tres_name_array[tres_pos], job_desc->tres_req_cnt[tres_pos], qos_ptr->max_tres_pu_ctld[tres_pos], qos_ptr->name);
            rc = false;
            goto end_it;
        } else if (job_desc->tres_req_cnt[tres_pos] > qos_ptr->grp_tres_ctld[tres_pos]) {
            if (reason)
                *reason = _get_tres_state_reason(tres_pos, WAIT_QOS_GRP_UNK);

            debug2("job submit for user %s(%u): "
                   "min tres(%s) request %"
            PRIu64
            " exceeds "
            "group max tres limit %"
            PRIu64
            " for qos '%s'", user_name, job_desc->user_id, assoc_mgr_tres_name_array[tres_pos], job_desc->tres_req_cnt[tres_pos], qos_ptr->grp_tres_ctld[tres_pos], qos_ptr->name);
            rc = false;
            goto end_it;
        }
    }
    ...
    if ((qos_out_ptr->grp_submit_jobs == INFINITE) && (qos_ptr->grp_submit_jobs != INFINITE)) {

        qos_out_ptr->grp_submit_jobs = qos_ptr->grp_submit_jobs;

        if ((qos_ptr->usage->grp_used_submit_jobs + job_cnt) > qos_ptr->grp_submit_jobs) {
            if (reason)
            ...
            debug2("job submit for user %s(%u): group max submit job limit exceeded %u (used:%u + requested:%d) for qos '%s'",
                   user_name, job_desc->user_id, qos_ptr->grp_submit_jobs, qos_ptr->usage->grp_used_submit_jobs,
                   job_cnt, qos_ptr->name);
            rc = false;
            goto end_it;
        }
    }
    ...
    if (acct_policy_limit_set->time != ADMIN_SET_LIMIT) {
        if (!_validate_tres_time_limits(&tres_pos, &job_desc->time_limit, part_ptr->max_time, job_desc->tres_req_cnt,
                                        qos_ptr->max_tres_mins_pj_ctld, qos_out_ptr->max_tres_mins_pj_ctld,
                                        &acct_policy_limit_set->time, strict_checking)) {
            if (reason)
                *reason = _get_tres_state_reason(tres_pos, WAIT_QOS_MAX_UNK_MINS_PER_JOB);
            debug2("job submit for user %s(%u): "
                   "tres(%s) time limit request %"
            PRIu64
            " "
            "exceeds max per-job limit %"
            PRIu64
            " "
            "for qos '%s'", user_name, job_desc->user_id, assoc_mgr_tres_name_array[tres_pos], (
                    (uint64_t) job_desc->time_limit *
                    job_desc->tres_req_cnt[tres_pos]), qos_ptr->max_tres_mins_pj_ctld[tres_pos], qos_ptr->name);
            rc = false;
            goto end_it;
        }

        if (!_validate_tres_time_limits(&tres_pos, &job_desc->time_limit, part_ptr->max_time, job_desc->tres_req_cnt,
                                        qos_ptr->grp_tres_mins_ctld, qos_out_ptr->grp_tres_mins_ctld,
                                        &acct_policy_limit_set->time, strict_checking)) {
            if (reason)
                *reason = _get_tres_state_reason(tres_pos, WAIT_QOS_GRP_UNK_MIN);
            debug2("job submit for user %s(%u): "
                   "tres(%s) time limit request %"
            PRIu64
            " "
            "exceeds group max limit %"
            PRIu64
            " "
            "for qos '%s'", user_name, job_desc->user_id, assoc_mgr_tres_name_array[tres_pos], (
                    (uint64_t) job_desc->time_limit *
                    job_desc->tres_req_cnt[tres_pos]), qos_ptr->grp_tres_mins_ctld[tres_pos], qos_ptr->name);
            rc = false;
            goto end_it;
        }

        if (!_validate_tres_time_limits(&tres_pos, &job_desc->time_limit, part_ptr->max_time, job_desc->tres_req_cnt,
                                        qos_ptr->grp_tres_run_mins_ctld, qos_out_ptr->grp_tres_run_mins_ctld,
                                        &acct_policy_limit_set->time, strict_checking)) {
            if (reason)
                *reason = _get_tres_state_reason(tres_pos, WAIT_QOS_GRP_UNK_RUN_MIN);
            debug2("job submit for user %s(%u): "
                   "tres(%s) time limit request %"
            PRIu64
            " "
            "exceeds group max running limit %"
            PRIu64
            " "
            "for qos '%s'", user_name, job_desc->user_id, assoc_mgr_tres_name_array[tres_pos], (
                    (uint64_t) job_desc->time_limit *
                    job_desc->tres_req_cnt[tres_pos]), qos_ptr->grp_tres_run_mins_ctld[tres_pos], qos_ptr->name);
            rc = false;
            goto end_it;
        }

        if ((qos_out_ptr->max_wall_pj == INFINITE) && (qos_ptr->max_wall_pj != INFINITE) &&
            (!update_call || (job_desc->time_limit != NO_VAL))) {
            _set_time_limit(&job_desc->time_limit, part_ptr->max_time, qos_ptr->max_wall_pj,
                            &acct_policy_limit_set->time);
            qos_out_ptr->max_wall_pj = qos_ptr->max_wall_pj;

            if (strict_checking && job_desc->time_limit > qos_ptr->max_wall_pj) {
                if (reason)
                    *reason = WAIT_QOS_MAX_WALL_PER_JOB;
                debug2("job submit for user %s(%u): "
                       "time limit %u exceeds qos max %u", user_name, job_desc->user_id, job_desc->time_limit,
                       qos_ptr->max_wall_pj);
                rc = false;
                goto end_it;
            }
        ...
        if ((qos_out_ptr->grp_wall == INFINITE) && (qos_ptr->grp_wall != INFINITE) &&
            (!update_call || (job_desc->time_limit != NO_VAL))) {
            _set_time_limit(&job_desc->time_limit, part_ptr->max_time, qos_ptr->grp_wall, &acct_policy_limit_set->time);

            qos_out_ptr->grp_wall = qos_ptr->grp_wall;

            if (strict_checking && job_desc->time_limit > qos_ptr->grp_wall) {
                if (reason)
                    *reason = WAIT_QOS_GRP_WALL;
                debug2("job submit for user %s(%u): "
                       "time limit %u exceeds qos grp max %u", user_name, job_desc->user_id, job_desc->time_limit,
                       qos_ptr->grp_wall);
                rc = false;
                goto end_it;
            }
    ...
    if (!_validate_tres_limits_for_qos(&tres_pos, job_desc->tres_req_cnt, 0, NULL, qos_ptr->max_tres_pj_ctld, NULL,
                                       qos_out_ptr->max_tres_pj_ctld, acct_policy_limit_set->tres, strict_checking,
                                       1)) {
        if (reason)
            *reason = _get_tres_state_reason(tres_pos, WAIT_QOS_MAX_UNK_PER_JOB);

        debug2("job submit for user %s(%u): "
               "min tres(%s) request %"
        PRIu64
        " exceeds "
        "per-job max tres limit %"
        PRIu64
        " for qos '%s'", user_name, job_desc->user_id, assoc_mgr_tres_name_array[tres_pos], job_desc->tres_req_cnt[tres_pos], qos_ptr->max_tres_pj_ctld[tres_pos], qos_ptr->name);
        rc = false;
        goto end_it;
    }

    if (!_validate_tres_limits_for_qos(&tres_pos, job_desc->tres_req_cnt, job_desc->tres_req_cnt[TRES_ARRAY_NODE], NULL,
                                       qos_ptr->max_tres_pn_ctld, NULL, qos_out_ptr->max_tres_pn_ctld,
                                       acct_policy_limit_set->tres, strict_checking, 1)) {
        if (reason)
            *reason = _get_tres_state_reason(tres_pos, WAIT_QOS_MAX_UNK_PER_NODE);

        debug2("job submit for user %s(%u): "
               "min tres(%s) request %"
        PRIu64
        " exceeds "
        "per-node max tres limit %"
        PRIu64
        " for qos '%s'", user_name, job_desc->user_id, assoc_mgr_tres_name_array[tres_pos],
                job_desc->tres_req_cnt[tres_pos] /
                job_desc->tres_req_cnt[TRES_ARRAY_NODE], qos_ptr->max_tres_pn_ctld[tres_pos], qos_ptr->name);
        rc = false;
        goto end_it;
    }
    ...
    if ((qos_out_ptr->max_submit_jobs_pa == INFINITE) && (qos_ptr->max_submit_jobs_pa != INFINITE)) {
        slurmdb_used_limits_t *used_limits = acct_policy_get_acct_used_limits(&qos_ptr->usage->acct_limit_list,
                                                                              assoc_ptr->acct);

        qos_out_ptr->max_submit_jobs_pa = qos_ptr->max_submit_jobs_pa;

        if ((used_limits->submit_jobs + job_cnt) > qos_ptr->max_submit_jobs_pa) {
            if (reason)
            ...
            debug2("job submit for account %s: qos max submit job limit exceeded %u (used:%u + requested:%d) for qos '%s'",
                   assoc_ptr->acct, qos_ptr->max_submit_jobs_pa, used_limits->submit_jobs, job_cnt, qos_ptr->name);
            rc = false;
            goto end_it;
        }
    }

    if ((qos_out_ptr->max_submit_jobs_pu == INFINITE) && (qos_ptr->max_submit_jobs_pu != INFINITE)) {
        slurmdb_used_limits_t *used_limits = acct_policy_get_user_used_limits(&qos_ptr->usage->user_limit_list,
                                                                              job_desc->user_id);

        qos_out_ptr->max_submit_jobs_pu = qos_ptr->max_submit_jobs_pu;

        if ((used_limits->submit_jobs + job_cnt) > qos_ptr->max_submit_jobs_pu) {
            if (reason)
            ...
            debug2("job submit for user %s(%u): qos max submit job limit exceeded %u (used:%u + requested:%d) for qos '%s'",
                   user_name, job_desc->user_id, qos_ptr->max_submit_jobs_pu, used_limits->submit_jobs, job_cnt,
                   qos_ptr->name);
            rc = false;
            goto end_it;
        }
    ...
    if (!_validate_tres_limits_for_qos(&tres_pos, job_desc->tres_req_cnt, 0, NULL, qos_ptr->min_tres_pj_ctld, NULL,
                                       qos_out_ptr->min_tres_pj_ctld, acct_policy_limit_set->tres, strict_checking,
                                       0)) {
        if (reason)
            *reason = _get_tres_state_reason(tres_pos, WAIT_QOS_MIN_UNK);
        ...
        "per-job max tres limit %"
        PRIu64
        " for qos '%s'", user_name, job_desc->user_id, assoc_mgr_tres_name_array[tres_pos], job_desc->tres_req_cnt[tres_pos], qos_ptr->min_tres_pj_ctld[tres_pos], qos_ptr->name);
        rc = false;
        goto end_it;
    }
...
static int
_qos_job_runnable_pre_select(struct job_record *job_ptr, slurmdb_qos_rec_t *qos_ptr, slurmdb_qos_rec_t *qos_out_ptr) {
    uint32_t wall_mins;
    ...
    used_limits_a = acct_policy_get_acct_used_limits(&qos_ptr->usage->acct_limit_list, assoc_ptr->acct);
    ...
    if ((qos_out_ptr->grp_jobs == INFINITE) && (qos_ptr->grp_jobs != INFINITE)) {
        ...
        if (qos_ptr->usage->grp_used_jobs >= qos_ptr->grp_jobs) {
            ...
            debug2("%pJ being held, the job is at or exceeds group max jobs limit %u with %u for QOS %s", job_ptr,
                   qos_ptr->grp_jobs, qos_ptr->usage->grp_used_jobs, qos_ptr->name);
    ...
    if ((job_ptr->limit_set.time != ADMIN_SET_LIMIT) && (qos_out_ptr->grp_wall == INFINITE) &&
        (qos_ptr->grp_wall != INFINITE)) {
        if (time_limit == NO_VAL) {
            time_limit = job_ptr->time_limit;
            _set_time_limit(&time_limit, job_ptr->part_ptr->max_time, MIN(qos_ptr->grp_wall, qos_ptr->max_wall_pj),
                            &job_ptr->limit_set.time);

            if ((job_ptr->qos_ptr && (job_ptr->qos_ptr->usage_factor >= 0)) &&
                ((time_limit != INFINITE) || (job_ptr->qos_ptr->usage_factor < 1.0))) {
        ...
        if (wall_mins >= qos_ptr->grp_wall) {
            ...
            debug2("%pJ being held, the job is at or exceeds group wall limit %u with %u for QOS %s", job_ptr,
                   qos_ptr->grp_wall, wall_mins, qos_ptr->name);
            rc = false;
            goto end_it;
        } else if (safe_limits && ((wall_mins + time_limit) > qos_ptr->grp_wall)) {
            xfree(job_ptr->state_desc);
            job_ptr->state_reason = WAIT_QOS_GRP_WALL;
            debug2("%pJ being held, the job request will exceed group wall limit %u if ran with %u for QOS %s", job_ptr,
                   qos_ptr->grp_wall, wall_mins + time_limit, qos_ptr->name);
            rc = false;
            goto end_it;
        }
    }
    ...
    if ((qos_out_ptr->max_jobs_pa == INFINITE) && (qos_ptr->max_jobs_pa != INFINITE)) {

        qos_out_ptr->max_jobs_pa = qos_ptr->max_jobs_pa;

        if (used_limits_a->jobs >= qos_ptr->max_jobs_pa) {
            xfree(job_ptr->state_desc);
            job_ptr->state_reason = WAIT_QOS_MAX_JOB_PER_ACCT;
            debug2("%pJ being held, the job is at or exceeds max jobs per-acct (%s) limit %u with %u for QOS %s",
                   job_ptr, used_limits_a->acct, qos_ptr->max_jobs_pa, used_limits_a->jobs, qos_ptr->name);
            rc = false;
            goto end_it;
        }
    }

    if ((qos_out_ptr->max_jobs_pu == INFINITE) && (qos_ptr->max_jobs_pu != INFINITE)) {

        qos_out_ptr->max_jobs_pu = qos_ptr->max_jobs_pu;

        if (used_limits->jobs >= qos_ptr->max_jobs_pu) {
            xfree(job_ptr->state_desc);
            job_ptr->state_reason = WAIT_QOS_MAX_JOB_PER_USER;
            debug2("%pJ being held, the job is at or exceeds max jobs per-user limit %u with %u for QOS %s", job_ptr,
                   qos_ptr->max_jobs_pu, used_limits->jobs, qos_ptr->name);
            rc = false;
            goto end_it;
        }
    }
	 ...
	 */
    if ((job_ptr->limit_set.time != ADMIN_SET_LIMIT) && (qos_out_ptr->max_wall_pj == INFINITE) &&
        (qos_ptr->max_wall_pj != INFINITE)) {
        if (time_limit == NO_VAL) {
            time_limit = job_ptr->time_limit;
            _set_time_limit(&time_limit, job_ptr->part_ptr->max_time, qos_ptr->max_wall_pj, &job_ptr->limit_set.time);
        }
        ...
        if (time_limit > qos_out_ptr->max_wall_pj) {
            ...
            debug2("%pJ being held, time limit %u exceeds QOS max wall pj %u", job_ptr, time_limit,
...
static int
_qos_job_runnable_post_select(struct job_record *job_ptr, slurmdb_qos_rec_t *qos_ptr, slurmdb_qos_rec_t *qos_out_ptr,
                              uint64_t *tres_req_cnt, uint64_t *job_tres_time_limit) {
    ...
    for (i = 0; i < slurmctld_tres_cnt; i++) {
        tres_run_mins[i] = qos_ptr->usage->grp_used_tres_run_secs[i] / 60;
        tres_usage_mins[i] = (uint64_t)(qos_ptr->usage->usage_tres_raw[i] / 60.0);
    ...
    tres_usage = _validate_tres_usage_limits_for_qos(&tres_pos, qos_ptr->grp_tres_mins_ctld,
    ...
    switch (tres_usage) {
        case TRES_USAGE_CUR_EXCEEDS_LIMIT:
            xfree(job_ptr->state_desc);
            job_ptr->state_reason = _get_tres_state_reason(tres_pos, WAIT_QOS_GRP_UNK_MIN);
            debug2("%pJ being held, QOS %s group max tres(%s) minutes limit of %"
            PRIu64
            " is already at or exceeded with %"
            PRIu64, job_ptr, qos_ptr->name, assoc_mgr_tres_name_array[tres_pos], qos_ptr->grp_tres_mins_ctld[tres_pos], tres_usage_mins[tres_pos]);
            rc = false;
            goto end_it;
            break;
        case TRES_USAGE_REQ_EXCEEDS_LIMIT:
            xfree(job_ptr->state_desc);
            job_ptr->state_reason = _get_tres_state_reason(tres_pos, WAIT_QOS_GRP_UNK_MIN);
            debug2("%pJ being held, the job is requesting more than allowed with QOS %s's group max tres(%s) minutes of %"
            PRIu64
            " with %"
            PRIu64, job_ptr, qos_ptr->name, assoc_mgr_tres_name_array[tres_pos], qos_ptr->grp_tres_mins_ctld[tres_pos], job_tres_time_limit[tres_pos]);
            rc = false;
            goto end_it;
            break;
		 ...
		 */
            xfree(job_ptr->state_desc);
            job_ptr->state_reason = _get_tres_state_reason(tres_pos, WAIT_QOS_GRP_UNK_MIN);
            debug2("%pJ being held, the job is at or exceeds QOS %s's group max tres(%s) minutes of %"
            PRIu64
            " of which %"
            PRIu64
            " are still available but request is for %"
            PRIu64
            " (plus %"
            PRIu64
            " already in use) tres minutes (request tres count %"
            PRIu64
            ")", job_ptr, qos_ptr->name, assoc_mgr_tres_name_array[tres_pos], qos_ptr->grp_tres_mins_ctld[tres_pos],
                    qos_ptr->grp_tres_mins_ctld[tres_pos] -
                    tres_usage_mins[tres_pos], job_tres_time_limit[tres_pos], tres_run_mins[tres_pos], tres_req_cnt[tres_pos]);
            rc = false;
            goto end_it;
            break;
    ...
    _get_unique_job_node_cnt(job_ptr, qos_ptr->usage->grp_node_bitmap, &tres_req_cnt[TRES_ARRAY_NODE]);
    tres_usage = _validate_tres_usage_limits_for_qos(&tres_pos, qos_ptr->grp_tres_ctld, qos_out_ptr->grp_tres_ctld,
                                                     tres_req_cnt, qos_ptr->usage->grp_used_tres, NULL,
                                                     job_ptr->limit_set.tres, true);
    ...
    switch (tres_usage) {
        ...
        case TRES_USAGE_REQ_EXCEEDS_LIMIT:
            xfree(job_ptr->state_desc);
            job_ptr->state_reason = _get_tres_state_reason(tres_pos, WAIT_QOS_GRP_UNK);
            debug2("%pJ is being held, QOS %s min tres(%s) request %"
            PRIu64
            " exceeds group max tres limit %"
            PRIu64, job_ptr, qos_ptr->name, assoc_mgr_tres_name_array[tres_pos], tres_req_cnt[tres_pos], qos_ptr->grp_tres_ctld[tres_pos]);
            rc = false;
            goto end_it;
            break;
        case TRES_USAGE_REQ_NOT_SAFE_WITH_USAGE:
            xfree(job_ptr->state_desc);
            job_ptr->state_reason = _get_tres_state_reason(tres_pos, WAIT_QOS_GRP_UNK);
            debug2("%pJ being held, if allowed the job request will exceed QOS %s group max tres(%s) limit %"
            PRIu64
            " with already used %"
            PRIu64
            " + requested %"
            PRIu64, job_ptr, qos_ptr->name, assoc_mgr_tres_name_array[tres_pos], qos_ptr->grp_tres_ctld[tres_pos], qos_ptr->usage->grp_used_tres[tres_pos], tres_req_cnt[tres_pos]);
            rc = false;
            goto end_it;
        case TRES_USAGE_OKAY:
            /* all good */
    ...
    tres_usage = _validate_tres_usage_limits_for_qos(&tres_pos, qos_ptr->grp_tres_run_mins_ctld,
                                                     qos_out_ptr->grp_tres_run_mins_ctld, job_tres_time_limit,
                                                     tres_run_mins, NULL, NULL, true);
    switch (tres_usage) {
        case TRES_USAGE_CUR_EXCEEDS_LIMIT:
            /* not possible because the curr_usage sent in is NULL */
            break;
        case TRES_USAGE_REQ_EXCEEDS_LIMIT:
            xfree(job_ptr->state_desc);
            job_ptr->state_reason = _get_tres_state_reason(tres_pos, WAIT_QOS_GRP_UNK_RUN_MIN);
            debug2("%pJ is being held, QOS %s group max running tres(%s) minutes request %"
            PRIu64
            " exceeds limit %"
            PRIu64, job_ptr, qos_ptr->name, assoc_mgr_tres_name_array[tres_pos], job_tres_time_limit[tres_pos], qos_ptr->grp_tres_run_mins_ctld[tres_pos]);
            rc = false;
            goto end_it;
            break;
        case TRES_USAGE_REQ_NOT_SAFE_WITH_USAGE:
            xfree(job_ptr->state_desc);
            job_ptr->state_reason = _get_tres_state_reason(tres_pos, WAIT_QOS_GRP_UNK_RUN_MIN);
            debug2("%pJ being held, if allowed the job request will exceed QOS %s group max running tres(%s) minutes limit %"
            PRIu64
            " with already used %"
            PRIu64
            " + requested %"
            PRIu64, job_ptr, qos_ptr->name, assoc_mgr_tres_name_array[tres_pos], qos_ptr->grp_tres_run_mins_ctld[tres_pos], tres_run_mins[tres_pos], job_tres_time_limit[tres_pos]);
            rc = false;
            goto end_it;
            break;
        case TRES_USAGE_OKAY:
    ...
    if (!_validate_tres_limits_for_qos(&tres_pos, job_tres_time_limit, 0, NULL, qos_ptr->max_tres_mins_pj_ctld, NULL,
                                       qos_out_ptr->max_tres_mins_pj_ctld, job_ptr->limit_set.tres, 1, 1)) {
        xfree(job_ptr->state_desc);
        job_ptr->state_reason = _get_tres_state_reason(tres_pos, WAIT_QOS_MAX_UNK_MINS_PER_JOB);
        debug2("%pJ being held, the job is requesting more than allowed with QOS %s's max tres(%s) minutes of %"
        PRIu64
        " with %"
        PRIu64, job_ptr, qos_ptr->name, assoc_mgr_tres_name_array[tres_pos], qos_ptr->max_tres_mins_pj_ctld[tres_pos], job_tres_time_limit[tres_pos]);
        rc = false;
        goto end_it;
    }

    if (!_validate_tres_limits_for_qos(&tres_pos, tres_req_cnt, 0, NULL, qos_ptr->max_tres_pj_ctld, NULL,
                                       qos_out_ptr->max_tres_pj_ctld, job_ptr->limit_set.tres, 1, 1)) {
        xfree(job_ptr->state_desc);
        job_ptr->state_reason = _get_tres_state_reason(tres_pos, WAIT_QOS_MAX_UNK_PER_JOB);
        debug2("%pJ is being held, QOS %s min tres(%s) per job request %"
        PRIu64
        " exceeds max tres limit %"
        PRIu64, job_ptr, qos_ptr->name, assoc_mgr_tres_name_array[tres_pos], tres_req_cnt[tres_pos], qos_ptr->max_tres_pj_ctld[tres_pos]);
        rc = false;
        goto end_it;
    }

    if (!_validate_tres_limits_for_qos(&tres_pos, tres_req_cnt, tres_req_cnt[TRES_ARRAY_NODE], NULL,
                                       qos_ptr->max_tres_pn_ctld, NULL, qos_out_ptr->max_tres_pn_ctld,
                                       job_ptr->limit_set.tres, 1, 1)) {
        uint64_t req_per_node;
        xfree(job_ptr->state_desc);
        job_ptr->state_reason = _get_tres_state_reason(tres_pos, WAIT_QOS_MAX_UNK_PER_NODE);
        req_per_node = tres_req_cnt[tres_pos];
        if (tres_req_cnt[TRES_ARRAY_NODE] > 1)
            req_per_node /= tres_req_cnt[TRES_ARRAY_NODE];
        debug2("%pJ is being held, QOS %s min tres(%s) per node request %"
        PRIu64
        " exceeds max tres limit %"
        PRIu64, job_ptr, qos_ptr->name, assoc_mgr_tres_name_array[tres_pos], req_per_node, qos_ptr->max_tres_pn_ctld[tres_pos]);
        rc = false;
        goto end_it;
    }

    if (!_validate_tres_limits_for_qos(&tres_pos, tres_req_cnt, 0, NULL, qos_ptr->min_tres_pj_ctld, NULL,
                                       qos_out_ptr->min_tres_pj_ctld, job_ptr->limit_set.tres, 1, 0)) {
        xfree(job_ptr->state_desc);
        job_ptr->state_reason = _get_tres_state_reason(tres_pos, WAIT_QOS_MIN_UNK);
        debug2("%pJ is being held, QOS %s min tres(%s) per job request %"
        PRIu64
        " exceeds min tres limit %"
        PRIu64, job_ptr, qos_ptr->name, assoc_mgr_tres_name_array[tres_pos], tres_req_cnt[tres_pos], qos_ptr->min_tres_pj_ctld[tres_pos]);
        rc = false;
        goto end_it;
    }

    orig_node_cnt = tres_req_cnt[TRES_ARRAY_NODE];
    _get_unique_job_node_cnt(job_ptr, used_limits_a->node_bitmap, &tres_req_cnt[TRES_ARRAY_NODE]);
    tres_usage = _validate_tres_usage_limits_for_qos(&tres_pos, qos_ptr->max_tres_pa_ctld,
                                                     qos_out_ptr->max_tres_pa_ctld, tres_req_cnt, used_limits_a->tres,
    ...
    switch (tres_usage) {
		 ...
		 */
            xfree(job_ptr->state_desc);
            job_ptr->state_reason = _get_tres_state_reason(tres_pos, WAIT_QOS_MAX_UNK_PER_ACCT);
            debug2("%pJ is being held, QOS %s min tres(%s) request %"
            PRIu64
            " exceeds max tres per account (%s) limit %"
            PRIu64, job_ptr, qos_ptr->name, assoc_mgr_tres_name_array[tres_pos], tres_req_cnt[tres_pos], used_limits_a->acct, qos_ptr->max_tres_pa_ctld[tres_pos]);
            rc = false;
            goto end_it;
            break;
        case TRES_USAGE_REQ_NOT_SAFE_WITH_USAGE:
		 ...
		 */
            xfree(job_ptr->state_desc);
            job_ptr->state_reason = _get_tres_state_reason(tres_pos, WAIT_QOS_MAX_UNK_PER_ACCT);
            debug2("%pJ being held, if allowed the job request will exceed QOS %s max tres(%s) per account (%s) limit %"
            PRIu64
            " with already used %"
            PRIu64
            " + requested %"
            PRIu64, job_ptr, qos_ptr->name, assoc_mgr_tres_name_array[tres_pos], used_limits_a->acct, qos_ptr->max_tres_pa_ctld[tres_pos], used_limits_a->tres[tres_pos], tres_req_cnt[tres_pos]);
            rc = false;
            goto end_it;
        case TRES_USAGE_OKAY:
            /* all good */
    ...
    orig_node_cnt = tres_req_cnt[TRES_ARRAY_NODE];
    _get_unique_job_node_cnt(job_ptr, used_limits->node_bitmap, &tres_req_cnt[TRES_ARRAY_NODE]);
    tres_usage = _validate_tres_usage_limits_for_qos(&tres_pos, qos_ptr->max_tres_pu_ctld,
                                                     qos_out_ptr->max_tres_pu_ctld, tres_req_cnt, used_limits->tres,
                                                     NULL, job_ptr->limit_set.tres, true);
    tres_req_cnt[TRES_ARRAY_NODE] = orig_node_cnt;
    switch (tres_usage) {
        case TRES_USAGE_CUR_EXCEEDS_LIMIT:
		 ...
		 */
            xfree(job_ptr->state_desc);
            job_ptr->state_reason = _get_tres_state_reason(tres_pos, WAIT_QOS_MAX_UNK_PER_USER);
            debug2("%pJ is being held, QOS %s min tres(%s) request %"
            PRIu64
            " exceeds max tres per user limit %"
            PRIu64, job_ptr, qos_ptr->name, assoc_mgr_tres_name_array[tres_pos], tres_req_cnt[tres_pos], qos_ptr->max_tres_pu_ctld[tres_pos]);
            rc = false;
            goto end_it;
            break;
        case TRES_USAGE_REQ_NOT_SAFE_WITH_USAGE:
		 ...
		 */
            xfree(job_ptr->state_desc);
            job_ptr->state_reason = _get_tres_state_reason(tres_pos, WAIT_QOS_MAX_UNK_PER_USER);
            debug2("%pJ being held, if allowed the job request will exceed QOS %s max tres(%s) per user limit %"
            PRIu64
            " with already used %"
            PRIu64
            " + requested %"
            PRIu64, job_ptr, qos_ptr->name, assoc_mgr_tres_name_array[tres_pos], qos_ptr->max_tres_pu_ctld[tres_pos], used_limits->tres[tres_pos], tres_req_cnt[tres_pos]);
            rc = false;
            goto end_it;
        case TRES_USAGE_OKAY:
            /* all good */
...
static int _qos_job_time_out(struct job_record *job_ptr, slurmdb_qos_rec_t *qos_ptr, slurmdb_qos_rec_t *qos_out_ptr,
    ...
    tres_usage = _validate_tres_usage_limits_for_qos(&tres_pos, qos_ptr->grp_tres_mins_ctld,
                                                     qos_out_ptr->grp_tres_mins_ctld, NULL, NULL, tres_usage_mins, NULL,
                                                     false);
    switch (tres_usage) {
        case TRES_USAGE_CUR_EXCEEDS_LIMIT:
            last_job_update = now;
            info("%pJ timed out, the job is at or exceeds QOS %s's group max tres(%s) minutes of %"
            PRIu64
            " with %"
            PRIu64
            "", job_ptr, qos_ptr->name, assoc_mgr_tres_name_array[tres_pos], qos_ptr->grp_tres_mins_ctld[tres_pos], tres_usage_mins[tres_pos]);
    ...
    if ((qos_out_ptr->grp_wall == INFINITE) && (qos_ptr->grp_wall != INFINITE)) {
        ...
        if (wall_mins >= qos_ptr->grp_wall) {
            last_job_update = now;
            info("%pJ timed out, the job is at or exceeds QOS %s's group wall limit of %u with %u", job_ptr,
    ...
    tres_usage = _validate_tres_usage_limits_for_qos(&tres_pos, qos_ptr->max_tres_mins_pj_ctld,
                                                     qos_out_ptr->max_tres_mins_pj_ctld, job_tres_usage_mins, NULL,
                                                     NULL, NULL, true);
    switch (tres_usage) {
        case TRES_USAGE_CUR_EXCEEDS_LIMIT:
        ...
        case TRES_USAGE_REQ_EXCEEDS_LIMIT:
            last_job_update = now;
            info("%pJ timed out, the job is at or exceeds QOS %s's max tres(%s) minutes of %"
            PRIu64
            " with %"
            PRIu64, job_ptr, qos_ptr->name, assoc_mgr_tres_name_array[tres_pos], qos_ptr->max_tres_mins_pj_ctld[tres_pos], job_tres_usage_mins[tres_pos]);
            job_ptr->state_reason = FAIL_TIMEOUT;
            rc = false;
            goto end_it;
            break;
...
extern void acct_policy_alter_job(struct job_record *job_ptr, uint32_t new_time_limit) {
    ...
    for (i = 0; i < slurmctld_tres_cnt; i++) {
        if (i == TRES_ARRAY_ENERGY)
            continue;
        used_tres_run_secs[i] = job_ptr->tres_alloc_cnt[i] * time_limit_secs;
        new_used_tres_run_secs[i] = job_ptr->tres_alloc_cnt[i] * new_time_limit_secs;
    }
    ...
    _qos_alter_job(job_ptr, qos_ptr_1, used_tres_run_secs, new_used_tres_run_secs);
    _qos_alter_job(job_ptr, qos_ptr_2, used_tres_run_secs, new_used_tres_run_secs);
    ...
    while (assoc_ptr) {
        for (i = 0; i < slurmctld_tres_cnt; i++) {
            if (used_tres_run_secs[i] == new_used_tres_run_secs[i])
                continue;
            assoc_ptr->usage->grp_used_tres_run_secs[i] -= used_tres_run_secs[i];
            assoc_ptr->usage->grp_used_tres_run_secs[i] += new_used_tres_run_secs[i];
            debug2("altering %pJ assoc %u(%s/%s/%s) got %"
            PRIu64
            " just removed %"
            PRIu64
            " and added %"
            PRIu64, job_ptr, assoc_ptr->id, assoc_ptr->acct, assoc_ptr->user, assoc_ptr->partition, assoc_ptr->usage->grp_used_tres_run_secs[i], used_tres_run_secs[i], new_used_tres_run_secs[i]);
        }
...
static void _get_accrue_create_cnt(uint32_t *max_jobs_accrue, int *create_cnt, uint32_t in_accrue, uint32_t in_used) {
    /*
...
static void _add_accrue_time_internal(slurmdb_assoc_rec_t *assoc_ptr, slurmdb_qos_rec_t *qos_ptr_1,
                                      slurmdb_used_limits_t *used_limits_a1, slurmdb_used_limits_t *used_limits_u1,
                                      slurmdb_qos_rec_t *qos_ptr_2, slurmdb_used_limits_t *used_limits_a2,
                                      slurmdb_used_limits_t *used_limits_u2, int cnt) {
    if (slurmctld_conf.debug_flags & DEBUG_FLAG_ACCRUE) {
        info("%s: Adding %d to assoc_ptr %p (%p %p %p %p %p %p)", __func__, cnt, assoc_ptr, qos_ptr_1, used_limits_a1,
    ...
    while (assoc_ptr) {
        if (slurmctld_conf.debug_flags & DEBUG_FLAG_ACCRUE) {
            info("assoc_id %u(%s/%s/%s/%p) added %d count %d", assoc_ptr->id, assoc_ptr->acct, assoc_ptr->user,
                 assoc_ptr->partition, assoc_ptr->usage, cnt, assoc_ptr->usage->accrue_cnt);
        }
...
static void _remove_accrue_time_internal(slurmdb_assoc_rec_t *assoc_ptr, slurmdb_qos_rec_t *qos_ptr_1,
                                         slurmdb_used_limits_t *used_limits_a1, slurmdb_used_limits_t *used_limits_u1,
                                         slurmdb_qos_rec_t *qos_ptr_2, slurmdb_used_limits_t *used_limits_a2,
                                         slurmdb_used_limits_t *used_limits_u2, int cnt) {
    if (slurmctld_conf.debug_flags & DEBUG_FLAG_ACCRUE) {
        info("%s: Removing %d from assoc_ptr %p (%p %p %p %p %p %p)", __func__, cnt, assoc_ptr, qos_ptr_1,
             used_limits_a1, used_limits_u1, qos_ptr_2, used_limits_a2, used_limits_u2);
    }
    ...
    if (qos_ptr_1) {
        if (qos_ptr_1->usage->accrue_cnt >= cnt)
            ...
            error("%s: QOS %s accrue_cnt underflow", __func__, qos_ptr_1->name);
    ...
    if (used_limits_a1) {
        if (used_limits_a1->accrue_cnt >= cnt)
            ...
            if (qos_ptr_1) {
                error("%s: QOS %s acct %s accrue_cnt underflow", __func__, qos_ptr_1->name, used_limits_a1->acct);
            }
    ...
    if (used_limits_u1) {
        if (used_limits_u1->accrue_cnt >= cnt)
            ...
            if (qos_ptr_1) {
                error("%s: QOS %s user %u accrue_cnt underflow", __func__, qos_ptr_1->name, used_limits_u1->uid);
            }
    ...
    if (qos_ptr_2) {
        if (qos_ptr_2->usage->accrue_cnt)
            ...
            error("%s: QOS %s accrue_cnt underflow", __func__, qos_ptr_2->name);
    ...
    if (used_limits_a2) {
        if (used_limits_a2->accrue_cnt >= cnt)
            ...
            if (qos_ptr_2) {
                error("%s: QOS %s acct %s accrue_cnt underflow", __func__, qos_ptr_2->name, used_limits_a2->acct);
            }
    ...
    if (used_limits_u2) {
        if (used_limits_u2->accrue_cnt >= cnt)
            ...
            if (qos_ptr_2 && used_limits_a2) {
                error("%s: QOS %s user %u accrue_cnt underflow", __func__, qos_ptr_2->name, used_limits_a2->uid);
            }
    ...
    while (assoc_ptr) {
        if (assoc_ptr->usage->accrue_cnt >= cnt) {
            if (slurmctld_conf.debug_flags & DEBUG_FLAG_ACCRUE) {
                info("assoc_id %u(%s/%s/%s/%p) removed %d count %d", assoc_ptr->id, assoc_ptr->acct, assoc_ptr->user,
                     assoc_ptr->partition, assoc_ptr->usage, cnt, assoc_ptr->usage->accrue_cnt);
            }
            assoc_ptr->usage->accrue_cnt -= cnt;
        } else {
            error("%s: assoc_id %u(%s/%s/%s) accrue_cnt underflow", __func__, assoc_ptr->id, assoc_ptr->acct,
                  assoc_ptr->user, assoc_ptr->partition);
...
static bool _acct_policy_validate(job_desc_msg_t *job_desc, struct part_record *part_ptr, slurmdb_assoc_rec_t *assoc_in,
                                  slurmdb_qos_rec_t *qos_ptr_1, slurmdb_qos_rec_t *qos_ptr_2, uint32_t *reason,
    ...
    char *user_name = NULL;
    bool rc = true;
    assoc_mgr_lock_t locks = {.assoc = READ_LOCK, .qos = READ_LOCK, .tres = READ_LOCK};
    bool strict_checking;
    ...
    if (qos_ptr_1) {
        ...
        if (!(rc = _qos_policy_validate(job_desc, assoc_ptr, part_ptr, qos_ptr_1, &qos_rec, reason,
                                        acct_policy_limit_set, update_call, user_name, job_cnt, strict_checking)))
            goto end_it;
        if (!(rc = _qos_policy_validate(job_desc, assoc_ptr, part_ptr, qos_ptr_2, &qos_rec, reason,
                                        acct_policy_limit_set, update_call, user_name, job_cnt, strict_checking)))
            goto end_it;
    ...
    while (assoc_ptr) {
        ...
        if (!_validate_tres_limits_for_assoc(&tres_pos, job_desc->tres_req_cnt, 0, assoc_ptr->grp_tres_ctld,
                                             qos_rec.grp_tres_ctld, acct_policy_limit_set->tres, strict_checking,
                                             update_call, 1)) {
            if (reason)
                *reason = _get_tres_state_reason(tres_pos, WAIT_ASSOC_GRP_UNK);
            ...
            "group max tres limit %"
            PRIu64
            " for account %s", user_name, job_desc->user_id, assoc_mgr_tres_name_array[tres_pos], job_desc->tres_req_cnt[tres_pos], assoc_ptr->grp_tres_ctld[tres_pos], assoc_ptr->acct);
            rc = false;
            break;
        }
        ...
        if ((qos_rec.grp_submit_jobs == INFINITE) && (assoc_ptr->grp_submit_jobs != INFINITE) &&
            ((assoc_ptr->usage->used_submit_jobs + job_cnt) > assoc_ptr->grp_submit_jobs)) {
            if (reason)
            ...
            debug2("job submit for user %s(%u): group max submit job limit exceeded %u (used:%u + requested:%d) for account '%s'",
                   user_name, job_desc->user_id, assoc_ptr->grp_submit_jobs, assoc_ptr->usage->used_submit_jobs,
                   job_cnt, assoc_ptr->acct);
            rc = false;
            break;
        }
        ...
        tres_pos = 0;
        if (!update_call &&
            !_validate_tres_time_limits(&tres_pos, &job_desc->time_limit, part_ptr->max_time, job_desc->tres_req_cnt,
                                        assoc_ptr->grp_tres_mins_ctld, qos_rec.grp_tres_mins_ctld,
                                        &acct_policy_limit_set->time, strict_checking)) {
            if (reason)
                *reason = _get_tres_state_reason(tres_pos, WAIT_ASSOC_GRP_UNK_MIN);
            debug2("job submit for user %s(%u): "
                   "tres(%s) time limit request %"
            PRIu64
            " "
            ...
            PRIu64
            " "
            "for account '%s'", user_name, job_desc->user_id, assoc_mgr_tres_name_array[tres_pos], (
                    (uint64_t) job_desc->time_limit *
                    job_desc->tres_req_cnt[tres_pos]), assoc_ptr->grp_tres_mins_ctld[tres_pos], assoc_ptr->acct);
            rc = false;
            goto end_it;
        }
        ...
        if (!update_call &&
            !_validate_tres_time_limits(&tres_pos, &job_desc->time_limit, part_ptr->max_time, job_desc->tres_req_cnt,
                                        assoc_ptr->grp_tres_run_mins_ctld, qos_rec.grp_tres_run_mins_ctld,
                                        &acct_policy_limit_set->time, strict_checking)) {
            if (reason)
                *reason = _get_tres_state_reason(tres_pos, WAIT_ASSOC_GRP_UNK_RUN_MIN);
            debug2("job submit for user %s(%u): "
                   "tres(%s) time limit request %"
            PRIu64
            " "
            ...
            " for account '%s'", user_name, job_desc->user_id, assoc_mgr_tres_name_array[tres_pos], (
                    (uint64_t) job_desc->time_limit *
                    job_desc->tres_req_cnt[tres_pos]), assoc_ptr->grp_tres_run_mins_ctld[tres_pos], assoc_ptr->acct);
            rc = false;
            goto end_it;
        }

        if (!update_call &&
            !_validate_time_limit(&job_desc->time_limit, part_ptr->max_time, 1, assoc_ptr->grp_wall, &qos_rec.grp_wall,
                                  &acct_policy_limit_set->time, strict_checking, false)) {
            if (reason)
                *reason = WAIT_ASSOC_GRP_WALL;
            debug2("job submit for user %s(%u): "
                   "time limit %u exceeds max group %u for "
                   "account '%s'", user_name, job_desc->user_id, job_desc->time_limit, assoc_ptr->grp_wall,
                   assoc_ptr->acct);
            rc = false;
            break;
        }
        ...
        tres_pos = 0;
        if (!_validate_tres_limits_for_assoc(&tres_pos, job_desc->tres_req_cnt, 0, assoc_ptr->max_tres_ctld,
                                             qos_rec.max_tres_pj_ctld, acct_policy_limit_set->tres, strict_checking,
                                             update_call, 1)) {
            if (reason)
                *reason = _get_tres_state_reason(tres_pos, WAIT_ASSOC_MAX_UNK_PER_JOB);
            ...
            "max tres limit %"
            PRIu64
            " for account %s", user_name, job_desc->user_id, assoc_mgr_tres_name_array[tres_pos], job_desc->tres_req_cnt[tres_pos], assoc_ptr->max_tres_ctld[tres_pos], assoc_ptr->acct);
            rc = false;
            break;
        }

        tres_pos = 0;
        if (!_validate_tres_limits_for_assoc(&tres_pos, job_desc->tres_req_cnt, job_desc->tres_req_cnt[TRES_ARRAY_NODE],
                                             assoc_ptr->max_tres_pn_ctld, qos_rec.max_tres_pn_ctld,
                                             acct_policy_limit_set->tres, strict_checking, update_call, 1)) {
            if (reason)
                *reason = _get_tres_state_reason(tres_pos, WAIT_ASSOC_MAX_UNK_PER_NODE);
            ...
            "for account %s", user_name, job_desc->user_id, assoc_mgr_tres_name_array[tres_pos],
                    job_desc->tres_req_cnt[tres_pos] /
                    job_desc->tres_req_cnt[TRES_ARRAY_NODE], assoc_ptr->max_tres_pn_ctld[tres_pos], assoc_ptr->acct);
            rc = false;
            break;
        }
        ...
        if ((qos_rec.max_submit_jobs_pa == INFINITE) && (qos_rec.max_submit_jobs_pu == INFINITE) &&
            ...
            debug2("job submit for user %s(%u): account max submit job limit exceeded %u (used:%u + requested:%d) for account '%s'",
                   user_name, job_desc->user_id, assoc_ptr->max_submit_jobs, assoc_ptr->usage->used_submit_jobs,
                   job_cnt, assoc_ptr->acct);
            rc = false;
            break;
        }
        ...
        if (!update_call &&
            !_validate_tres_time_limits(&tres_pos, &job_desc->time_limit, part_ptr->max_time, job_desc->tres_req_cnt,
                                        assoc_ptr->max_tres_mins_ctld, qos_rec.max_tres_mins_pj_ctld,
                                        &acct_policy_limit_set->time, strict_checking)) {
            if (reason)
                *reason = _get_tres_state_reason(tres_pos, WAIT_ASSOC_MAX_UNK_MINS_PER_JOB);
            debug2("job submit for user %s(%u): "
                   "tres(%s) time limit request %"
            PRIu64
            " "
            ...
            "for account '%s'", user_name, job_desc->user_id, assoc_mgr_tres_name_array[tres_pos], (
                    (uint64_t) job_desc->time_limit *
                    job_desc->tres_req_cnt[tres_pos]), assoc_ptr->max_tres_mins_ctld[tres_pos], assoc_ptr->acct);
            rc = false;
            break;
        }

        if (!update_call && !_validate_time_limit(&job_desc->time_limit, part_ptr->max_time, 1, assoc_ptr->max_wall_pj,
                                                  &qos_rec.max_wall_pj, &acct_policy_limit_set->time, strict_checking,
                                                  false)) {
            if (reason)
                *reason = WAIT_ASSOC_MAX_WALL_PER_JOB;
            debug2("job submit for user %s(%u): "
                   "time limit %u exceeds max %u for "
                   "account '%s'", user_name, job_desc->user_id, job_desc->time_limit, assoc_ptr->max_wall_pj,
                   assoc_ptr->acct);
            rc = false;
            break;
        }
...
extern bool acct_policy_validate(job_desc_msg_t *job_desc, struct part_record *part_ptr, slurmdb_assoc_rec_t *assoc_in,
                                 slurmdb_qos_rec_t *qos_ptr, uint32_t *reason,
                                 acct_policy_limit_set_t *acct_policy_limit_set, bool update_call) {
    ...
    assoc_mgr_unlock(&locks);
    rc = _acct_policy_validate(job_desc, part_ptr, assoc_in, qos_ptr_1, qos_ptr_2, reason, acct_policy_limit_set,
                               update_call);
    return rc;
}
...
extern bool acct_policy_validate_pack(List submit_job_list) {
    ...
    acct_policy_limit_set.tres = xmalloc(sizeof(uint16_t) * slurmctld_tres_cnt);
    ...
    while ((job_limit1 = (pack_limits_t *) list_next(iter1))) {
        ...
        if (job_limit1->assoc_ptr) {
            job_cnt = 1;
            memcpy(job_desc.tres_req_cnt, job_ptr1->tres_req_cnt, tres_req_size);
            iter2 = list_iterator_create(pack_limit_list);
            while ((job_limit2 = (pack_limits_t *) list_next(iter2))) {
                if ((job_limit2 == job_limit1) || (job_limit2->assoc_ptr != job_limit1->assoc_ptr))
                    continue;
                job_ptr2 = job_limit2->job_ptr;
                for (i = 0; i < g_tres_count; i++) {
                    job_desc.tres_req_cnt[i] += job_ptr2->tres_req_cnt[i];
                }
                job_cnt++;
            }
            ...
            if (job_cnt > 1) {
				...
				*/
                rc = _acct_policy_validate(&job_desc, job_ptr1->part_ptr, job_limit1->assoc_ptr, job_limit1->qos_ptr_1,
                                           job_limit1->qos_ptr_2, &reason, &acct_policy_limit_set, false);
...
extern bool acct_policy_job_runnable_state(struct job_record *job_ptr) {
    ...
    if ((job_ptr->state_reason >= WAIT_QOS_GRP_CPU && job_ptr->state_reason <= WAIT_ASSOC_MAX_SUB_JOB) ||
        (job_ptr->state_reason == WAIT_ASSOC_JOB_LIMIT) || (job_ptr->state_reason == WAIT_ASSOC_RESOURCE_LIMIT) ||
        (job_ptr->state_reason == WAIT_ASSOC_TIME_LIMIT) || (job_ptr->state_reason == WAIT_QOS_JOB_LIMIT) ||
...
extern bool acct_policy_job_runnable_pre_select(struct job_record *job_ptr, bool assoc_mgr_locked) {
    ...
    if (qos_ptr_1 && !(rc = _qos_job_runnable_pre_select(job_ptr, qos_ptr_1, &qos_rec)))
        goto end_it;

    /* If qos_ptr_1 didn't set the value use the 2nd QOS to set the limit */
    if (qos_ptr_2 && !(rc = _qos_job_runnable_pre_select(job_ptr, qos_ptr_2, &qos_rec)))
        goto end_it;
    ...
    while (assoc_ptr) {
        ...
        if ((qos_rec.grp_jobs == INFINITE) && (assoc_ptr->grp_jobs != INFINITE) &&
            (assoc_ptr->usage->used_jobs >= assoc_ptr->grp_jobs)) {
            xfree(job_ptr->state_desc);
            job_ptr->state_reason = WAIT_ASSOC_GRP_JOB;
            debug2("%pJ being held, assoc %u is at or exceeds group max jobs limit %u with %u for account %s", job_ptr,
                   assoc_ptr->id, assoc_ptr->grp_jobs, assoc_ptr->usage->used_jobs, assoc_ptr->acct);
        ...
        if ((job_ptr->limit_set.time != ADMIN_SET_LIMIT) && (qos_rec.grp_wall == INFINITE) &&
            (assoc_ptr->grp_wall != INFINITE)) {
            if (time_limit == NO_VAL) {
                time_limit = job_ptr->time_limit;
                _set_time_limit(&time_limit, job_ptr->part_ptr->max_time,
                                MIN(assoc_ptr->grp_wall, assoc_ptr->max_wall_pj), &job_ptr->limit_set.time);
            ...
            if (wall_mins >= assoc_ptr->grp_wall) {
                ...
                debug2("%pJ being held, assoc %u is at or exceeds group wall limit %u with %u for account %s", job_ptr,
                       assoc_ptr->id, assoc_ptr->grp_wall, wall_mins, assoc_ptr->acct);
                rc = false;
                goto end_it;
            } else if (safe_limits && ((wall_mins + time_limit) > assoc_ptr->grp_wall)) {
                xfree(job_ptr->state_desc);
                job_ptr->state_reason = WAIT_ASSOC_GRP_WALL;
                debug2("%pJ being held, the job request with assoc %u will exceed group wall limit %u if ran with %u for account %s",
                       job_ptr, assoc_ptr->id, assoc_ptr->grp_wall, wall_mins + time_limit, assoc_ptr->acct);
                rc = false;
                goto end_it;
            }
        }
        ...
        if ((qos_rec.max_jobs_pa == INFINITE) && (qos_rec.max_jobs_pu == INFINITE) &&
            (assoc_ptr->max_jobs != INFINITE) && (assoc_ptr->usage->used_jobs >= assoc_ptr->max_jobs)) {
            xfree(job_ptr->state_desc);
            job_ptr->state_reason = WAIT_ASSOC_MAX_JOBS;
            debug2("%pJ being held, assoc %u is at or exceeds max jobs limit %u with %u for account %s", job_ptr,
                   assoc_ptr->id, assoc_ptr->max_jobs, assoc_ptr->usage->used_jobs, assoc_ptr->acct);
            rc = false;
            goto end_it;
        }
		 ...
		 * submission and job can not run, then kill it
		 */
        if ((job_ptr->limit_set.time != ADMIN_SET_LIMIT) && (qos_rec.max_wall_pj == INFINITE) &&
            (assoc_ptr->max_wall_pj != INFINITE)) {
            if (time_limit == NO_VAL) {
                time_limit = job_ptr->time_limit;
                _set_time_limit(&time_limit, job_ptr->part_ptr->max_time, assoc_ptr->max_wall_pj,
                                &job_ptr->limit_set.time);
            ...
            if (time_limit > assoc_ptr->max_wall_pj) {
                xfree(job_ptr->state_desc);
                job_ptr->state_reason = WAIT_ASSOC_MAX_WALL_PER_JOB;
                debug2("%pJ being held, time limit %u exceeds account max %u", job_ptr, job_ptr->time_limit,
                       time_limit);
                rc = false;
                goto end_it;
            }
...
acct_policy_job_runnable_post_select(struct job_record *job_ptr, uint64_t *tres_req_cnt, bool assoc_mgr_locked) {
    ...
    time_limit = job_ptr->time_limit;
    _set_time_limit(&time_limit, job_ptr->part_ptr->max_time, job_ptr->part_ptr->default_time, NULL);
    ...
    /* check the first QOS setting it's values in the qos_rec */
    if (qos_ptr_1 &&
        !(rc = _qos_job_runnable_post_select(job_ptr, qos_ptr_1, &qos_rec, tres_req_cnt, job_tres_time_limit)))
        goto end_it;

    /* If qos_ptr_1 didn't set the value use the 2nd QOS to set the limit */
    if (qos_ptr_2 &&
        !(rc = _qos_job_runnable_post_select(job_ptr, qos_ptr_2, &qos_rec, tres_req_cnt, job_tres_time_limit)))
        goto end_it;

    assoc_ptr = job_ptr->assoc_ptr;
    while (assoc_ptr) {
        for (i = 0; i < slurmctld_tres_cnt; i++) {
            tres_usage_mins[i] = (uint64_t)(assoc_ptr->usage->usage_tres_raw[i] / 60);
            tres_run_mins[i] = assoc_ptr->usage->grp_used_tres_run_secs[i] / 60;
		 ...
		 */
        tres_usage = _validate_tres_usage_limits_for_assoc(&tres_pos, assoc_ptr->grp_tres_mins_ctld,
                                                           qos_rec.grp_tres_mins_ctld, job_tres_time_limit,
                                                           tres_run_mins, tres_usage_mins, job_ptr->limit_set.tres,
                                                           safe_limits);
        switch (tres_usage) {
            case TRES_USAGE_CUR_EXCEEDS_LIMIT:
                xfree(job_ptr->state_desc);
                job_ptr->state_reason = _get_tres_state_reason(tres_pos, WAIT_ASSOC_GRP_UNK_MIN);
                debug2("%pJ being held, assoc %u(%s/%s/%s) group max tres(%s) minutes limit of %"
                PRIu64
                " is already at or exceeded with %"
                PRIu64, job_ptr, assoc_ptr->id, assoc_ptr->acct, assoc_ptr->user, assoc_ptr->partition, assoc_mgr_tres_name_array[tres_pos], assoc_ptr->grp_tres_mins_ctld[tres_pos], tres_usage_mins[tres_pos]);
                rc = false;
                goto end_it;
                break;
            case TRES_USAGE_REQ_EXCEEDS_LIMIT:
                xfree(job_ptr->state_desc);
                job_ptr->state_reason = _get_tres_state_reason(tres_pos, WAIT_ASSOC_GRP_UNK_MIN);
                debug2("%pJ being held, the job is requesting more than allowed with assoc %u(%s/%s/%s) group max tres(%s) minutes of %"
                PRIu64
                " with %"
                PRIu64, job_ptr, assoc_ptr->id, assoc_ptr->acct, assoc_ptr->user, assoc_ptr->partition, assoc_mgr_tres_name_array[tres_pos], assoc_ptr->grp_tres_mins_ctld[tres_pos], job_tres_time_limit[tres_pos]);
                rc = false;
                goto end_it;
                break;
            case TRES_USAGE_REQ_NOT_SAFE_WITH_USAGE:
			 ...
			 */
                xfree(job_ptr->state_desc);
                job_ptr->state_reason = _get_tres_state_reason(tres_pos, WAIT_ASSOC_GRP_UNK_MIN);
                debug2("%pJ being held, the job is at or exceeds assoc %u(%s/%s/%s) group max tres(%s) minutes of %"
                PRIu64
                " of which %"
                PRIu64
                " are still available but request is for %"
                ...
                " already in use) tres minutes (request tres count %"
                PRIu64
                ")", job_ptr, assoc_ptr->id, assoc_ptr->acct, assoc_ptr->user, assoc_ptr->partition, assoc_mgr_tres_name_array[tres_pos], assoc_ptr->grp_tres_mins_ctld[tres_pos],
        ...
        orig_node_cnt = tres_req_cnt[TRES_ARRAY_NODE];
        _get_unique_job_node_cnt(job_ptr, assoc_ptr->usage->grp_node_bitmap, &tres_req_cnt[TRES_ARRAY_NODE]);
        tres_usage = _validate_tres_usage_limits_for_assoc(&tres_pos, assoc_ptr->grp_tres_ctld, qos_rec.grp_tres_ctld,
                                                           tres_req_cnt, assoc_ptr->usage->grp_used_tres, NULL,
                                                           job_ptr->limit_set.tres, true);
        tres_req_cnt[TRES_ARRAY_NODE] = orig_node_cnt;
        switch (tres_usage) {
            case TRES_USAGE_CUR_EXCEEDS_LIMIT:
            ...
            case TRES_USAGE_REQ_EXCEEDS_LIMIT:
                xfree(job_ptr->state_desc);
                job_ptr->state_reason = _get_tres_state_reason(tres_pos, WAIT_ASSOC_GRP_UNK);
                debug2("%pJ is being held, assoc %u(%s/%s/%s) min tres(%s) request %"
                PRIu64
                " exceeds group max tres limit %"
                PRIu64, job_ptr, assoc_ptr->id, assoc_ptr->acct, assoc_ptr->user, assoc_ptr->partition, assoc_mgr_tres_name_array[tres_pos], tres_req_cnt[tres_pos], assoc_ptr->grp_tres_ctld[tres_pos]);
                rc = false;
                goto end_it;
                break;
            case TRES_USAGE_REQ_NOT_SAFE_WITH_USAGE:
                xfree(job_ptr->state_desc);
                job_ptr->state_reason = _get_tres_state_reason(tres_pos, WAIT_ASSOC_GRP_UNK);
                debug2("%pJ being held, if allowed the job request will exceed assoc %u(%s/%s/%s) group max tres(%s) limit %"
                PRIu64
                " with already used %"
                PRIu64
                " + requested %"
                PRIu64, job_ptr, assoc_ptr->id, assoc_ptr->acct, assoc_ptr->user, assoc_ptr->partition, assoc_mgr_tres_name_array[tres_pos], assoc_ptr->grp_tres_ctld[tres_pos], assoc_ptr->usage->grp_used_tres[tres_pos], tres_req_cnt[tres_pos]);
                rc = false;
                goto end_it;
            case TRES_USAGE_OKAY:
                /* all good */
        ...
        tres_usage = _validate_tres_usage_limits_for_assoc(&tres_pos, assoc_ptr->grp_tres_run_mins_ctld,
                                                           qos_rec.grp_tres_run_mins_ctld, job_tres_time_limit,
                                                           tres_run_mins, NULL, NULL, true);
        switch (tres_usage) {
            case TRES_USAGE_CUR_EXCEEDS_LIMIT:
                /* not possible because the curr_usage sent in is NULL*/
            ...
            case TRES_USAGE_REQ_EXCEEDS_LIMIT:
                xfree(job_ptr->state_desc);
                job_ptr->state_reason = _get_tres_state_reason(tres_pos, WAIT_ASSOC_GRP_UNK_RUN_MIN);
                debug2("%pJ is being held, assoc %u(%s/%s/%s) group max running tres(%s) minutes request limit %"
                PRIu64
                " exceeds limit %"
                PRIu64, job_ptr, assoc_ptr->id, assoc_ptr->acct, assoc_ptr->user, assoc_ptr->partition, assoc_mgr_tres_name_array[tres_pos], tres_run_mins[tres_pos], assoc_ptr->grp_tres_run_mins_ctld[tres_pos]);
                rc = false;
                goto end_it;
                break;
            case TRES_USAGE_REQ_NOT_SAFE_WITH_USAGE:
                xfree(job_ptr->state_desc);
                job_ptr->state_reason = _get_tres_state_reason(tres_pos, WAIT_ASSOC_GRP_UNK_RUN_MIN);
                debug2("%pJ being held, if allowed the job request will exceed assoc %u(%s/%s/%s) group max running tres(%s) minutes limit %"
                PRIu64
                " with already used %"
                PRIu64
                " + requested %"
                PRIu64, job_ptr, assoc_ptr->id, assoc_ptr->acct, assoc_ptr->user, assoc_ptr->partition, assoc_mgr_tres_name_array[tres_pos], assoc_ptr->grp_tres_run_mins_ctld[tres_pos], tres_run_mins[tres_pos], job_tres_time_limit[tres_pos]);
                rc = false;
                goto end_it;
                break;
            case TRES_USAGE_OKAY:
        ...
        if (!_validate_tres_limits_for_assoc(&tres_pos, job_tres_time_limit, 0, assoc_ptr->max_tres_mins_ctld,
                                             qos_rec.max_tres_mins_pj_ctld, job_ptr->limit_set.tres, 1, 0, 1)) {
            xfree(job_ptr->state_desc);
            job_ptr->state_reason = _get_tres_state_reason(tres_pos, WAIT_ASSOC_MAX_UNK_MINS_PER_JOB);
            debug2("%pJ being held, the job is requesting more than allowed with assoc %u(%s/%s/%s) max tres(%s) minutes of %"
            PRIu64
            " with %"
            PRIu64, job_ptr, assoc_ptr->id, assoc_ptr->acct, assoc_ptr->user, assoc_ptr->partition, assoc_mgr_tres_name_array[tres_pos], assoc_ptr->max_tres_mins_ctld[tres_pos], job_tres_time_limit[tres_pos]);
            rc = false;
            goto end_it;
        }

        if (!_validate_tres_limits_for_assoc(&tres_pos, tres_req_cnt, 0, assoc_ptr->max_tres_ctld,
                                             qos_rec.max_tres_pj_ctld, job_ptr->limit_set.tres, 1, 0, 1)) {
            xfree(job_ptr->state_desc);
            job_ptr->state_reason = _get_tres_state_reason(tres_pos, WAIT_ASSOC_MAX_UNK_PER_JOB);
            debug2("%pJ is being held, the job is requesting more than allowed with assoc %u(%s/%s/%s) max tres(%s) limit of %"
            PRIu64
            " with %"
            PRIu64, job_ptr, assoc_ptr->id, assoc_ptr->acct, assoc_ptr->user, assoc_ptr->partition, assoc_mgr_tres_name_array[tres_pos], assoc_ptr->max_tres_ctld[tres_pos], tres_req_cnt[tres_pos]);
            rc = false;
            break;
        }

        if (!_validate_tres_limits_for_assoc(&tres_pos, tres_req_cnt, tres_req_cnt[TRES_ARRAY_NODE],
                                             assoc_ptr->max_tres_pn_ctld, qos_rec.max_tres_pn_ctld,
                                             job_ptr->limit_set.tres, 1, 0, 1)) {
            xfree(job_ptr->state_desc);
            job_ptr->state_reason = _get_tres_state_reason(tres_pos, WAIT_ASSOC_MAX_UNK_PER_NODE);
            debug2("%pJ is being held, the job is requesting more than allowed with assoc %u(%s/%s/%s) max tres(%s) per node limit of %"
            PRIu64
            " with %"
            PRIu64, job_ptr, assoc_ptr->id, assoc_ptr->acct, assoc_ptr->user, assoc_ptr->partition, assoc_mgr_tres_name_array[tres_pos], assoc_ptr->max_tres_pn_ctld[tres_pos], tres_req_cnt[tres_pos]);
            rc = false;
            break;
        }
...
extern uint32_t acct_policy_get_max_nodes(struct job_record *job_ptr, uint32_t *wait_reason) {
    uint64_t max_nodes_limit = INFINITE64, qos_max_p_limit = INFINITE64, grp_nodes = INFINITE64;
    ...
    if (qos_ptr_1) {
        uint64_t max_nodes_pj = qos_ptr_1->max_tres_pj_ctld[TRES_ARRAY_NODE];
        uint64_t max_nodes_pu = qos_ptr_1->max_tres_pu_ctld[TRES_ARRAY_NODE];
        uint64_t max_nodes_pa = qos_ptr_1->max_tres_pa_ctld[TRES_ARRAY_NODE];

        grp_nodes = qos_ptr_1->grp_tres_ctld[TRES_ARRAY_NODE];

        if (qos_ptr_2) {
            if (max_nodes_pa == INFINITE64)
                max_nodes_pa = qos_ptr_2->max_tres_pa_ctld[TRES_ARRAY_NODE];
            if (max_nodes_pj == INFINITE64)
                max_nodes_pj = qos_ptr_2->max_tres_pj_ctld[TRES_ARRAY_NODE];
            if (max_nodes_pu == INFINITE64)
                max_nodes_pu = qos_ptr_2->max_tres_pu_ctld[TRES_ARRAY_NODE];
            if (grp_nodes == INFINITE64)
                grp_nodes = qos_ptr_2->grp_tres_ctld[TRES_ARRAY_NODE];
        }
	...
	*/
    while (assoc_ptr) {
        if ((!qos_ptr_1 || (grp_nodes == INFINITE64)) && (assoc_ptr->grp_tres_ctld[TRES_ARRAY_NODE] != INFINITE64) &&
            (assoc_ptr->grp_tres_ctld[TRES_ARRAY_NODE] < max_nodes_limit)) {
            max_nodes_limit = assoc_ptr->grp_tres_ctld[TRES_ARRAY_NODE];
            *wait_reason = WAIT_ASSOC_GRP_NODE;
            grp_set = 1;
        }

        if (!parent && (qos_max_p_limit == INFINITE64) && (assoc_ptr->max_tres_ctld[TRES_ARRAY_NODE] != INFINITE64) &&
            (assoc_ptr->max_tres_ctld[TRES_ARRAY_NODE] < max_nodes_limit)) {
            max_nodes_limit = assoc_ptr->max_tres_ctld[TRES_ARRAY_NODE];
...
extern int acct_policy_update_pending_job(struct job_record *job_ptr) {
    ...
    if (!accounting_enforce || !IS_JOB_PENDING(job_ptr) || !(accounting_enforce & ACCOUNTING_ENFORCE_LIMITS))
    ...
    memcpy(&acct_policy_limit_set, &job_ptr->limit_set, sizeof(acct_policy_limit_set_t));
    job_desc.tres_req_cnt = tres_req_cnt;
    /* copy all the tres requests over */
    memcpy(job_desc.tres_req_cnt, job_ptr->tres_req_cnt, sizeof(uint64_t) * slurmctld_tres_cnt);
    ...
    if (!acct_policy_validate(&job_desc, job_ptr->part_ptr, job_ptr->assoc_ptr, job_ptr->qos_ptr,
                              &job_ptr->state_reason, &acct_policy_limit_set, 0)) {
        info("%s: exceeded association/qos's cpu, node, memory or time limit for %pJ", __func__, job_ptr);
        return SLURM_ERROR;
    }
...
extern bool acct_policy_job_time_out(struct job_record *job_ptr) {
	 ...
	 */
    if (!(accounting_enforce & ACCOUNTING_ENFORCE_LIMITS) || (accounting_enforce & ACCOUNTING_ENFORCE_SAFE))
        return false;
    ...
    time_delta = (uint64_t)(((now - job_ptr->start_time) - job_ptr->tot_sus_time) / 60);
    ...
    for (i = 0; i < slurmctld_tres_cnt; i++) {
        ...
        if (job_ptr->tres_alloc_cnt[i]) {
            job_tres_usage_mins[i] = (time_delta * job_ptr->tres_alloc_cnt[i]) + 1;
        }
    }

    /* check the first QOS setting it's values in the qos_rec */
    if (qos_ptr_1 && !_qos_job_time_out(job_ptr, qos_ptr_1, &qos_rec, job_tres_usage_mins))
        goto job_failed;

    /* If qos_ptr_1 didn't set the value use the 2nd QOS to set the limit */
    if (qos_ptr_2 && !_qos_job_time_out(job_ptr, qos_ptr_2, &qos_rec, job_tres_usage_mins))
        goto job_failed;

    /* handle any association stuff here */
    while (assoc) {
        for (i = 0; i < slurmctld_tres_cnt; i++)
            tres_usage_mins[i] = (uint64_t)(assoc->usage->usage_tres_raw[i] / 60.0);
        wall_mins = assoc->usage->grp_used_wall / 60;

        tres_usage = _validate_tres_usage_limits_for_assoc(&tres_pos, assoc->grp_tres_mins_ctld,
                                                           qos_rec.grp_tres_mins_ctld, NULL, NULL, tres_usage_mins,
                                                           NULL, false);
        switch (tres_usage) {
            case TRES_USAGE_CUR_EXCEEDS_LIMIT:
                last_job_update = now;
                info("%pJ timed out, the job is at or exceeds assoc %u(%s/%s/%s) group max tres(%s) minutes of %"
                PRIu64
                " with %"
                PRIu64, job_ptr, assoc->id, assoc->acct, assoc->user, assoc->partition, assoc_mgr_tres_name_array[tres_pos], assoc->grp_tres_mins_ctld[tres_pos], tres_usage_mins[tres_pos]);
                job_ptr->state_reason = FAIL_TIMEOUT;
                goto job_failed;
                break;
            case TRES_USAGE_REQ_EXCEEDS_LIMIT:
        ...
        if ((qos_rec.grp_wall == INFINITE) && (assoc->grp_wall != INFINITE) && (wall_mins >= assoc->grp_wall)) {
            info("%pJ timed out, assoc %u is at or exceeds group wall limit %u with %u for account %s", job_ptr,
                 assoc->id, assoc->grp_wall, wall_mins, assoc->acct);
            job_ptr->state_reason = FAIL_TIMEOUT;
            break;
        }
        ...
        tres_usage = _validate_tres_usage_limits_for_assoc(&tres_pos, assoc->max_tres_mins_ctld,
                                                           qos_rec.max_tres_mins_pj_ctld, job_tres_usage_mins, NULL,
                                                           NULL, NULL, true);
        ...
        switch (tres_usage) {
            ...
            case TRES_USAGE_REQ_EXCEEDS_LIMIT:
                last_job_update = now;
                info("%pJ timed out, the job is at or exceeds assoc %u(%s/%s/%s) max tres(%s) minutes of %"
                PRIu64
                " with %"
                PRIu64, job_ptr, assoc->id, assoc->acct, assoc->user, assoc->partition, assoc_mgr_tres_name_array[tres_pos], assoc->max_tres_mins_ctld[tres_pos], job_tres_usage_mins[tres_pos]);
                job_ptr->state_reason = FAIL_TIMEOUT;
                goto job_failed;
                break;
            case TRES_USAGE_REQ_NOT_SAFE_WITH_USAGE:
...
extern int acct_policy_handle_accrue_time(struct job_record *job_ptr, bool assoc_mgr_locked) {
    ...
    if (qos_ptr_1) {
        used_limits_a1 = acct_policy_get_acct_used_limits(&qos_ptr_1->usage->acct_limit_list, assoc_ptr->acct);
        used_limits_u1 = acct_policy_get_user_used_limits(&qos_ptr_1->usage->user_limit_list, job_ptr->user_id);
    }

    if (qos_ptr_2) {
        used_limits_a2 = acct_policy_get_acct_used_limits(&qos_ptr_2->usage->acct_limit_list, assoc_ptr->acct);
        used_limits_u2 = acct_policy_get_user_used_limits(&qos_ptr_2->usage->user_limit_list, job_ptr->user_id);
    }

    /* We have started running, let's clear us out of the mix. */
    if (details_ptr->accrue_time) {
        if (!(job_ptr->bit_flags & JOB_ACCRUE_OVER) && !IS_JOB_PENDING(job_ptr)) {
            int job_cnt;
            /*
            ...
            _remove_accrue_time_internal(job_ptr->assoc_ptr, qos_ptr_1, used_limits_a1, used_limits_u1, qos_ptr_2,
                                         used_limits_a2, used_limits_u2, job_cnt);
        }

        /* We already have our time and we aren't an array, endit */
        if (!IS_JOB_PENDING(job_ptr) || !job_ptr->array_recs || !job_ptr->array_recs->task_cnt)
            goto endit;
    } else if (!IS_JOB_PENDING(job_ptr))
        goto endit;

    if (qos_ptr_1)
        _get_accrue_create_cnt(&max_jobs_accrue, &create_cnt, qos_ptr_1->grp_jobs_accrue, qos_ptr_1->usage->accrue_cnt);
    if (used_limits_a1)
        _get_accrue_create_cnt(&max_jobs_accrue, &create_cnt, qos_ptr_1->max_jobs_accrue_pa,
                               used_limits_a1->accrue_cnt);

    if (used_limits_u1)
        _get_accrue_create_cnt(&max_jobs_accrue, &create_cnt, qos_ptr_1->max_jobs_accrue_pu,
                               used_limits_u1->accrue_cnt);
    if (qos_ptr_2)
        _get_accrue_create_cnt(&max_jobs_accrue, &create_cnt, qos_ptr_2->grp_jobs_accrue, qos_ptr_2->usage->accrue_cnt);
    if (used_limits_a2)
        _get_accrue_create_cnt(&max_jobs_accrue, &create_cnt, qos_ptr_2->max_jobs_accrue_pa,
                               used_limits_a2->accrue_cnt);

    if (used_limits_u2)
        _get_accrue_create_cnt(&max_jobs_accrue, &create_cnt, qos_ptr_2->max_jobs_accrue_pu,
    ...
    while (assoc_ptr) {
        ...
        _get_accrue_create_cnt(&max_jobs_accrue, &create_cnt, assoc_ptr->grp_jobs_accrue, assoc_ptr->usage->accrue_cnt);
        /* We don't need to look at the regular limits for
		 * parents since we have pre-propogated them, so just
		 * continue with the next parent
		 */
        if (!parent)
            _get_accrue_create_cnt(&max_jobs_accrue, &create_cnt, assoc_ptr->max_jobs_accrue,
    ...
    if ((max_jobs_accrue == INFINITE) || (create_cnt && (!job_ptr->array_recs || !job_ptr->array_recs->task_cnt))) {
        if (!details_ptr->accrue_time && job_ptr->details->begin_time) {
            /*
			 * If no limit and begin_time hasn't happened yet
			 * then set accrue_time to now.
			 */
            details_ptr->accrue_time = ((max_jobs_accrue == INFINITE) && details_ptr->begin_time)
            ...
            _add_accrue_time_internal(job_ptr->assoc_ptr, qos_ptr_1, used_limits_a1, used_limits_u1, qos_ptr_2,
                                      used_limits_a2, used_limits_u2, create_cnt);
        }
    ...
    if (!create_cnt) {
        if (slurmctld_conf.debug_flags & DEBUG_FLAG_ACCRUE)
            info("%s: %pJ can't accrue, we are over a limit", __func__, job_ptr);
        goto endit;
    }
    ...
    for (i = 0; i < create_cnt; i++) {
        ...
        if (!details_ptr) {
            ...
            rc = SLURM_ERROR;
            _add_accrue_time_internal(job_ptr->assoc_ptr, qos_ptr_1, used_limits_a1, used_limits_u1, qos_ptr_2,
                                      used_limits_a2, used_limits_u2, i - 1);
            goto endit;
        }
	 ...
	 */
    _add_accrue_time_internal(job_ptr->assoc_ptr, qos_ptr_1, used_limits_a1, used_limits_u1, qos_ptr_2, used_limits_a2,
                              used_limits_u2, create_cnt);
...
extern void acct_policy_add_accrue_time(struct job_record *job_ptr, bool assoc_mgr_locked) {
    ...
    _add_accrue_time_internal(assoc_ptr, qos_ptr_1, used_limits_a1, used_limits_u1, qos_ptr_2, used_limits_a2,
                              used_limits_u2, job_cnt);
    if (!assoc_mgr_locked)
        assoc_mgr_unlock(&locks);
}
...
extern void acct_policy_remove_accrue_time(struct job_record *job_ptr, bool assoc_mgr_locked) {
    ...
    _remove_accrue_time_internal(assoc_ptr, qos_ptr_1, used_limits_a1, used_limits_u1, qos_ptr_2, used_limits_a2,
                                 used_limits_u2, job_cnt);
...
extern uint32_t acct_policy_get_prio_thresh(struct job_record *job_ptr, bool assoc_mgr_locked) {
    ...
    assoc_mgr_lock_t locks = {READ_LOCK, NO_LOCK, READ_LOCK, NO_LOCK, NO_LOCK, NO_LOCK, NO_LOCK};
...
extern slurmdb_used_limits_t *acct_policy_get_acct_used_limits(List *acct_limit_list, char *acct) {
    ...
    if (!(used_limits = list_find_first(*acct_limit_list, _find_used_limits_for_acct, acct))) {
...
extern slurmdb_used_limits_t *acct_policy_get_user_used_limits(List *user_limit_list, uint32_t user_id) {
    ...
    if (!(used_limits = list_find_first(*user_limit_list, _find_used_limits_for_user, &user_id))) {
        int i = sizeof(uint64_t) * slurmctld_tres_cnt;
