...
{
	printf("this is controller.c  edited by dadan\n");
...
static int _accounting_mark_all_nodes_down(char *reason)
{
	...
	for (i = 0; i < node_record_count; i++, node_ptr++)
	{
		...
		if ((rc = clusteracct_storage_g_node_down(
				 acct_db_conn,
				 node_ptr, event_time,
				 reason, slurmctld_conf.slurm_user_id)) == SLURM_ERROR)
			break;
	}
	return rc;
}
		...
		if (!association_based_accounting)
		{
			...
			list_append(update_object.objects, tres_rec);
				 ...
				 assoc_mgr_fill_in_tres(
					 ...
					 ACCOUNTING_ENFORCE_TRES, NULL, 0) != SLURM_SUCCESS)
		{
			if (!add_list)
...
static void _queue_reboot_msg(void)
{
		 ...
		 i < node_record_count; i++, node_ptr++)
	{
		...
		/* only active idle nodes, don't reboot
                 ...
                 * jobs on them
                 */
		if (IS_NODE_IDLE(node_ptr) && !IS_NODE_NO_RESPOND(node_ptr) && !IS_NODE_POWER_UP(node_ptr) && node_ptr->sus_job_cnt == 0)
			want_reboot = true;
...
static void *_slurmctld_background(void *no_data)
{
	...
	slurmctld_lock_t purge_job_locks = {
		.conf = READ_LOCK, .job = WRITE_LOCK, .node = WRITE_LOCK, .fed = READ_LOCK};
	...
	while (1)
	{
		...
		if (slurmctld_conf.group_time &&
			(difftime(now, last_group_time) >= slurmctld_conf.group_time))
		{
			lock_slurmctld(part_write_lock);
			now = time(NULL);
			last_group_time = now;
...
static void *_assoc_cache_mgr(void *no_data)
{
	...
	assoc_mgr_lock_t locks =
		{.assoc = READ_LOCK, .qos = READ_LOCK, .tres = WRITE_LOCK, .user = READ_LOCK};
