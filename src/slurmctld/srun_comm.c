 ...
 */
static void
_srun_agent_launch(slurm_addr_t *addr, char *host, slurm_msg_type_t type, void *msg_args, uint16_t protocol_version) {
...
extern void srun_allocate(struct job_record *job_ptr) {
    ...
    xassert(job_ptr);
    if (!job_ptr || !job_ptr->alloc_resp_port || !job_ptr->alloc_node || !job_ptr->resp_host || !job_ptr->job_resrcs ||
        !job_ptr->job_resrcs->cpu_array_cnt)
        return;

    if (job_ptr->pack_job_id == 0) {
        addr = xmalloc(sizeof(struct sockaddr_in));
        slurm_set_addr(addr, job_ptr->alloc_resp_port, job_ptr->resp_host);

        msg_arg = build_alloc_msg(job_ptr, SLURM_SUCCESS, NULL);
        _srun_agent_launch(addr, job_ptr->alloc_node, RESPONSE_RESOURCE_ALLOCATION, msg_arg,
                           job_ptr->start_protocol_ver);
    } else if (_pending_pack_jobs(job_ptr)) {
        return;
    } else if ((pack_leader = find_job_record(job_ptr->pack_job_id))) {
        addr = xmalloc(sizeof(struct sockaddr_in));
        slurm_set_addr(addr, pack_leader->alloc_resp_port, pack_leader->resp_host);
        ...
        while ((pack_job = (struct job_record *) list_next(iter))) {
            ...
            msg_arg = build_alloc_msg(pack_job, SLURM_SUCCESS, NULL);
            list_append(job_resp_list, msg_arg);
            msg_arg = NULL;
        }
        list_iterator_destroy(iter);
        _srun_agent_launch(addr, job_ptr->alloc_node, RESPONSE_JOB_PACK_ALLOCATION, job_resp_list,
                           job_ptr->start_protocol_ver);
    } else {
        error("%s: Can not find pack job leader %pJ", __func__, job_ptr);
    }
}
...
extern void srun_allocate_abort(struct job_record *job_ptr) {
    if (job_ptr && job_ptr->alloc_resp_port && job_ptr->alloc_node && job_ptr->resp_host) {
        slurm_addr_t *addr;
        ...
        _srun_agent_launch(addr, job_ptr->alloc_node, SRUN_JOB_COMPLETE, msg_arg, job_ptr->start_protocol_ver);
    }
}
...
extern void srun_node_fail(struct job_record *job_ptr, char *node_name) {
    ...
    while ((step_ptr = (struct step_record *) list_next(step_iterator))) {
        if (step_ptr->step_node_bitmap == NULL)   /* pending step */
            continue;
        if ((bit_position >= 0) && (!bit_test(step_ptr->step_node_bitmap, bit_position)))
            continue;    /* job step not on this node */
        if ((step_ptr->port == 0) || (step_ptr->host == NULL) || (step_ptr->batch_step) || (step_ptr->host[0] == '\0'))
            continue;
        addr = xmalloc(sizeof(struct sockaddr_in));
        ...
        _srun_agent_launch(addr, step_ptr->host, SRUN_NODE_FAIL, msg_arg, step_ptr->start_protocol_ver);
    }
    ...
    if (job_ptr->other_port && job_ptr->alloc_node && job_ptr->resp_host) {
        ...
        msg_arg->step_id = NO_VAL;
        msg_arg->nodelist = xstrdup(node_name);
        _srun_agent_launch(addr, job_ptr->alloc_node, SRUN_NODE_FAIL, msg_arg, job_ptr->start_protocol_ver);
    }
}
...
extern void srun_ping(void) {
    ...
    time_t old = now - (slurmctld_conf.inactive_limit / 3) + slurmctld_conf.msg_timeout + 1;
    ...
    while ((job_ptr = (struct job_record *) list_next(job_iterator))) {
        ...
        if ((job_ptr->time_last_active <= old) && job_ptr->other_port && job_ptr->alloc_node && job_ptr->resp_host) {
            addr = xmalloc(sizeof(struct sockaddr_in));
            slurm_set_addr(addr, job_ptr->other_port, job_ptr->resp_host);
            msg_arg = xmalloc(sizeof(srun_ping_msg_t));
            msg_arg->job_id = job_ptr->job_id;
            msg_arg->step_id = NO_VAL;
            _srun_agent_launch(addr, job_ptr->alloc_node, SRUN_PING, msg_arg, job_ptr->start_protocol_ver);
        }
    }
...
extern void srun_step_timeout(struct step_record *step_ptr, time_t timeout_val) {
    ...
    if (step_ptr->batch_step || !step_ptr->port || !step_ptr->host || (step_ptr->host[0] == '\0'))
        return;
    ...
    _srun_agent_launch(addr, step_ptr->host, SRUN_TIMEOUT, msg_arg, step_ptr->start_protocol_ver);
}
...
extern void srun_timeout(struct job_record *job_ptr) {
    ...
    if (job_ptr->other_port && job_ptr->alloc_node && job_ptr->resp_host) {
        ...
        _srun_agent_launch(addr, job_ptr->alloc_node, SRUN_TIMEOUT, msg_arg, job_ptr->start_protocol_ver);
    }
...
extern int srun_user_message(struct job_record *job_ptr, char *msg) {
    ...
    if (job_ptr->other_port && job_ptr->resp_host && job_ptr->resp_host[0]) {
        addr = xmalloc(sizeof(struct sockaddr_in));
        slurm_set_addr(addr, job_ptr->other_port, job_ptr->resp_host);
        msg_arg = xmalloc(sizeof(srun_user_msg_t));
        msg_arg->job_id = job_ptr->job_id;
        msg_arg->msg = xstrdup(msg);
        _srun_agent_launch(addr, job_ptr->resp_host, SRUN_USER_MSG, msg_arg, job_ptr->start_protocol_ver);
...
#endif
        notify_msg_ptr = (job_notify_msg_t *) xmalloc(sizeof(job_notify_msg_t));
...
extern bool srun_job_suspend(struct job_record *job_ptr, uint16_t op) {
    ...
    if (job_ptr->other_port && job_ptr->alloc_node && job_ptr->resp_host) {
        ...
        msg_arg->job_id = job_ptr->job_id;
        msg_arg->op = op;
        _srun_agent_launch(addr, job_ptr->alloc_node, SRUN_REQUEST_SUSPEND, msg_arg, job_ptr->start_protocol_ver);
        msg_sent = true;
    }
    return msg_sent;
}
...
extern void srun_step_missing(struct step_record *step_ptr, char *node_list) {
    ...
    if (step_ptr->port && step_ptr->host && step_ptr->host[0]) {
        ...
        _srun_agent_launch(addr, step_ptr->host, SRUN_STEP_MISSING, msg_arg, step_ptr->start_protocol_ver);
    }
}
...
extern void srun_exec(struct step_record *step_ptr, char **argv) {
    ...
    if (step_ptr->port && step_ptr->host && step_ptr->host[0]) {
        ...
        _srun_agent_launch(addr, step_ptr->host, SRUN_EXEC, msg_arg, step_ptr->start_protocol_ver);
    } else {
        error("srun_exec %pS lacks communication channel", step_ptr);
    }
}
