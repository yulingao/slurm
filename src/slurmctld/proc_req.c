...
static void _create_pack_job_id_set(hostset_t jobid_hostset,
                                    uint32_t pack_job_offset, char **pack_job_id_set);
...
void slurmctld_req(slurm_msg_t *msg, connection_arg_t *arg) {
    ...
    if (slurmctld_conf.debug_flags & DEBUG_FLAG_PROTOCOL) {
        ...
        if (msg->conn) {
            info("%s: received opcode %s from persist conn on (%s)%s", __func__,
                 p, msg->conn->cluster_name, msg->conn->rem_host);
        } else if (arg) {
            char inetbuf[64];
            ...
            error(
                  "%s: No arg given and this doesn't appear to be a persistent connection, this should never happen",
                  __func__);
        }
    }
    ...
    switch (msg->msg_type) {
        ...
        case REQUEST_PERSIST_INIT:
            if (msg->conn)
                error(
                      "We already have a persistent connect, this should never happen");
...
static void _fill_ctld_conf(slurm_ctl_conf_t *conf_ptr) {
    ...
    conf_ptr->accounting_storage_backup_host = xstrdup(
    ...
    conf_ptr->acct_gather_filesystem_type = xstrdup(
                                                    conf->acct_gather_filesystem_type);
    conf_ptr->acct_gather_interconnect_type = xstrdup(
                                                      conf->acct_gather_interconnect_type);
    conf_ptr->acct_gather_profile_type = xstrdup(
    ...
    conf_ptr->job_credential_private_key = xstrdup(
                                                   conf->job_credential_private_key);
    conf_ptr->job_credential_public_certificate = xstrdup(
    ...
    conf_ptr->propagate_rlimits_except = xstrdup(
    ...
    conf_ptr->slurmctld_primary_off_prog = xstrdup(
                                                   conf->slurmctld_primary_off_prog);
    conf_ptr->slurmctld_primary_on_prog = xstrdup(
...
static int _valid_id(char *caller, job_desc_msg_t *msg, uid_t uid, gid_t gid) {
    ...
    if (uid != msg->user_id) {
        error("%s: Requested UID=%u doesn't match user UID=%u.", caller,
              msg->user_id, uid);
        return ESLURM_USER_ID_MISSING;
    }
    
    if (gid != msg->group_id) {
        error("%s: Requested GID=%u doesn't match user GID=%u.", caller,
              msg->group_id, gid);
...
static void _kill_job_on_msg_fail(uint32_t job_id) {
    ...
    slurmctld_lock_t job_write_lock = { NO_LOCK, WRITE_LOCK, WRITE_LOCK,
        NO_LOCK, READ_LOCK };
...
extern resource_allocation_response_msg_t* build_alloc_msg(
    ...
    int i;
    resource_allocation_response_msg_t *alloc_msg = xmalloc(
    ...
    if (job_ptr->job_resrcs && job_ptr->job_resrcs->cpu_array_cnt) {
        ...
        alloc_msg->cpu_count_reps = xmalloc(
    ...
    alloc_msg->select_jobinfo = select_g_select_jobinfo_copy(
    ...
    if (job_ptr->details) {
        ...
        if (job_ptr->details->env_cnt) {
            ...
            alloc_msg->environment = xmalloc(
                                             sizeof(char*) * alloc_msg->env_size);
            for (i = 0; i < alloc_msg->env_size; i++) {
                alloc_msg->environment[i] = xstrdup(
...
static void _slurm_rpc_allocate_pack(slurm_msg_t *msg) {
    ...
    *first_job_ptr = NULL;
    ...
    while ((job_desc_msg = (job_desc_msg_t*) list_next(iter))) {
        ...
        if ((error_code = _valid_id("REQUEST_JOB_PACK_ALLOCATION", job_desc_msg,
                                    uid, gid))) {
            break;
        }
        ...
        if ((msg->protocol_version < SLURM_19_05_PROTOCOL_VERSION)
            ...
            err_msg = xstrdup(
                              "Het jobs from old Slurm versions are not possible");
    ...
    if (error_code) {
        ...
        if (fed_mgr_fed_rec) {
            ...
            if (fed_mgr_job_allocate(msg, job_desc_msg, true, uid,
                                     msg->protocol_version, &job_id, &error_code, &err_msg)) {
                reject_job = true;
            } else if (!(job_ptr = find_job_record(job_id))) {
                error(
                      "%s: can't find fed job that was just created. this should never happen",
                      __func__);
                reject_job = true;
                error_code = SLURM_ERROR;
            }
...
static void _slurm_rpc_dump_conf(slurm_msg_t *msg) {
    ...
    slurmctld_lock_t config_read_lock = { READ_LOCK, READ_LOCK, NO_LOCK,
        READ_LOCK, READ_LOCK };
    uid_t uid = g_slurm_auth_get_uid(msg->auth_cred);
...
static void _slurm_rpc_dump_jobs(slurm_msg_t *msg) {
    ...
    slurmctld_lock_t job_read_lock = { READ_LOCK, READ_LOCK, NO_LOCK, READ_LOCK,
        READ_LOCK };
    uid_t uid = g_slurm_auth_get_uid(msg->auth_cred);
    ...
    if ((job_info_request_msg->last_update - 1) >= last_job_update) {
        ...
        if (job_info_request_msg->job_ids) {
            pack_spec_jobs(&dump, &dump_size, job_info_request_msg->job_ids,
...
static void _slurm_rpc_get_fed(slurm_msg_t *msg) {
    ...
    ;
    slurm_msg_t response_msg;
    slurmctld_lock_t fed_read_lock = { NO_LOCK, NO_LOCK, NO_LOCK, NO_LOCK,
        READ_LOCK };
    uid_t uid = g_slurm_auth_get_uid(msg->auth_cred);
...
static void _slurm_rpc_dump_front_end(slurm_msg_t *msg) {
    ...
    slurmctld_lock_t node_read_lock = { READ_LOCK, NO_LOCK, NO_LOCK, READ_LOCK,
        NO_LOCK };
    uid_t uid = g_slurm_auth_get_uid(msg->auth_cred);
...
static void _slurm_rpc_dump_nodes(slurm_msg_t *msg) {
    ...
    if ((node_req_msg->last_update - 1) >= last_node_update) {
        ...
        pack_all_node(&dump, &dump_size, node_req_msg->show_flags, uid,
...
static void _slurm_rpc_dump_node_single(slurm_msg_t *msg) {
    ...
    slurmctld_lock_t node_write_lock = { READ_LOCK, NO_LOCK, READ_LOCK,
        READ_LOCK, NO_LOCK };
    uid_t uid = g_slurm_auth_get_uid(msg->auth_cred);
    ...
    select_g_select_nodeinfo_set_all();
#endif
    pack_one_node(&dump, &dump_size, node_req_msg->show_flags, uid,
...
static void _slurm_rpc_dump_partitions(slurm_msg_t *msg) {
    ...
    if ((slurmctld_conf.private_data & PRIVATE_DATA_PARTITIONS)
        ...
        pack_all_part(&dump, &dump_size, part_req_msg->show_flags, uid,
                      msg->protocol_version);
        unlock_slurmctld(part_read_lock);
...
static void _slurm_rpc_epilog_complete(slurm_msg_t *msg,
    ...
    slurmctld_lock_t job_write_lock = { READ_LOCK, WRITE_LOCK, WRITE_LOCK,
        NO_LOCK, NO_LOCK };
    uid_t uid = g_slurm_auth_get_uid(msg->auth_cred);
    ...
    if (epilog_msg->return_code)
        error("%s: epilog error %pJ Node=%s Err=%s %s", __func__, job_ptr,
              epilog_msg->node_name, slurm_strerror(epilog_msg->return_code),
              TIME_STR);
    else
...
static void _slurm_rpc_job_step_kill(uint32_t uid, slurm_msg_t *msg) {
    ...
    if (slurmctld_conf.debug_flags & DEBUG_FLAG_STEPS) {
        info(
             "Processing RPC: REQUEST_CANCEL_JOB_STEP JobId=%u StepId=%u uid=%u",
             job_step_kill_msg->job_id, job_step_kill_msg->job_step_id, uid);
    }
    _throttle_start(&active_rpc_cnt);
...
static void _slurm_rpc_complete_prolog(slurm_msg_t *msg) {
    ...
    /* return result */
    if (error_code) {
        info("_slurm_rpc_complete_prolog JobId=%u: %s ", comp_msg->job_id,
...
static void _slurm_rpc_complete_batch_script(slurm_msg_t *msg,
    ...
    if (!validate_slurm_user(uid)) {
        error("A non superuser %u tried to complete batch JobId=%u", uid,
              comp_msg->job_id);
    ...
    if (association_based_accounting && job_ptr
        ...
        if (!step_ptr) {
            if (msg->protocol_version >=
                ...
                error(
                      "%s: Could not find batch step for %pJ, this should never happen",
                      __func__, job_ptr);
            } else {
                debug2(
                       "%s: Batch step complete from old version of Slurm received for job %pJ",
                       __func__, job_ptr);
            }
        ...
        if (step_ptr->step_id != SLURM_BATCH_SCRIPT) {
            error(
                  "%s: %pJ Didn't find batch step, found step %u. This should never happen.",
    ...
    if ((comp_msg->slurm_rc == ESLURMD_JOB_NOTRUNNING)
        ...
        info("slurmd error running JobId=%u from %s=%s: %s", comp_msg->job_id,
             msg_title, nodes, slurm_strerror(comp_msg->slurm_rc));
        ...
        error("Slurmd error running JobId=%u on %s=%s: %s", comp_msg->job_id,
              msg_title, nodes, slurm_strerror(comp_msg->slurm_rc));
        ...
        if (error_code == SLURM_SUCCESS) {
            ...
            error_code = drain_nodes(comp_msg->node_name,
                                     "batch job complete failure", slurmctld_conf.slurm_user_id);
...
static void _slurm_rpc_dump_batch_script(slurm_msg_t *msg) {
    ...
    START_TIMER;
    debug3("Processing RPC: REQUEST_BATCH_SCRIPT from uid=%d for JobId=%u", uid,
           job_id_msg->job_id);
    lock_slurmctld(job_read_lock);
    ...
    if (error_code) {
        ...
        if (slurmctld_conf.debug_flags & DEBUG_FLAG_STEPS) {
            if ((error_code == ESLURM_PROLOG_RUNNING)
                ...
                debug("%s for suspended JobId=%u: %s", __func__,
                      req_step_msg->job_id, slurm_strerror(error_code));
            } else {
                info("%s for JobId=%u: %s", __func__, req_step_msg->job_id,
                     slurm_strerror(error_code));
            }
        }
        slurm_send_rc_msg(msg, error_code);
        ...
        if (slurmctld_conf.debug_flags & DEBUG_FLAG_STEPS)
            sched_info("%s: %pS %s %s", __func__, step_rec,
                       req_step_msg->node_list,
                       TIME_STR);
...
static void _slurm_rpc_job_step_get_info(slurm_msg_t *msg) {
    ...
    if ((request->last_update - 1) >= last_job_update) {
        ...
        error_code = pack_ctld_job_step_info_response_msg(request->job_id,
                                                          request->step_id, uid, request->show_flags, buffer,
                                                          msg->protocol_version);
        unlock_slurmctld(job_read_lock);
        ...
        if (error_code == SLURM_SUCCESS) {
            ...
            if (job_desc_msg->job_id == NO_VAL) {
                ...
                error_code = job_allocate(job_desc_msg, false,
                                          true, &resp, true, uid, &job_ptr, &err_msg,
                                          msg->protocol_version);
...
static void _slurm_rpc_node_registration(slurm_msg_t *msg,
    ...
    if (error_code) {
        error("_slurm_rpc_node_registration node=%s: %s",
              node_reg_stat_msg->node_name, slurm_strerror(error_code));
        slurm_send_rc_msg(msg, error_code);
    } else {
...
static void _slurm_rpc_job_alloc_info(slurm_msg_t *msg) {
    ...
    slurmctld_lock_t job_read_lock = { READ_LOCK, READ_LOCK, READ_LOCK, NO_LOCK,
        NO_LOCK };
    uid_t uid = g_slurm_auth_get_uid(msg->auth_cred);
    ...
    if (error_code || (job_ptr == NULL) || (job_ptr->job_resrcs == NULL)) {
        unlock_slurmctld(job_read_lock);
        debug2("%s: JobId=%u, uid=%u: %s", __func__, job_info_msg->job_id, uid,
               slurm_strerror(error_code));
        slurm_send_rc_msg(msg, error_code);
    } else {
        debug("%s: JobId=%u NodeList=%s %s", __func__, job_info_msg->job_id,
    ...
    if (!job_ptr->pack_job_list) {
        ...
        while ((pack_job = (struct job_record*) list_next(iter))) {
            ...
            if (working_cluster_rec) {
                ...
                set_remote_working_response(job_info_resp_msg, pack_job,
                                            job_info_msg->req_cluster);
...
static slurm_addr_t* _build_node_addr(char *node_list, uint32_t node_cnt,
    ...
    if ((host_list = hostlist_create(node_list)) == NULL) {
        error("%s hostlist_create error for pack JobId=%u (%s): %m", __func__,
              pack_job_id, node_list);
        return NULL;
    }
    ...
    while ((this_node_name = hostlist_shift(host_list))) {
        if ((node_ptr = find_node_record(this_node_name))) {
            memcpy(node_addr + node_inx, &node_ptr->slurm_addr,
                   sizeof(slurm_addr_t));
            node_inx++;
        } else {
            error("%s: Invalid node %s in pack JobId=%u", __func__,
                  this_node_name, pack_job_id);
            error_code = SLURM_ERROR;
        }
        free(this_node_name);
        if (error_code != SLURM_SUCCESS)
...
static void _slurm_rpc_job_sbcast_cred(slurm_msg_t *msg) {
    ...
    if (job_info_msg->pack_job_offset == NO_VAL) {
        ...
        if (job_ptr && job_ptr->pack_job_list) { /* Do full pack job */
            ...
            if (!error_code) {
                ...
                node_addr = _build_node_addr(node_list, node_cnt,
                                             job_ptr->job_id);
                if (!node_addr)
                    error_code = SLURM_ERROR;
            }
    ...
    if ((error_code == SLURM_SUCCESS) && job_ptr
        ...
        if (!step_ptr) {
            ...
            error_code = ESLURM_INVALID_JOB_ID;
                   ...
                   && (step_ptr->step_layout->node_cnt != job_ptr->node_cnt)) {
    ...
    if ((sbcast_cred = create_sbcast_cred(slurmctld_config.cred_ctx,
                                          &sbcast_arg, msg->protocol_version)) == NULL) {
        unlock_slurmctld(job_read_lock);
        error("_slurm_rpc_job_sbcast_cred %pJ cred create error", job_ptr);
        slurm_send_rc_msg(msg, SLURM_ERROR);
    } else {
        if (job_ptr && (job_info_msg->step_id != NO_VAL)) {
            info("_slurm_rpc_job_sbcast_cred %pJ NodeList=%s %s", job_ptr,
                 node_list, TIME_STR);
        } else {
...
static void _slurm_rpc_reconfigure_controller(slurm_msg_t *msg) {
    ...
    slurmctld_lock_t config_write_lock = { WRITE_LOCK, WRITE_LOCK, WRITE_LOCK,
        WRITE_LOCK, NO_LOCK };
    uid_t uid = g_slurm_auth_get_uid(msg->auth_cred);
...
static void _slurm_rpc_shutdown_controller(slurm_msg_t *msg) {
    ...
    if (msg->msg_type == REQUEST_CONTROL) {
        ...
        if (slurmctld_config.resume_backup)
            error(
                  "%s: REQUEST_CONTROL reply but backup not completely done relinquishing control.  Old state possible",
                  __func__);
    }
...
static void _slurm_rpc_step_complete(slurm_msg_t *msg, bool running_composite) {
    ...
    if (slurmctld_conf.debug_flags & DEBUG_FLAG_STEPS)
        info(
             "Processing RPC: REQUEST_STEP_COMPLETE for JobId=%u StepId=%u nodes %u-%u rc=%u uid=%d",
    ...
    error_code = job_step_complete(req->job_id, req->job_step_id, uid, false,
                                   step_rc);
    if (!running_composite) {
        unlock_slurmctld(job_write_lock);
        _throttle_fini(&active_rpc_cnt);
    ...
    if (error_code) {
        if (slurmctld_conf.debug_flags & DEBUG_FLAG_STEPS)
            info("%s 1 JobId=%u StepId=%u %s", __func__, req->job_id,
                 req->job_step_id, slurm_strerror(error_code));
        slurm_send_rc_msg(msg, error_code);
    } else {
        if (slurmctld_conf.debug_flags & DEBUG_FLAG_STEPS)
            sched_info("%s JobId=%u StepId=%u %s", __func__, req->job_id,
                       req->job_step_id,
                       TIME_STR);
        slurm_send_rc_msg(msg, SLURM_SUCCESS);
        dump_job = true;
...
static void _slurm_rpc_step_layout(slurm_msg_t *msg) {
    ...
    if (error_code || (job_ptr == NULL)) {
        ...
        if (error_code == ESLURM_ACCESS_DENIED) {
            ...
            if (slurmctld_conf.debug_flags & DEBUG_FLAG_STEPS)
                info("%s: JobId=%u, uid=%u: %s", __func__, req->job_id, uid,
                     slurm_strerror(error_code));
        }
        slurm_send_rc_msg(msg, error_code);
        return;
    ...
    if (!step_ptr) {
        unlock_slurmctld(job_read_lock);
        if (slurmctld_conf.debug_flags & DEBUG_FLAG_STEPS)
            info("%s: %pJ StepId=%u Not Found", __func__, job_ptr,
                 req->step_id);
...
static void _slurm_rpc_submit_batch_job(slurm_msg_t *msg) {
    ...
    job_desc_msg_t *job_desc_msg = (job_desc_msg_t*) msg->data;
    /* Locks: Read config, read job, read node, read partition */
    slurmctld_lock_t job_read_lock = { READ_LOCK, READ_LOCK, READ_LOCK,
        READ_LOCK, READ_LOCK };
    ...
    if ((error_code = _valid_id("REQUEST_SUBMIT_BATCH_JOB", job_desc_msg, uid,
                                gid))) {
        reject_job = true;
        goto send_msg;
    }
...
static void _slurm_rpc_submit_batch_pack_job(slurm_msg_t *msg) {
    ...
    while ((job_desc_msg = (job_desc_msg_t*) list_next(iter))) {
        ...
        if (err_msg) {
            ...
            while (tok) {
                ...
                if (job_submit_user_msg)
                    sep = "\n";
                xstrfmtcat(job_submit_user_msg, "%s%d: %s", sep,
                           pack_job_offset, tok);
                tok = strtok_r(NULL, "\n", &save_ptr);
            }
            xfree(err_msg);
    ...
    while ((job_desc_msg = (job_desc_msg_t*) list_next(iter))) {
        ...
        if (!job_desc_msg->burst_buffer) {
            xfree(job_desc_msg->script);
            if (!(job_desc_msg->script = bb_g_build_pack_script(script,
                                                                pack_job_offset))) {
...
static void _slurm_rpc_update_job(slurm_msg_t *msg) {
    ...
    if (error_code == SLURM_SUCCESS) {
        ...
        while (error_code == ESLURM_JOB_SETTING_DB_INX) {
            ...
            if (error_code == ESLURM_JOB_SETTING_DB_INX) {
                ...
                if (job_desc_msg->job_id_str) {
                    debug(
                          "%s: We cannot update JobId=%s at the moment, we are setting the db index, waiting",
                          __func__, job_desc_msg->job_id_str);
                } else {
                    debug(
                          "%s: We cannot update JobId=%u at the moment, we are setting the db index, waiting",
    ...
    if (error_code) {
        if (job_desc_msg->job_id_str) {
            info("%s: JobId=%s uid=%d: %s", __func__, job_desc_msg->job_id_str,
                 uid, slurm_strerror(error_code));
        } else {
            info("%s: JobId=%u uid=%d: %s", __func__, job_desc_msg->job_id, uid,
                 slurm_strerror(error_code));
        }
    } else {
        if (job_desc_msg->job_id_str) {
...
static void _slurm_rpc_update_front_end(slurm_msg_t *msg) {
    ...
    if (error_code) {
        info("_slurm_rpc_update_front_end for %s: %s",
             update_front_end_msg_ptr->name, slurm_strerror(error_code));
        slurm_send_rc_msg(msg, error_code);
    } else {
...
static void _slurm_rpc_update_node(slurm_msg_t *msg) {
    ...
    slurmctld_lock_t node_write_lock = { NO_LOCK, WRITE_LOCK, WRITE_LOCK,
    ...
    if (error_code) {
        info("_slurm_rpc_update_node for %s: %s",
             update_node_msg_ptr->node_names, slurm_strerror(error_code));
        slurm_send_rc_msg(msg, error_code);
    } else {
...
static void _slurm_rpc_update_layout(slurm_msg_t *msg) {
    ...
    /* return result */
    if (error_code) {
        info("_slurm_rpc_update_layout for %s: %s", msg_ptr->layout,
             slurm_strerror(error_code));
        slurm_send_rc_msg(msg, error_code);
    } else {
...
static void _slurm_rpc_update_partition(slurm_msg_t *msg) {
    ...
    slurmctld_lock_t part_write_lock = { READ_LOCK, WRITE_LOCK, READ_LOCK,
        WRITE_LOCK, NO_LOCK };
    uid_t uid = g_slurm_auth_get_uid(msg->auth_cred);
...
static void _slurm_rpc_update_powercap(slurm_msg_t *msg) {
    ...
    slurmctld_lock_t config_write_lock = { WRITE_LOCK, NO_LOCK, READ_LOCK,
        NO_LOCK, NO_LOCK };
    uid_t uid = g_slurm_auth_get_uid(msg->auth_cred);
...
static void _slurm_rpc_delete_partition(slurm_msg_t *msg) {
    ...
    slurmctld_lock_t part_write_lock = { NO_LOCK, WRITE_LOCK, READ_LOCK,
        WRITE_LOCK, NO_LOCK };
    uid_t uid = g_slurm_auth_get_uid(msg->auth_cred);
...
static void _slurm_rpc_resv_create(slurm_msg_t *msg) {
    ...
    slurmctld_lock_t node_write_lock = { READ_LOCK, READ_LOCK, WRITE_LOCK,
        READ_LOCK, NO_LOCK };
    uid_t uid = g_slurm_auth_get_uid(msg->auth_cred);
...
static void _slurm_rpc_resv_update(slurm_msg_t *msg) {
    ...
    /* return result */
    if (error_code) {
        info("_slurm_rpc_resv_update reservation=%s: %s", resv_desc_ptr->name,
             slurm_strerror(error_code));
        slurm_send_rc_msg(msg, error_code);
    } else {
...
static void _slurm_rpc_layout_show(slurm_msg_t *msg) {
    layout_info_request_msg_t *layout_req_msg =
    (layout_info_request_msg_t*) msg->data;
    DEF_TIMERS
    ;
    slurm_msg_t response_msg;
    char *dump;
    ...
    if (layout_req_msg->layout_type == NULL) {
        ...
        if (layouts_pack_layout(layout_req_msg->layout_type,
                                layout_req_msg->entities, layout_req_msg->type,
                                layout_req_msg->flags, buffer) != SLURM_SUCCESS) {
            debug2("%s: unable to get layout[%s]", __func__,
...
inline static void _slurm_rpc_suspend(slurm_msg_t *msg) {
    ...
    info("Processing RPC: REQUEST_SUSPEND(%s) from uid=%u", op,
         (unsigned int) uid);
    ...
    if (!job_ptr)
        error_code = ESLURM_INVALID_JOB_ID;
             ...
             && fed_mgr_is_origin_job(job_ptr) && IS_JOB_REVOKED(job_ptr)
             && job_ptr->fed_details->cluster_lock
             && (job_ptr->fed_details->cluster_lock
                 != fed_mgr_cluster_rec->fed.id)) {
        
        /* Route to the cluster that is running the job. */
        slurmdb_cluster_rec_t *dst = fed_mgr_get_cluster_by_id(
                                                               job_ptr->fed_details->cluster_lock);
        if (dst) {
            slurm_send_reroute_msg(msg, dst);
            info("%s: %s %pJ uid %d routed to %s", __func__,
                 rpc_num2string(msg->msg_type), job_ptr, uid, dst->name);
...
inline static void _slurm_rpc_top_job(slurm_msg_t *msg) {
    ...
    if (error_code) {
        info("%s for %s %s", __func__, top_ptr->job_id_str,
             slurm_strerror(error_code));
    } else {
...
inline static void _slurm_rpc_checkpoint(slurm_msg_t *msg) {
    ...
    debug2("Processing RPC: REQUEST_CHECKPOINT(%s) from uid=%u", op,
           (unsigned int) uid);
    ...
    if (error_code) {
        if (ckpt_ptr->step_id == SLURM_BATCH_SCRIPT) {
            info("_slurm_rpc_checkpoint %s %u: %s", op, ckpt_ptr->job_id,
                 slurm_strerror(error_code));
        } else {
            info("_slurm_rpc_checkpoint %s JobId=%u StepId=%u: %s", op,
                 ckpt_ptr->job_id, ckpt_ptr->step_id,
                 slurm_strerror(error_code));
        }
    } else {
        if (ckpt_ptr->step_id == SLURM_BATCH_SCRIPT) {
            info("_slurm_rpc_checkpoint %s for JobId=%u %s", op,
                 ckpt_ptr->job_id, TIME_STR);
        } else {
            info("_slurm_rpc_checkpoint %s for JobId=%u StepId=%u %s", op,
...
inline static void _slurm_rpc_get_topo(slurm_msg_t *msg) {
    ...
    topo_resp_msg->topo_array = xmalloc(
                                        sizeof(topo_info_t) * topo_resp_msg->record_count);
    for (i = 0; i < topo_resp_msg->record_count; i++) {
        ...
        topo_resp_msg->topo_array[i].name = xstrdup(
                                                    switch_record_table[i].name);
        topo_resp_msg->topo_array[i].nodes = xstrdup(
                                                     switch_record_table[i].nodes);
        topo_resp_msg->topo_array[i].switches = xstrdup(
                                                        switch_record_table[i].switches);
    }
    unlock_slurmctld(node_read_lock);
...
inline static void _slurm_rpc_job_notify(slurm_msg_t *msg) {
    ...
    if (!job_ptr)
        error_code = ESLURM_INVALID_JOB_ID;
    else if (job_ptr->batch_flag && fed_mgr_cluster_rec && job_ptr->fed_details
             && fed_mgr_is_origin_job(job_ptr) && IS_JOB_REVOKED(job_ptr)
             && job_ptr->fed_details->cluster_lock
             && (job_ptr->fed_details->cluster_lock
                 != fed_mgr_cluster_rec->fed.id)) {
...
inline static void _slurm_rpc_set_debug_flags(slurm_msg_t *msg) {
    uid_t uid = g_slurm_auth_get_uid(msg->auth_cred);
    slurmctld_lock_t config_write_lock = { WRITE_LOCK, READ_LOCK, READ_LOCK,
        READ_LOCK, READ_LOCK };
...
inline static void _slurm_rpc_set_debug_level(slurm_msg_t *msg) {
    int debug_level;
    uid_t uid = g_slurm_auth_get_uid(msg->auth_cred);
    slurmctld_lock_t config_write_lock = { WRITE_LOCK, NO_LOCK, NO_LOCK,
        NO_LOCK, NO_LOCK };
...
inline static void _slurm_rpc_accounting_update_msg(slurm_msg_t *msg) {
    ...
    if (update_ptr->update_list && list_count(update_ptr->update_list)) {
        ...
        if ((object = list_find_first(update_ptr->update_list,
                                      _find_update_object_in_list, &fed_type))) {
#if HAVE_SYS_PRCTL_H
...
inline static void _slurm_rpc_dump_spank(slurm_msg_t *msg) {
    int rc = SLURM_SUCCESS;
    spank_env_request_msg_t *spank_req_msg =
    (spank_env_request_msg_t*) msg->data;
    ...
    START_TIMER;
    debug("Processing RPC: REQUEST_SPANK_ENVIRONMENT from uid=%d JobId=%u", uid,
          spank_req_msg->job_id);
    if (!validate_slurm_user(uid)) {
        rc = ESLURM_USER_ID_MISSING;
...
inline static void _slurm_rpc_kill_job(slurm_msg_t *msg) {
    ...
    slurmctld_lock_t fed_job_read_lock = { NO_LOCK, READ_LOCK, NO_LOCK, NO_LOCK,
        READ_LOCK };
    slurmctld_lock_t lock = { READ_LOCK, WRITE_LOCK, WRITE_LOCK, NO_LOCK,
        READ_LOCK };
    uid_t uid;
    int cc;
    ...
    if (cc == ESLURM_ALREADY_DONE) {
        debug2("%s: job_str_signal() JobId=%s sig %d returned %s", __func__,
               kill->sjob_id, kill->signal, slurm_strerror(cc));
    } else if (cc != SLURM_SUCCESS) {
        info("%s: job_str_signal() JobId=%s sig %d returned %s", __func__,
             kill->sjob_id, kill->signal, slurm_strerror(cc));
    } else {
...
static void _slurm_rpc_composite_msg(slurm_msg_t *msg) {
    ...
    _slurm_rpc_comp_msg_list(comp_msg, &run_scheduler, comp_resp_msg.msg_list,
                             &start_tv, sched_timeout);
    unlock_slurmctld(job_write_lock);
    _throttle_fini(&active_rpc_cnt);
    ...
    while ((next_msg = list_next(itr))) {
        ...
        switch (next_msg->msg_type) {
            case MESSAGE_COMPOSITE:
                ...
                comp_resp_msg->msg_list = list_create(
                                                      _slurmctld_free_comp_msg_list);
                ...
                _slurm_rpc_comp_msg_list(ncomp_msg, run_scheduler,
                                         comp_resp_msg->msg_list, start_tv, timeout);
                if (list_count(comp_resp_msg->msg_list)) {
                     ...
                     */
                    memcpy(&comp_resp_msg->sender, &ncomp_msg->sender,
                           sizeof(slurm_addr_t));
...
static void _slurm_rpc_assoc_mgr_info(slurm_msg_t *msg) {
    ...
    assoc_mgr_info_get_pack_msg(&dump, &dump_size,
                                (assoc_mgr_info_request_msg_t*) msg->data, uid, acct_db_conn,
                                msg->protocol_version);
...
end_it:
    ...
    if (slurm_persist_send_msg(&p_tmp, ret_buf) != SLURM_SUCCESS) {
        debug("Problem sending response to connection %d uid(%d)", p_tmp.fd,
              uid);
    }
    ...
    while ((single_req_buf = list_next(iter))) {
        ...
        switch (sub_msg.msg_type) {
            ...
            case REQUEST_SIB_MSG:
                ...
                break;
            default:
                error("%s: Unsupported Message Type:%s", __func__,
                      rpc_num2string(sub_msg.msg_type));
        }
    ...
    if (!msg->conn && fed_mgr_fed_rec) {
        ...
        if (origin_id && (origin_id != fed_mgr_cluster_rec->fed.id)) {
            ...
            if (!dst) {
                ...
                slurm_send_reroute_msg(msg, dst);
                info("%s: %s JobId=%u uid %d routed to %s", __func__,
                     rpc_num2string(msg->msg_type), job_id, uid, dst->name);
            }
...
inline static void _slurm_rpc_set_fs_dampening_factor(slurm_msg_t *msg) {
    uid_t uid = g_slurm_auth_get_uid(msg->auth_cred);
    slurmctld_lock_t config_write_lock = { WRITE_LOCK, WRITE_LOCK, READ_LOCK,
        READ_LOCK, READ_LOCK };
