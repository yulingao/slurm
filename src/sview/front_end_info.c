...
static display_data_t display_data_front_end[] = {{G_TYPE_INT,     SORTID_POS,               NULL,              false, EDIT_NONE,  refresh_front_end, create_model_front_end, admin_edit_front_end},
                                                  {G_TYPE_STRING,  SORTID_NAME,              "Name",            false, EDIT_NONE,  refresh_front_end, create_model_front_end, admin_edit_front_end},
                                                  {G_TYPE_STRING,  SORTID_COLOR,             NULL,              true,  EDIT_COLOR, refresh_front_end, create_model_front_end, admin_edit_front_end},
                                                  {G_TYPE_STRING,  SORTID_STATE,             "State",           false, EDIT_MODEL, refresh_front_end, create_model_front_end, admin_edit_front_end},
                                                  {G_TYPE_STRING,  SORTID_BOOT_TIME,         "BootTime",        false, EDIT_NONE,  refresh_front_end, create_model_front_end, admin_edit_front_end},
                                                  {G_TYPE_STRING,  SORTID_SLURMD_START_TIME, "SlurmdStartTime", false, EDIT_NONE,  refresh_front_end, create_model_front_end, admin_edit_front_end},
                                                  {G_TYPE_STRING,  SORTID_REASON,            "Reason",          false, EDIT_NONE,  refresh_front_end, create_model_front_end, admin_edit_front_end},
                                                  {G_TYPE_STRING,  SORTID_ALLOW_GROUPS,      "Allow Groups",    false, EDIT_NONE,  refresh_front_end, create_model_front_end, admin_edit_front_end},
                                                  {G_TYPE_STRING,  SORTID_ALLOW_USERS,       "Allow Users",     false, EDIT_NONE,  refresh_front_end, create_model_front_end, admin_edit_front_end},
                                                  {G_TYPE_STRING,  SORTID_DENY_GROUPS,       "Deny Groups",     false, EDIT_NONE,  refresh_front_end, create_model_front_end, admin_edit_front_end},
                                                  {G_TYPE_STRING,  SORTID_DENY_USERS,        "Deny Users",      false, EDIT_NONE,  refresh_front_end, create_model_front_end, admin_edit_front_end},
                                                  {G_TYPE_STRING,  SORTID_VERSION,           "Version",         false, EDIT_NONE,  refresh_front_end, create_model_front_end, admin_edit_front_end},
                                                  {G_TYPE_INT,     SORTID_COLOR_INX,         NULL,              false, EDIT_NONE,  refresh_front_end, create_model_front_end, admin_edit_front_end},
                                                  {G_TYPE_POINTER, SORTID_NODE_INX,          NULL,              false, EDIT_NONE,  refresh_front_end, create_model_front_end, admin_edit_front_end},
                                                  {G_TYPE_INT,     SORTID_UPDATED,           NULL,              false, EDIT_NONE,  refresh_resv,      create_model_resv,      admin_edit_resv},
                                                  {G_TYPE_NONE,    -1,                       NULL,              false, EDIT_NONE}};

static display_data_t options_data_front_end[] = {{G_TYPE_INT,    SORTID_POS,     NULL,                    false, EDIT_NONE},
                                                  {G_TYPE_STRING, INFO_PAGE,      "Full Info",             true,  FRONT_END_PAGE},
                                                  {G_TYPE_STRING, FRONT_END_PAGE, "Drain Front End Node",  true,  ADMIN_PAGE},
                                                  {G_TYPE_STRING, FRONT_END_PAGE, "Resume Front End Node", true,  ADMIN_PAGE},
                                                  {G_TYPE_NONE,   -1,             NULL,                    false, EDIT_NONE}};
...
static void _admin_front_end(GtkTreeModel *model, GtkTreeIter *iter, char *type, char *node_list);

static void _process_each_front_end(GtkTreeModel *model, GtkTreePath *path, GtkTreeIter *iter, gpointer userdata);
...
static void _layout_front_end_record(GtkTreeView *treeview, sview_front_end_info_t *sview_front_end_info, int update) {

    GtkTreeIter iter;
    front_end_info_t *front_end_ptr = sview_front_end_info->front_end_ptr;
    GtkTreeStore *treestore = GTK_TREE_STORE(gtk_tree_view_get_model(treeview));

    if (!treestore)
        return;

    add_display_treestore_line(update, treestore, &iter, find_col_name(display_data_front_end, SORTID_NAME),
                               front_end_ptr->name);

    add_display_treestore_line(update, treestore, &iter, find_col_name(display_data_front_end, SORTID_STATE),
                               sview_front_end_info->state);

    add_display_treestore_line(update, treestore, &iter, find_col_name(display_data_front_end, SORTID_BOOT_TIME),
                               sview_front_end_info->boot_time);

    add_display_treestore_line(update, treestore, &iter,
                               find_col_name(display_data_front_end, SORTID_SLURMD_START_TIME),
                               sview_front_end_info->slurmd_start_time);

    add_display_treestore_line(update, treestore, &iter, find_col_name(display_data_front_end, SORTID_REASON),
                               sview_front_end_info->reason);

    add_display_treestore_line(update, treestore, &iter, find_col_name(display_data_front_end, SORTID_ALLOW_GROUPS),
                               front_end_ptr->allow_groups);

    add_display_treestore_line(update, treestore, &iter, find_col_name(display_data_front_end, SORTID_ALLOW_USERS),
                               front_end_ptr->allow_users);

    add_display_treestore_line(update, treestore, &iter, find_col_name(display_data_front_end, SORTID_DENY_GROUPS),
                               front_end_ptr->deny_groups);

    add_display_treestore_line(update, treestore, &iter, find_col_name(display_data_front_end, SORTID_DENY_USERS),
                               front_end_ptr->deny_users);

    add_display_treestore_line(update, treestore, &iter, find_col_name(display_data_front_end, SORTID_VERSION),
                               front_end_ptr->version);
}

static void _update_front_end_record(sview_front_end_info_t *sview_front_end_info_ptr, GtkTreeStore *treestore) {
    front_end_info_t *front_end_ptr;

    front_end_ptr = sview_front_end_info_ptr->front_end_ptr;

    /* Combining these records provides a slight performance improvement */
    gtk_tree_store_set(treestore, &sview_front_end_info_ptr->iter_ptr, SORTID_ALLOW_GROUPS, front_end_ptr->allow_groups,
                       SORTID_ALLOW_USERS, front_end_ptr->allow_users, SORTID_BOOT_TIME,
                       sview_front_end_info_ptr->boot_time, SORTID_COLOR,
                       sview_colors[sview_front_end_info_ptr->color_inx], SORTID_COLOR_INX,
                       sview_front_end_info_ptr->color_inx, SORTID_DENY_GROUPS, front_end_ptr->deny_groups,
                       SORTID_DENY_USERS, front_end_ptr->deny_users, SORTID_NODE_INX,
                       sview_front_end_info_ptr->node_inx, SORTID_NAME, front_end_ptr->name, SORTID_REASON,
                       sview_front_end_info_ptr->reason, SORTID_SLURMD_START_TIME,
                       sview_front_end_info_ptr->slurmd_start_time, SORTID_STATE, sview_front_end_info_ptr->state,
                       SORTID_UPDATED, 1, SORTID_VERSION, front_end_ptr->version, -1);

    return;
}

static void _append_front_end_record(sview_front_end_info_t *sview_front_end_info_ptr, GtkTreeStore *treestore) {
    gtk_tree_store_append(treestore, &sview_front_end_info_ptr->iter_ptr, NULL);
    gtk_tree_store_set(treestore, &sview_front_end_info_ptr->iter_ptr, SORTID_POS, sview_front_end_info_ptr->pos, -1);
...
static void _update_info_front_end(List info_list, GtkTreeView *tree_view) {
    ...
    while ((sview_front_end_info = list_next(itr))) {
        ...
        if (sview_front_end_info->iter_set) {
            gtk_tree_model_get(model, &sview_front_end_info->iter_ptr, SORTID_NAME, &name, -1);
            if (xstrcmp(name, sview_front_end_info->front_end_name)) {
                /* Bad pointer */
                sview_front_end_info->iter_set = false;
        ...
        if (sview_front_end_info->iter_set)
            _update_front_end_record(sview_front_end_info, GTK_TREE_STORE(model));
        else {
...
static List _create_front_end_info_list(front_end_info_msg_t *front_end_info_ptr, int changed) {
    char *upper = NULL;
    ...
    for (i = 0; i < front_end_info_ptr->record_count; i++) {
        ...
        if (last_list_itr) {
            while ((sview_front_end_info_ptr = list_next(last_list_itr))) {
                if (!xstrcmp(sview_front_end_info_ptr->front_end_name, front_end_ptr->name)) {
                    list_remove(last_list_itr);
                    _front_end_info_free(sview_front_end_info_ptr);
                    break;
                }
            }
        ...
        if (!sview_front_end_info_ptr)
            sview_front_end_info_ptr = xmalloc(sizeof(sview_front_end_info_t));
        ...
        if (g_node_info_ptr) {
            ...
            sview_front_end_info_ptr->node_inx[1] = g_node_info_ptr->record_count - 1;
            sview_front_end_info_ptr->node_inx[2] = -1;
        } else
            sview_front_end_info_ptr->node_inx[0] = -1;
        if (front_end_ptr->boot_time) {
            slurm_make_time_str(&front_end_ptr->boot_time, time_str, sizeof(time_str));
            sview_front_end_info_ptr->boot_time = xstrdup(time_str);
        }
        if (front_end_ptr->slurmd_start_time) {
            slurm_make_time_str(&front_end_ptr->slurmd_start_time, time_str, sizeof(time_str));
            sview_front_end_info_ptr->slurmd_start_time = xstrdup(time_str);
        }
        upper = node_state_string(front_end_ptr->node_state);
        sview_front_end_info_ptr->state = str_tolower(upper);

        if (front_end_ptr->reason && front_end_ptr->reason_time && (front_end_ptr->reason_uid != NO_VAL)) {
            struct passwd *pw = NULL;

            if ((pw = getpwuid(front_end_ptr->reason_uid)))
                snprintf(user, sizeof(user), "%s", pw->pw_name);
            else
                snprintf(user, sizeof(user), "Unk(%u)", front_end_ptr->reason_uid);
            slurm_make_time_str(&front_end_ptr->reason_time, time_str, sizeof(time_str));
            sview_front_end_info_ptr->reason = xstrdup_printf("%s [%s@%s]", front_end_ptr->reason, user, time_str);
        } else {
            sview_front_end_info_ptr->reason = xstrdup(front_end_ptr->reason);
        }
    ...
    while ((sview_fe_info = list_next(itr))) {
        ...
        if (xstrcmp(front_end_ptr->name, name) == 0) {
            ...
            while (sview_fe_info->node_inx[j] >= 0) {
                change_grid_color(popup_win->grid_button_list, sview_fe_info->node_inx[j],
                                  sview_fe_info->node_inx[j + 1], sview_fe_info->color_inx, true, 0);
                j += 2;
            }
            _layout_front_end_record(treeview, sview_fe_info, update);
            found = 1;
            break;
        }
    }
...
extern int get_new_info_front_end(front_end_info_msg_t **info_ptr, int force) {
    ...
    if (g_front_end_info_ptr && !force && ((now - last) < working_sview_config.refresh_delay)) {
    ...
    if (g_front_end_info_ptr) {
        error_code = slurm_load_front_end(g_front_end_info_ptr->last_update, &new_front_end_ptr);
        ...
        error_code = slurm_load_front_end((time_t) NULL, &new_front_end_ptr);
        changed = 1;
    }
...
extern void
admin_edit_front_end(GtkCellRendererText *cell, const char *path_string, const char *new_text, gpointer data) {
    ...
    switch (column) {
        case SORTID_STATE:
            gtk_tree_model_get(GTK_TREE_MODEL(treestore), &iter, SORTID_NAME, &node_list, -1);
            _admin_front_end(GTK_TREE_MODEL(treestore), &iter, (char *) new_text, node_list);
            g_free(node_list);
        default:
            break;
    }
...
extern void get_info_front_end(GtkTable *table, display_data_t *display_data) {
    ...
    if (!set_opts)
        set_page_opts(FRONT_END_PAGE, display_data_front_end, SORTID_CNT, _initial_page_opts);
    set_opts = true;
    ...
    if (error_code == SLURM_NO_CHANGE_IN_DATA) {
        ...
        if (display_widget)
            gtk_widget_destroy(display_widget);
        view = ERROR_VIEW;
        sprintf(error_char, "slurm_load_front_end: %s", slurm_strerror(slurm_get_errno()));
    ...
    if (!path) {
        ...
        while ((fe_ptr = list_next(itr))) {
            ...
            while (fe_ptr->node_inx[j] >= 0) {
                change_grid_color(grid_button_list, fe_ptr->node_inx[j], fe_ptr->node_inx[j + 1], fe_ptr->color_inx,
                                  true, 0);
                j += 2;
            }
        }
    ...
    if (!display_widget) {
        ...
        /* since this function sets the model of the tree_view
           ...
           the return value */
        create_treestore(tree_view, display_data_front_end, SORTID_CNT, SORTID_NAME, SORTID_COLOR);
    }
...
extern void specific_info_front_end(popup_info_t *popup_win) {
    ...
    resv_error_code = get_new_info_front_end(&front_end_info_ptr, popup_win->force_refresh);
    ...
    while ((sview_front_end_info_ptr = list_next(itr))) {
        ...
        switch (spec_info->type) {
            ...
            case NODE_PAGE:
                break;
            case JOB_PAGE:
                if (xstrcmp(front_end_ptr->name, search_info->gchar_data))
                    continue;
                break;
            case RESV_PAGE:
    ...
    _update_info_front_end(send_resv_list, GTK_TREE_VIEW(spec_info->display_widget));
...
extern void set_menus_front_end(void *arg, void *arg2, GtkTreePath *path, int type) {
...
extern void popup_all_front_end(GtkTreeModel *model, GtkTreeIter *iter, int id) {
    ...
    if (!popup_win) {
        if (id == INFO_PAGE)
            popup_win = create_popup_info(id, FRONT_END_PAGE, title);
        else {
            popup_win = create_popup_info(FRONT_END_PAGE, id, title);
        }
    } else {
        g_free(name);
...
static void _process_each_front_end(GtkTreeModel *model, GtkTreePath *path, GtkTreeIter *iter, gpointer user_data) {
    char *name = NULL;
...
extern void
select_admin_front_end(GtkTreeModel *model, GtkTreeIter *iter, display_data_t *display_data, GtkTreeView *treeview) {
    if (treeview) {
        char *node_list;
        hostlist_t hl = NULL;
        ...
        gtk_tree_selection_selected_foreach(gtk_tree_view_get_selection(treeview), _process_each_front_end, &user_data);
...
static void _admin_front_end(GtkTreeModel *model, GtkTreeIter *iter, char *type, char *node_list) {
    ...
    snprintf(tmp_char, sizeof(tmp_char), "Are you sure you want to set state of front end node %s "
    ...
    if (rc == GTK_RESPONSE_OK) {
        ...
        if (entry) {
            front_end_update_msg.reason = xstrdup(gtk_entry_get_text(GTK_ENTRY(entry)));
            if (!front_end_update_msg.reason || !strlen(front_end_update_msg.reason)) {
                lower = g_strdup_printf("You need a reason to do that.");
                display_edit_note(lower);
                g_free(lower);
                goto end_it;
            }
            rc = uid_from_string(getlogin(), &front_end_update_msg.reason_uid);
            if (rc < 0)
        ...
        if (rc == SLURM_SUCCESS) {
            lower = g_strdup_printf("Nodes %s updated successfully.", node_list);
            display_edit_note(lower);
            g_free(lower);
        } else {
            lower = g_strdup_printf("Problem updating nodes %s: %s", node_list, slurm_strerror(rc));
            display_edit_note(lower);
            g_free(lower);
        }
    }
