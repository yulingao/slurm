...
void *_refresh_thr(gpointer arg) {
    ...
    sleep(5);
    gdk_threads_enter();
    gtk_statusbar_remove(GTK_STATUSBAR(main_statusbar), STATUS_REFRESH, msg_id);
    //gdk_flush();
    gdk_threads_leave();
    return NULL;
}

static gboolean _delete_popup(GtkWidget *widget, GtkWidget *event, gpointer data) {
...
void _search_entry(sview_search_info_t *sview_search_info) {
    ...
    if (sview_search_info->int_data == NO_VAL &&
        (!sview_search_info->gchar_data || !strlen(sview_search_info->gchar_data))) {
    ...
    switch (sview_search_info->search_type) {
        ...
        case SEARCH_JOB_ID:
            id = JOB_PAGE;
            snprintf(title, 100, "Job %s info", sview_search_info->gchar_data);
            break;
        case SEARCH_JOB_USER:
            id = JOB_PAGE;
            snprintf(title, 100, "Job(s) info for user %s", sview_search_info->gchar_data);
            break;
        case SEARCH_PARTITION_NAME:
            id = PART_PAGE;
            snprintf(title, 100, "Partition %s info", sview_search_info->gchar_data);
            break;
        ...
        case SEARCH_NODE_NAME:
            id = NODE_PAGE;
            snprintf(title, 100, "Node(s) %s info", sview_search_info->gchar_data);
            break;
        ...
        case SEARCH_RESERVATION_NAME:
            id = RESV_PAGE;
            snprintf(title, 100, "Reservation %s info", sview_search_info->gchar_data);
            break;
        default:
    ...
    memcpy(popup_win->spec_info->search_info, sview_search_info, sizeof(sview_search_info_t));
...
static GtkTreeStore *_local_create_treestore_2cols(GtkWidget *popup, int x, int y) {
    ...
    gtk_window_set_default_size(GTK_WINDOW(popup), x, y);
...
static void
_gtk_print_key_pairs(List config_list, char *title, bool first, GtkTreeStore *treestore, GtkTreeIter *iter) {
    ListIterator itr = NULL;
    ...
    add_display_treestore_line_with_font(update, treestore, iter, title, NULL, "bold");

    itr = list_iterator_create(config_list);
    while ((key_pair = list_next(itr)))
        add_display_treestore_line(update, treestore, iter, key_pair->name, key_pair->value);
    list_iterator_destroy(itr);
}

static void
_gtk_print_config_plugin_params_list(List l, char *title, bool first, GtkTreeStore *treestore, GtkTreeIter *iter) {
    ListIterator itr = NULL;
    ...
    while ((p = list_next(itr))) {
        add_display_treestore_line_with_font(update, treestore, iter, p->name, NULL, "italic");
        _gtk_print_key_pairs(p->key_pairs, NULL, 1, treestore, iter);
    }
    list_iterator_destroy(itr);
}

static void _layout_conf_ctl(GtkTreeStore *treestore, slurm_ctl_conf_info_msg_t *slurm_ctl_conf_ptr) {
    ...
    snprintf(tmp_str, sizeof(tmp_str), "Configuration data as of %s", time_str);

    ret_list = slurm_ctl_conf_2_key_pairs(slurm_ctl_conf_ptr);
    _gtk_print_key_pairs(ret_list, tmp_str, 1, treestore, &iter);
    FREE_NULL_LIST(ret_list);

    _gtk_print_key_pairs(slurm_ctl_conf_ptr->acct_gather_conf, "Account Gather", 0, treestore, &iter);

    _gtk_print_key_pairs(slurm_ctl_conf_ptr->cgroup_conf, "Cgroup Support", 0, treestore, &iter);

    _gtk_print_key_pairs(slurm_ctl_conf_ptr->ext_sensors_conf, "External Sensors", 0, treestore, &iter);

    xstrcat(tmp_title, "Node Features:");
    _gtk_print_config_plugin_params_list(slurm_ctl_conf_ptr->node_features_conf, tmp_title, 0, treestore, &iter);
    xfree(tmp_title);

    xstrcat(tmp_title, "Slurmctld Plugstack Plugins:");
    _gtk_print_config_plugin_params_list(slurm_ctl_conf_ptr->slurmctld_plugstack_conf, tmp_title, 0, treestore, &iter);
    xfree(tmp_title);

    _gtk_print_key_pairs(slurm_ctl_conf_ptr->select_conf_key_pairs, select_title, 0, treestore, &iter);
}
...
static void _layout_conf_dbd(GtkTreeStore *treestore) {
    ...
    char *acct_storage_backup_host = slurm_get_accounting_storage_backup_host();
    ...
    add_display_treestore_line_with_font(update, treestore, &iter, "Slurm Configuration data as of", tmp_str, "bold");

    add_display_treestore_line(update, treestore, &iter, "AccountingStorageBackupHost", acct_storage_backup_host);
    add_display_treestore_line(update, treestore, &iter, "AccountingStorageHost", acct_storage_host);
    add_display_treestore_line(update, treestore, &iter, "AccountingStorageLoc", acct_storage_loc);
    add_display_treestore_line(update, treestore, &iter, "AccountingStoragePass", acct_storage_pass);
    sprintf(tmp_str, "%u", acct_storage_port);
    add_display_treestore_line(update, treestore, &iter, "AccountingStoragePort", tmp_str);
    add_display_treestore_line(update, treestore, &iter, "AccountingStorageType", acct_storage_type);
    add_display_treestore_line(update, treestore, &iter, "AccountingStorageUser", acct_storage_user);
    add_display_treestore_line(update, treestore, &iter, "AuthType", auth_type);
    sprintf(tmp_str, "%u sec", msg_timeout);
    add_display_treestore_line(update, treestore, &iter, "MessageTimeout", tmp_str);
    add_display_treestore_line(update, treestore, &iter, "PluginDir", plugin_dir);
    private_data_string(private_data, tmp_str, sizeof(tmp_str));
    add_display_treestore_line(update, treestore, &iter, "PrivateData", tmp_str);
    user_name = uid_to_string_cached(slurm_user_id);
    sprintf(tmp_str, "%s(%u)", user_name, slurm_user_id);
    add_display_treestore_line(update, treestore, &iter, "SlurmUserId", tmp_str);
    add_display_treestore_line(update, treestore, &iter, "SLURM_CONF", default_slurm_config_file);
    add_display_treestore_line(update, treestore, &iter, "SLURM_VERSION", SLURM_VERSION_STRING);
    sprintf(tmp_str, "%u", track_wckey);
    add_display_treestore_line(update, treestore, &iter, "TrackWCKey", tmp_str);
...
extern void create_config_popup(GtkAction *action, gpointer user_data) {
    GtkWidget *popup = gtk_dialog_new_with_buttons("Slurm Config Info", GTK_WINDOW(user_data),
                                                   GTK_DIALOG_DESTROY_WITH_PARENT, GTK_STOCK_CLOSE, GTK_RESPONSE_OK,
                                                   NULL);
    GtkTreeStore *treestore = _local_create_treestore_2cols(popup, 600, 400);
    ...
    g_signal_connect(G_OBJECT(popup), "delete_event", G_CALLBACK(_delete_popup), NULL);
    g_signal_connect(G_OBJECT(popup), "response", G_CALLBACK(_delete_popup), NULL);
...
extern void create_dbconfig_popup(GtkAction *action, gpointer user_data) {
    List dbd_config_list = NULL;
    GtkWidget *popup = gtk_dialog_new_with_buttons("Slurm Database Config Info", GTK_WINDOW(user_data),
                                                   GTK_DIALOG_DESTROY_WITH_PARENT, GTK_STOCK_CLOSE, GTK_RESPONSE_OK,
                                                   NULL);
    GtkTreeStore *treestore = _local_create_treestore_2cols(popup, 600, 400);
...
extern void create_daemon_popup(GtkAction *action, gpointer user_data) {
    GtkWidget *popup = gtk_dialog_new_with_buttons("Slurm Daemons running", GTK_WINDOW(user_data),
                                                   GTK_DIALOG_DESTROY_WITH_PARENT, GTK_STOCK_CLOSE, GTK_RESPONSE_OK,
                                                   NULL);
    int i, update = 0;
    ...
    GtkTreeStore *treestore = _local_create_treestore_2cols(popup, 300, 100);
    GtkTreeIter iter;
    gtk_window_set_type_hint(GTK_WINDOW(popup), GDK_WINDOW_TYPE_HINT_NORMAL);
    g_signal_connect(G_OBJECT(popup), "delete_event", G_CALLBACK(_delete_popup), NULL);
    g_signal_connect(G_OBJECT(popup), "response", G_CALLBACK(_delete_popup), NULL);
    ...
    for (i = 1; i < conf->control_cnt; i++) {
        if ((b = conf->control_machine[i])) {
            if (!xstrcmp(b, me) || !xstrcasecmp(b, "localhost"))
                ctld = 1;
        }
    }
    if (conf->control_machine[0]) {
        ...
        while (token) {
            if ((xstrcmp(token, me) == 0) || (xstrcasecmp(token, "localhost") == 0)) {
                ctld = 1;
                break;
            }
    ...
    if (actld && ctld)
        add_display_treestore_line(update, treestore, &iter, "Slurmctld", "1");
    if (actld && d)
        add_display_treestore_line(update, treestore, &iter, "Slurmd", "1");
...
extern void create_create_popup(GtkAction *action, gpointer user_data) {
    GtkWidget *popup = gtk_dialog_new_with_buttons("Create", GTK_WINDOW(user_data),
    ...
    if (!xstrcmp(name, "batch_job")) {
        ...
        label = gtk_label_new("Partition creation specifications\n\n"
    ...
    if (response == GTK_RESPONSE_OK) {
        ...
        switch (sview_search_info.search_type) {
            case CREATE_BATCH_JOB:
                response = slurm_submit_batch_job(job_msg, &slurm_alloc_msg);
                if (response == SLURM_SUCCESS) {
                    temp = g_strdup_printf("Job %u submitted", slurm_alloc_msg->job_id);
                } else {
                    temp = g_strdup_printf("Problem submitting job: %s", slurm_strerror(slurm_get_errno()));
                }
            ...
            case CREATE_PARTITION:
                ...
                if (response == SLURM_SUCCESS) {
                    temp = g_strdup_printf("Partition %s created", part_msg->name);
                } else {
                    temp = g_strdup_printf("Problem creating partition: %s", slurm_strerror(slurm_get_errno()));
                }
                display_edit_note(temp);
                g_free(temp);
                break;
            case CREATE_RESERVATION:
                ...
                if (res_name) {
                    temp = g_strdup_printf("Reservation %s created", res_name);
                    free(res_name);
                } else {
                    temp = g_strdup_printf("Problem creating reservation: %s", slurm_strerror(slurm_get_errno()));
                }
                display_edit_note(temp);
                g_free(temp);
                break;
...
extern void create_search_popup(GtkAction *action, gpointer user_data) {
    GtkWidget *popup = gtk_dialog_new_with_buttons("Search", GTK_WINDOW(user_data),
                                                   GTK_DIALOG_MODAL | GTK_DIALOG_DESTROY_WITH_PARENT, NULL);
    ...
    if (!xstrcmp(name, "jobid")) {
        ...
        display_data_t pulldown_display_data[] = {{G_TYPE_NONE, JOB_PENDING,     "Pending",       true,  -1},
        ...
        display_data_t pulldown_display_data[] = {{G_TYPE_NONE, PARTITION_UP,       "Up",       true,  -1},
        ...
        display_data_t pulldown_display_data[] = {{G_TYPE_NONE, NODE_STATE_ALLOCATED,                    "Allocated",  true,  -1},
                                                  {G_TYPE_NONE, NODE_STATE_COMPLETING,                   "Completing", true,  -1},
                                                  {G_TYPE_NONE, NODE_STATE_DOWN,                         "Down",       true,  -1},
                                                  {G_TYPE_NONE, NODE_STATE_ALLOCATED |
                                                                NODE_STATE_DRAIN,                        "Draining",   true,  -1},
                                                  {G_TYPE_NONE, NODE_STATE_IDLE |
                                                                NODE_STATE_DRAIN,                        "Drained",    true,  -1},
                                                  {G_TYPE_NONE, NODE_STATE_FAIL,                         "Fail",       true,  -1},
                                                  {G_TYPE_NONE, NODE_STATE_FAIL |
                                                                NODE_STATE_ALLOCATED,                    "Failing",    true,  -1},
                                                  ...
                                                  {G_TYPE_NONE, NODE_STATE_NO_RESPOND,                   "No Respond", true,  -1},
                                                  {G_TYPE_NONE, NODE_STATE_NET |
                                                                NODE_STATE_IDLE,                         "PerfCTRs",   true,  -1},
                                                  {G_TYPE_NONE, NODE_STATE_POWER_SAVE,                   "Power Down", true,  -1},
                                                  {G_TYPE_NONE, NODE_STATE_POWER_UP,                     "Power Up",   true,  -1},
                                                  {G_TYPE_NONE, NODE_STATE_REBOOT,                       "Reboot",     true,  -1},
                                                  {G_TYPE_NONE, NODE_STATE_RES |
                                                                NODE_STATE_IDLE,                         "Reserved",   true,  -1},
    ...
    gtk_box_pack_start(GTK_BOX(GTK_DIALOG(popup)->vbox), label, false, false, 0);
    gtk_box_pack_start(GTK_BOX(GTK_DIALOG(popup)->vbox), entry, false, false, 0);
    ...
    if (response == GTK_RESPONSE_OK) {
        ...
        switch (sview_search_info.search_type) {
            ...
            case SEARCH_PARTITION_STATE:
                ...
                gtk_tree_model_get(model, &iter, 0, &sview_search_info.int_data, -1);
                break;
            case SEARCH_JOB_ID:
            case SEARCH_JOB_USER:
            case SEARCH_PARTITION_NAME:
            case SEARCH_NODE_NAME:
            case SEARCH_RESERVATION_NAME:
                sview_search_info.gchar_data = g_strdup(gtk_entry_get_text(GTK_ENTRY(entry)));
                break;
            default:
...
extern void change_refresh_popup(GtkAction *action, gpointer user_data) {
    ...
    GtkWidget *label = NULL;
    GtkObject *adjustment = gtk_adjustment_new(working_sview_config.refresh_delay, 1, 10000, 5, 60, 0);
    GtkWidget *spin_button = gtk_spin_button_new(GTK_ADJUSTMENT(adjustment), 1, 0);
    GtkWidget *popup = gtk_dialog_new_with_buttons("Refresh Interval", GTK_WINDOW(user_data),
                                                   GTK_DIALOG_MODAL | GTK_DIALOG_DESTROY_WITH_PARENT, NULL);
    GError *error = NULL;
    int response = 0;
    char *temp = NULL;
    ...
    if (response == GTK_RESPONSE_OK) {
        working_sview_config.refresh_delay = gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(spin_button));
        temp = g_strdup_printf("Refresh Interval set to %d seconds.", working_sview_config.refresh_delay);
        gtk_statusbar_pop(GTK_STATUSBAR(main_statusbar), STATUS_REFRESH);
        response = gtk_statusbar_push(GTK_STATUSBAR(main_statusbar), STATUS_REFRESH, temp);
        g_free(temp);
        if (!sview_thread_new(_refresh_thr, GINT_TO_POINTER(response), false, &error)) {
...
extern void change_grid_popup(GtkAction *action, gpointer user_data) {
    ...
    int width = working_sview_config.grid_x_width, hori = working_sview_config.grid_hori, vert = working_sview_config.grid_vert;
    GtkWidget *popup = gtk_dialog_new_with_buttons("Grid Properties", GTK_WINDOW(user_data),
                                                   GTK_DIALOG_MODAL | GTK_DIALOG_DESTROY_WITH_PARENT, NULL);
    GError *error = NULL;
    int response = 0;
    char *temp = NULL;
    ...
    adjustment = gtk_adjustment_new(working_sview_config.grid_x_width, 1, 1000, 1, 60, 0);
    ...
    adjustment = gtk_adjustment_new(working_sview_config.grid_hori, 1, 1000, 1, 60, 0);
    ...
    adjustment = gtk_adjustment_new(working_sview_config.grid_vert, 1, 1000, 1, 60, 0);
    ...
    */
    ...
    if (response == GTK_RESPONSE_OK) {
        working_sview_config.grid_x_width = gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(width_sb));
        working_sview_config.grid_hori = gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(hori_sb));
        working_sview_config.grid_vert = gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(vert_sb));
        memcpy(&default_sview_config, &working_sview_config, sizeof(sview_config_t));
        if ((width == working_sview_config.grid_x_width) && (hori == working_sview_config.grid_hori) &&
            ...
            temp = g_strdup_printf("Grid set to %d nodes breaks "
                                   "at %d H and %d V.", working_sview_config.grid_x_width,
                                   working_sview_config.grid_hori, working_sview_config.grid_vert);
            ...
            if ((width > working_sview_config.grid_x_width) && grid_button_list) {
...
extern void about_popup(GtkAction *action, gpointer user_data) {
    ...
    GtkWidget *popup = gtk_dialog_new_with_buttons("About", GTK_WINDOW(user_data),
                                                   GTK_DIALOG_MODAL | GTK_DIALOG_DESTROY_WITH_PARENT, NULL);
    char *version = NULL;
...
extern void usage_popup(GtkAction *action, gpointer user_data) {
    ...
    GtkWidget *popup = gtk_dialog_new_with_buttons("Usage", GTK_WINDOW(user_data),
                                                   GTK_DIALOG_MODAL | GTK_DIALOG_DESTROY_WITH_PARENT, NULL);
    char *help_msg = "sview can be used to view and modify many of Slurm's\n"
                     "records.\n\n"
...
extern void display_fed_disabled_popup(const char *title) {
    char tmp_char[100];
    GtkWidget *label = NULL;
    GtkDialog *dialog = GTK_DIALOG(gtk_dialog_new_with_buttons(title, GTK_WINDOW(main_window),
                                                               GTK_DIALOG_MODAL | GTK_DIALOG_DESTROY_WITH_PARENT,
                                                               NULL));
    label = gtk_dialog_add_button(dialog, GTK_STOCK_OK, GTK_RESPONSE_OK);
    gtk_window_set_default(GTK_WINDOW(dialog), label);
    snprintf(tmp_char, sizeof(tmp_char), "Disabled in a federated view.\n"
